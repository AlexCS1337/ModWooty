#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 7 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 8 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 9 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 10 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 11 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 14 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 23 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 25 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
bgEntity_t *pm_entSelf = ((void *)0);
bgEntity_t *pm_entVeh = ((void *)0);
...
float	pm_spectatorfriction = 2.5f;
...
bgEntity_t *PM_BGEntForNum( int num )
{
	...
	if (!pm)
	{
		(void)( (!!(!"You cannot call PM_BGEntForNum outside of pm functions!")) || (_wassert(L"!\"You cannot call PM_BGEntForNum outside of pm functions!\"", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 178), 0) );
		return ((void *)0);
	}

	if (!pm->baseEnt)
	{
		(void)( (!!(!"Base entity address not set")) || (_wassert(L"!\"Base entity address not set\"", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 184), 0) );
		return ((void *)0);
	}

	if (!pm->entSize)
	{
		(void)( (!!(!"sizeof(ent) is 0, impossible (not set?)")) || (_wassert(L"!\"sizeof(ent) is 0, impossible (not set?)\"", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 190), 0) );
		return ((void *)0);
	}

	(void)( (!!(num >= 0 && num < (1<<10))) || (_wassert(L"num >= 0 && num < MAX_GENTITIES", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 194), 0) );
...
#line 245 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"


qboolean __inline PM_IsRocketTrooper(void)
{
...
void PM_pitch_roll_for_slope( bgEntity_t *forwhom, vec3_t pass_slope, vec3_t storeAngles )
{
	...
	if( !pass_slope || VectorCompare( vec3_origin, pass_slope ) )
	{
		...
		((startspot)[0]=(pm->ps->origin)[0],(startspot)[1]=(pm->ps->origin)[1],(startspot)[2]=(pm->ps->origin)[2]);
		startspot[2] += pm->mins[2] + 4;
		((endspot)[0]=(startspot)[0],(endspot)[1]=(startspot)[1],(endspot)[2]=(startspot)[2]);
		endspot[2] -= 300;
		pm->trace( &trace, pm->ps->origin, vec3_origin, vec3_origin, endspot, forwhom->s.number, (0x00000001|0x00001000) );
		...
		((slope)[0]=(trace.plane.normal)[0],(slope)[1]=(trace.plane.normal)[1],(slope)[2]=(trace.plane.normal)[2]);
	}
	else
	{
		((slope)[0]=(pass_slope)[0],(slope)[1]=(pass_slope)[1],(slope)[2]=(pass_slope)[2]);
	}
	...
	if ( forwhom->s.NPC_class == CLASS_VEHICLE )
	{
		...
		tempAngles[0] = tempAngles[2] = 0;
		tempAngles[1] = pVeh->m_vOrientation[1];
		AngleVectors( tempAngles, ovf, ovr, ((void *)0) );
	}
	else
	{
		AngleVectors( pm->ps->viewangles, ovf, ovr, ((void *)0) );
	}

	vectoangles( slope, new_angles );
	pitch = new_angles[0] + 90;
	new_angles[2] = new_angles[0] = 0;

	AngleVectors( new_angles, nvf, ((void *)0), ((void *)0) );

	mod = ((nvf)[0]*(ovr)[0]+(nvf)[1]*(ovr)[1]+(nvf)[2]*(ovr)[2]);
	...
	dot = ((nvf)[0]*(ovf)[0]+(nvf)[1]*(ovf)[1]+(nvf)[2]*(ovf)[2]);

	if ( storeAngles )
	{
		storeAngles[0] = dot * pitch;
		storeAngles[2] = ((1-Q_fabs(dot)) * pitch * mod);
	}
	else 
	{
		float oldmins2;

		pm->ps->viewangles[0] = dot * pitch;
		pm->ps->viewangles[2] = ((1-Q_fabs(dot)) * pitch * mod);
		oldmins2 = pm->mins[2];
		pm->mins[2] = -24 + 12 * fabs(pm->ps->viewangles[0])/180.0f;
...
static int pm_flying = 0;

void PM_SetSpecialMoveValues (void)
{
	bgEntity_t *pEnt;
	
	if (pm->ps->clientNum < 32)
	{ 
		pm_flying = 0;
		return;
	}
...
static void PM_SetVehicleAngles( vec3_t normal )
{
	...
	((vAngles)[0]=(vAngles)[1]=(vAngles)[2]=0);
	if ( pm->waterlevel > 0 )
	{
		
		
		vAngles[0] += (pm->ps->viewangles[0]-vAngles[0])*0.75f + (pitchBias*0.5);
	}
	else if ( normal )
	{
		PM_pitch_roll_for_slope( pEnt, normal, vAngles );
		if ( (pml.groundTrace.contents&(0x00000004|0x00020000|0x00000002)) )
		{
			
			
			vAngles[0] += (pm->ps->viewangles[0]-vAngles[0])*0.5f + (pitchBias*0.5f);
		}
	}
	else
	{
		
		vAngles[0] = pm->ps->viewangles[0]*0.5f + pitchBias;
	...
	if ( pVeh->m_pVehicleInfo->rollLimit > 0 )
	{
		...
		vec3_t	velocity;
		float	speed;
		((velocity)[0]=(pm->ps->velocity)[0],(velocity)[1]=(pm->ps->velocity)[1],(velocity)[2]=(pm->ps->velocity)[2]);
		velocity[2] = 0.0f;
		...
		if ( speed > 32.0f || speed < -32.0f ) 
		{
			...
			((tempVAngles)[0]=(pVeh->m_vOrientation)[0],(tempVAngles)[1]=(pVeh->m_vOrientation)[1],(tempVAngles)[2]=(pVeh->m_vOrientation)[2]);
			tempVAngles[2] = 0;
			AngleVectors( tempVAngles, ((void *)0), rt, ((void *)0) );
			dp = ((velocity)[0]*(rt)[0]+(velocity)[1]*(rt)[1]+(velocity)[2]*(rt)[2]);
			side = speed * dp;
			vAngles[2] -= side;
		}
	}
	...
	{
		if ( vAngles[0] > pVeh->m_pVehicleInfo->pitchLimit )
		{
			vAngles[0] = pVeh->m_pVehicleInfo->pitchLimit;
		}
		else if ( vAngles[0] < -pVeh->m_pVehicleInfo->pitchLimit )
		{
			vAngles[0] = -pVeh->m_pVehicleInfo->pitchLimit;
		}
	}

	if ( vAngles[2] > pVeh->m_pVehicleInfo->rollLimit )
	{
		vAngles[2] = pVeh->m_pVehicleInfo->rollLimit;
	}
	else if ( vAngles[2] < -pVeh->m_pVehicleInfo->rollLimit )
	{
		vAngles[2] = -pVeh->m_pVehicleInfo->rollLimit;
	}
	...
	if ( pVeh && pVeh->m_pVehicleInfo )
	{
		...
		if ( pVeh->m_pVehicleInfo->speedDependantTurning )
		{
			if ( pVeh->m_LandTrace.fraction >= 1.0f 
				|| pVeh->m_LandTrace.plane.normal[2] < 0.8f  )
			{
...
#line 725 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"



typedef struct gentity_s gentity_t;
gentity_t *G_PlayEffectID(const int fxID, vec3_t org, vec3_t ang);
#line 731 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 733 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
void PM_HoverTrace( void )
{
	...
	relativeWaterLevel = pm->waterlevel; 
	if ( pm->waterlevel && relativeWaterLevel >= 0 )
	{
		...
		{
			float floatHeight = (pVeh->m_pVehicleInfo->bouyancy * ((pm->maxs[2]-pm->mins[2])*0.5f)) - (hoverHeight*0.5f);
		...
		if (pm->waterlevel <= 1)
		{
			if ( fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1]) > 100 )
			{
				if ( Q_irand( pml.frametime, 100 ) >= 50 )
				{
					...
					vAng[0] = vAng[2] = 0;
					vAng[1] = pVeh->m_vOrientation[1];
					AngleVectors( vAng, fxAxis[2], fxAxis[1], fxAxis[0] );
					((wakeOrg)[0]=(pm->ps->origin)[0],(wakeOrg)[1]=(pm->ps->origin)[1],(wakeOrg)[2]=(pm->ps->origin)[2]);
...
#line 800 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
				}
			}
		}
	}
	else
	{
		...
		{
			traceContents |= (0x00000004|0x00020000|0x00000002);
		}
		...
		else if ( trace->plane.normal[2] >= minNormal ) 
		{
			if ( trace->fraction < 1.0f )
			{
				...
				if ( (trace->contents&(0x00000004|0x00020000|0x00000002)) )
				{
					if ( fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1]) > 100 )
					{
						if ( Q_irand( pml.frametime, 100 ) >= 50 )
...
#line 864 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
						}
					}
				}
	...
	else
	{
		...
		{
			pVeh->m_vAngularVelocity = pVeh->m_vOrientation[1] - pVeh->m_vPrevOrientation[1];
...
static void PM_Friction( void ) {
	...
	float	drop;
	bgEntity_t *pEnt = ((void *)0);
	
	vel = pm->ps->velocity;
	
	((vec)[0]=(vel)[0],(vec)[1]=(vel)[1],(vec)[2]=(vel)[2]);
	if ( pml.walking ) {
		vec[2] = 0;	
	}
	...
	else if ( pm_flying != 1 && pm_flying != 2 )
	{
		
		if ( pm->waterlevel <= 1 ) {
			if ( pml.walking && !(pml.groundTrace.surfaceFlags & 0x00004000) ) {
	...
	else if ( pm->ps->groundEntityNum < 32 )
	{
		drop = 0;
	}
...
static void PM_Accelerate( vec3_t wishdir, float wishspeed, float accel )
{
	...
	{ 
		...
		currentspeed = ((pm->ps->velocity)[0]*(wishdir)[0]+(pm->ps->velocity)[1]*(wishdir)[1]+(pm->ps->velocity)[2]*(wishdir)[2]);
		addspeed = wishspeed - currentspeed;
		if (addspeed <= 0 && pm->ps->clientNum < 32) {
			return;
		}
	...
	{ 
		...
		((wishVelocity)[0]=(wishdir)[0]*(wishspeed),(wishVelocity)[1]=(wishdir)[1]*(wishspeed),(wishVelocity)[2]=(wishdir)[2]*(wishspeed));
		((pushDir)[0]=(wishVelocity)[0]-(pm->ps->velocity)[0],(pushDir)[1]=(wishVelocity)[1]-(pm->ps->velocity)[1],(pushDir)[2]=(wishVelocity)[2]-(pm->ps->velocity)[2]);
		...
		((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(pushDir)[0]*(canPush),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(pushDir)[1]*(canPush),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(pushDir)[2]*(canPush));
	}
}
...
void PM_SetPMViewAngle(playerState_t *ps, vec3_t angle, usercmd_t *ucmd)
{
	...
	{ 
		...
		cmdAngle = ((int)((angle[i])*65536/360) & 65535);
		ps->delta_angles[i] = cmdAngle - ucmd->angles[i];
	}
	((ps->viewangles)[0]=(angle)[0],(ps->viewangles)[1]=(angle)[1],(ps->viewangles)[2]=(angle)[2]);
}
...
qboolean PM_AdjustAngleForWallRun( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
...

	{
		...
		((maxs)[0]=(15), (maxs)[1]=(15), (maxs)[2]=(24));
		((fwdAngles)[0]=(0), (fwdAngles)[1]=(pm->ps->viewangles[1]), (fwdAngles)[2]=(0));

		AngleVectors( fwdAngles, fwd, rt, ((void *)0) );
		...
		((traceTo)[0]=(ps->origin)[0]+(rt)[0]*(dist),(traceTo)[1]=(ps->origin)[1]+(rt)[1]*(dist),(traceTo)[2]=(ps->origin)[2]+(rt)[2]*(dist));
		
		pm->trace( &trace, ps->origin, mins, maxs, traceTo, ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );

		if ( trace.fraction < 1.0f 
			&& (trace.plane.normal[2] >= 0.0f && trace.plane.normal[2] <= 0.7f) )
		{
			trace_t	trace2;
			vec3_t traceTo2;
			vec3_t	wallRunFwd, wallRunAngles;
			
			((wallRunAngles)[0]=(wallRunAngles)[1]=(wallRunAngles)[2]=0);
			wallRunAngles[1] = vectoyaw( trace.plane.normal )+yawAdjust;
			AngleVectors( wallRunAngles, wallRunFwd, ((void *)0), ((void *)0) );

			((traceTo2)[0]=(pm->ps->origin)[0]+(wallRunFwd)[0]*(32),(traceTo2)[1]=(pm->ps->origin)[1]+(wallRunFwd)[1]*(32),(traceTo2)[2]=(pm->ps->origin)[2]+(wallRunFwd)[2]*(32));
			pm->trace( &trace2, pm->ps->origin, mins, maxs, traceTo2, pm->ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );
			if ( trace2.fraction < 1.0f && ((trace2.plane.normal)[0]*(wallRunFwd)[0]+(trace2.plane.normal)[1]*(wallRunFwd)[1]+(trace2.plane.normal)[2]*(wallRunFwd)[2]) <= -0.999f )
			{
				trace.fraction = 1.0f;
			}
		} 
			...
			&& (trace.plane.normal[2] >= 0.0f&&trace.plane.normal[2] <= 0.7f) )
		{
			...
			ps->viewangles[1] = vectoyaw( trace.plane.normal )+yawAdjust;

			PM_SetPMViewAngle(ps, ps->viewangles, ucmd);

			ucmd->angles[1] = ((int)((ps->viewangles[1])*65536/360) & 65535) - ps->delta_angles[1];
			if ( doMove )
			{
				...
				if ( ps->legsTimer >= 100 )
				{
					...
					{
						speed = g_speed.integer/2;
					}
					else if ( ucmd->forwardmove > 0 )
					{
						speed = (float)g_speed.integer;
					}
					((ps->velocity)[0]=(fwd)[0]*(speed),(ps->velocity)[1]=(fwd)[1]*(speed),(ps->velocity)[2]=(fwd)[2]*(speed));
				}

				ps->velocity[2] = zVel+3.14f;
				
				((ps->velocity)[0]=(ps->velocity)[0]+(rt)[0]*(dist),(ps->velocity)[1]=(ps->velocity)[1]+(rt)[1]*(dist),(ps->velocity)[2]=(ps->velocity)[2]+(rt)[2]*(dist));
			}
			ucmd->forwardmove = 0;
			return qtrue;
		}
		else if ( doMove )
		{
			if ( (ps->legsAnim) == BOTH_WALL_RUN_RIGHT )
			{
				PM_ContinueLegsAnim(BOTH_WALL_RUN_RIGHT);
	
				PM_SetAnim(1|2, BOTH_WALL_RUN_RIGHT, 1|4, 0);
			...
			{
				PM_ContinueLegsAnim(BOTH_WALL_RUN_LEFT);

				PM_SetAnim(1|2, BOTH_WALL_RUN_LEFT, 1|4, 0);
				pm->ps->legsTimer = 1000;
...
qboolean PM_AdjustAngleForWallRunUp( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	if ( ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START )
	{
		...
		AngleVectors( fwdAngles, fwd, ((void *)0), ((void *)0) );
		((traceTo)[0]=(ps->origin)[0]+(fwd)[0]*(dist),(traceTo)[1]=(ps->origin)[1]+(fwd)[1]*(dist),(traceTo)[2]=(ps->origin)[2]+(fwd)[2]*(dist));
		pm->trace( &trace, ps->origin, mins, maxs, traceTo, ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );
		if ( trace.fraction > 0.5f )
		{
			trace_t	trace2;
			...
			((top)[0]=(trace.endpos)[0],(top)[1]=(trace.endpos)[1],(top)[2]=(trace.endpos)[2]);
			top[2] += (pm->mins[2]*-1) + 4.0f;
			((bottom)[0]=(top)[0],(bottom)[1]=(top)[1],(bottom)[2]=(top)[2]);
			bottom[2] -= 64.0f;
			pm->trace( &trace2, top, pm->mins, pm->maxs, bottom, ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );
			if ( !trace2.allsolid 
				&& !trace2.startsolid 
				&& trace2.fraction < 1.0f 
				&& trace2.plane.normal[2] > 0.7f )
			{
				((pm->ps->velocity)[0]=(fwd)[0]*(100),(pm->ps->velocity)[1]=(fwd)[1]*(100),(pm->ps->velocity)[2]=(fwd)[2]*(100));
				pm->ps->velocity[2] += 400;
				PM_SetAnim(1|2, BOTH_FORCEWALLRUNFLIP_ALT, 1|2, 0);
				pm->ps->pm_flags |= 2;
			...
			(trace.plane.normal[2] >= 0.0f&&trace.plane.normal[2]<=0.4f) )
		{
			...
			trace_t	trace2;
			((traceTo)[0]=(ps->origin)[0],(traceTo)[1]=(ps->origin)[1],(traceTo)[2]=(ps->origin)[2]);
			traceTo[2] += 64;
			pm->trace( &trace2, ps->origin, mins, maxs, traceTo, ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );
			...
			else
			{
				...
				if (1) 
				{
					if ( doMove )
					{
						...
						((ps->velocity)[0]=(trace.plane.normal)[0]*(-dist*trace.fraction),(ps->velocity)[1]=(trace.plane.normal)[1]*(-dist*trace.fraction),(ps->velocity)[2]=(trace.plane.normal)[2]*(-dist*trace.fraction));
		...
		if ( doMove )
		{
			((ps->velocity)[0]=(fwd)[0]*(-300.0f),(ps->velocity)[1]=(fwd)[1]*(-300.0f),(ps->velocity)[2]=(fwd)[2]*(-300.0f));
			ps->velocity[2] += 200;
			
			
			PM_SetAnim(1|2, BOTH_FORCEWALLRUNFLIP_END, 1|2, 0);
			ps->pm_flags |= 2;
...
qboolean PM_AdjustAngleForWallJump( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
		...
		|| (pm->ps->pm_flags&16384) )
	{
		...
		((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[1]), (mins)[2]=(0));
		((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[1]), (maxs)[2]=(24));
		((fwdAngles)[0]=(0), (fwdAngles)[1]=(pm->ps->viewangles[1]), (fwdAngles)[2]=(0));
		...
		case BOTH_FORCEWALLHOLD_RIGHT:
			AngleVectors( fwdAngles, ((void *)0), checkDir, ((void *)0) );
			yawAdjust = -90;
			break;
		case BOTH_FORCEWALLREBOUND_LEFT:
		case BOTH_FORCEWALLHOLD_LEFT:
			AngleVectors( fwdAngles, ((void *)0), checkDir, ((void *)0) );
			((checkDir)[0]=(checkDir)[0]*(-1),(checkDir)[1]=(checkDir)[1]*(-1),(checkDir)[2]=(checkDir)[2]*(-1));
			yawAdjust = 90;
			break;
		case BOTH_FORCEWALLREBOUND_FORWARD:
		case BOTH_FORCEWALLHOLD_FORWARD:
			AngleVectors( fwdAngles, checkDir, ((void *)0), ((void *)0) );
			yawAdjust = 180;
			break;
		...
		if ( !pm->debugMelee )
		{
			if ( ucmd->upmove > 0 && pm->ps->fd.forcePower > 5)
			{
				if ( BG_InReboundHold( ps->legsAnim ) )
				{
					PM_SetAnim( 1|2, BOTH_SPINATTACK7, 1|2, 0 );
					pm->ps->fd.forcePower = pm->ps->fd.forcePower - 5;
				...
				else
				{
					...
					{
						ps->saberHolstered = 2;
						PM_SetAnim( 1|2, BOTH_FORCEWALLRELEASE_FORWARD+(ps->legsAnim-BOTH_FORCEWALLHOLD_FORWARD), 1|2, 0 );
						ps->legsTimer = ps->torsoTimer = 150;
					}
				}
			}
		}
		((traceTo)[0]=(ps->origin)[0]+(checkDir)[0]*(dist),(traceTo)[1]=(ps->origin)[1]+(checkDir)[1]*(dist),(traceTo)[2]=(ps->origin)[2]+(checkDir)[2]*(dist));
		pm->trace( &trace, ps->origin, mins, maxs, traceTo, ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );
		if ( 
			ps->legsTimer > 100 &&
			trace.fraction < 1.0f && 
			fabs(trace.plane.normal[2]) <= 0.2f )
		{
			...
			if (1)
			{
				if ( doMove )
				{
					...
					((ps->velocity)[0]=(trace.plane.normal)[0]*(-128.0f),(ps->velocity)[1]=(trace.plane.normal)[1]*(-128.0f),(ps->velocity)[2]=(trace.plane.normal)[2]*(-128.0f));
				}
			}
			ucmd->upmove = 0;
			ps->pm_flags |= 16384;
			...
			&& (ps->pm_flags&16384))
		{
			...
			ps->pm_flags &= ~16384;
			ps->velocity[0] = ps->velocity[1] = g_walljumpXvelocity.integer;
			((ps->velocity)[0]=(checkDir)[0]*(-1*g_walljumpYvelocity.integer),(ps->velocity)[1]=(checkDir)[1]*(-1*g_walljumpYvelocity.integer),(ps->velocity)[2]=(checkDir)[2]*(-1*g_walljumpYvelocity.integer));
			ps->velocity[2] = BG_ForceWallJumpStrength()*g_walljumpZvelocity.integer;
			...
			BG_ForcePowerDrain( ps, FP_LEVITATION, g_walljumpforce.integer );
			...
			else
			{
				...
				PM_SetAnim(2,BOTH_FORCEJUMP1,1|2|4, 0);
			}
...
static qboolean PM_CheckJump( void ) 
{
	...
	if ( pm->ps->weapon == WP_SABER )
	{
		...
		saberInfo_t *saber2 = BG_MySaber( pm->ps->clientNum, 1 );
		if ( saber1
			&& (saber1->saberFlags&(1<<17)) )
		{
			allowFlips = qfalse;
		}
		if ( saber2
			&& (saber2->saberFlags&(1<<17)) )
		{
			allowFlips = qfalse;
		}
	}

	if (pm->ps->groundEntityNum != ((1<<10)-1) || pm->ps->origin[2] < pm->ps->fd.forceJumpZStart)
	{
					...
					if ( curHeight > forceJumpHeight[0] )
					{
						if ( !(pm->ps->fd.forcePowersActive&(1<<FP_LEVITATION)) )
						{
							...
							else if ( pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 )
							{
								...
								((facingAngles)[0]=(0), (facingAngles)[1]=(pm->ps->viewangles[1]), (facingAngles)[2]=(0));

								AngleVectors( facingAngles, facingFwd, facingRight, ((void *)0) );
								dotR = ((facingRight)[0]*(pm->ps->velocity)[0]+(facingRight)[1]*(pm->ps->velocity)[1]+(facingRight)[2]*(pm->ps->velocity)[2]);
								dotF = ((facingFwd)[0]*(pm->ps->velocity)[0]+(facingFwd)[1]*(pm->ps->velocity)[1]+(facingFwd)[2]*(pm->ps->velocity)[2]);
						...
						{ 
							if ( pm->ps->legsTimer < 1 )
							{
								...
								if ( newAnim != -1 )
								{
									...
									PM_SetAnim( parts, newAnim, 1|2, 150 );
								}
							}
						}
					}
...
#line 2143 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"

	
	if ( pm->cmd.upmove < 10 && pm->ps->groundEntityNum != ((1<<10)-1)) {
		return qfalse;
	}
	...
	if ( pm->ps->gravity <= 0 )
	{
		...
		AngleVectors( pm->ps->viewangles, forward, ((void *)0), ((void *)0) );
		((back)[0]=(pm->ps->origin)[0]+(forward)[0]*(-8),(back)[1]=(pm->ps->origin)[1]+(forward)[1]*(-8),(back)[2]=(pm->ps->origin)[2]+(forward)[2]*(-8));
		pm->trace( &trace, pm->ps->origin, pm->mins, pm->maxs, back, pm->ps->clientNum, pm->tracemask );

		if ( trace.fraction <= 1.0f )
		{
			((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(forward)[0]*(225*2),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(forward)[1]*(225*2),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(forward)[2]*(225*2));
			PM_SetAnim(2,BOTH_FORCEJUMP1,1|2|4, 150);
		}
		pm->cmd.upmove = 0;
		...
		BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION) )
	{
		...
		if ( pm->ps->weapon == WP_SABER )
		{
			...
			saberInfo_t *saber2 = BG_MySaber( pm->ps->clientNum, 1 );
			if ( saber1
				&& (saber1->saberFlags&(1<<13)) )
			{
				
			}
			if ( saber2
				&& (saber2->saberFlags&(1<<13)) )
			{
				
			}
			if ( saber1
				&& (saber1->saberFlags&(1<<14)) )
			{
				allowWallFlips = qfalse;
			}
			if ( saber2
				&& (saber2->saberFlags&(1<<14)) )
			{
				allowWallFlips = qfalse;
			}
			if ( saber1
			...
			if ( saber1
				&& (saber1->saberFlags&(1<<15)) )
			{
				allowWallGrabs = qfalse;
			}
			if ( saber2
				&& (saber2->saberFlags&(1<<15)) )
			{
				allowWallGrabs = qfalse;
			}
		}
		...
		if ( pm->ps->groundEntityNum != ((1<<10)-1) )
		{
			...
			else if ( pm->cmd.forwardmove < 0 && !(pm->cmd.buttons&1) )
			{
				if ( allowFlips )
				{
			...
			if ( anim != -1 )
			{
				...
				int contents = (0x00000001|0x00000010|0x00000100|0x00001000);
				...
				case BOTH_WALL_RUN_LEFT:
					doTrace = qtrue;
					((traceto)[0]=(pm->ps->origin)[0]+(right)[0]*(-16),(traceto)[1]=(pm->ps->origin)[1]+(right)[1]*(-16),(traceto)[2]=(pm->ps->origin)[2]+(right)[2]*(-16));
					break;
				...
				case BOTH_WALL_FLIP_BACK1:
					...
					doTrace = qtrue;
					((traceto)[0]=(pm->ps->origin)[0]+(fwd)[0]*(-16),(traceto)[1]=(pm->ps->origin)[1]+(fwd)[1]*(-16),(traceto)[2]=(pm->ps->origin)[2]+(fwd)[2]*(-16));
					break;
				}
				...
				if ( doTrace )
				{
					...
					((wallNormal)[0]=(trace.plane.normal)[0],(wallNormal)[1]=(trace.plane.normal)[1],(wallNormal)[2]=(trace.plane.normal)[2]);
					VectorNormalize( wallNormal );
					((idealNormal)[0]=(pm->ps->origin)[0]-(traceto)[0],(idealNormal)[1]=(pm->ps->origin)[1]-(traceto)[1],(idealNormal)[2]=(pm->ps->origin)[2]-(traceto)[2]);
				...
				if ( !doTrace || (trace.fraction < 1.0f && (trace.entityNum < 32 || ((wallNormal)[0]*(idealNormal)[0]+(wallNormal)[1]*(idealNormal)[1]+(wallNormal)[2]*(idealNormal)[2]) > 0.7)) )
				{
						...
						if ( anim == BOTH_WALL_FLIP_LEFT )
						{
							...
							((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(right)[0]*(150),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(right)[1]*(150),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(right)[2]*(150));
						}
						...
						{
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
							((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(fwd)[0]*(-150),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(fwd)[1]*(-150),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(fwd)[2]*(-150));
						}

						
						if ( doTrace && anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT )
						{
							if (trace.entityNum < 32)
							{
							...
							|| anim == BOTH_WALL_RUN_RIGHT)
						{
							...
							PM_SetAnim( parts, anim, 1|4, 0 );							
							

							if (pm->cmd.buttons & 1){
							PM_GrabWallForJump( BOTH_FORCEWALLHOLD_FORWARD );
							}else{
							pm->ps->legsTimer = 1000;
		...
		else 
		{
			...
			if ( legsAnim == BOTH_WALL_RUN_LEFT || legsAnim == BOTH_WALL_RUN_RIGHT )
			{
				...
				((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[0]), (mins)[2]=(0));
				((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[0]), (maxs)[2]=(24));
				((fwdAngles)[0]=(0), (fwdAngles)[1]=(pm->ps->viewangles[1]), (fwdAngles)[2]=(0));
				...
				if ( anim != -1 )
				{
					pm->trace( &trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, 0x00000001|0x00000100 );
					if ( trace.fraction < 1.0f )
					{
						int parts = 0;
						...
						if ( anim == BOTH_WALL_RUN_LEFT_FLIP )
						{
								...
								pm->ps->velocity[0] *= 0.7f;
							pm->ps->velocity[1] *= 0.7f;
							((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(right)[0]*(150),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(right)[1]*(150),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(right)[2]*(150));
						}
			...
			else if ( pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START )
			{
				...
				((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[0]), (mins)[2]=(0.0f));
				((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[0]), (maxs)[2]=(24.0f));
				
				((fwdAngles)[0]=(0), (fwdAngles)[1]=(pm->ps->viewangles[1]), (fwdAngles)[2]=(0.0f));
				AngleVectors( fwdAngles, fwd, ((void *)0), ((void *)0) );

				(void)( (!!(pm_entSelf)) || (_wassert(L"pm_entSelf", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 2536), 0) ); 
				...
				if ( anim != -1 )
				{
					...
					if ( trace.fraction < 1.0f )
					{
						...
						((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(fwd)[0]*(-300),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(fwd)[1]*(-300),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(fwd)[2]*(-300));
				...
				)
			{
				...
				((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[1]), (mins)[2]=(pm->mins[2]));
				((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[1]), (maxs)[2]=(pm->maxs[2]));
				((fwdAngles)[0]=(0), (fwdAngles)[1]=(pm->ps->viewangles[1]), (fwdAngles)[2]=(0));

				AngleVectors( fwdAngles, fwd, ((void *)0), ((void *)0) );
				((traceto)[0]=(pm->ps->origin)[0]+(fwd)[0]*(32),(traceto)[1]=(pm->ps->origin)[1]+(fwd)[1]*(32),(traceto)[2]=(pm->ps->origin)[2]+(fwd)[2]*(32));

				pm->trace( &trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000) );
				((idealNormal)[0]=(pm->ps->origin)[0]-(traceto)[0],(idealNormal)[1]=(pm->ps->origin)[1]-(traceto)[1],(idealNormal)[2]=(pm->ps->origin)[2]-(traceto)[2]);
				VectorNormalize( idealNormal );
				...
				if ( trace.fraction < 1.0f )
				{
					...
					PM_SetAnim( parts, BOTH_WALL_FLIP_BACK1, 1|2, 0 );
					...
					BG_ForcePowerDrain( pm->ps, FP_LEVITATION, 20 );
				...
				|| pm->ps->legsAnim == BOTH_INAIR1 ) )
			{
				if ( allowWallRuns )
				{
					...
					int parts = 2;
					int contents = (0x00000001|0x00001000);
					...
					if (1) 
					{
						...
						((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[1]), (mins)[2]=(0.0f));
						((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[1]), (maxs)[2]=(24.0f));
						((fwdAngles)[0]=(0), (fwdAngles)[1]=(pm->ps->viewangles[1]), (fwdAngles)[2]=(0.0f));
						...
						if ( trace.fraction < 1.0f
							...
							&&traceEnt
							&&traceEnt->s.solid!=0xffffff)
							||((trace.plane.normal)[0]*(idealNormal)[0]+(trace.plane.normal)[1]*(idealNormal)[1]+(trace.plane.normal)[2]*(idealNormal)[2])>0.4) 
							)
						{
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
							if ( wallWalkAnim == BOTH_FORCEWALLRUNFLIP_START )
							{
								pm->ps->velocity[2] = forceJumpStrength[FORCE_LEVEL_3]/1.6f;
							}
							else
							{
								((pm->ps->velocity)[0]=(pm->ps->velocity)[0]+(fwd)[0]*(-150),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]+(fwd)[1]*(-150),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]+(fwd)[2]*(-150));
								pm->ps->velocity[2] += 175.0f;
							}
							
							
							PM_SetAnim( parts, wallWalkAnim, 1|2|4, 0 );
				...
				&& pm->ps->velocity[2] < -50))
					...
					&& BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION)
					&& (pm->ps->origin[2]-pm->ps->fd.forceJumpZStart) < (forceJumpHeightMax[FORCE_LEVEL_3]-(BG_ForceWallJumpStrength()/2.0f)) 
					...
					)
			{
				if ( allowWallGrabs )
				{
					...
					if ( anim != -1 )
					{
						...
						((traceto)[0]=(pm->ps->origin)[0]+(checkDir)[0]*(8),(traceto)[1]=(pm->ps->origin)[1]+(checkDir)[1]*(8),(traceto)[2]=(pm->ps->origin)[2]+(checkDir)[2]*(8));
						pm->trace( &trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, 0x00000001 );
						((idealNormal)[0]=(pm->ps->origin)[0]-(traceto)[0],(idealNormal)[1]=(pm->ps->origin)[1]-(traceto)[1],(idealNormal)[2]=(pm->ps->origin)[2]-(traceto)[2]);
						VectorNormalize( idealNormal );
						traceEnt = PM_BGEntForNum(trace.entityNum);
						if ( trace.fraction < 1.0f
							
							&&fabs(trace.plane.normal[2]) <= 0.6f
							&&((trace.entityNum<((1<<10)-2)&&traceEnt&&traceEnt->s.solid!=0xffffff)||((trace.plane.normal)[0]*(idealNormal)[0]+(trace.plane.normal)[1]*(idealNormal)[1]+(trace.plane.normal)[2]*(idealNormal)[2])>0.7) )
						{
							float dot = ((pm->ps->velocity)[0]*(trace.plane.normal)[0]+(pm->ps->velocity)[1]*(trace.plane.normal)[1]+(pm->ps->velocity)[2]*(trace.plane.normal)[2]);
							if ( dot < 1.0f )
							{
	...
	if ( pm->cmd.upmove > 0 
		...
		&& !PM_IsRocketTrooper()
		&& (pm->ps->weaponTime > 0||pm->cmd.buttons&1) )
	{
				...
				if ( pm->ps->fd.saberAnimLevel == FORCE_LEVEL_2 )
				{
					...
					{ 
						...
						
						{
							...
							pm->ps->pm_flags |= 2;
							pm->ps->groundEntityNum = ((1<<10)-1);
							((pml.groundTrace.plane.normal)[0]=(pml.groundTrace.plane.normal)[1]=(pml.groundTrace.plane.normal)[2]=0);
...
static qboolean	PM_CheckWaterJump( void ) {
	...
	((spot)[0]=(pm->ps->origin)[0]+(flatforward)[0]*(30),(spot)[1]=(pm->ps->origin)[1]+(flatforward)[1]*(30),(spot)[2]=(pm->ps->origin)[2]+(flatforward)[2]*(30));
	spot[2] += 4;
	...
	((pm->ps->velocity)[0]=(pml.forward)[0]*(200),(pm->ps->velocity)[1]=(pml.forward)[1]*(200),(pm->ps->velocity)[2]=(pml.forward)[2]*(200));
...
#line 3016 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	PM_Friction ();
	...
	if ( pml.groundPlane && ((pm->ps->velocity)[0]*(pml.groundTrace.plane.normal)[0]+(pm->ps->velocity)[1]*(pml.groundTrace.plane.normal)[1]+(pm->ps->velocity)[2]*(pml.groundTrace.plane.normal)[2]) < 0 ) {
		...
		((pm->ps->velocity)[0]=(pm->ps->velocity)[0]*(vel),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]*(vel),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]*(vel));
	}
...
static void PM_FlyVehicleMove( void ) 
{
	...
	if ( pm->ps->gravity && pm->ps->velocity[2] < 0 && pm->ps->groundEntityNum == ((1<<10)-1) )
	{
	...
	{
		PM_Friction ();
		if ( pm->ps->velocity[2] < 0 && pm->ps->groundEntityNum != ((1<<10)-1) )
		{
			pm->ps->velocity[2] = 0;	
		}
	}
	...
	if ( pm->ps->clientNum >= 32 )
	{
		...
		else
		{
			...
			((wishvel)[0]=(pm->ps->moveDir)[0]*(pm->ps->speed),(wishvel)[1]=(pm->ps->moveDir)[1]*(pm->ps->speed),(wishvel)[2]=(pm->ps->moveDir)[2]*(pm->ps->speed));
			((wishdir)[0]=(pm->ps->moveDir)[0],(wishdir)[1]=(pm->ps->moveDir)[1],(wishdir)[2]=(pm->ps->moveDir)[2]);
		}
	}
	else
	{
	...
	if ( wishspeed < 0 )
	{
		...
		((wishvel)[0]=(wishvel)[0]*(-1.0f),(wishvel)[1]=(wishvel)[1]*(-1.0f),(wishvel)[2]=(wishvel)[2]*(-1.0f));
		((wishdir)[0]=(wishdir)[0]*(-1.0f),(wishdir)[1]=(wishdir)[1]*(-1.0f),(wishdir)[2]=(wishdir)[2]*(-1.0f));
	}
...
static void PM_FlyMove( void ) {
	...
	if ( pm->ps->pm_type == PM_SPECTATOR && pm->cmd.buttons & 128) {
...
#line 3242 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	}
	PM_Friction();
...
#line 3368 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	}
	else if ( gPMDoSlowFall )
	{
		((wishvel)[0]=(wishvel)[1]=(wishvel)[2]=0);
	}
	...
	{ 
		...
		if (pm->cmd.upmove <= 0)
		{
            ((wishvel)[0]=(wishvel)[0]*(0.8f),(wishvel)[1]=(wishvel)[1]*(0.8f),(wishvel)[2]=(wishvel)[2]*(0.8f));
		}
		else
		{ 
            ((wishvel)[0]=(wishvel)[0]*(2.0f),(wishvel)[1]=(wishvel)[1]*(2.0f),(wishvel)[2]=(wishvel)[2]*(2.0f));
		}
	}
	else
	{
	...
	if (pm->ps->torsoAnim == FACE_SMILE && g_candivecrane.integer > 0 && pm->cmd.upmove < 0){
...
static void PM_WalkMove( void ) {
	...
	if (pm->ps->brokenLimbs == -10){
		return;
	}

	if ( pm->waterlevel > 2 && ((pml.forward)[0]*(pml.groundTrace.plane.normal)[0]+(pml.forward)[1]*(pml.groundTrace.plane.normal)[1]+(pml.forward)[2]*(pml.groundTrace.plane.normal)[2]) > 0 ) {
	...
	if (pm->ps->electrifyTime > level.time){
		return;
	}	
	#line 3485 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	...
	PM_ClipVelocity (pml.forward, pml.groundTrace.plane.normal, pml.forward, 1.001f );
	PM_ClipVelocity (pml.right, pml.groundTrace.plane.normal, pml.right, 1.001f );
	...
	if ( pm->ps->clientNum >= 32 && !VectorCompare( pm->ps->moveDir, vec3_origin ) )
	{
		bgEntity_t *pEnt = pm_entSelf;
	...
	else if ( (pm->ps->pm_flags & 4) && !BG_InRoll(pm->ps, pm->ps->legsAnim) &&
	...
	else if ( ( pml.groundTrace.surfaceFlags & 0x00004000 ) || pm->ps->pm_flags & 64 )
	{
		accelerate = pm_airaccelerate;
	}
	else
	...
	if ( ( pml.groundTrace.surfaceFlags & 0x00004000 ) || pm->ps->pm_flags & 64 )
	{
		pm->ps->velocity[2] -= pm->ps->gravity * pml.frametime;
	}
...
static void PM_DeadMove( void ) {
	...
	forward -= 20;
	if ( forward <= 0 ) {
		((pm->ps->velocity)[0]=(pm->ps->velocity)[1]=(pm->ps->velocity)[2]=0);
	} else {
		VectorNormalize (pm->ps->velocity);
		((pm->ps->velocity)[0]=(pm->ps->velocity)[0]*(forward),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]*(forward),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]*(forward));
	}
}
...
static void PM_NoclipMove( void ) {
	...
	if (speed < 1)
	{
		((pm->ps->velocity)[0]=(vec3_origin)[0],(pm->ps->velocity)[1]=(vec3_origin)[1],(pm->ps->velocity)[2]=(vec3_origin)[2]);
	}
	else
	{
		drop = 0;
		...
		((pm->ps->velocity)[0]=(pm->ps->velocity)[0]*(newspeed),(pm->ps->velocity)[1]=(pm->ps->velocity)[1]*(newspeed),(pm->ps->velocity)[2]=(pm->ps->velocity)[2]*(newspeed));
	}
	...
	((pm->ps->origin)[0]=(pm->ps->origin)[0]+(pm->ps->velocity)[0]*(pml.frametime),(pm->ps->origin)[1]=(pm->ps->origin)[1]+(pm->ps->velocity)[1]*(pml.frametime),(pm->ps->origin)[2]=(pm->ps->origin)[2]+(pm->ps->velocity)[2]*(pml.frametime));
}
...
{
	if ( pml.groundTrace.surfaceFlags & 0x00400000 ) 
	{
		return 0;
	}
	return ( pml.groundTrace.surfaceFlags & 0x1f );
}

static int PM_TryJK2Roll( void )
{
	trace_t	trace;
	int		anim = -1;
	vec3_t fwd, right, traceto, mins, maxs, fwdAngles;
	...
	((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[1]), (mins)[2]=(pm->mins[2]+18));
	((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[1]), (maxs)[2]=(16));
	...
	{ 
		if ( pm->ps->pm_flags & 16 ) 
		{
			anim = BOTH_ROLL_B;
			((traceto)[0]=(pm->ps->origin)[0]+(fwd)[0]*(-64),(traceto)[1]=(pm->ps->origin)[1]+(fwd)[1]*(-64),(traceto)[2]=(pm->ps->origin)[2]+(fwd)[2]*(-64));
		}
		else
		{
			anim = BOTH_ROLL_F;
	...
	{ 
		anim = BOTH_ROLL_R;
		((traceto)[0]=(pm->ps->origin)[0]+(right)[0]*(64),(traceto)[1]=(pm->ps->origin)[1]+(right)[1]*(64),(traceto)[2]=(pm->ps->origin)[2]+(right)[2]*(64));
	}
	else if ( pm->cmd.rightmove < 0 )
	{ 
		anim = BOTH_ROLL_L;
...
static int PM_TryRoll( void )
{
	...
	((mins)[0]=(pm->mins[0]), (mins)[1]=(pm->mins[1]), (mins)[2]=(pm->mins[2]+18));
	((maxs)[0]=(pm->maxs[0]), (maxs)[1]=(pm->maxs[1]), (maxs)[2]=(pm->ps->crouchheight));
...
static void PM_CrashLandEffect( void )
{
	...
	if ( delta >= 30 ) 
	{
		vec3_t bottom;
		int	effectID = -1;
		int material = (pml.groundTrace.surfaceFlags&0x1f);
		((bottom)[0]=(pm->ps->origin[0]), (bottom)[1]=(pm->ps->origin[1]), (bottom)[2]=(pm->ps->origin[2]+pm->mins[2]+1));
		switch ( material )
		{
		case 17:
...
#line 3968 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
static void PM_CrashLand( void ) {
	...
	PM_CrashLandEffect();
#line 4008 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		...
		pm->ps->legsAnim == BOTH_A7_KICK_L_AIR)
	{
		...
		if ( landAnim != -1 )
		{
			...
			{
				PM_SetAnim(1|2, landAnim, 1|2, 0);
			}
			else
			{
				PM_SetAnim(2, landAnim, 1|2, 0);
			}
		}
	}
	...
	if ( delta < 1 && level.clients[pm->ps->clientNum].sess.karma >= 0) {
		return;
	}else if (delta < 1){
		delta = 2;
	}
	...
	if ( pm->ps->pm_flags & 1 ) 
	{
			...
			pm->ps->forceHandExtend == HANDEXTEND_NONE )
		{
			...
			if (PM_InRollComplete(pm->ps, pm->ps->legsAnim))
			{
				...
				pm->ps->legsTimer = 0;
				pm->ps->legsAnim = 0;
				PM_SetAnim(1|2,BOTH_LAND1,1|2, 150);
				pm->ps->legsTimer = 130;
			}
	...
	if ( !(pml.groundTrace.surfaceFlags & 0x00040000) )  {
		if (delta > 7)
		{
...
static int PM_CorrectAllSolid( trace_t *trace ) {
	...
	for (i = -1; i <= 1; i++) {
		for (j = -1; j <= 1; j++) {
			for (k = -1; k <= 1; k++) {
				((point)[0]=(pm->ps->origin)[0],(point)[1]=(pm->ps->origin)[1],(point)[2]=(pm->ps->origin)[2]);
				point[0] += (float) i;
				point[1] += (float) j;
				point[2] += (float) k;
...
static void PM_GroundTraceMissed( void ) {
	...
	if ( pm->ps->pm_type == PM_FLOAT ) 
	{
		...
		PM_SetAnim(parts, BOTH_CHOKE3, 1, 100);
	}
	else if ( pm->ps->pm_type == PM_JETPACK ) 
	{
	...
	else if ( pm->ps->groundEntityNum != ((1<<10)-1) || (pm->ps->legsAnim) == BOTH_CHOKE3 ) 
	{
		...
		if ( trace.fraction == 1.0 || pm->ps->pm_type == PM_FLOAT ) {
			if ( pm->ps->velocity[2] <= 0 && !(pm->ps->pm_flags&2))
			{
				
				PM_SetAnim(2,BOTH_INAIR1,0, 100);
				pm->ps->pm_flags &= ~8;
			}
			else if ( pm->cmd.forwardmove >= 0 ) 
			{
				PM_SetAnim(2,BOTH_JUMP1,1, 100);
				pm->ps->pm_flags &= ~8;
			} 
			else 
			{
				PM_SetAnim(2,BOTH_JUMPBACK1,1, 100);
				pm->ps->pm_flags |= 8;
			}
	...
	{ 
		...
		PM_SetAnim(1|2,BOTH_INAIR1,1|2, 150);
		pm->ps->inAirAnim = qtrue;
	}
...
static void PM_GroundTrace( void ) {
	...
	if ( pm->ps->velocity[2] > 0 && ((pm->ps->velocity)[0]*(trace.plane.normal)[0]+(pm->ps->velocity)[1]*(trace.plane.normal)[1]+(pm->ps->velocity)[2]*(trace.plane.normal)[2]) > 10 ) {
		if ( pm->debugLevel ) {
...
#line 4537 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
static void PM_SetWaterLevel( void ) {
	...
	point[0] = pm->ps->origin[0];
	point[1] = pm->ps->origin[1];
	point[2] = pm->ps->origin[2] + -24 + 1;	
	cont = pm->pointcontents( point, pm->ps->clientNum );

	if ( cont & (0x00000004|0x00000002|0x00020000) ) {
		sample2 = pm->ps->viewheight - -24;
		sample1 = sample2 / 2;

		pm->watertype = cont;
		pm->waterlevel = 1;
		point[2] = pm->ps->origin[2] + -24 + sample1;
		cont = pm->pointcontents (point, pm->ps->clientNum );
		if ( cont & (0x00000004|0x00000002|0x00020000) ) {
			pm->waterlevel = 2;
			point[2] = pm->ps->origin[2] + -24 + sample2;
			cont = pm->pointcontents (point, pm->ps->clientNum );
			if ( cont & (0x00000004|0x00000002|0x00020000) ){
				pm->waterlevel = 3;
			}
...
{
	if ( (pm->ps->pm_flags&128) )
	{
		
		trace_t	trace;
		vec3_t end, curMins, curMaxs;
	
		((end)[0]=(pm->ps->origin[0]), (end)[1]=(pm->ps->origin[1]), (end)[2]=(pm->ps->origin[2]+-24)); 
		((curMins)[0]=(pm->mins[0]), (curMins)[1]=(pm->mins[1]), (curMins)[2]=(0)); 
		((curMaxs)[0]=(pm->maxs[0]), (curMaxs)[1]=(pm->maxs[1]), (curMaxs)[2]=(pm->ps->standheight)); 
		...
		if ( !trace.allsolid && !trace.startsolid )
		{
			...
			else
			{
				...
				float updist = ((1.0f-trace.fraction) * --24);
...
static void PM_CheckDuck (void)
{
	...
	if ( pm->ps->m_iVehicleNum > 0 && pm->ps->m_iVehicleNum < ((1<<10)-1) )
	{
			 ...
			 pm_entVeh->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL))
		{
			...
			{ 
				((pm->mins)[0]=(pm->mins)[1]=(pm->mins)[2]=0);
				((pm->maxs)[0]=(pm->maxs)[1]=(pm->maxs)[2]=0);
...
#line 4730 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
			}
		}
	}
	else
	{
		...
		if (pm->ps->pm_type == PM_DEAD && pm->ps->clientNum < 32)
		{
			pm->maxs[2] = -8;
			pm->ps->viewheight = -16;
			return;
	...
	if (pm->ps->pm_flags & 1)
	{
		...
		#line 4806 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"

		pm->maxs[2] = pm->ps->crouchheight;
		pm->ps->viewheight = (16+-4);
	...
	else
	{
		...
		gentity_t *self = &g_entities[pm->ps->clientNum];
		Jedi_Decloak( self );
		#line 4823 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		pm->maxs[2] = pm->ps->standheight;
		pm->ps->viewheight = (40+-4);
	}
}
...
qboolean PM_SwimmingAnim( int anim )
{
	switch ( anim )
	{
	case BOTH_SWIM_IDLE1:		
	case BOTH_SWIMFORWARD:		
	case BOTH_SWIMBACKWARD:		
		return qtrue;
		break;
	}
	return qfalse;
...
void PM_AnglesForSlope( const float yaw, const vec3_t slope, vec3_t angles )
{
	...
	((angles)[0]=(0), (angles)[1]=(yaw), (angles)[2]=(0));
	AngleVectors( angles, ovf, ovr, ((void *)0) );
	...
	angles[1] = 0;
	angles[0] = dot * pitch;
	angles[2] = ((1-Q_fabs(dot)) * pitch * mod);
}
...
void PM_FootSlopeTrace( float *pDiff, float *pInterval )
{
	...
	((footLOrg)[0]=(footLPoint)[0],(footLOrg)[1]=(footLPoint)[1],(footLOrg)[2]=(footLPoint)[2]);
	((footROrg)[0]=(footRPoint)[0],(footROrg)[1]=(footRPoint)[1],(footROrg)[2]=(footRPoint)[2]);

	
	footLOrg[2] = pm->ps->origin[2] + pm->mins[2] + 1;
	footROrg[2] = pm->ps->origin[2] + pm->mins[2] + 1;
	((footLBot)[0]=(footLOrg[0]), (footLBot)[1]=(footLOrg[1]), (footLBot)[2]=(footLOrg[2] - interval*10));
	((footRBot)[0]=(footROrg[0]), (footRBot)[1]=(footROrg[1]), (footRBot)[2]=(footROrg[2] - interval*10));

	
	((footMins)[0]=(-3), (footMins)[1]=(-3), (footMins)[2]=(0));
	((footMaxs)[0]=(3), (footMaxs)[1]=(3), (footMaxs)[2]=(1));

	pm->trace( &trace, footLOrg, footMins, footMaxs, footLBot, pm->ps->clientNum, pm->tracemask );
	((footLBot)[0]=(trace.endpos)[0],(footLBot)[1]=(trace.endpos)[1],(footLBot)[2]=(trace.endpos)[2]);
	((footLSlope)[0]=(trace.plane.normal)[0],(footLSlope)[1]=(trace.plane.normal)[1],(footLSlope)[2]=(trace.plane.normal)[2]);

	pm->trace( &trace, footROrg, footMins, footMaxs, footRBot, pm->ps->clientNum, pm->tracemask );
	((footRBot)[0]=(trace.endpos)[0],(footRBot)[1]=(trace.endpos)[1],(footRBot)[2]=(trace.endpos)[2]);
	((footRSlope)[0]=(trace.plane.normal)[0],(footRSlope)[1]=(trace.plane.normal)[1],(footRSlope)[2]=(trace.plane.normal)[2]);
...
qboolean PM_AdjustStandAnimForSlope( void )
{
	...
	{
		if ( destAnim > legsAnim && pm->ps->slopeRecalcTime < pm->cmd.serverTime )
		{
			legsAnim++;
			pm->ps->slopeRecalcTime = pm->cmd.serverTime + 100;
		}
		else if ( destAnim < legsAnim && pm->ps->slopeRecalcTime < pm->cmd.serverTime )
		{
			legsAnim--;
			pm->ps->slopeRecalcTime = pm->cmd.serverTime + 100;
		}
...
static void PM_Footsteps( void ) {
	...
	if (pm->ps->saberMove == LS_SPINATTACK)
	{
	...
	else if ( pm->ps->groundEntityNum == ((1<<10)-1) ) {
		...
		if (  pm->xyspeed < 5 ) {
			...
			{
				if ( (pm->ps->eFlags2&(1<<1)) )
				{
					PM_ContinueLegsAnim( BOTH_STAND4 );
					
				}
				else if ( (pm->ps->eFlags2&(1<<2)) )
				{
			...
			else if ( (pm->ps->pm_flags & 1) || (pm->ps->pm_flags & 4) ) {
				if ((pm->ps->legsAnim) != BOTH_CROUCH1IDLE)
				{
					PM_SetAnim(2, BOTH_CROUCH1IDLE, setAnimFlags, 100);
				}
				else
				{
	...
	else if ( pm->ps->pm_flags & 1 )
	{
		...
		if( ( g_canjk2roll.integer >= 1 && pm->cmd.buttons & 16)
		||
		
	( ( (PM_RunningAnim( pm->ps->legsAnim )
			&&VectorLengthSquared(pm->ps->velocity)>=30000) 
		...
		{ 
			if ( pm->ps->pm_flags & 16 ) {
				...
				{
					PM_SetAnim(2, BOTH_CROUCH1WALKBACK, setAnimFlags, 100);
				}
				else
				{
				...
				{
					PM_SetAnim(2, BOTH_CROUCH1WALK, setAnimFlags, 100);
				}
				else
				{
	...
	else
	{
		...
		if (desiredAnim != -1)
		{
			...
			{
				PM_SetAnim(2, desiredAnim, setAnimFlags, 100);
			}
			else
			{
...
#line 5984 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 5994 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 6007 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	...
	if ( impact_splash )
	{
		...
		((start)[0]=(pm->ps->origin)[0],(start)[1]=(pm->ps->origin)[1],(start)[2]=(pm->ps->origin)[2]);
		((end)[0]=(pm->ps->origin)[0],(end)[1]=(pm->ps->origin)[1],(end)[2]=(pm->ps->origin)[2]);

		
		start[2] += 10;
		end[2] -= 40;

		pm->trace( &tr, start, vec3_origin, vec3_origin, end, pm->ps->clientNum, (0x00000004|0x00000002|0x00020000) );

		if ( tr.fraction < 1.0f )
		{
			if ( (tr.contents&0x00000002) )
			{
				G_PlayEffect( EFFECT_LAVA_SPLASH, tr.endpos, tr.plane.normal );
			}
			else if ( (tr.contents&0x00020000) )
			{
...
#line 6044 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
void PM_BeginWeaponChange( int weapon ) {
	...
	PM_SetAnim(1, TORSO_DROPWEAP1, 1, 0);
...
void PM_FinishWeaponChange( void ) {
	...
	else
	{
		...
		PM_SetAnim(1, TORSO_RAISEWEAP1, 1, 0);
	}
	pm->ps->weapon = weapon;
...
#line 6143 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 6155 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	
	minAutoAimDist = Distance( entOrg, camPos ) + (bgEnt->m_pVehicle->m_pVehicleInfo->length/2.0f) + 200.0f;

	((newEnd)[0]=(end)[0],(newEnd)[1]=(end)[1],(newEnd)[2]=(end)[2]);
	((viewDir2End)[0]=(end)[0]-(camPos)[0],(viewDir2End)[1]=(end)[1]-(camPos)[1],(viewDir2End)[2]=(end)[2]-(camPos)[2]);
	VectorNormalize( viewDir2End );
	((extraEnd)[0]=(camPos)[0]+(viewDir2End)[0]*(20000.0f),(extraEnd)[1]=(camPos)[1]+(viewDir2End)[1]*(20000.0f),(extraEnd)[2]=(camPos)[2]+(viewDir2End)[2]*(20000.0f));


	trap_Trace( camTrace, camPos, vec3_origin, vec3_origin, extraEnd, 
		bgEnt->s.number, 0x00000001|0x00000100 );



#line 6170 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"

	if ( !camTrace->allsolid
		&& !camTrace->startsolid
		&& camTrace->fraction < 1.0f
		&& (camTrace->fraction*20000.0f) > minAutoAimDist 
		&& ((camTrace->fraction*20000.0f)-Distance( entOrg, camPos )) < bestDist )
	{
		((newEnd)[0]=(camTrace->endpos)[0],(newEnd)[1]=(camTrace->endpos)[1],(newEnd)[2]=(camTrace->endpos)[2]);
...
void PM_RocketLock( float lockDist, qboolean vehicleLock )
{
	...
	if ( vehicleLock )
	{
		...
		((muzzlePoint)[0]=(pm->ps->origin)[0],(muzzlePoint)[1]=(pm->ps->origin)[1],(muzzlePoint)[2]=(pm->ps->origin)[2]);
		((ang)[0]=(muzzlePoint)[0]+(forward)[0]*(lockDist),(ang)[1]=(muzzlePoint)[1]+(forward)[1]*(lockDist),(ang)[2]=(muzzlePoint)[2]+(forward)[2]*(lockDist));
	}
	else
	{
		AngleVectors( pm->ps->viewangles, forward, right, up );

		AngleVectors(pm->ps->viewangles, ang, ((void *)0), ((void *)0));

		((muzzlePoint)[0]=(pm->ps->origin)[0],(muzzlePoint)[1]=(pm->ps->origin)[1],(muzzlePoint)[2]=(pm->ps->origin)[2]);
		((muzzleOffPoint)[0]=(WP_MuzzlePoint[WP_ROCKET_LAUNCHER])[0],(muzzleOffPoint)[1]=(WP_MuzzlePoint[WP_ROCKET_LAUNCHER])[1],(muzzleOffPoint)[2]=(WP_MuzzlePoint[WP_ROCKET_LAUNCHER])[2]);
	...
	pm->trace(&tr, muzzlePoint, ((void *)0), ((void *)0), ang, pm->ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000));
...
#line 6449 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
				(void)( (!!(pm->ps->weapon > WP_NONE)) || (_wassert(L"pm->ps->weapon > WP_NONE", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 6449), 0) );
...
#line 6488 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 6526 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 6537 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"

		
		pm->cmd.buttons |= 128;
		pm->ps->eFlags |= ((1<<9)|(1<<10));
	}
...
int PM_ItemUsable(playerState_t *ps, int forcedUse)
{
	...
	case HI_SEEKER:
		...
		yawonly[2] = 0;
		yawonly[0] = 0;
		yawonly[1] = ps->viewangles[1];

		((mins)[0]=(-8), (mins)[1]=(-8), (mins)[2]=(0));
		((maxs)[0]=(8), (maxs)[1]=(8), (maxs)[2]=(24));

		AngleVectors(yawonly, fwd, ((void *)0), ((void *)0));
		...
		pm->trace(&tr, ps->origin, mins, maxs, trtest, ps->clientNum, (0x00000001|0x00000010|0x00000100|0x00001000));
	...
	case HI_SHIELD:
		...
		AngleVectors (ps->viewangles, fwd, ((void *)0), ((void *)0));
		fwd[2] = 0;
		((dest)[0]=(ps->origin)[0]+(fwd)[0]*(64),(dest)[1]=(ps->origin)[1]+(fwd)[1]*(64),(dest)[2]=(ps->origin)[2]+(fwd)[2]*(64));
		pm->trace(&tr, ps->origin, mins, maxs, dest, ps->clientNum, (0x00000001|0x00000100|0x00000200|0x00001000) );
		if (tr.fraction > 0.9 && !tr.startsolid && !tr.allsolid)
		{
			((pos)[0]=(tr.endpos)[0],(pos)[1]=(tr.endpos)[1],(pos)[2]=(tr.endpos)[2]);
			((dest)[0]=(pos[0]), (dest)[1]=(pos[1]), (dest)[2]=(pos[2] - 512));
			pm->trace( &tr, pos, mins, maxs, dest, ps->clientNum, (0x00000001|0x00001000) );
...
backAgain:
	...
	{ 
		...
		PM_SetAnim(1|2, Anim, iFlags, iBlend);
	}
}
...
static void PM_Weapon( void )
{
	...
	int amount;
	int		killAfterItem = 0;
	bgEntity_t *veh = ((void *)0);
	qboolean vehicleRocketLock = qfalse;
	gentity_t *gent = (gentity_t *)pm_entSelf;

	if (gent->client->grappleState > 10 && gent->client->grappleState <= 20 ){
		return;
	}
	if (pm->ps->brokenLimbs == -10){
		return;
...
#line 7044 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		}
	}
	...
	{ 
		...
		pm->ps->legsAnim = 987;
		pm->ps->legsTimer = level.time + 1000;
	...
	{ 
		...
		case HANDEXTEND_KNOCKDOWN:
			if (pm->ps->forceDodgeAnim)
			{
				...
				{ 
					int originalDAnim = pm->ps->forceDodgeAnim-8; 
					if (originalDAnim == 2)
					{
		...
		if (!seperateOnTorso)
		{ 
			PM_SetAnim(1, desiredAnim, 1|2, 100);
			pm->ps->torsoTimer = 1;
		}

		if (playFullBody)
		{ 
			PM_SetAnim(1|2, desiredAnim, 1|2, 100);
			...
			(pm->ps->forceHandExtend == HANDEXTEND_CHOKE && pm->ps->groundEntityNum == ((1<<10)-1)) )
		{ 
			if (seperateOnTorso)
			{
				PM_SetAnim(2, desiredAnim, 1|2, 100);
				pm->ps->legsTimer = 1;

				PM_SetAnim(1, desiredOnTorso, 1|2, 100);
				pm->ps->torsoTimer = 1;
			}
			else
			{
	...
	{		
			...
			pm->ps->weapon == WP_DET_PACK)
		{
			...
			{	
				if ((pm->ps->torsoAnim) == WeaponAttackAnim[pm->ps->weapon] &&
					(pm->ps->weaponTime-700) <= 0 && !gent->client->grappleState)
				{
					PM_StartTorsoAnim( WeaponReadyAnim[pm->ps->weapon] );
				}
			}
	...
	if ( pm->ps->clientNum < 32 && pm->ps->persistant[PERS_TEAM] == TEAM_SPECTATOR ) {
			return;
	}
	...
	{
		if ( pm->ps->clientNum < 32 && pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] != -1 )
		{
		...
		if (PM_CanSetWeaponAnims()){
			...
			else
			{
				...
				else
				{
					...
					else if (!gent->client->grappleState)
					{
						PM_StartTorsoAnim( WeaponReadyAnim[pm->ps->weapon] );
					}
				}
	...
	if (PM_CanSetWeaponAnims() && !PM_IsRocketTrooper() &&
		...
		pm->ps->weapon != WP_EMPLACED_GUN && !gent->client->grappleState)
	{
		PM_StartTorsoAnim( WeaponReadyAnim[pm->ps->weapon] );
	}
		...
		(pm->ps->weapon != WP_DISRUPTOR || pm->ps->zoomMode != 1))
	{
		...
		else if (PM_CanSetWeaponAnims() && !gent->client->grappleState)
		{
			PM_StartTorsoAnim( WeaponReadyAnim[pm->ps->weapon] );
		}
	}
...
#line 7754 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 7839 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 7851 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
#line 7852 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
			}
			...
			{ 
					...
					!BG_KickingAnim(pm->ps->legsAnim))
				{
					...
					if (kickMove != -1)
					{
						...
						ent = &g_entities[pm->ps->clientNum];
						#line 7875 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
				...
				if (pm->ps->torsoAnim != pm->ps->legsAnim)
				{
					PM_SetAnim(1|2, pm->ps->legsAnim, 1|2, 0);
				}
				pm->ps->weaponTime = 0;
				return;
			}
			...
			{ 
				...
				if (pm->ps->torsoAnim == BOTH_MELEE2)
				{
					desTAnim = BOTH_PULL_IMPALE_SWING;
				}
				if (pm->ps->torsoAnim == BOTH_PULL_IMPALE_SWING)
				{
					if (Q_irand(0,10) < 3){
					desTAnim = BOTH_THERMAL_THROW;
					}else{
					desTAnim = BOTH_THERMAL_READY;
					}
				}
...
#line 8186 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	}
}
...
#line 8228 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 8249 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 8251 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
...
#line 8326 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		else
		{
			if ( i == 0 ) {
		...
		ps->viewangles[i] = ((temp)*(360.0/65536));
	}
}
...

{
	...
	{
		if ( !(pm->ps->eFlags & (1<<10)) && (pm->cmd.buttons & 128) 
)
		{
		...
		else if ( !(pm->cmd.buttons & 128 ) && pm->ps->zoomLockTime < pm->cmd.serverTime)
		{
		...
		else if (!(pm->ps->eFlags & (1<<10)) && (pm->cmd.buttons & 128) &&
	...
	if ( pm->ps->weapon == WP_DISRUPTOR)
	{
		if ( pm->cmd.buttons & 1 && pm->ps->zoomMode == 1 && pm->ps->zoomLocked)
		{
			
			pm->cmd.buttons |= 128;
			pm->ps->eFlags |= (1<<10);
		}
		else if ( pm->cmd.buttons & 128 && pm->ps->zoomMode == 1 && pm->ps->zoomLocked)
		{
			pm->cmd.buttons &= ~128;
			pm->ps->eFlags &= ~(1<<10);
		}
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime)
{
	...
	if ( cmd->forwardmove < 0 && !(cmd->buttons&16) && pm->ps->groundEntityNum != ((1<<10)-1) )
	{
					 ...
					 vec3_t origin, vec3_t angles, vec3_t scale, int blendTime, qboolean forceHalt)
{
	...
	(void)( (!!(bgHumanoidAnimations[basePose].firstFrame > 0)) || (_wassert(L"bgHumanoidAnimations[basePose].firstFrame > 0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 9038), 0) );
	...
	if (!*ikInProgress && !forceHalt)
	{
		...
		((ikP.pcjMins)[0]=(0), (ikP.pcjMins)[1]=(0), (ikP.pcjMins)[2]=(0));
		((ikP.pcjMaxs)[0]=(0), (ikP.pcjMaxs)[1]=(0), (ikP.pcjMaxs)[2]=(0));

		
		ikP.blendTime = blendTime;
		((ikP.origin)[0]=(origin)[0],(ikP.origin)[1]=(origin)[1],(ikP.origin)[2]=(origin)[2]);
		((ikP.angles)[0]=(angles)[0],(ikP.angles)[1]=(angles)[1],(ikP.angles)[2]=(angles)[2]);
		ikP.angles[0] = 0;
		ikP.pcjOverrides = 0;
		ikP.radius = 10.0f;
		((ikP.scale)[0]=(scale)[0],(ikP.scale)[1]=(scale)[1],(ikP.scale)[2]=(scale)[2]);
		...
		ikP.forceAnimOnBone = qfalse; 

		
		
		
		if (!strap_G2API_SetBoneIKState(ghoul2, time, ((void *)0), IKS_DYNAMIC, &ikP))
		{
			(void)( (!!(!"Failed to init IK system for g2 instance!")) || (_wassert(L"!\"Failed to init IK system for g2 instance!\"", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 9075), 0) );
		}
		...
		if (strap_G2API_SetBoneIKState(ghoul2, time, "lhumerus", IKS_DYNAMIC, &ikP))
		{
			...
			((ikP.pcjMins)[0]=(-90.0f), (ikP.pcjMins)[1]=(-20.0f), (ikP.pcjMins)[2]=(-20.0f));
			((ikP.pcjMaxs)[0]=(30.0f), (ikP.pcjMaxs)[1]=(20.0f), (ikP.pcjMaxs)[2]=(-20.0f));
	...
	{ 
		...
		((ikM.desiredOrigin)[0]=(desiredPos)[0],(ikM.desiredOrigin)[1]=(desiredPos)[1],(ikM.desiredOrigin)[2]=(desiredPos)[2]); 

		((tAngles)[0]=(angles)[0],(tAngles)[1]=(angles)[1],(tAngles)[2]=(angles)[2]);
		...
		((torg)[0]=(lHand)[0]-(desiredPos)[0],(torg)[1]=(lHand)[1]-(desiredPos)[1],(torg)[2]=(lHand)[2]-(desiredPos)[2]);
		...
		((ikM.origin)[0]=(origin)[0],(ikM.origin)[1]=(origin)[1],(ikM.origin)[2]=(origin)[2]); 

		ikM.boneName[0] = 0;
		if (strap_G2API_IKMove(ghoul2, time, &ikM))
		{
			
			((tuParms.angles)[0]=(angles)[0],(tuParms.angles)[1]=(angles)[1],(tuParms.angles)[2]=(angles)[2]);
			tuParms.angles[0] = 0;

			((tuParms.position)[0]=(origin)[0],(tuParms.position)[1]=(origin)[1],(tuParms.position)[2]=(origin)[2]);
			((tuParms.scale)[0]=(scale)[0],(tuParms.scale)[1]=(scale)[1],(tuParms.scale)[2]=(scale)[2]);

			tuParms.me = ent->number;
			((tuParms.velocity)[0]=(tuParms.velocity)[1]=(tuParms.velocity)[2]=0);
	...
	{ 
		...
		strap_G2API_SetBoneIKState(ghoul2, time, "lhumerus", IKS_NONE, ((void *)0));
		strap_G2API_SetBoneIKState(ghoul2, time, "lradius", IKS_NONE, ((void *)0));
		
		
		strap_G2API_SetBoneAngles(ghoul2, 0, "lhumerus", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, ((void *)0), 0, time);
		strap_G2API_SetBoneAngles(ghoul2, 0, "lradius", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, ((void *)0), 0, time);
		...
		strap_G2API_SetBoneIKState(ghoul2, time, ((void *)0), IKS_NONE, ((void *)0));
...
void BG_UpdateLookAngles( int lookingDebounceTime, vec3_t lastHeadAngles, int time, vec3_t lookAngles, float lookSpeed, float minPitch, float maxPitch, float minYaw, float maxYaw, float minRoll, float maxRoll )
{
	...
	if ( lookingDebounceTime > time )
	{
		...
		if ( lookAngles[0] > maxPitch )
		{
			lookAngles[0] = maxPitch;
		}
		else if ( lookAngles[0] < minPitch )
		{
			lookAngles[0] = minPitch;
		}
		if ( lookAngles[1] > maxYaw )
		{
			lookAngles[1] = maxYaw;
		}
		else if ( lookAngles[1] < minYaw )
		{
			lookAngles[1] = minYaw;
		}
		if ( lookAngles[2] > maxRoll )
		{
			lookAngles[2] = maxRoll;
		}
		else if ( lookAngles[2] < minRoll )
		{
			lookAngles[2] = minRoll;
		}

		
		
		((oldLookAngles)[0]=(lastHeadAngles)[0],(oldLookAngles)[1]=(lastHeadAngles)[1],(oldLookAngles)[2]=(lastHeadAngles)[2]);
		((lookAnglesDiff)[0]=(lookAngles)[0]-(oldLookAngles)[0],(lookAnglesDiff)[1]=(lookAngles)[1]-(oldLookAngles)[1],(lookAnglesDiff)[2]=(lookAngles)[2]-(oldLookAngles)[2]);
		...
		{
			lookAngles[0] = AngleNormalize180( oldLookAngles[0]+(lookAnglesDiff[0]*fFrameInter*lookSpeed) );
			lookAngles[1] = AngleNormalize180( oldLookAngles[1]+(lookAnglesDiff[1]*fFrameInter*lookSpeed) );
			lookAngles[2] = AngleNormalize180( oldLookAngles[2]+(lookAnglesDiff[2]*fFrameInter*lookSpeed) );
		}
	}
	
	((lastHeadAngles)[0]=(lookAngles)[0],(lastHeadAngles)[1]=(lookAngles)[1],(lastHeadAngles)[2]=(lookAngles)[2]);
}
...
{
	vec3_t	lA;
	((lA)[0]=(lookAngles)[0],(lA)[1]=(lookAngles)[1],(lA)[2]=(lookAngles)[2]);
	
	if ( lA[0] < headClampMinAngles[0] )
	{
		lA[0] = headClampMinAngles[0];
	}
	else if ( lA[0] > headClampMaxAngles[0] )
	{
		lA[0] = headClampMaxAngles[0];
	}

	if ( lA[1] < headClampMinAngles[1] )
	{
		lA[1] = headClampMinAngles[1];
	}
	else if ( lA[1] > headClampMaxAngles[1] )
	{
		lA[1] = headClampMaxAngles[1];
	}

	if ( lA[2] < headClampMinAngles[2] )
	{
		lA[2] = headClampMinAngles[2];
	}
	else if ( lA[2] > headClampMaxAngles[2] )
	{
		lA[2] = headClampMaxAngles[2];
	}

	
	if ( thoracicAngles[0] )
	{
		thoracicAngles[0] = (thoracicAngles[0] + (lA[0] * 0.4)) * 0.5f;
	}
	else
	{
		thoracicAngles[0] = lA[0] * 0.4;
	}
	if ( thoracicAngles[1] )
	{
		thoracicAngles[1] = (thoracicAngles[1] + (lA[1] * 0.1)) * 0.5f;
	}
	else
	{
		thoracicAngles[1] = lA[1] * 0.1;
	}
	if ( thoracicAngles[2] )
	{
		thoracicAngles[2] = (thoracicAngles[2] + (lA[2] * 0.1)) * 0.5f;
	}
	else
	{
		thoracicAngles[2] = lA[2] * 0.1;
	}

	neckAngles[0] = lA[0] * 0.2f;
	neckAngles[1] = lA[1] * 0.3f;
	neckAngles[2] = lA[2] * 0.3f;

	headAngles[0] = lA[0] * 0.4;
	headAngles[1] = lA[1] * 0.6;
	headAngles[2] = lA[2] * 0.6;
	...
	strap_G2API_SetBoneAngles(ghoul2, 0, "cranium", headAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
	strap_G2API_SetBoneAngles(ghoul2, 0, "cervical", neckAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
	strap_G2API_SetBoneAngles(ghoul2, 0, "thoracic", thoracicAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
}
							...
							vec3_t ulAngles, vec3_t llAngles, vec3_t modelScale, float *tPitchAngle, float *tYawAngle, int *corrTime )
{
	...
	viewAngles[1] = AngleDelta( cent_lerpAngles[1], angles[1] );
...
#line 9395 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	...
	thoracicAngles[0] = viewAngles[0]*0.20f;
	llAngles[0] = viewAngles[0]*0.40f;
	ulAngles[0] = viewAngles[0]*0.40f;

	thoracicAngles[1] = viewAngles[1]*0.20f;
	ulAngles[1] = viewAngles[1]*0.35f;
	llAngles[1] = viewAngles[1]*0.45f;

	thoracicAngles[2] = viewAngles[2]*0.20f;
	ulAngles[2] = viewAngles[2]*0.35f;
	llAngles[2] = viewAngles[2]*0.45f;
}
...
extern qboolean BG_SaberLockBreakAnim( int anim ); 
void BG_G2PlayerAngles(void *ghoul2, int motionBolt, entityState_t *cent, int time, vec3_t cent_lerpOrigin,
					   vec3_t cent_lerpAngles, vec3_t legs[3], vec3_t legsAngles, qboolean *tYawing,
					   qboolean *tPitching, qboolean *lYawing, float *tYawAngle, float *tPitchAngle,
					   float *lYawAngle, int frametime, vec3_t turAngles, vec3_t modelScale, int ciLegs,
					   ...
					   entityState_t *emplaced, int *crazySmoothFactor)
{
	...
	{ 
		...
		((forcedAngles)[0]=(forcedAngles)[1]=(forcedAngles)[2]=0);
		forcedAngles[1] = cent_lerpAngles[1];
		forcedAngles[2] = cent_lerpAngles[2];
		AnglesToAxis( forcedAngles, legs );
		((legsAngles)[0]=(forcedAngles)[0],(legsAngles)[1]=(forcedAngles)[1],(legsAngles)[2]=(forcedAngles)[2]);

		if (cent->number < 32)
		{
			strap_G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
			strap_G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
			strap_G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
			strap_G2API_SetBoneAngles(ghoul2, 0, "thoracic", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
			strap_G2API_SetBoneAngles(ghoul2, 0, "cervical", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
		}
		return;
	}
	...
	((headAngles)[0]=(cent_lerpAngles)[0],(headAngles)[1]=(cent_lerpAngles)[1],(headAngles)[2]=(cent_lerpAngles)[2]);
	headAngles[1] = AngleMod( headAngles[1] );
	((legsAngles)[0]=(legsAngles)[1]=(legsAngles)[2]=0);
	((torsoAngles)[0]=(torsoAngles)[1]=(torsoAngles)[2]=0);
	...
	*tYawAngle = torsoAngles[1];

	

	((velocity)[0]=(cent->pos.trDelta)[0],(velocity)[1]=(cent->pos.trDelta)[1],(velocity)[2]=(cent->pos.trDelta)[2]);
	...
	if ( speed ) {
		...
		side = speed * ((velocity)[0]*(axis[1])[0]+(velocity)[1]*(axis[1])[1]+(velocity)[2]*(axis[1])[2]);
		legsAngles[2] -= side;

		side = speed * ((velocity)[0]*(axis[0])[0]+(velocity)[1]*(axis[0])[1]+(velocity)[2]*(axis[0])[2]);
		legsAngles[0] += side;
	}
	...
	if ( cent->groundEntityNum == ((1<<10)-1) ||
		 cent->forceFrame ||
		 (cent->weapon == WP_EMPLACED_GUN && emplaced) )
	{ 
		((velPos)[0]=(cent_lerpOrigin)[0],(velPos)[1]=(cent_lerpOrigin)[1],(velPos)[2]=(cent_lerpOrigin)[2]);
	}

	((velAng)[0]=(cent_lerpOrigin)[0]-(velPos)[0],(velAng)[1]=(cent_lerpOrigin)[1]-(velPos)[1],(velAng)[2]=(cent_lerpOrigin)[2]-(velPos)[2]);

	if (!VectorCompare(velAng, vec3_origin))
	{
		vectoangles(velAng, velAng);

		if (velAng[1] <= legsAngles[1])
		{
			degrees_negative = (legsAngles[1] - velAng[1]);
			degrees_positive = (360 - legsAngles[1]) + velAng[1];
		}
		else
		{
			degrees_negative = legsAngles[1] + (360 - velAng[1]);
			degrees_positive = (velAng[1] - legsAngles[1]);
		}
	...
	if (cent->m_iVehicleNum)
	{ 
		*lYawAngle = legsAngles[1];
	}
	else
	{
		BG_SwingAngles( legsAngles[1], 0, 90, 0.65, lYawAngle, lYawing, frametime );
	}
	legsAngles[1] = *lYawAngle;
	...
	{ 
		...
		((legsAngles)[0]=(legsAngles)[1]=(legsAngles)[2]=0);
		legsAngles[1] = cent_lerpAngles[1];
	}
	...
	((turAngles)[0]=(legsAngles)[0],(turAngles)[1]=(legsAngles)[1],(turAngles)[2]=(legsAngles)[2]);

	AnglesToAxis( legsAngles, legs );

	((viewAngles)[0]=(cent_lerpAngles)[0],(viewAngles)[1]=(cent_lerpAngles)[1],(viewAngles)[2]=(cent_lerpAngles)[2]);
	viewAngles[1] = viewAngles[2] = 0;
	viewAngles[0] *= 0.5;

	((angles)[0]=(0), (angles)[1]=(legsAngles[1]), (angles)[2]=(0));
	...
	{ 
		...
		((facingAngles)[0]=(emplaced->pos.trBase)[0]-(cent_lerpOrigin)[0],(facingAngles)[1]=(emplaced->pos.trBase)[1]-(cent_lerpOrigin)[1],(facingAngles)[2]=(emplaced->pos.trBase)[2]-(cent_lerpOrigin)[2]);
		vectoangles(facingAngles, facingAngles);

		if (emplaced->weapon == WP_NONE)
		{ 
			((legsAngles)[0]=(facingAngles)[0],(legsAngles)[1]=(facingAngles)[1],(legsAngles)[2]=(facingAngles)[2]);
			AnglesToAxis( legsAngles, legs );
		}
		else
		{ 
			float dif = AngleSubtract(cent_lerpAngles[1], facingAngles[1]);
			...
			((facingAngles)[0]=(-16.0f), (facingAngles)[1]=(-dif), (facingAngles)[2]=(0.0f));
			...
			{ 
				...
				strap_G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", llAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
				strap_G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", ulAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
				strap_G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 

				((facingAngles)[0]=(facingAngles)[0]+(thoracicAngles)[0],(facingAngles)[1]=(facingAngles)[1]+(thoracicAngles)[1],(facingAngles)[2]=(facingAngles)[2]+(thoracicAngles)[2]);

				if (cent->legsAnim == BOTH_STRAFE_LEFT1)
				{ 
					facingAngles[1] -= 32.0f;
				}
			}
			else
			{
			...
			((facingAngles)[0]=(facingAngles)[0]*(0.6f),(facingAngles)[1]=(facingAngles)[1]*(0.6f),(facingAngles)[2]=(facingAngles)[2]*(0.6f));
			strap_G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", vec3_origin, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
			((facingAngles)[0]=(facingAngles)[0]*(0.8f),(facingAngles)[1]=(facingAngles)[1]*(0.8f),(facingAngles)[2]=(facingAngles)[2]*(0.8f));
			strap_G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", facingAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
			((facingAngles)[0]=(facingAngles)[0]*(0.8f),(facingAngles)[1]=(facingAngles)[1]*(0.8f),(facingAngles)[2]=(facingAngles)[2]*(0.8f));
			strap_G2API_SetBoneAngles(ghoul2, 0, "thoracic", facingAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 

			
			((facingAngles)[0]=(0.0f), (facingAngles)[1]=(dif), (facingAngles)[2]=(0.0f));
			((facingAngles)[0]=(facingAngles)[0]*(0.6f),(facingAngles)[1]=(facingAngles)[1]*(0.6f),(facingAngles)[2]=(facingAngles)[2]*(0.6f));
			strap_G2API_SetBoneAngles(ghoul2, 0, "cervical", facingAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
	...
	((eyeAngles)[0]=(cent_lerpAngles)[0],(eyeAngles)[1]=(cent_lerpAngles)[1],(eyeAngles)[2]=(cent_lerpAngles)[2]);
...
void BG_G2ATSTAngles(void *ghoul2, int time, vec3_t cent_lerpAngles )
{
	strap_G2API_SetBoneAngles(ghoul2, 0, "thoracic", cent_lerpAngles, 0x0002, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time); 
}
...
static __inline void PM_CmdForSaberMoves(usercmd_t *ucmd)
#line 9925 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
{
			...
			&& pm->ps->saberMove == LS_BUTTERFLY_LEFT ) )
	{
		...
		{ 
			if ( pm->ps->legsTimer >= 100 
				&& (aLen - pm->ps->legsTimer) >= 250 ) 
			{ 
				
				ucmd->forwardmove = 127;
			}

			if ( (pm->ps->legsTimer >= 900 
					&& aLen - pm->ps->legsTimer >= 950 ) 
				|| ( pm->ps->legsTimer >= 1600 
					&& aLen - pm->ps->legsTimer >= 400 ) ) 
			{ 
				if ( pm->ps->groundEntityNum != ((1<<10)-1) )
				{ 
					if ( pm->ps->groundEntityNum >= 32 )
					{
		...
		pm->ps->legsAnim == BOTH_JUMPATTACK7)
	{
			...
			&& aLen - pm->ps->legsTimer >= 400 )
		{
			if ( pm->ps->groundEntityNum != ((1<<10)-1) )
			{
				...
				((yawAngles)[0]=(0), (yawAngles)[1]=(pm->ps->viewangles[1]+180), (yawAngles)[2]=(0));
				AngleVectors( yawAngles, backDir, 0, 0 );
				((pm->ps->velocity)[0]=(backDir)[0]*(100),(pm->ps->velocity)[1]=(backDir)[1]*(100),(pm->ps->velocity)[2]=(backDir)[2]*(100));
...
#line 10108 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		{
			setAngles = qtrue;
			clampMin[0] = -pVeh->m_pVehicleInfo->lookPitch;
			clampMax[0] = pVeh->m_pVehicleInfo->lookPitch;
			clampMin[1] = clampMax[1] = 0;
			clampMin[2] = clampMax[2] = -1;
		}
	}
	else 
	{
		...
		for ( i = 0; i < 2; i++ )
		{
			...
			{
				setAngles = qtrue;
				clampMin[0] = veh->m_pVehicle->m_pVehicleInfo->turret[i].pitchClampUp;
				clampMax[0] = veh->m_pVehicle->m_pVehicleInfo->turret[i].pitchClampDown;
				clampMin[1] = veh->m_pVehicle->m_pVehicleInfo->turret[i].yawClampRight;
				clampMax[1] = veh->m_pVehicle->m_pVehicleInfo->turret[i].yawClampLeft;
				clampMin[2] = clampMax[2] = 0;
				break;
			}
		}
	}
...
void PM_VehForcedTurning(bgEntity_t *veh)
{
	...
	((dir)[0]=(dst->s.origin)[0]-(veh->playerState->origin)[0],(dir)[1]=(dst->s.origin)[1]-(veh->playerState->origin)[1],(dir)[2]=(dst->s.origin)[2]-(veh->playerState->origin)[2]);
	vectoangles(dir, dir);

	yawD = AngleSubtract(pm->ps->viewangles[1], dir[1]);
	pitchD = AngleSubtract(pm->ps->viewangles[0], dir[0]);
	...
	pm->ps->viewangles[1] = AngleSubtract(pm->ps->viewangles[1], yawD);
	pm->ps->viewangles[0] = AngleSubtract(pm->ps->viewangles[0], pitchD);

	PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
#line 10276 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
}
...
void PM_VehFaceHyperspacePoint(bgEntity_t *veh)
{
	...
	{
		float timeFrac = ((float)(pm->cmd.serverTime-veh->playerState->hyperSpaceTime))/4000;
		...
		if ( timeFrac < 0.75f )
		{
			...
			else if ( !(veh->playerState->eFlags2&(1<<5)))
			{
				veh->playerState->eFlags2 |= (1<<5);
			}
		}
	}
}

#line 10438 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
										...
										void (*localTrace)(trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask))
{
	...
	{
		((maxs)[0]=(veh->m_pVehicleInfo->width/2.0f), (maxs)[1]=(veh->m_pVehicleInfo->width/2.0f), (maxs)[2]=(veh->m_pVehicleInfo->height+-24));
		((mins)[0]=(veh->m_pVehicleInfo->width/-2.0f), (mins)[1]=(veh->m_pVehicleInfo->width/-2.0f), (mins)[2]=(-24));
		return;
	}
	else
	{
		...
		((point[0])[0]=(origin)[0]+(axis[0])[0]*(veh->m_pVehicleInfo->length/2.0f),(point[0])[1]=(origin)[1]+(axis[0])[1]*(veh->m_pVehicleInfo->length/2.0f),(point[0])[2]=(origin)[2]+(axis[0])[2]*(veh->m_pVehicleInfo->length/2.0f));
		((point[1])[0]=(origin)[0]+(axis[0])[0]*(-veh->m_pVehicleInfo->length/2.0f),(point[1])[1]=(origin)[1]+(axis[0])[1]*(-veh->m_pVehicleInfo->length/2.0f),(point[1])[2]=(origin)[2]+(axis[0])[2]*(-veh->m_pVehicleInfo->length/2.0f));
		
		((point[0])[0]=(point[0])[0]+(axis[2])[0]*(veh->m_pVehicleInfo->height/2.0f),(point[0])[1]=(point[0])[1]+(axis[2])[1]*(veh->m_pVehicleInfo->height/2.0f),(point[0])[2]=(point[0])[2]+(axis[2])[2]*(veh->m_pVehicleInfo->height/2.0f));
		((point[2])[0]=(point[0])[0]+(axis[2])[0]*(-veh->m_pVehicleInfo->height),(point[2])[1]=(point[0])[1]+(axis[2])[1]*(-veh->m_pVehicleInfo->height),(point[2])[2]=(point[0])[2]+(axis[2])[2]*(-veh->m_pVehicleInfo->height));
		((point[1])[0]=(point[1])[0]+(axis[2])[0]*(veh->m_pVehicleInfo->height/2.0f),(point[1])[1]=(point[1])[1]+(axis[2])[1]*(veh->m_pVehicleInfo->height/2.0f),(point[1])[2]=(point[1])[2]+(axis[2])[2]*(veh->m_pVehicleInfo->height/2.0f));
		((point[3])[0]=(point[1])[0]+(axis[2])[0]*(-veh->m_pVehicleInfo->height),(point[3])[1]=(point[1])[1]+(axis[2])[1]*(-veh->m_pVehicleInfo->height),(point[3])[2]=(point[1])[2]+(axis[2])[2]*(-veh->m_pVehicleInfo->height));

		((point[4])[0]=(origin)[0]+(axis[1])[0]*(veh->m_pVehicleInfo->width/2.0f),(point[4])[1]=(origin)[1]+(axis[1])[1]*(veh->m_pVehicleInfo->width/2.0f),(point[4])[2]=(origin)[2]+(axis[1])[2]*(veh->m_pVehicleInfo->width/2.0f));
		((point[5])[0]=(origin)[0]+(axis[1])[0]*(-veh->m_pVehicleInfo->width/2.0f),(point[5])[1]=(origin)[1]+(axis[1])[1]*(-veh->m_pVehicleInfo->width/2.0f),(point[5])[2]=(origin)[2]+(axis[1])[2]*(-veh->m_pVehicleInfo->width/2.0f));
		
		((point[4])[0]=(point[4])[0]+(axis[2])[0]*(veh->m_pVehicleInfo->height/2.0f),(point[4])[1]=(point[4])[1]+(axis[2])[1]*(veh->m_pVehicleInfo->height/2.0f),(point[4])[2]=(point[4])[2]+(axis[2])[2]*(veh->m_pVehicleInfo->height/2.0f));
		((point[6])[0]=(point[4])[0]+(axis[2])[0]*(-veh->m_pVehicleInfo->height),(point[6])[1]=(point[4])[1]+(axis[2])[1]*(-veh->m_pVehicleInfo->height),(point[6])[2]=(point[4])[2]+(axis[2])[2]*(-veh->m_pVehicleInfo->height));
		((point[5])[0]=(point[5])[0]+(axis[2])[0]*(veh->m_pVehicleInfo->height/2.0f),(point[5])[1]=(point[5])[1]+(axis[2])[1]*(veh->m_pVehicleInfo->height/2.0f),(point[5])[2]=(point[5])[2]+(axis[2])[2]*(veh->m_pVehicleInfo->height/2.0f));
		((point[7])[0]=(point[5])[0]+(axis[2])[0]*(-veh->m_pVehicleInfo->height),(point[7])[1]=(point[5])[1]+(axis[2])[1]*(-veh->m_pVehicleInfo->height),(point[7])[2]=(point[5])[2]+(axis[2])[2]*(-veh->m_pVehicleInfo->height));
		



		
		((newMins)[0]=(origin)[0],(newMins)[1]=(origin)[1],(newMins)[2]=(origin)[2]);
		((newMaxs)[0]=(origin)[0],(newMaxs)[1]=(origin)[1],(newMaxs)[2]=(origin)[2]);
		...
		((newMins)[0]=(newMins)[0]-(origin)[0],(newMins)[1]=(newMins)[1]-(origin)[1],(newMins)[2]=(newMins)[2]-(origin)[2]);
		((newMaxs)[0]=(newMaxs)[0]-(origin)[0],(newMaxs)[1]=(newMaxs)[1]-(origin)[1],(newMaxs)[2]=(newMaxs)[2]-(origin)[2]);
		...
		{
			((mins)[0]=(newMins)[0],(mins)[1]=(newMins)[1],(mins)[2]=(newMins)[2]);
			((maxs)[0]=(newMaxs)[0],(maxs)[1]=(newMaxs)[1],(maxs)[2]=(newMaxs)[2]);
		}
...
void PM_MoveForKata(usercmd_t *ucmd)
{
	if ( pm->ps->legsAnim == BOTH_A7_SOULCAL
		&& pm->ps->saberMove == LS_STAFF_SOULCAL && pm->ps->legsAnim != BOTH_KYLE_PA_1 && pm->ps->legsAnim != BOTH_KYLE_PA_2 && pm->ps->legsAnim != BOTH_KYLE_PA_3 )
	{
		ucmd->upmove = 0;
...
void PmoveSingle (pmove_t *pmove) {
		...
		pm->ps->saberMove == LS_DUAL_LR || pm->ps->saberMove == LS_DUAL_FB)
	{
		...
		{ 
			...
			{
				pm->ps->viewangles[1] += pml.frametime*240.0f;
				PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
			}
		}
		stiffenedUp = qtrue;
		...
		&& BG_FullBodyTauntAnim( pm->ps->torsoAnim ) )
	{
			...
			|| pm->cmd.upmove )
		{
			...
			{
				PM_SetAnim( 1|2, BOTH_MEDITATE_END, 1|2, 0 );
			}
			else
			{
				pm->ps->legsTimer = pm->ps->torsoTimer = 0;
...
#line 10860 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	...
	{ 
		...
		{ 
			pm->ps->weaponstate = WEAPON_READY;
			pm->ps->weaponTime = 1000;
			PM_AddEventWithParm(EV_WEAPON_CHARGE, WP_DISRUPTOR); 
			pm->cmd.upmove = 0;
		}
	}
	else if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
	...
	((pml.previous_origin)[0]=(pm->ps->origin)[0],(pml.previous_origin)[1]=(pm->ps->origin)[1],(pml.previous_origin)[2]=(pm->ps->origin)[2]);

	
	((pml.previous_velocity)[0]=(pm->ps->velocity)[0],(pml.previous_velocity)[1]=(pm->ps->velocity)[1],(pml.previous_velocity)[2]=(pm->ps->velocity)[2]);
	...
	{ 
		...
		(void)( (!!(veh && veh->m_pVehicle)) || (_wassert(L"veh && veh->m_pVehicle", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 11015), 0) );
		if ( veh && veh->m_pVehicle )
		{
		...
		&&pm->ps->m_iVehicleNum)
	{
		...
		if (veh && veh->playerState &&
			(pm->cmd.serverTime-veh->playerState->hyperSpaceTime) < 4000)
		{ 
	...
	{
		vec3_t vFwd, fwdAng;
		((fwdAng)[0]=(0.0f), (fwdAng)[1]=(pm->ps->viewangles[1]), (fwdAng)[2]=(0.0f));

		AngleVectors( fwdAng, vFwd, ((void *)0), ((void *)0) );
		if ( pm->ps->groundEntityNum == ((1<<10)-1) )
		{
			float savZ = pm->ps->velocity[2];
			((pm->ps->velocity)[0]=(vFwd)[0]*(100),(pm->ps->velocity)[1]=(vFwd)[1]*(100),(pm->ps->velocity)[2]=(vFwd)[2]*(100));
...
#line 11077 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	...
	if ( pm->cmd.upmove < 10 && !(pm->ps->pm_flags & 16384)) {
	...
	{
		pm->cmd.upmove = 0;
		pm->cmd.forwardmove = 20;
		pm->cmd.rightmove *= 0.08;
	}
	...
	if (pm->ps->pm_type == PM_JETPACK)
	{
		...
		if (pm->cmd.upmove > 0 && pm->ps->velocity[2] < 384)
		{ 
			float addIn = 12.0f;
	...
	if (!pm->ps->m_iVehicleNum &&
		pm_entSelf->s.NPC_class!=CLASS_VEHICLE&&
		pm_entSelf->s.NPC_class!=CLASS_RANCOR&&
		pm->ps->groundEntityNum < ((1<<10)-2) &&
		pm->ps->groundEntityNum >= 32)
	{ 
...
#line 11352 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
	}
	...
	{ 
		...
		(void)( (!!(veh && veh->playerState && veh->m_pVehicle && veh->s.number >= 32)) || (_wassert(L"veh && veh->playerState && veh->m_pVehicle && veh->s.number >= MAX_CLIENTS", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 11360), 0) );

		if (veh->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER)
		{ 
			veh->m_pVehicle->m_vOrientation[0] = pm->ps->viewangles[0];
		}
...
#line 11373 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		}
		else
		{
			...
			int i = 0;
#line 11380 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"

			(void)( (!!(self && self->playerState && self->s.number < 32)) || (_wassert(L"self && self->playerState && self->s.number < MAX_CLIENTS", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 11381), 0) );
			...
			while (i < veh->m_pVehicle->m_iNumPassengers)
			{
				...
				{
					gentity_t *thePassenger = (gentity_t *)veh->m_pVehicle->m_ppPassengers[i]; 
...
#line 11470 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
		}
		noAnimate = qtrue;
	}
	...
	if (!pm->ps->m_iVehicleNum 
		|| pm_entSelf->s.NPC_class==CLASS_VEHICLE 
		|| (!(pm->ps->eFlags&(1<<8))&&PM_WeaponOkOnVehicle(pm->cmd.weapon)) )
	{ 
	...
	{ 
		...
		(void)( (!!(veh->m_pVehicle)) || (_wassert(L"veh->m_pVehicle", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c", 11592), 0) );
...
#line 11668 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_pmove.c"
