...
qboolean ClientInactivityTimer( gclient_t *client ) {
	if ( ! g_inactivity.integer ) {
		...
		if ( level.time > client->inactivityTime && !IsABot(client->ps.clientNum)) {
			char	userinfo[MAX_INFO_STRING];
			//trap_DropClient( client - level.clients, "Dropped due to inactivity" );
			//return qfalse;

				strcpy(client->pers.netname, "¡AFK");
				client->pers.netnameTime = level.time + 500;
				...
				Info_SetValueForKey( userinfo, "name", "¡AFK" );
		...
		case EV_ROLL:
			{
				...
				else
				{
					...
					{
						damage = delta*0.32; //good enough for now, I guess
					}
				}
...
#define MAX_JEDIMASTER_FOV		135 ///100

#define MAX_SIGHT_DISTANCE		1500
#define MAX_SIGHT_FOV			135 ///100
...
void ClientThink_real( gentity_t *ent ) {
	...
	if (rand()%100 >= 66){
		if (ent->client && ent->chain && ent->chain->s.pos.trBase && ent->chain->r.svFlags & SVF_OWNERNOTSHARED){
			///VectorCopy(ent->chain->s.angles, ent->s.angles);
			///VectorCopy(ent->chain->s.angles2, ent->s.angles2);
			///ent->chain->s.apos = ent->s.apos;
				/////ent->client->ps.viewangles
			///VectorCopy(ent->chain->s.angles, ent->client->renderInfo.muzzleDirOld);
			///VectorScale( ent->client->renderInfo.muzzleDir, rand()%(ent->client->sess.dex*ent->client->sess.con), ent->chain->s.pos.trBase );
			if (ent->client->sess.dex){
			///VectorCopy(ent->client->renderInfo.crotchPoint, ent->parent->s.pos.trBase);
			///VectorCopy(ent->client->renderInfo.muzzleDir, ent->parent->s.pos.trDelta);
				ent->client->ps.userFloat1+=(ent->client->sess.dex*1.2f);
				///VectorScale( ent->client->renderInfo.muzzleDir, 100+ent->client->ps.userFloat1, ent->parent->s.pos.trDelta );
				VectorScale( ent->client->renderInfo.muzzleDir, 100+ent->client->ps.userFloat1, ent->chain->s.pos.trDelta );
			///VectorNormalize(ent->parent->s.pos.trDelta);
			}

			///VectorScale( ent->client->ps.origin, 270.0f*(1.2+ent->client->sess.dex/24.0f), ent->chain->s.pos.trBase );
			///VectorNormalize(ent->chain->s.pos.trDelta);
			...
			{
				///trap_SendServerCommand( -1, va("print \"anti-crashz0r hack!\n\""));
				return;
			}
...
#define	MIN_NPC_SPEED	16.0f
	...
	if (client->bodyGrabIndex != ENTITYNUM_NONE)
	{
		...
		else
		{
			...
			{
				bodyDir[2] /= 3;////= 0; //don't want it floating
	...
	if (ent->client->ps.duelInProgress)
	{
			...
			duelAgainst->client->ps.duelIndex != ent->s.number)
		{
			...
			///G_Damage(duelAgainst,ent,ent,NULL,ent->client->ps.origin,9999,DAMAGE_NO_PROTECTION,MOD_UNKNOWN);
		...
		else
		{
			...
			if (subLen >= 4096*1.5)
			{
				...
				///x = ent->client->ps.fallingToDeath
	...
	*/
	...
	{ //die!
		if (ent->health > 0)
		{
			...
			///if (Q_irand(0,1) == 0){
			///	G_AddEvent( otherKiller, Q_irand( EV_VICTORY1, EV_VICTORY3 ), 0 );
			///}else{
			///	G_AddEvent( otherKiller, Q_irand( EV_GLOAT1, EV_GLOAT3 ), 0 );
			///}
	...
	*/
		...
		(pm.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.time))
	{
		...
		{ //these are the only two where you wouldn't care about a delay between
			ent->client->lastGenCmdTime = level.time + 1200; ///300; //default 100ms debounce between issuing the same command.
		}
	...
	if (client->ps.forceKickFlip && g_flipkickvelocity.integer > 0)
	{
		...
		{int xpgain;
			if ( faceKicked && faceKicked->client && faceKicked->health 
				...
				/*|| ent->client->ps.legsAnim == 900*/) )
			{//push them away and do pain  //baby rancors?
				vec3_t oppDir;
				...
				G_Damage( faceKicked, ent, ent, oppDir, client->ps.origin, strength*(g_flipkickdamage.integer/20.0f), /*DAMAGE_NO_ARMOR*/DAMAGE_EXTRA_KNOCKBACK, MOD_MELEE );
				...
				//BG_ForcePowerDrain( &client->ps, FP_LEVITATION, 25 );
					 ...
					 (!BG_SaberInAttack(faceKicked->client->ps.saberMove) && !PM_SaberInStart(faceKicked->client->ps.saberMove) && !PM_SaberInReturn(faceKicked->client->ps.saberMove) && !PM_SaberInTransition(faceKicked->client->ps.saberMove)) )
				{
						...
						faceKicked->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN)
					{
						if (BG_KnockDownable(&faceKicked->client->ps) 
							&& (Q_irand(0,50) > faceKicked->client->sess.dex))//&& Q_irand(1, faceKicked->client->ps.iModelScale) <= 80)
						{ //only actually knock over sometimes, but always do velocity hit
							...
							faceKicked->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
							faceKicked->client->ps.forceHandExtendTime = level.time + 950/*1100*/ + Q_irand(1,300);//
			...
			///faceKicked->client->grappleIndex = ent->s.number;
			//faceKicked->client->grappleState = 1;
			//ent->client->grappleIndex = faceKicked->s.number;
			//ent->client->grappleState = 20;
			G_SetAnim(faceKicked, NULL, SETANIM_BOTH, BOTH_KYLE_GRAB, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_HOLDLESS, 0);
			faceKicked->client->ps.torsoTimer += 350; //make the hand stick out a little longer than it normally would
			...
			////G_InstaGrabSomeMofos(faceKicked,ent);
			...
			///trap_SendServerCommand( -1, va("cp \"%s ^5counter KO'd!\n\"",faceKicked->client->pers.netname) );

			//ent->client->ps.weaponTime = ent->client->ps.torsoTimer;
			///ent->client->ps.torsoAnim = 1155;
			///ent->client->ps.torsoTimer += 500;
			

			////faceKicked->client->ps.forceKickFlip = ent->s.number+1;
			////G_SetAnim(faceKicked, NULL, SETANIM_BOTH, 894, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_HOLDLESS, 0);
...
extern int GetLevel(gclient_t *cl)
{
	...
	//level = (int)(sqrt((float)xp * 0.33f)*1.2);
	//level = (int)sqrt(xp * (xp * 1.5f)); //3000?

	/*
	if ( Q_stricmp ("sub", cl->sess.admin_name) ==  0)
	{
		somefloat = (90.5f);
	}else{
		somefloat = (90.0f);
	}
	level = (int)(xp/somefloat);
	*/
	...
	/*if (xp < 50){
		level = (int)((float)xp/10.0f); //0-5
	}else if(xp < 150){
		level = 5+(int)((float)(xp-50)/15.0f);
	}else if(xp < 450){
		level = 5+(20/3)+(int)((float)(xp-150)/20.0f);
	}else if(xp < 1350){
		level = 5+(20/3)+(15)+(int)((float)(xp-450)/25.0f);
	}else{
		level = 5+(20/3)+(15)+(36)+(int)((float)(xp-1350)/30.0f);
	}*/
...
//	char foo[64];
	...
	int lc = 0, ac = 0, bc = 0; 
		...
		if (cl->sess.karma == 0 || !cl->sess.karma){
		len = trap_FS_FOpenFile(va("ranks/%s/%s.aln",dirkey,correctname), &f, FS_READ) ;  
        if(len > 0) { 
			char *cnf;
			cnf = malloc(len); 
			...
			trap_SendServerCommand( -1, va("print \"Debug align %s %d\n\"",cl->pers.netname, cl->sess.karma) );
			
		}trap_FS_FCloseFile(f); 
		}
		if (cl->sess.str == 0 || !cl->sess.str){
		len = trap_FS_FOpenFile(va("ranks/%s/%s.str",dirkey,correctname), &f, FS_READ) ;  
        if(len > 0) { 
			char *cnf;
			cnf = malloc(len); 
			trap_FS_Read(cnf, len, f); 
			...
			trap_SendServerCommand( -1, va("print \"Debug str %s %d\n\"",cl->pers.netname, cl->sess.karma) );
			
		}trap_FS_FCloseFile(f); 
		}
		if (cl->sess.dex == 0 || !cl->sess.dex){
		len = trap_FS_FOpenFile(va("ranks/%s/%s.dex",dirkey,correctname), &f, FS_READ) ;  
        if(len > 0) { 
			char *cnf;
			cnf = malloc(len); 
			trap_FS_Read(cnf, len, f); 
			...
			trap_SendServerCommand( -1, va("print \"Debug dex %s %d\n\"",cl->pers.netname, cl->sess.karma) );
			
		}trap_FS_FCloseFile(f); 
		}		if (cl->sess.con == 0 || !cl->sess.con){
		len = trap_FS_FOpenFile(va("ranks/%s/%s.con",dirkey,correctname), &f, FS_READ) ;  
        if(len > 0) { 
			char *cnf;
			cnf = malloc(len); 
			trap_FS_Read(cnf, len, f); 
			...
			trap_SendServerCommand( -1, va("print \"Debug con %s %d\n\"",cl->pers.netname, cl->sess.karma) );
			
		}trap_FS_FCloseFile(f); 
		}if (cl->sess.admin_number == 0 || !cl->sess.admin_number){
		len = trap_FS_FOpenFile(va("ranks/%s/%s.gold",dirkey,correctname), &f, FS_READ) ;  
        if(len > 0) { 
			char *cnf;
			cnf = malloc(len); 
			trap_FS_Read(cnf, len, f); 
			...
			trap_SendServerCommand( -1, va("print \"Debug gold %s %d\n\"",cl->pers.netname, cl->sess.karma) );
			
		}trap_FS_FCloseFile(f); 
		}*/
	///cl->ps.hackingTime = level.time + (cl->sess.karma * 10);
	///cl->ps.hackingBaseTime = (cl->sess.karma * 10);
	...
	/*
	if (g_gametype.integer == GT_POWERDUEL){
		percpoints = ent->client->sess.wins;
		//cl->sess.xp = ent->client->sess.wins;
	}else{
		percpoints = GetLevel(ent->client);
	}
	percpct = (percpoints / 100.0f);*/

	///cl->saber[0].knockbackScale *= (ent->client->sess.str / 10);
	cl->saber[0].maxChain = (ent->client->sess.dex / 2);

	//cl->saber[0].blade[0].length = 26 + (ent->client->sess.dex);
	//cl->saber[0].blade[0].extendDebounce += (ent->client->sess.con - 14);

	//cl->ps.iModelScale = 78+(percpoints*1.45);
	cl->ps.iModelScale = 120-(ent->client->sess.con*1.1f);
	//VectorSet(ent->modelScale, 0.78f+(percpct*1.45f),  0.78f+(percpct*1.45f),  0.78f+(percpct*1.45f));
	VectorSet(ent->modelScale, 1.20f-((ent->client->sess.con/100.0f)*1.1f),  1.20f-((ent->client->sess.con/100.0f)*1.1f),  1.20f-((ent->client->sess.con/100.0f)*1.1f));
	//cl->saber[0].moveSpeedScale = 1.15f-(percpct*1.15f);
	cl->saber[0].moveSpeedScale = .76f+((ent->client->sess.dex/100.0f)*1.12f);
	//cl->saber[0].animSpeedScale = 1.15f-(percpct*1.15f);
	cl->saber[0].animSpeedScale = .76f+((ent->client->sess.dex/100.0f)*1.12f);
	//cl->saber[0].damageScale =  0.76f+(percpct*1.85f);
	cl->saber[0].damageScale =  0.76f+((ent->client->sess.str/100.0f)*1.12f);
	//cl->saber[0].damageScale2 =  0.76f+(percpct*1.85f);
	cl->saber[0].damageScale2 =  0.76f+((ent->client->sess.str/100.0f)*1.12f);
	///WP_ForcePowerRegenerate( level.clients[cl->ps.clientNum], (int) (cl->saber[0].moveSpeedScale * 2.0f));
	//cl->ps.fd.forcePowerRegenDebounceTime *= cl->saber[0].damageScale;
	...
	if (cl->ps.iModelScale < 50){
		...
		VectorSet(ent->modelScale, 0.50f, 0.50f, 0.50f);
	}	
	if (cl->ps.iModelScale > 220){
		cl->ps.iModelScale = 220;
		VectorSet(ent->modelScale, 2.2f, 2.2f, 2.2f);
	}
	...
	/*if (percpoints >= 20){
		...
		//client->jetPackOn = 1;
		//client->ps.pm_type = PM_JETPACK;
		cl->ps.stats[STAT_WEAPONS] |= ( 1 << WP_ROCKET_LAUNCHER );
		cl->ps.ammo[AMMO_ROCKETS] = 1;
...
extern void ShowRPGStats(int entNum, int clientNum)
{
	...
	gclient_t *cl = &level.clients[clientNum];
	char		userinfo[MAX_INFO_STRING];
	char		lastip[MAX_INFO_STRING],correctname[MAX_INFO_STRING], dirkey[MAX_INFO_STRING];
...
//}
	...
	////if (
	////	(cl->sess.str == 0 && cl->sess.dex == 0) || 
	////	(cl->sess.dex == 0 && cl->sess.con == 0)/* || 
	////	(cl->sess.karma == 0 && cl->sess.admin_number == 0)*/ 
	////	){
		firstTime=qtrue;
	////}else{
		////firstTime=qfalse;
	...
	if (entNum == clientNum){
        ...
        if(len > 0) { 
			...
			///*(cnf + len) = '\0'; 
			cl->sess.karma = atoi(cnf);
			///trap_SendServerCommand( -1, va("print \"%s^3 recall Alignment: %d\n\"",cl->pers.netname, cl->sess.karma) );
			///G_Printf ( "rec karma: %d (%s)\n", cl->sess.karma, cl->pers.netname );
		}trap_FS_FCloseFile(f); 
		...
		///if (cl->sess.str == 0){
			...
			if(len > 0) { 
				...
				///*(cnf + len) = '\0'; 
				cl->sess.str = atoi(cnf);
				///G_Printf ( "rec str: %d (%s)\n", cl->sess.str, cl->pers.netname );
				///trap_SendServerCommand( clientNum, va("print \"%s^3 recall STR: %d\n\"",cl->pers.netname, cl->sess.str) );
		...
		///if (cl->sess.dex == 0){
			...
			if(len > 0) { 
				...
				///*(cnf + len) = '\0'; 
				cl->sess.dex = atoi(cnf);
				///G_Printf ( "rec dex: %d (%s)\n", cl->sess.dex, cl->pers.netname );
				///trap_SendServerCommand( clientNum, va("print \"%s^3 recall DEX: %d\n\"",cl->pers.netname, cl->sess.dex) );
			}trap_FS_FCloseFile(f); 
		///}
		...
		len = trap_FS_FOpenFile(va("ranks/cn/%s.con",correctname), &f, FS_READ) ;  
			if(len > 0) { 
				...
				///*(cnf + len) = '\0'; 
				cl->sess.con = atoi(cnf);
				///G_Printf ( "rec con: %d (%s)\n", cl->sess.con, cl->pers.netname );
				///trap_SendServerCommand( clientNum, va("print \"%s^3 recall CON: %d\n\"",cl->pers.netname, cl->sess.con) );
        ...
        if(len > 0) { 
			...
			///*(cnf + len) = '\0'; 
			cl->sess.admin_number = atoi(cnf);
			///trap_SendServerCommand( clientNum, va("print \"%s^3 recall Gold: %d\n\"",cl->pers.netname, cl->sess.admin_number) );
        ...
        if(len > 0) { 
			...
			///trap_SendServerCommand( clientNum, va("print \"%s^3 recall Level: %d\n\"",cl->pers.netname, cl->sess.wins) );
	...
	len = trap_FS_FOpenFile(va("ranks/xp/%s.xp",correctname), &f, FS_READ) ;  
        if(len > 0) { 
			...
			///*(cnf + len) = '\0'; 
			cl->sess.xp = atoi(cnf);
			///trap_SendServerCommand( -1, va("print \"%s^3 recall XP: %d\n\"",cl->pers.netname, cl->sess.xp) );
	...
	len = trap_FS_FOpenFile(va("ranks/style/%s.style",correctname), &f, FS_READ) ;  
        if(len > 0) { 
			...
			*(cnf + len) = '\0'; 
			...
			///trap_SendServerCommand( -1, va("print \"%s^3 recall saberstance: %i\n\"",cl->pers.netname,cl->sess.saberLevel ) );
			}
		}trap_FS_FCloseFile(f); 
	...
	len = trap_FS_FOpenFile(va("ranks/ip/%s.ip",correctname), &f, FS_READ) ;  
        if(len > 0) { 
			...
			*(cnf + len) = '\0'; 
			...
			if (Q_stricmp (smallvalue,smallcnf) != 0 /*&& smallvalue[1]>0*/ && smallvalue[0] >= "0" &&  smallvalue[0] <= "9"   && smallcnf[0] >= "0" &&  smallcnf[0] <= "9"){
				...
				if (Q_stricmp (cl->pers.netname, "Padawan") != 0 && Q_stricmp (cl->pers.netname, "Player") != 0 && 
					...
					///strcpy(cl->sess.admin_name,"mooted");
					///trap_SendServerCommand( -1, va("print \"%s ^1mooting, suspending privs\n\"",cl->pers.netname) );
					///WriteKeys(clientNum,7,qtrue);
				}

				    ///WriteKeys(clientNum,8,qtrue);
			}else if (Q_stricmp (smallvalue,smallcnf) == 0){
				///////trap_SendServerCommand( -1, va("print \"%s ^4subnet verified (%s)\n\"",cl->pers.netname, smallcnf) );
								
				if ((Q_stricmp (cl->sess.admin_name, "sub") == 0 || Q_stricmp (cl->sess.admin_name, "tsub") == 0 || Q_stricmp (cl->sess.admin_name, "rsub") == 0)){
					//////trap_SendServerCommand( -1, va("print \"%sadmin: (%s)\n\"",cl->sess.admin_name,cl->pers.netname) );
				}
					///WriteKeys(clientNum,8,qtrue);
			}
		}else if (!IsABot(clientNum)){

			///////trap_SendServerCommand( -1, va("print \"%s ^3subnet tolerated (for now)\n\"",cl->pers.netname) );
				if ((Q_stricmp (cl->sess.admin_name, "sub") == 0 || Q_stricmp (cl->sess.admin_name, "tsub") == 0 || Q_stricmp (cl->sess.admin_name, "rsub") == 0)){
					strcpy(cl->sess.admin_name,"");
					///////trap_SendServerCommand( -1, va("print \"%s ^3suspending privs\n\"",cl->pers.netname) );
					///WriteKeys(clientNum,7,qtrue);
				}
	...
	len = trap_FS_FOpenFile(va("ranks/stype/%s.stype",correctname), &f, FS_READ) ;  
        if(len > 0) { 
			...
			*(cnf + len) = '\0'; 
			...
			///trap_SendServerCommand( -1, va("print \"%s^3 recall saber1: %s\n\"",cl->pers.netname,cl->sess.saberType ) );
        ...
        if(len > 0) { 
			...
			*(cnf + len) = '\0'; 
			...
			///trap_SendServerCommand( -1, va("print \"%s^3 recall saber2: %s\n\"",cl->pers.netname,cl->sess.saber2Type ) );
		}trap_FS_FCloseFile(f); 
        ...
        if(len > 0) { 
			...
			*(cnf + len) = '\0'; 
			...
			///trap_SendServerCommand( -1, va("print \"%s^3 recall saber-offense level (melee=0): %i\n\"",cl->pers.netname,ent->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] ) );
		}trap_FS_FCloseFile(f); 

		/*len = trap_FS_FOpenFile(va("ranks/%s/mdl/%s.mdl",dirkey,correctname), &f, FS_READ) ;  
        if(len > 0) { 
			char *cnf;
			cnf = malloc(len); 
			trap_FS_Read(cnf, len, f); 
	...
	if (con <= 6){
		...
		/*
		str = (int) ((((cl->saber[0].damageScale - 1.0f) *2.4f)+1.0f) *18.0f);
		dex = (int) ((((cl->saber[0].moveSpeedScale - 1.0f) *3.5f)+1.0f) *15.5f);
		con = (int) ((((100.0f - cl->ps.iModelScale) *2.0f)+80.0f)/ 5.55f);
		*/
	...
	///trap_GetUserinfo( clientNum, userinfo, sizeof( userinfo ) );
	///value = Info_ValueForKey (userinfo, "ip");
	///valueint = ((value[0]*value[1]*value[2])/100);
...
/*
		conmod = 115 -(int)(cl->ps.iModelScale);
		dexmod = (int)(cl->saber[0].animSpeedScale*100.0f)-90;
		strmod = (int)(cl->saber[0].damageScale*100.0f)-90;
		if (strmod < 0){
			strcpy(strmodcolor,"^1");
			strcpy(strmodfix," pct ^7dmg");
		}else if (strmod == 0){
			strcpy(strmodcolor,"^4");
			strcpy(strmodfix," pct ^7dmg");
		}else{
			strcpy(strmodcolor,"^2+");
			strcpy(strmodfix," pct ^7dmg");
		}
		if (dexmod < 0){
			strcpy(dexmodcolor,"^1");
			strcpy(dexmodfix," pct ^7speed");
		}else if (dexmod == 0){
			strcpy(dexmodcolor,"^4");
			strcpy(dexmodfix," pct ^7speed");
		}else{
			strcpy(dexmodcolor,"^2+");
			strcpy(dexmodfix," pct ^7speed");
		}
		if (conmod < 0){
			strcpy(conmodcolor,"^1");
			strcpy(conmodfix," pct ^7life");
		}else if (conmod == 0){
			strcpy(conmodcolor,"^4");
			strcpy(conmodfix," pct ^7life");
		}else{
			strcpy(conmodcolor,"^2+");
			strcpy(conmodfix," pct ^7life");
		}
*/
	...
	if(entNum == clientNum){
		...
		int dscale = (int)((cl->saber[0].damageScale*100.0f));
		int movescale = (int)((cl->saber[0].moveSpeedScale*100.0f));
		int modelscale = (int)(100-(cl->ps.iModelScale));

		trap_SendServerCommand(entNum, va("print \"^7Name: %s^7\n^5Level %i %s%s%s%s ^5(%i XP)\n^3%i gold ^7$$ %s%i ^5STR (%i pct), %s%i ^5DEX (%i pct), %s%i ^5CON (%i pct)\n^0Align: %s ^0(%i)\n^7%s\n\"", 
		cl->pers.netname,
		percpoints, 
		foostr,
		foodex,
...
void ClientEndFrame( gentity_t *ent ) {
	...
	if ( !g_synchronousClients.integer 
		...
		///|| ent->client->ps.pm_type == PM_JETPACK || ent->client->ps.pm_type == PM_FLOAT) 
		&& ent->client
		&& IsABot(ent->client)
