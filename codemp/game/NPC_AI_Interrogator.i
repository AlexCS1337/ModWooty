...
void Interrogator_PartsMove(void)
{
	...
	if ( TIMER_Done(NPC,"syringeDelay") )
	{
		...
		{
			NPC->pos1[1]=Q_irand( 300, 360 );	
		}
		else 
		{
			NPC->pos1[1]=Q_irand( 0, 60 );	
		}
	...
	if ( TIMER_Done(NPC,"scalpelDelay") )
	{
		...
		if ( NPCInfo->localState == LSTATE_BLADEDOWN )	
		{
			...
			{
				NPC->pos2[0] = 180;
				NPCInfo->localState = LSTATE_BLADEUP;	
			}
		}
		else											
		{
			...
			{
				NPC->pos2[0] = 360;
				NPCInfo->localState = LSTATE_BLADEDOWN;	
...
void Interrogator_Strafe( void )
{
	...
	if ( tr.fraction > 0.9f )
	{
		((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[0]+(right)[0]*(32 * dir),(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[1]+(right)[1]*(32 * dir),(NPC->client->ps.velocity)[2]=(NPC->client->ps.velocity)[2]+(right)[2]*(32 * dir));
...
void Interrogator_Melee( qboolean visible, qboolean advance )
{
	if ( TIMER_Done( NPC, "attackDelay" ) )	
	{
		...
		if ( NPC->r.currentOrigin[2] >= NPC->enemy->r.currentOrigin[2]+NPC->enemy->r.mins[2] && NPC->r.currentOrigin[2]+NPC->r.mins[2]+8 < NPC->enemy->r.currentOrigin[2]+NPC->enemy->r.maxs[2] )
		{
			...
			G_Damage( NPC->enemy, NPC, NPC, 0, 0, 2, 0x00000004, MOD_MELEE );
...
void Interrogator_Attack( void )
{
	...
	advance		= (qboolean)(distance > 64*64 );
