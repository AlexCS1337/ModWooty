...
{
	if ( ((rand () & 0x7fff) / ((float)0x7fff)) < failChance )
	{
		return;
	}
...
static qboolean ST_Move( void )
{
	...
	if ( moved == qfalse )
	{
		if ( !trap_ICARUS_TaskIDPending( NPC, TID_MOVE_NAV ) )
		{
			if ( info.blocker && info.blocker->NPC && NPCInfo->group != ((void *)0) && info.blocker->NPC->group == NPCInfo->group )
			{
...
void NPC_BSST_Sleep( void )
{
	int alertEvent = NPC_CheckAlertEvents( qfalse, qtrue, -1, qfalse, AEL_MINOR );

	
	if ( alertEvent >= 0 )
	{
		
		if ( level.alertEvents[alertEvent].level == AEL_DISCOVERED && (NPCInfo->scriptFlags&0x00000800) )
		{ 
...
qboolean NPC_CheckEnemyStealth( gentity_t *target )
{
	...
	if ( !(target->client->ps.pm_flags&1)
		&& (NPCInfo->scriptFlags&0x00000800)
	...
	if ( clearLOS )
	{
		...
		((targ_org)[0]=(target->r.currentOrigin[0]), (targ_org)[1]=(target->r.currentOrigin[1]), (targ_org)[2]=(target->r.currentOrigin[2]+target->r.maxs[2]-4));
		...
		speed_rating		= ( target_speed / 250 );
		turning_rating		= 5.0f;
		light_level			= (255/255); 
		FOV_perc			= 1.0f - ( hAngle_perc + vAngle_perc ) * 0.5f;	
		vis_rating			= 0.0f;
		...
		dist_influence	= 0.35f * ( ( 1.0f - dist_rating ) );
		
		fov_influence		= 0.40f * ( 1.0f - FOV_perc );
		
		light_influence	= ( light_level - 0.5f ) * 0.25f;
		...
		target_rating += speed_rating * 0.25f;
		target_rating += turning_rating * 0.25f;
		...
		if ( NPC->client->NPC_class == CLASS_SWAMPTROOPER )
		{
			realize = (float)( 0.6f * 0.75 );
			cautious = (float)( 0.6f * 0.75 ) * 0.75f;
		}
		else
		{
			realize = (float)0.6f;
		...
		if ( target_rating > realize && (NPCInfo->scriptFlags&0x00000800) )
		{
			G_SetEnemy( NPC, target );
			NPCInfo->enemyLastSeenTime = level.time;
			TIMER_Set( NPC, "attackDelay", Q_irand( 500, 2500 ) );
		...
		if ( target_rating > cautious && !(NPCInfo->scriptFlags&0x00002000) )
		{
			...
			else if ( TIMER_Get( NPC, "enemyLastVisible" ) <= level.time + 500 && (NPCInfo->scriptFlags&0x00000800) )	
			{
...
static qboolean NPC_ST_InvestigateEvent( int eventID, qboolean extraSuspicious )
{
	...
	{
		if ( level.alertEvents[eventID].level == AEL_DISCOVERED && (NPCInfo->scriptFlags&0x00000800) )
		{
	...
	{
		if ( level.alertEvents[eventID].light < Q_irand( 30, 180 ) )
		{
			return qfalse;
		}
	}
	...
	((NPCInfo->investigateGoal)[0]=(level.alertEvents[eventID].position)[0],(NPCInfo->investigateGoal)[1]=(level.alertEvents[eventID].position)[1],(NPCInfo->investigateGoal)[2]=(level.alertEvents[eventID].position)[2]);
	...
	if ( level.alertEvents[eventID].level > AEL_MINOR && NPCInfo->investigateCount > 1 && (NPCInfo->scriptFlags&0x00000400) )
	{
		
		if ( G_ExpandPointToBBox( NPCInfo->investigateGoal, NPC->r.mins, NPC->r.maxs, NPC->s.number, ((NPC->clipmask&~0x00000100)|0x00000040) ) )
		{
			
			vec3_t	end;
			trace_t	trace;
			((end)[0]=(NPCInfo->investigateGoal)[0],(end)[1]=(NPCInfo->investigateGoal)[1],(end)[2]=(NPCInfo->investigateGoal)[2]);
			end[2] -= 512;
			trap_Trace( &trace, NPCInfo->investigateGoal, NPC->r.mins, NPC->r.maxs, end, ((1<<10)-1), ((NPC->clipmask&~0x00000100)|0x00000040) );
			...
			{
				((NPCInfo->investigateGoal)[0]=(trace.endpos)[0],(NPCInfo->investigateGoal)[1]=(trace.endpos)[1],(NPCInfo->investigateGoal)[2]=(trace.endpos)[2]);
				NPC_SetMoveGoal( NPC, NPCInfo->investigateGoal, 16, qtrue, -1, ((void *)0) );
				NPCInfo->localState = LSTATE_INVESTIGATE;
			}
		}
		else
		{
			int id = NPC_FindCombatPoint( NPCInfo->investigateGoal, NPCInfo->investigateGoal, NPCInfo->investigateGoal, 0x00000040|0x00001000, 0, -1 );

			if ( id != -1 )
			{
				NPC_SetMoveGoal( NPC, level.combatPoints[id].origin, 16, qtrue, id, ((void *)0) );
				NPCInfo->localState = LSTATE_INVESTIGATE;
			}
		}
		...
		if ( NPCInfo->investigateDebounceTime+NPCInfo->pauseTime > level.time )
		{
			if ( NPCInfo->group && 
				...
				!Q_irand( 0, 3 ) )
			{
				ST_Speech( NPCInfo->group->commander, SPEECH_LOOK, 0 );
			}
			else
			{
				ST_Speech( NPC, SPEECH_LOOK, 0 );
			}
		}
		else
		{
...
static void ST_OffsetLook( float offset, vec3_t out )
{
	...
	((out)[0]=(NPC->r.currentOrigin)[0]+(forward)[0]*(64),(out)[1]=(NPC->r.currentOrigin)[1]+(forward)[1]*(64),(out)[2]=(NPC->r.currentOrigin)[2]+(forward)[2]*(64));
...
static void ST_LookAround( void )
{
	...
	if ( perc < 0.25 )
	{
		((lookPos)[0]=(NPCInfo->investigateGoal)[0],(lookPos)[1]=(NPCInfo->investigateGoal)[1],(lookPos)[2]=(NPCInfo->investigateGoal)[2]);
	}
	else if ( perc < 0.5f )		
	{
...
void NPC_BSST_Investigate( void )
{
	...
	if ( NPCInfo->localState == LSTATE_INVESTIGATE && (NPCInfo->goalEntity!=((void *)0)) )
	{
...
void NPC_BSST_Patrol( void )
{
	...
	if ( NPC->client->NPC_class == CLASS_IMPERIAL || NPC->client->NPC_class == CLASS_IMPWORKER )
	{
		if ( ucmd.forwardmove || ucmd.rightmove || ucmd.upmove )
		{
			...
			{
				if ( (ucmd.buttons&16) && !(NPCInfo->scriptFlags&0x00000020) )
				{
					
					NPC_SetAnim( NPC, 1, BOTH_STAND4, 1|2 );
		...
		else
		{
			...
			{
				NPC_SetAnim( NPC, 1|2, BOTH_STAND4, 1|2 );
...
static void ST_CheckMoveState( void )
{
	...
	if ( ( NPCInfo->goalEntity != NPC->enemy ) && ( NPCInfo->goalEntity != ((void *)0) ) )
	{
...
static void ST_CheckFireState( void )
{
	...
	if ( !hitAlly 
		...
		&& NPCInfo->group 
		&& (NPCInfo->group->numState[SQUAD_RETREAT]>0||NPCInfo->group->numState[SQUAD_TRANSITION]>0||NPCInfo->group->numState[SQUAD_SCOUT]>0) )
	{
...
void ST_TrackEnemy( gentity_t *self, vec3_t enemyPos )
{
	...
	NPC_SetMoveGoal( self, enemyPos, 16, qfalse, -1, ((void *)0) );
}
...
void ST_TransferMoveGoal( gentity_t *self, gentity_t *other )
{
	...
	else
	{
		...
		{
			NPC_SetMoveGoal( other, self->NPC->tempGoal->r.currentOrigin, self->NPC->goalRadius, ((self->NPC->tempGoal->flags&0x00020000)?qtrue:qfalse), -1, ((void *)0) );
		}
		else
		{
...
int ST_GetCPFlags( void )
{
	...
	if ( NPC && NPCInfo->group )
	{
		if ( NPC == NPCInfo->group->commander && NPC->client->NPC_class == CLASS_IMPERIAL )
		{
			...
			cpFlags = (0x00000002|0x00000001|0x00000100|0x00004000|0x00020000);
		}
		else if ( NPCInfo->group->morale < 0 )
		{
			cpFlags = (0x00000001|0x00000100|0x00004000|0x00020000);
		}
		else if ( NPCInfo->group->morale < NPCInfo->group->numGroup )
		{
			...
			if ( moraleDrop < -6 )
			{
				cpFlags = (0x00000004|0x00020000|0x00000001|0x00000100|0x00004000);
			}
			else if ( moraleDrop < -3 )
			{
				cpFlags = (0x00020000|0x00000001|0x00000100|0x00004000);
			}
			else if ( moraleDrop < 0 )
			{
				cpFlags = (0x00000001|0x00000100|0x00004000);
			}
		}
		else
		{
			...
			if ( moraleBoost > 20 )
			{
				cpFlags = (0x00000002|0x00000800|0x00000200);
			}
			else if ( moraleBoost > 15 )
			{
				cpFlags = (0x00000002|0x00000400|0x00000200);
			}
			else if ( moraleBoost > 10 )
			{
				cpFlags = (0x00000002|0x00000200);
			}
		}
	}
	if ( !cpFlags )
	{
		...
		{
		case 0:
			cpFlags = (0x00000002|0x00000001|0x00000010);
			break;
		case 1:
			cpFlags = (0x00000002|0x00000001|0x00000200);
			break;
		case 2:
			cpFlags = (0x00000002|0x00000001|0x00000400|0x00000200);
			break;
		case 3:
			cpFlags = (0x00000002|0x00000001|0x00000800|0x00000200);
			break;
		}
	}
	if ( NPC && (NPCInfo->scriptFlags&0x00100000) )
	{
		cpFlags &= ~(0x00000800|0x00000200|0x00000400);
		cpFlags |= 0x00000010;
	}
	return cpFlags;
}
...
void ST_Commander( void )
{
	...
	if ( group->enemy == ((void *)0) || group->enemy->client == ((void *)0) )
	{
		return;
	}
	...
	if ( group->lastSeenEnemyTime < level.time - 180000 )
	{
		...
		group->enemy->waypoint = NAV_FindClosestWaypointForEnt( group->enemy, -1 );
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			else if ( group->enemy->waypoint == -1 || (trap_Nav_GetPathCost( NPC->waypoint, group->enemy->waypoint ) >= 16777216) )
			{
	...
	for ( i = curMemberNum; i < lastMemberNum; i++ )
	{
		...
		if ( NPCInfo->squadState != SQUAD_RETREAT )
		{
			if ( NPC->client->ps.weapon == WP_NONE )
			{
				if ( NPCInfo->goalEntity == ((void *)0) || NPCInfo->goalEntity->enemy == ((void *)0) || NPCInfo->goalEntity->enemy->s.eType != ET_ITEM )
				{
			...
			else if ( NPCInfo->localState == LSTATE_UNDERFIRE )
			{
				...
				case WP_SABER:
					...
					{
						cpFlags |= (0x00000020|0x00000001|0x00000100|0x00020000);
						if ( !group->commander || group->commander->NPC->rank  < RANK_ENSIGN )
						{
				...
				if ( NPC->health <= 10 )
				{
					if ( !group->commander || group->commander->NPC->rank < RANK_ENSIGN )
					{
						cpFlags |= (0x00000004|0x00000100|0x00020000);
						squadState = SQUAD_RETREAT;
					}
				}
			}
			else
			{
				if ( trap_InPVS( NPC->r.currentOrigin, group->enemy->r.currentOrigin ) )
				{
					if ( NPC->client->ps.weapon == WP_ROCKET_LAUNCHER &&
						DistanceSquared( group->enemy->r.currentOrigin, NPC->r.currentOrigin ) < 16384 &&
						NPCInfo->squadState != SQUAD_TRANSITION )
					{
						cpFlags |= (0x00000020|0x00000002|0x00000100);
						avoidDist = 256;
					}
					else
					{
				...
				else
				{
					...
					if ( !cpFlags )
					{
						...
						if ( TIMER_Done( NPC, "duck" ) )
						{
							if ( TIMER_Done( NPC, "stand" ) )
							{
								if ( NPCInfo->combatPoint == -1 || (level.combatPoints[NPCInfo->combatPoint].flags&0x00000001) )
								{
									if ( !Q_irand( 0, 3 ) )
									{
		...
		if ( NPCInfo->scriptFlags&0x00100000 )
		{
			cpFlags &= ~(0x00000800|0x00000200|0x00000400);
			cpFlags |= 0x00000010;
		}
		...
		if ( cpFlags )
		{
			...
			if ( cp == -1 )
			{
				cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, group->enemy->r.currentOrigin, cpFlags|0x00001000, avoidDist, NPCInfo->lastFailedCombatPoint );
			}
			while ( cp == -1 && cpFlags != 0 )
			{
				if ( cpFlags & 0x00000040 )
				{
					cpFlags &= ~0x00000040;
				}
				else if ( cpFlags & 0x00000080 )
				{
					cpFlags &= ~0x00000080;
				}
				else if ( cpFlags & 0x00000008 )
				{
					cpFlags &= ~0x00000008;
				}
				else if ( cpFlags & 0x00000010 )
				{
					cpFlags &= ~0x00000010;
				}
				else if ( cpFlags & 0x00000800 )
				{
					cpFlags &= ~0x00000800;
				}
				else if ( cpFlags & 0x00004000 )
				{
					cpFlags &= ~0x00004000;
				}
				else if ( cpFlags & 0x00000400 )
				{
					cpFlags &= ~0x00000400;
					
					cpFlags |= 0x00000200;
				}
				else if ( cpFlags & 0x00000200 )
				{
					cpFlags &= ~0x00000200;
				}
				else if ( cpFlags & 0x00000001 )
				{
					cpFlags &= ~0x00000001;
					
					cpFlags |= 0x00000008;
				}
				else if ( cpFlags & 0x00000002 )
				{
					cpFlags &= ~0x00000002;
				}
				else if ( cpFlags & 0x00000020 )
				{
					cpFlags &= ~0x00000020;
				}
				else if ( cpFlags & 0x00020000 )
				{
					cpFlags &= ~0x00020000;
				}
				else if ( cpFlags &0x00000004 )
				{
					cpFlags &= ~0x00000004;
					
					cpFlags |= (0x00000001|0x00000020);
				}
				else if ( cpFlags & 0x00000100 )
				{
					cpFlags &= ~0x00000100;
				}
				else
				...
				cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, group->enemy->r.currentOrigin, cpFlags|0x00001000, avoidDist, -1 );
			} 
			...
			if ( cp != -1 )
			{
				...
				TIMER_Set( NPC, "roamTime", 16777216 );
				TIMER_Set( NPC, "verifyCP", Q_irand( 1000, 3000 ) );
					...
					else 
					{
						if ( group->numGroup > 1 )
						{
							...
							if ( !Q_irand( 0, 3 ) )
							{
								...
								((eDir2Me)[0]=(NPC->r.currentOrigin)[0]-(group->enemy->r.currentOrigin)[0],(eDir2Me)[1]=(NPC->r.currentOrigin)[1]-(group->enemy->r.currentOrigin)[1],(eDir2Me)[2]=(NPC->r.currentOrigin)[2]-(group->enemy->r.currentOrigin)[2]);
								VectorNormalize( eDir2Me );

								((eDir2CP)[0]=(level.combatPoints[NPCInfo->combatPoint].origin)[0]-(group->enemy->r.currentOrigin)[0],(eDir2CP)[1]=(level.combatPoints[NPCInfo->combatPoint].origin)[1]-(group->enemy->r.currentOrigin)[1],(eDir2CP)[2]=(level.combatPoints[NPCInfo->combatPoint].origin)[2]-(group->enemy->r.currentOrigin)[2]);
...
#line 2457 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_ai_stormtrooper.c"
			ST_Commander();
...
#line 2473 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_ai_stormtrooper.c"
		}
	}
	...
	if ( !faceEnemy )
	{
		if ( !move )
		{
			((NPCInfo->lastPathAngles)[0]=(NPC->client->ps.viewangles)[0],(NPCInfo->lastPathAngles)[1]=(NPC->client->ps.viewangles)[1],(NPCInfo->lastPathAngles)[2]=(NPC->client->ps.viewangles)[2]);
		}
		NPCInfo->desiredYaw = NPCInfo->lastPathAngles[1];
