...
// E11 Blaster
//---------
#define BLASTER_SPREAD				1.9f//1.2f
#define BLASTER_VELOCITY			1800//2300//?
#define BLASTER_DAMAGE				10

// Tenloss Disruptor
//----------
#define DISRUPTOR_MAIN_DAMAGE			12 //40
#define DISRUPTOR_MAIN_DAMAGE_SIEGE		50
#define DISRUPTOR_NPC_MAIN_DAMAGE_CUT	0.25f

#define DISRUPTOR_ALT_DAMAGE			36 //125
...
// Heavy Repeater
//----------
#define REPEATER_SPREAD				1.6f///1.4f
#define	REPEATER_DAMAGE				9
#define	REPEATER_VELOCITY			2400
...
// DEMP2
//----------
#define	DEMP2_DAMAGE				12
#define	DEMP2_VELOCITY				4200///1400
...
#define FLECHETTE_DAMAGE			9///12//15
#define FLECHETTE_VEL				3500
#define FLECHETTE_SIZE				1
#define FLECHETTE_MINE_RADIUS_CHECK	256
#define FLECHETTE_ALT_DAMAGE		20///60
#define FLECHETTE_ALT_SPLASH_DAM	20///60
#define FLECHETTE_ALT_SPLASH_RAD	25

// Personal Rocket Launcher
//---------
#define	ROCKET_VELOCITY				1550///1050
#define	ROCKET_DAMAGE				25//30//100?
#define	ROCKET_SPLASH_DAMAGE		25//30//100?
#define	ROCKET_SPLASH_RADIUS		100
...
#define	CONC_DAMAGE					1 //25 //75 //150
#define	CONC_NPC_DAMAGE_EASY		1
#define	CONC_NPC_DAMAGE_NORMAL		1
#define	CONC_NPC_DAMAGE_HARD		1
#define	CONC_SPLASH_DAMAGE			-1//40 //50
#define	CONC_SPLASH_RADIUS			600 //300
//alt
#define CONC_ALT_DAMAGE				0//25 //100
#define CONC_ALT_NPC_DAMAGE_EASY	0
#define CONC_ALT_NPC_DAMAGE_MEDIUM	0
#define CONC_ALT_NPC_DAMAGE_HARD	0

// Stun Baton
//--------------
#define STUN_BATON_DAMAGE			2
#define STUN_BATON_ALT_DAMAGE		2
#define STUN_BATON_RANGE			24

// Melee
//--------------
#define MELEE_SWING1_DAMAGE			8//10
#define MELEE_SWING2_DAMAGE			10//12
#define MELEE_RANGE					16///13//8

// ATST Main Gun
//--------------
#define ATST_MAIN_VEL				4000	// 
#define ATST_MAIN_DAMAGE			15///25		// 
#define ATST_MAIN_SIZE				3		// make it easier to hit things

// ATST Side Gun
//---------------
#define ATST_SIDE_MAIN_DAMAGE				15///75
#define ATST_SIDE_MAIN_VELOCITY				1200
...
#define ATST_SIDE_ALT_DAMAGE				40///130
...
//---------------------------------------------------------
{
	...
	ent->client->ps.userFloat1 = 100; //distance
	///VectorScale(start,1.5f,start);
	start[2] += 55;
	missile = CreateMissile( start, dir, (int)velocity*speedfactor, 10000, ent, altFire );
	////demp2ProjectileEffect
	...
	if (Q_irand(12,200) < ent->client->sess.con && !autoAnim){
		...
		missile->nextthink = level.time + 1000 + (150*ent->client->sess.con);///level.time + 1000;
		supah = qtrue;
		WP_FireHadokenBlasterMissile(ent, start, dir, altFire, damage, velocity+(velocity*((float)Q_irand(1,100)/100.0f)), mod, qtrue);
		WP_FireHadokenBlasterMissile(ent, start, dir, altFire, damage, velocity+(velocity*((float)Q_irand(1,100)/100.0f)), mod, qtrue);
		///WP_FireHadokenBlasterMissile(ent, start, dir, altFire, damage, Q_irand(150,250), mod, qtrue);
		...
		///missile->physicsBounce = ent->client->sess.con/ent->client->sess.str;
		missile->nextthink = level.time + 500 + (50*ent->client->sess.con);///level.time + 1000;
	
	}

	missile->think = G_FreeEntity;

	VectorSet( missile->r.maxs, 4, 4, 4 );
	VectorScale( missile->r.maxs, -1, missile->r.mins );

	///VectorSet(missile->modelScale, (int)2*dmgfactor,(int)2*dmgfactor,(int)2*dmgfactor);
	missile->mass = 6;//8
	missile->flags |= FL_BOUNCE;
	missile->dflags = DAMAGE_NO_PROTECTION;///DAMAGE_NO_TEAM_PROTECTION;

	missile->splashMethodOfDeath = MOD_FORCE_DARK;
	missile->splashDamage = 1;////REPEATER_ALT_SPLASH_DAMAGE;

	///missile->r.svFlags |= SVF_CAPSULE; //only guided hadokens

	missile->methodOfDeath = mod;
	missile->clipmask = MASK_SHOT;//// | CONTENTS_LIGHTSABER;
	///missile->think = G_FreeEntity;
	/////G_AddEvent(missile, EV_BODYFADE, 0);
	...
	///return BOTH_FORCE_DRAIN_GRAB_START;

	/*if (supah){
		return BOTH_FORCE_ABSORB;
	}else{
		return 1339;
	}*/

///G_SoundAtLoc( ent->client->ps.origin, CHAN_WEAPON, G_SoundIndex("sound/hadoken.wav") );
	/*missile = CreateMissile( ent->client->ps.origin, forward , 410, 10000, ent, qtrue );
	///missile->activator = ent;
	VectorSet( missile->r.maxs, 6, 6, 5 );
	VectorScale( missile->r.maxs, -1, missile->r.mins );
	missile->model = "models/players/Jawa/model.glm";
	VectorSet(missile->modelScale, 1.24f-((ent->client->sess.con/100.0f)*1.12f),  1.24f-((ent->client->sess.con/100.0f)*1.12f),  1.24f-((ent->client->sess.con/100.0f)*1.12f));
	missile->classname = "bowcaster_proj";
	missile->s.weapon = WP_BOWCASTER;
	missile->damage = 4;
	missile->dflags = DAMAGE_DEATH_KNOCKBACK;
	missile->methodOfDeath = MOD_CRUSH;
	missile->clipmask = MASK_SHOT | CONTENTS_LIGHTSABER;
...
gentity_t *WP_FireHadokenLikeBlasterMissile( gentity_t *ent, vec3_t start, vec3_t dir, qboolean altFire, int damage, int velocity, int mod, qboolean autoAnim )
//---------------------------------------------------------
{
	gentity_t *missile;
	float dmgfactor = .25+ent->client->sess.str/22.0f;
	...
	if (autoAnim){
		...
		missile->damage = 0;
		///missile->physicsBounce = ent->client->sess.con/ent->client->sess.str;
		missile->nextthink = level.time; /// + -5000 + (50*ent->client->sess.con);///level.time + 1000;
...
//---------------------------------------------------------
{
	int	damage	= 2;////REPEATER_ALT_DAMAGE;
	...
	VectorScale( missile->r.maxs, -1, missile->r.mins );
	missile->s.pos.trType = TR_GRAVITY;
	missile->s.pos.trDelta[2] +=75.0f; ////40.0f; //give a slight boost in the upward direction
	missile->damage = damage;
	...
	missile->clipmask = MASK_SHOT | CONTENTS_LIGHTSABER;
	missile->splashDamage = 2;////REPEATER_ALT_SPLASH_DAMAGE;
...
//---------------------------------------------------------
{
	...
	{
		// add some slop to the alt-fire direction
		angs[PITCH] += crandom() * 2.7f;///REPEATER_SPREAD;
		angs[YAW]	+= crandom() * 2.7f;///REPEATER_SPREAD;
...
{//a rail-gun-like beam
	...
	////VectorMA( ent->client->ps.velocity, -275, forward, ent->client->ps.velocity );
	VectorMA( ent->client->ps.velocity, -119, forward, ent->client->ps.velocity );
...
//	}
	...
	for ( i = 0; i < traces; i++ )
	{
		...
		if (d_projectileGhoul2Collision.integer)
		{
			trap_G2Trace( &tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT | CONTENTS_ITEM, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
		}
		else
		{
			trap_Trace( &tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT | CONTENTS_ITEM);
		}
...
//	shotDist = VectorNormalize( dir );
	...
	/////tent = G_TempEntity(tr.endpos, EV_POWERUP_QUAD);
	tent->s.eventParm = DirToByte(tr.plane.normal);
	...
	*/
	...
	for ( dist = 0; dist < shotDist; dist += 64 )
	{
		...
		G_PlayEffectID( G_EffectIndex( "sparks/spark_exp_nosnd" ), spot, actualAngles );
	}
...
{//a fast rocket-like projectile
	...
	//Shove us backwards for half a second
	////VectorMA( ent->client->ps.velocity, -275, forward, ent->client->ps.velocity );
	VectorMA( ent->client->ps.velocity, 360, forward, ent->client->ps.velocity );
...
//	}
	...
	//Shove us forwards? for half a second
	////VectorMA( ent->client->ps.velocity, 325, forward, ent->client->ps.velocity );
	VectorMA( ent->client->ps.velocity, 380, forward, ent->client->ps.velocity );
	...
	*/
	...
	///unwoot?
	/*
	missile = CreateMissile( start, forward, vel, 5000, ent, qfalse );/////10000, ent, qfalse );

	missile->classname = "conc_proj";
	////missile->classname = "turbo_proj";
	missile->s.weapon = WP_CONCUSSION;////WP_CONCUSSION;
	missile->mass = 15;///10;
	...
	if ( tr_ent && tr_ent->takedamage )
	{
		...
		{ //if it's a player then use the shock effect
			...
			{//not on vehicles
				...
				{//can zap animals
					tr_ent->client->ps.electrifyTime = level.time + Q_irand( 100, 200 );///Q_irand( 3000, 4000 );
				}
			}
			else
			{
...
void WP_FireMelee( gentity_t *ent, qboolean alt_fire )
{
	...
	///if (BG_CrouchAnim(ent->client->ps.legsAnim) || BG_CrouchAnim(ent->client->ps.torsoAnim)){
		///goto crouching;
	///}
anim = 0;
	if (ent->client->pers.cmd.rightmove < 0 && ent->client->pers.cmd.forwardmove < 0)
			...
			{ ///char *path;
				...
				anim=BOTH_A7_KICK_BF; //BOTH_PULL_IMPALE_SWING
...
goto crouching;
			...
			{goto crouching;
				...
				if (Q_irand(0,100) >= 50){
					G_SetAnim(ent, NULL, SETANIM_LEGS, BOTH_STAND1+Q_irand(0,12), 3, 0); 
					ent->client->ps.torsoTimer += 500;
				}else{
					G_SetAnim(ent, NULL, SETANIM_LEGS, BOTH_STAND1, 3, 0); 
					ent->client->ps.torsoTimer += 500;
				}
			}
	...
	{ //hit something
		...
		{ //damage them, do more damage if we're in the second right hook
			...
			int dmg = Q_irand( g_meleedamage.integer*0.7f, (int)(g_meleedamage.integer*1.05f));

			if (ent->client && ent->client->ps.torsoAnim == BOTH_MELEE2)
			{ //do a tad bit more damage on the second swing
				///dmg = MELEE_SWING2_DAMAGE;
				dmg = Q_irand( g_meleedamage.integer*0.85f, (int)(g_meleedamage.integer*1.15f));
			}

			if (ent->client && ent->client->ps.torsoAnim == BOTH_THERMAL_READY)
			{ //do a tad bit more damage on the second swing
				dmg = Q_irand( g_meleedamage.integer*0.9f, (int)(g_meleedamage.integer*1.2f));
				
			}
			if (ent->client && ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_SWING)
			{ //do a tad bit more damage on the second swing
				dmg = Q_irand( (int)(g_meleedamage.integer*0.95f), (int)(g_meleedamage.integer*1.25f));
				if (ent->client->sess.karma >= 16){
					dmg = Q_irand( (int)(g_meleedamage.integer*1.0f), (int)(g_meleedamage.integer*1.35f));
					if (Q_irand(0,100) > 85){
					...
					ent->client->ps.powerups[PW_SPEED] = level.time + 250;
					G_LogWeaponPowerup(ent->s.number, PW_SPEED);
				}
			}
			if (ent->client && ent->client->ps.torsoAnim == BOTH_THERMAL_THROW)
			{ //do a tad bit more damage on the second swing
				dmg = Q_irand( (int)(g_meleedamage.integer*1.0f), (int)(g_meleedamage.integer*1.3f));
				if (Q_irand(0,100) > 85){
			...
			//punchin' damage dmg
			///if (tr_ent->client){
			///dmg *= (ent->client->saber[0].damageScale / tr_ent->client->saber[0].damageScale) ;
			///}else{
			
			if (ent->client->saber[0].moveSpeedScale > 0.5f){
				dmg /= (ent->client->saber[0].moveSpeedScale - 0.4f) ;
			}

			if (ent->client->saber[0].damageScale >= 0.3f){
				if (ent->client->saber[0].damageScale <= 1.0f){
					dmg *= (ent->client->saber[0].damageScale - 0.3f) ;
				}
				if (ent->client->saber[0].damageScale < 1.5f){
					dmg *= (ent->client->saber[0].damageScale - 0.3f) ;
				}else{
					dmg *= 1.5f;
				}
			...
			if (tr_ent->client){
			//trap_SendServerCommand( -1, va("cp \"debug: %d health\n\"",tr_ent->client->ps.stats[STAT_HEALTH]) );
			}

			if (ent->client->ps.torsoTimer > 25 || dmg <= 2){
				G_Damage( tr_ent, ent, ent, forward, tr.endpos, dmg, DAMAGE_HALF_ARMOR_REDUCTION, MOD_MELEE );
			}else{
				dmg *= 0.1f;

				//ent->client->ps.powerups[PW_QUAD] = level.time + 150;
				//G_LogWeaponPowerup(ent->s.number, PW_QUAD);

				ent->chain = WP_FireHadokenLikeBlasterMissile(ent, ent->client->renderInfo.muzzlePoint, ent->client->renderInfo.muzzleDir, 0, 1, 1, MOD_FORCE_DARK, qfalse);


				G_Damage( tr_ent, ent, ent, forward, tr.endpos, dmg, DAMAGE_EXTRA_KNOCKBACK, MOD_MELEE );
			}
			return;
