...
int CanGetToVector(vec3_t org1, vec3_t org2, vec3_t mins, vec3_t maxs)
{
	...
	trap_Trace(&tr, org1, mins, maxs, org2, ENTITYNUM_NONE, MASK_ALL/*MASK_SOLID*/);
...
int ConnectTrail(int startindex, int endindex, qboolean behindTheScenes)
{
	...
	if (!g_RMG.integer)
	{
		branchDistance = 48; ///30; ////TABLE_BRANCH_DISTANCE;
	}
	else
	{
		branchDistance = 48; ////512; //be less precise here, terrain is fairly broad, and we don't want to take an hour precalculating
	}
...
int RepairPaths(qboolean behindTheScenes)
{
	...
	int ctRet;
	vec3_t a;
	float maxDistFactor = 1200; ///1600; ///400
