...
void CorpsePhysics( gentity_t *self )
{
	...
	if ( self->client->ps.groundEntityNum != ((1<<10)-1) && !(self->s.eFlags&(1<<26)) )
	{
		
		pitch_roll_for_slope( self, ((void *)0) );
	}

	if ( eventClearTime == level.time + 200 )
	{
		if ( !(self->client->ps.eFlags&(1<<8)) )
		{
	...
	if (self->client->respawnTime < (level.time+500))
	{
		...
		if (self->client->ps.eFlags & (1<<26))
		{
			self->r.contents = 0;
		}
		else if ((self->client->NPC_class != CLASS_MARK1) && (self->client->NPC_class != CLASS_INTERROGATOR))	
		{
...
void NPC_RemoveBody( gentity_t *self )
{
	...
	self->NPC->nextBStateThink = level.time + 50;
		...
		|| self->client->NPC_class == CLASS_MARK2 )
	{
		...
		{
			if ( !self->activator || !self->activator->client || !(self->activator->client->ps.eFlags2&(1<<0)) )
			{
				G_FreeEntity( self );
			}
		}
	...
	if ( self->NPC && self->NPC->timeOfDeath <= level.time )
	{
		...
		if ( self->enemy )
		{
			...
			if (!trap_ICARUS_IsRunning(self->s.number))
			{
				if ( !self->activator || !self->activator->client || !(self->activator->client->ps.eFlags2&(1<<0)) )
				{
					if ( self->client && self->client->ps.saberEntityNum > 0 && self->client->ps.saberEntityNum < ((1<<10)-2) )
					{
...
static void NPC_RemoveBodyEffect(void)
{
	...
	if ( !NPC || !NPC->client || (NPC->s.eFlags & (1<<8)) )
		return;
...
void pitch_roll_for_slope( gentity_t *forwhom, vec3_t pass_slope )
{
	...
	if( !pass_slope || VectorCompare( vec3_origin, pass_slope ) )
	{
		...
		((startspot)[0]=(forwhom->r.currentOrigin)[0],(startspot)[1]=(forwhom->r.currentOrigin)[1],(startspot)[2]=(forwhom->r.currentOrigin)[2]);
		startspot[2] += forwhom->r.mins[2] + 4;
		((endspot)[0]=(startspot)[0],(endspot)[1]=(startspot)[1],(endspot)[2]=(startspot)[2]);
		endspot[2] -= 300;
		trap_Trace( &trace, forwhom->r.currentOrigin, vec3_origin, vec3_origin, endspot, forwhom->s.number, (0x00000001|0x00001000) );
	...
	AngleVectors( forwhom->r.currentAngles, ovf, ovr, ((void *)0) );
	...
	if ( forwhom->client )
	{
		...
		forwhom->client->ps.viewangles[0] = dot * pitch;
		forwhom->client->ps.viewangles[2] = ((1-Q_fabs(dot)) * pitch * mod);
		oldmins2 = forwhom->r.mins[2];
		forwhom->r.mins[2] = -24 + 12 * fabs(forwhom->client->ps.viewangles[0])/180.0f;
	...
	{
		forwhom->r.currentAngles[0] = dot * pitch;
		forwhom->r.currentAngles[2] = ((1-Q_fabs(dot)) * pitch * mod);
	}
}
...

	{
		...
		if ( level.time >= NPCInfo->timeOfDeath + BodyRemovalPadTime( NPC ) )
		{
			...
			else
			{
				...
				{
					NPC->client->ps.eFlags |= (1<<8);
					NPCInfo->timeOfDeath = level.time + 50 * 8;
				}
				else
					NPCInfo->timeOfDeath = level.time + 50 * 4;
			}
			return;
		}
	}
...
void NPC_ShowDebugInfo (void)
{
	if ( showBBoxes )
	{
		...
		while( (found = G_Find( found, ((int)&(((gentity_t *)0)->classname)), "NPC" ) ) != ((void *)0) )
		{
			if ( trap_InPVS( found->r.currentOrigin, g_entities[0].r.currentOrigin ) )
			{
				((mins)[0]=(found->r.currentOrigin)[0]+(found->r.mins)[0],(mins)[1]=(found->r.currentOrigin)[1]+(found->r.mins)[1],(mins)[2]=(found->r.currentOrigin)[2]+(found->r.mins)[2]);
				((maxs)[0]=(found->r.currentOrigin)[0]+(found->r.maxs)[0],(maxs)[1]=(found->r.currentOrigin)[1]+(found->r.maxs)[1],(maxs)[2]=(found->r.currentOrigin)[2]+(found->r.maxs)[2]);
...
{
	if ( NPCInfo->scriptFlags & 0x00000001 )
	{
	...
	if(NPCInfo->scriptFlags & 0x00000020)
	{
		ucmd.buttons &= ~16;
	}
	else if(NPCInfo->scriptFlags & 0x00000002)
	{
		if ( NPCInfo->charmedTime > level.time && (ucmd.forwardmove || ucmd.rightmove) )
		{
		}
	...
	if(NPCInfo->scriptFlags & 0x00000008)
	{
		ucmd.buttons |= 32;
		ucmd.rightmove = 127;
		ucmd.forwardmove = 0;
	...
	else if(NPCInfo->scriptFlags & 0x00000010)
	{
		ucmd.buttons |= 32;
		ucmd.rightmove = -127;
		ucmd.forwardmove = 0;
	...
	if ( (NPCInfo->scriptFlags & 0x00000040) && (ucmd.buttons & 1) )
	{
		ucmd.buttons |= 128;
	}
}
...
void NPC_HandleAIFlags (void)
{
	...
	if ( NPCInfo->aiFlags & 0x00002000 )
	{
		
		NPCInfo->aiFlags &= ~0x00002000;
...
void NPC_CheckAttackHold(void)
{
	...
	{
		((vec)[0]=(NPC->enemy->r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(vec)[1]=(NPC->enemy->r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(vec)[2]=(NPC->enemy->r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
		...
		else if ( ( NPCInfo->attackHold ) && ( ucmd.buttons & 1 ) )
		{
...
void NPC_KeepCurrentFacing(void)
{
	if(!ucmd.angles[1])
	{
		ucmd.angles[1] = ((int)((client->ps.viewangles[1])*65536/360) & 65535) - client->ps.delta_angles[1];
	}

	if(!ucmd.angles[0])
	{
		ucmd.angles[0] = ((int)((client->ps.viewangles[0])*65536/360) & 65535) - client->ps.delta_angles[0];
	}
}
...
void NPC_RunBehavior( int team, int bState )
{
	...
	else if ( NPCInfo->scriptFlags & 0x00010000 )
	{
		NPC_BSDefault();
	}
	else
	{
		...
		case NPCTEAM_ENEMY:
			...
			if ( NPC->client->ps.weapon == WP_DISRUPTOR && (NPCInfo->scriptFlags & 0x00000040) )
			{
...
void NPC_ExecuteBState ( gentity_t *self)
{
	...
	if ( NPC->client->ps.saberLockTime && NPC->client->ps.saberLockEnemy != ((1<<10)-1) )
	{
	...
	if ( NPC->enemy )
	{
		if(NPC->enemy->flags & 0x00040000)
		{
			ucmd.buttons &= ~1;
			ucmd.buttons &= ~128;
		}
		else if ( NPC->client->playerTeam != NPCTEAM_ENEMY && NPC->enemy->NPC && (NPC->enemy->NPC->surrenderTime > level.time || (NPC->enemy->NPC->scriptFlags&0x00010000)) )
		{
			ucmd.buttons &= ~1;
			ucmd.buttons &= ~128;
		}
	...
	if(!(ucmd.buttons & 1) && NPC->attackDebounceTime > level.time)
	{
		if(client->ps.weapon == WP_SABER )
		{
			NPC_SetAnim(NPC,1,TORSO_WEAPONREADY1,0);
		}
		else if(client->ps.weapon == WP_BRYAR_PISTOL)
		{
			NPC_SetAnim(NPC,1,TORSO_WEAPONREADY3,0);
		}
	...
	else if ( !NPC->enemy )
	{

		{
			...
			{
				NPC_SetAnim(NPC,1,TORSO_WEAPONIDLE3,0);
			}
		}
	}
	...
	{
		NPCInfo->last_ucmd.buttons &= ~(1|128);
	}
...
void NPC_CheckInSolid(void)
{
	trace_t	trace;
	vec3_t	point;
	((point)[0]=(NPC->r.currentOrigin)[0],(point)[1]=(NPC->r.currentOrigin)[1],(point)[2]=(NPC->r.currentOrigin)[2]);
	point[2] -= 0.25;

	trap_Trace(&trace, NPC->r.currentOrigin, NPC->r.mins, NPC->r.maxs, point, NPC->s.number, NPC->clipmask);
	if(!trace.startsolid && !trace.allsolid)
	{
		((NPCInfo->lastClearOrigin)[0]=(NPC->r.currentOrigin)[0],(NPCInfo->lastClearOrigin)[1]=(NPC->r.currentOrigin)[1],(NPCInfo->lastClearOrigin)[2]=(NPC->r.currentOrigin)[2]);
	}
	else
	{
...
#line 1826 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc.c"
void NPC_Think ( gentity_t *self)
{
	...
	((oldMoveDir)[0]=(self->client->ps.moveDir)[0],(oldMoveDir)[1]=(self->client->ps.moveDir)[1],(oldMoveDir)[2]=(self->client->ps.moveDir)[2]);
	if (self->s.NPC_class != CLASS_VEHICLE)
	{ 
		((self->client->ps.moveDir)[0]=(self->client->ps.moveDir)[1]=(self->client->ps.moveDir)[2]=0);
	}
	...
	if ( self->health <= 0 ) 
	{
		...
		((self->client->ps.origin)[0]=(self->r.currentOrigin)[0],(self->client->ps.origin)[1]=(self->r.currentOrigin)[1],(self->client->ps.origin)[2]=(self->r.currentOrigin)[2]);
		return;
	}

	
	if ( debugNPCFreeze.value || (NPC->r.svFlags&0x00008000) ) 
	{
		NPC_UpdateAngles( qtrue, qtrue );
		ClientThink(self->s.number, &ucmd);
	...
	self->nextthink = level.time + 50/2;
	...
	while (i < 32)
	{
		...
		if (player->inuse && player->client && player->client->sess.sessionTeam != TEAM_SPECTATOR &&
			!(player->client->ps.pm_flags & 4096))
		{
...
#line 1928 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc.c"
		...
		{
			NPCInfo->nextBStateThink = level.time + 50/2;
		}
		else
		{
			NPCInfo->nextBStateThink = level.time + 50;
		}
...
#line 1957 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc.c"
	}
	else
	{
		((self->client->ps.moveDir)[0]=(oldMoveDir)[0],(self->client->ps.moveDir)[1]=(oldMoveDir)[1],(self->client->ps.moveDir)[2]=(oldMoveDir)[2]);
...
{	
	...
	G_SetAnim(ent, ((void *)0), setAnimParts, anim, setAnimFlags, 0);
