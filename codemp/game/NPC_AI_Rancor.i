...
qboolean Rancor_CheckRoar( gentity_t *self )
{
	...
	{
		self->wait = 1;
		self->client->ps.eFlags2 |= (1<<2);
		NPC_SetAnim( self, 1|2, BOTH_STAND1TO2, 1 | 2 );
...
void Rancor_Patrol( void )
{
	...
	else
	{
		...
		{
			TIMER_Set( NPC, "patrolTime", (2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) * 5000 + 5000 );
		}
	}
...
void Rancor_DropVictim( gentity_t *self )
{
	...
	if ( self->activator )
	{
		...
		{
			self->activator->client->ps.eFlags2 &= ~(1<<0);
			self->activator->client->ps.hasLookTarget = qfalse;
			self->activator->client->ps.lookTarget = ((1<<10)-1);
			self->activator->client->ps.viewangles[2] = 0;
			SetClientViewAngle( self->activator, self->activator->client->ps.viewangles );
			self->activator->r.currentAngles[0] = self->activator->r.currentAngles[2] = 0;
		...
		if ( self->activator->health <= 0 )
		{
			
			{
				...
				else
				{
					if ( self->activator->client )
					{
						self->activator->client->ps.eFlags |= (1<<8);
					}
		...
		if ( DistanceSquared( radiusEnt->r.currentOrigin, boltOrg ) <= radiusSquared )
		{
				...
				&& radiusEnt->client->NPC_class != CLASS_VEHICLE )
			{
				...
				else if ( radiusEnt->client )
				{
					...
					NPC_SetAnim( radiusEnt, 1|2, BOTH_SWIM_IDLE1, 1 | 2 );
				}
			}
			else
			{
				...
				((angs)[0]=(NPC->client->ps.viewangles)[0],(angs)[1]=(NPC->client->ps.viewangles)[1],(angs)[2]=(NPC->client->ps.viewangles)[2]);
				angs[1] += flrand( 25, 50 );
				angs[0] = flrand( -25, -15 );
				AngleVectors( angs, pushDir, ((void *)0), ((void *)0) );
				if ( radiusEnt->client->NPC_class != CLASS_RANCOR
					&& radiusEnt->client->NPC_class != CLASS_ATST )
				{
					G_Damage( radiusEnt, NPC, NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand( 25, 40 ), 0x00000002|0x00000004, MOD_MELEE );
...
void Rancor_Smash( void )
{
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( distSq <= radiusSquared )
		{
			...
			{
				G_Damage( radiusEnt, NPC, NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand( 10, 25 ), 0x00000002|0x00000004, MOD_MELEE );
			}
			if ( radiusEnt->health > 0 
				&& radiusEnt->client
				&& radiusEnt->client->NPC_class != CLASS_RANCOR
...
void Rancor_Bite( void )
{
	...
	numEnts = NPC_GetEntsNearBolt( radiusEntNums, radius, NPC->client->renderInfo.crotchBolt, boltOrg );
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( DistanceSquared( radiusEnt->r.currentOrigin, boltOrg ) <= radiusSquared )
		{
			G_Damage( radiusEnt, NPC, NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand( 15, 30 ), 0x00000002|0x00000004, MOD_MELEE );
			if ( radiusEnt->health <= 0 && radiusEnt->client )
			{
				if ( !Q_irand( 0, 1 ) )
				{
					...
					{
						NPC_SetAnim( radiusEnt, 1|2, BOTH_DEATH17, 1 | 2 );
					}
					else if ( hitLoc == G2_MODELPART_WAIST )
					{
						NPC_SetAnim( radiusEnt, 1|2, BOTH_DEATHBACKWARD2, 1 | 2 );
					}
...
void Rancor_Attack( float distance, qboolean doCharge )
{
	if ( !TIMER_Exists( NPC, "attacking" ) )
	{
		...
		else if ( NPC->count == 1 && NPC->activator )
		{
			...
			{
				NPC_SetAnim( NPC, 1|2, BOTH_ATTACK1, 1 | 2 );
				TIMER_Set( NPC, "attack_dmg", 450 );
			}
			else
			{
				NPC_SetAnim( NPC, 1|2, BOTH_ATTACK3, 1 | 2 );
				...
				if ( NPC->activator->health > 0 && NPC->activator->client )
				{
					...
					NPC_SetAnim( NPC->activator, 1, BOTH_FALLDEATH1, 1 | 2 );
					if ( NPC->activator->NPC )
					{
						TossClientItems( NPC );
						NPC->activator->NPC->nextBStateThink = 16777216;
					}
				}
			}
		}
		...
		{
			vec3_t	fwd, yawAng;
			((yawAng)[0]=(0), (yawAng)[1]=(NPC->client->ps.viewangles[1]), (yawAng)[2]=(0));
			AngleVectors( yawAng, fwd, ((void *)0), ((void *)0) );
			((NPC->client->ps.velocity)[0]=(fwd)[0]*(distance*1.5f),(NPC->client->ps.velocity)[1]=(fwd)[1]*(distance*1.5f),(NPC->client->ps.velocity)[2]=(fwd)[2]*(distance*1.5f));
			NPC->client->ps.velocity[2] = 150;
			NPC->client->ps.groundEntityNum = ((1<<10)-1);

			NPC_SetAnim( NPC, 1|2, BOTH_MELEE2, 1 | 2 );
			TIMER_Set( NPC, "attack_dmg", 1250 );
		}
		else if ( !Q_irand(0, 1) )
		{
			NPC_SetAnim( NPC, 1|2, BOTH_MELEE1, 1 | 2 );
			TIMER_Set( NPC, "attack_dmg", 1000 );
		}
		else
		{
			NPC_SetAnim( NPC, 1|2, BOTH_ATTACK2, 1 | 2 );
			TIMER_Set( NPC, "attack_dmg", 1000 );
		}

		TIMER_Set( NPC, "attacking", NPC->client->ps.legsTimer + ((rand () & 0x7fff) / ((float)0x7fff)) * 200 );
	}
	...
	if ( TIMER_Done2( NPC, "attack_dmg", qtrue ) )
	{
		...
		case BOTH_MELEE1:
			...
			G_ScreenShake( shakePos, ((void *)0), 4.0f, 1000, qfalse );
		...
		case BOTH_ATTACK1:
			...
			{
				G_Damage( NPC->activator, NPC, NPC, vec3_origin, NPC->activator->r.currentOrigin, Q_irand( 25, 40 ), 0x00000002|0x00000004, MOD_MELEE );
				if ( NPC->activator->health <= 0 )
				{
					...
					NPC_SetAnim( NPC->activator, 1|2, BOTH_SWIM_IDLE1, 1 | 2 );
				}
		...
		case BOTH_ATTACK3:
			if ( NPC->count == 1 && NPC->activator )
			{
				...
				G_Damage( NPC->activator, NPC, NPC, vec3_origin, NPC->activator->r.currentOrigin, NPC->enemy->health+10, 0x00000008|0x00000002|0x00000004|0x00002000, MOD_MELEE );
				if ( NPC->activator->client )
				{
					NPC->activator->client->ps.forceHandExtend = HANDEXTEND_NONE;
					NPC->activator->client->ps.forceHandExtendTime = 0;
	...
	else if ( TIMER_Done2( NPC, "attack_dmg2", qtrue ) )
	{
		...
		case BOTH_ATTACK3:
			if ( NPC->count == 1 && NPC->activator )
			{
				...
				if ( NPC->activator->client )
				{
					NPC->activator->client->ps.eFlags |= (1<<8);
				}
				NPC->count = 2;
...
void Rancor_Combat( void )
{
...

	{
		...
		advance = (qboolean)( distance > (NPC->r.maxs[0]+128) ? qtrue : qfalse  );
		doCharge = qfalse;

		if ( advance )
		{
			vec3_t	yawOnlyAngles;
			((yawOnlyAngles)[0]=(0), (yawOnlyAngles)[1]=(NPC->r.currentAngles[1]), (yawOnlyAngles)[2]=(0));
		...
		if (( advance  ) && TIMER_Done( NPC, "attacking" )) 
		{
			if ( TIMER_Done2( NPC, "takingPain", qtrue ))
			{
				NPCInfo->localState = 0;
				...
				&& self->client->ps.legsAnim != BOTH_ATTACK2 )
			{
...

				{
					if ( self->health > 100 || hitByRancor )
					{
						...
						((self->s.angles)[0]=(self->NPC->lastPathAngles)[0],(self->s.angles)[1]=(self->NPC->lastPathAngles)[1],(self->s.angles)[2]=(self->NPC->lastPathAngles)[2]);

						if ( self->count == 1 )
						{
							NPC_SetAnim( self, 1|2, BOTH_PAIN2, 1 | 2 );
						}
						else
						{
							NPC_SetAnim( self, 1|2, BOTH_PAIN1, 1 | 2 );
						}
						TIMER_Set( self, "takingPain", self->client->ps.legsTimer+Q_irand(0, 500) );

						if ( self->NPC )
						{
							self->NPC->localState = 1;
						}
					}
				}
			}
...
void Rancor_CheckDropVictim( void )
{
	...
	((mins)[0]=(NPC->activator->r.mins[0]-1), (mins)[1]=(NPC->activator->r.mins[1]-1), (mins)[2]=(0));
	((maxs)[0]=(NPC->activator->r.maxs[0]+1), (maxs)[1]=(NPC->activator->r.maxs[1]+1), (maxs)[2]=(1));
	((start)[0]=(NPC->activator->r.currentOrigin[0]), (start)[1]=(NPC->activator->r.currentOrigin[1]), (start)[2]=(NPC->activator->r.absmin[2])); 
	((end)[0]=(NPC->activator->r.currentOrigin[0]), (end)[1]=(NPC->activator->r.currentOrigin[1]), (end)[2]=(NPC->activator->r.absmax[2]-1)); 
...
void Rancor_Crush(void)
{
	...
	if (!NPC ||
		!NPC->client ||
		NPC->client->ps.groundEntityNum >= ((1<<10)-2))
	{ 
		return;
	}
	...
	{ 
		G_Damage(crush, NPC, NPC, ((void *)0), NPC->r.currentOrigin, 200, 0, MOD_CRUSH);
	}
}
...
void NPC_BSRancor_Default( void )
{
	...
	NPC->client->ps.eFlags2 &= ~((1<<1)|(1<<3));
	if ( NPC->count )
	{
		NPC->client->ps.eFlags2 |= (1<<1);
		if ( NPC->count == 2 )
		{
			NPC->client->ps.eFlags2 |= (1<<3);
		}
	}
	else
	{
	...
	if ( NPC->enemy )
	{
		...
		else if ( !NPC->count )
		{
			...
			{
				TIMER_Remove( NPC, "lookForNewEnemy" );
			...
			{
				gentity_t *newEnemy, *sav_enemy = NPC->enemy;
				NPC->enemy = ((void *)0);
