...
void G_AttackDelay( gentity_t *self, gentity_t *enemy )
{
	...
	{//delay their attack based on how far away they're facing from enemy
		...
		{//invert
			attDelay = 1200-attDelay;//052505: 2000-attDelay;
		}
		////061304attDelay += floor( (DotProduct( fwd, dir )+1.0f) * 2000.0f );//add up to 4000ms delay if they're facing away
...
//	}
	...
	if ( self->NPC && self->client )//061304&& self->client->ps.weapon == WP_SABER )
	{
...
gentity_t *NPC_PickEnemy( gentity_t *closestTo, int enemyTeam, qboolean checkVis, qboolean findPlayersFirst, qboolean findClosest )
{
	...
	{//try to find a player first
		...
		if( newenemy->client)////052504 who knows? && !(newenemy->flags & FL_NOTARGET) && !(newenemy->s.eFlags & EF_NODRAW))
		{
