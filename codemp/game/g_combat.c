...
void body_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	qboolean doDisint = qtrue;///qfalse;
...
//	char		adverb[64];
	...
	//mwstats birthdate for flatfile compression
	ltime -= 1113983094;

  ///printf( "The time is %s\n", ctime( &ltime ) );
  ///if ( attacker->client){
  //G_AddVoiceEvent( attacker, Q_irand( EV_VICTORY1, EV_VICTORY3 ), 2000 );
...
///}
	if (g_logstats.integer >= 1 
		...
		&& !self->NPC_type
		&& !attacker->NPC_type
		///&& !IsABot(self->s.number)
		///&& !IsABot(attacker->s.number)
		){
...
if ( attacker->client && self->client 
	...
	for (j=0;j< level.maxclients;j++){
		...
		int xpgain;
			...
			/*xpgain = ((((self->client->sess.xp)/2)- attacker->client->sess.xp)/ 3.0) + 1;
			if (xpgain < 1){
					xpgain = 1;
			}*/
			xpgain = level.clients[j].sess.betamt;//*xpgain*2);
			...
			//trap_SendServerCommand( -1, va("cp \"^5%s ^5won %igp on %s^5.\n^5%s ^5won a %igp commission.\n\"",  level.clients[j].pers.netname, xpgain, attacker->client->pers.netname, attacker->client->pers.netname, (xpgain / 4)) );
			level.clients[j].sess.bettor = 0;
		...
		if (attacker->client 
			...
			&& level.clients[j].sess.bettor > 0 
			////&& level.clients[j].sess.betamt >= 10
			){
			int xpgain;
			/////trap_SendServerCommand( -1, va("print \"^5%s ^5lost the bet on %s. \n\"",  level.clients[j].pers.netname, self->client->pers.netname) );
			xpgain = level.clients[j].sess.betamt;//*xpgain*2);
	...
	*/
	...
	if ( attacker ) {
		...
		if ( attacker->client ) {
			...
			//G_AddEvent( attacker, EV_POWERUP_BATTLESUIT, 0 );
	...
	/*if (self->health > -10 && self->health <= 0){
		strcpy(adverb,"");
	}
	if (self->health > -20 && self->health <= -10){
		strcpy(adverb,"definitely ");
	}
	if (self->health > -30 && self->health <= -20){
		strcpy(adverb,"excruciatingly ");
	}
	if (self->health > -40 && self->health <= -30){
		strcpy(adverb,"exceedingly ");
	}*/
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			{ //in duel, if you kill yourself, the person you are dueling against gets a kill for it
				...
				if (otherClNum >= 0 && otherClNum < MAX_CLIENTS &&
					...
					//so npcs don't give away victory
					&& self->s.eType != ET_NPC)
				{


					/*RoundXP(otherClNum, qtrue, 1);
					RoundXP(self->s.number, qfalse, 1);
					clientNum = otherClNum;
				...
				//so npcs don't trigger some kind of condition
				else if (self->s.eType != ET_NPC)
				{
					AddScore( attacker, self->r.currentOrigin, -1 );
				}
			}
			//just pduel/duel for now
			else if (self->s.eType != ET_NPC)
			{
		...
		if (g_gametype.integer == GT_DUEL)
		{ int clientNum = -1;//in duel, if you kill yourself, the person you are dueling against gets a kill for it
			int otherClNum = -1;
			if (level.sortedClients[0] == self->s.number)
			{
				otherClNum = level.sortedClients[1];
	...
	
	{
		...
		{ //Some droids don't have death anims
			...
			self->client->respawnTime = level.time + 15000;//((self->client->animations[anim].numFrames*40)/(50.0f / self->client->animations[anim].frameLerp))+300;
...
void G_Dismember( gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath )
{
	...
	*/
	...
	{ //The enemy has valid saber positions between this and last frame. Use them to factor in direction of the limb.
		...
		VectorScale(limb->epVelocity, 0.65f, limb->epVelocity);
...
void G_CheckForDismemberment(gentity_t *ent, gentity_t *enemy, vec3_t point, int damage, int deathAnim, qboolean postDeath)
{
	...
	if (ent->client && ent->client->ps.customRGBA[3] != 99)
	{
		...
		////trap_SendServerCommand( -1, va("cp \"^1Fatality.\n\""));
...
if (attacker && targ && attacker->client && targ->client && !attacker->NPC_type && !targ->NPC_type && strcmp(attacker->client->pers.netname,targ->client->pers.netname)
		...
		&& Q_irand(0,240) == 44 
		&& ((mod == MOD_MELEE && (dflags & DAMAGE_NO_DISMEMBER)) || mod == MOD_SABER)
		){
		...
		///SetRPGStats(attacker, attacker->s.number);
...
///	if (targ && targ->client && targ->client->)
	{
...
///	*////unwoot?
	...
	if (targ->client && !take && mod == MOD_MELEE){
		if (Q_irand(0,100) > 95){
			take = 1;
		}
	}
...
if (attacker->client 
		...
		/*&& ((attacker->client->sess.duelTeam == targ->client->sess.duelTeam 
	...
	if (targ->client 
		&& (targ->client->ps.eFlags & EF_TALK) 
		&& attacker->client 
		&& damage > 0 && targ->health > 0 && !targ->client->iAmALoser ){
		LoseKarma(attacker->s.number, "Honor: the truce.", 1);
	}
	///else if (targ->client && (targ->client->ps.eFlags & EF_CONNECTION) && attacker->client){
	///	LoseKarma(attacker->s.number, "Honor: the weak.", 1);
	///}
	else if ((targ->health <= 0 
		//&& !(targ->s.eFlags & EF2_USE_ALT_ANIM)
|| (targ->client && targ->client->iAmALoser 
	//&& !(targ->client->ps.eFlags & EF2_USE_ALT_ANIM)
	
	))
		
&& attacker->client && Q_irand(0,4) == 2){
			gentity_t *tent;
				///(int)ent->client->ps.origin[2] == (int)other->client->ps.origin[2] && other->client->ps.torsoAnim != FACE_SMILE && other->client->ps.electrifyTime <= level.time )
		
				G_SetAnim(attacker, NULL, SETANIM_TORSO, BOTH_DODGE_HOLD_FL + Q_irand(0,1), 1|2, 0);
				G_SetAnim(attacker, NULL, SETANIM_LEGS, BOTH_DODGE_HOLD_FL + Q_irand(0,1), 1|2, 0);
				attacker->client->ps.legsTimer += 800;


				LoseKarma(attacker->s.number, "Honor: the social compact.", 3);	
				G_Damage(attacker, attacker, attacker, NULL, attacker->client->ps.origin, -2, DAMAGE_NO_DAMAGE, MOD_TELEFRAG);

				///if (Q_irand(0,2) != 1){
				///G_Sound( attacker, CHAN_VOICE, G_SoundIndex( "sound/ambience/thunder4.mp3" ) );
				///}else{
				...
				if (targ->client && !(targ->client->ps.eFlags & EF2_USE_ALT_ANIM)){
					targ->client->ps.eFlags |= EF2_USE_ALT_ANIM;
				}else if (targ && !(targ->s.eFlags & EF2_USE_ALT_ANIM)){
					targ->s.eFlags |= EF2_USE_ALT_ANIM;
				}
				
				///LoseKarma(attacker->s.number, "Honor: the unliving.", 1);
	}
	else if (targ->client  
		&& attacker->client
		...
		&& strcmp(attacker->client->pers.netname, targ->client->pers.netname) 
		&& damage > 0){
		/////LoseKarma(attacker->s.number, "Honor: the disarmed.", 1);
	}


	/*else if (targ->client && attacker->client 
		&& damage > 0 
		&& PM_InRealKnockDown( &targ->client->ps ) 
		&& mod != MOD_FALLING
		&& mod != MOD_SUICIDE
		&& mod != MOD_UNKNOWN
		&& mod != MOD_TEAM_CHANGE
	...
	if (attacker->client && targ->client){
		...
		//G_Dismember( targ, targ, dir, Q_irand(0,8), 90, 0, BOTH_DEATH1, qfalse );
		...
		if (damage > 34 && damage < 50){
			xpgain = Q_irand(0,2);
			/////laggy-- trap_SendServerCommand(-1, va("print \"%s ^3has gained ^3%i^5 XP\n\"", attplayer, xpgain) );
			//trap_SendServerCommand(-1, va("cp \"%s ^5has gained ^3%i^5 XP\n\"", attacker->client->pers.netname, xpgain) );
			attacker->client->sess.xp += xpgain;
			///trap_SendServerCommand( -1, va("cp \"%s^5 caused ^6%s %s\n ^1injury to %s ^1(%i dmg^1).\n\"",attplayer, plural, modgerund, targplayer, damage) );
			/////laggy-- trap_SendServerCommand( -1, va("print \"%s^5 caused ^6%s %s\n ^4injury to %s ^1(%i dmg^1).\n\"",attplayer, plural, modgerund, targplayer, damage) );
			...
			///trap_SendServerCommand( -1, va("cp \"%s^1 dealt ^6%s %s\n ^1critical blow ^1to %s ^1(%i dmg^1).\n\"",attplayer, plural, modgerund, targplayer, damage) );
			///////trap_SendServerCommand( -1, va("print \"%s^1 dealt ^6%s %s ^1critical blow ^1to %s ^1(%i dmg^1).\n\"",attplayer, plural, modgerund, targplayer, damage) );
		}else if (damage >= 70 && damage < 100){

			xpgain = Q_irand(0,4);
			///trap_SendServerCommand(-1, va("print \"%s ^3has gained ^3%i^5 XP\n\"", attplayer, xpgain) );
			///trap_SendServerCommand(-1, va("cp \"%s ^5has gained ^3%i^5 XP\n\"", attacker->client->pers.netname, xpgain) );
			attacker->client->sess.xp += xpgain;

			///////trap_SendServerCommand( -1, va("print \"%s^1 dealt ^6%s %s ^6^MASSIVE critical^ ^1to %s ^1(%i dmg^1)!\n\"",attplayer, plural, modgerund, targplayer, damage) );
			G_Sound( targ, CHAN_VOICE, G_SoundIndex( "sound/FLAWLESS.WAV" ) );
			///trap_SendServerCommand( -1, va("cp \"%s^1 dealt ^6%s %s\n ^1^MASSIVE critical^ ^1to %s ^1(%i dmg^1)!\n\"",attplayer, plural, modgerund, targplayer, damage) );
		...
		//	trap_SendServerCommand(-1, va("cp \"%s ^5has gained ^3%i^5 XP\n\"", attacker->client->pers.netname, xpgain) );
			...
			G_Sound( targ, CHAN_VOICE, G_SoundIndex( "sound/FLAWLESS.WAV" ) );
			///trap_SendServerCommand( -1, va("cp \"%s^1 was ^6%sly\n ^1@RELIEVED OF COMMAND@ ^1by %s ^1(%i dmg^1)!!\n\"",targplayer, modgerund, attplayer, damage) );
			}
	}
