#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_saga.c"
...
#line 13 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_saga.c"
...
#line 14 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_saga.c"
...
static char gParseObjectives[16384];
...
{
	trap_SetConfigstring(((((37+256)+1)+1)+1), va("%i", team));
}
...
void InitSiegeMode(void)
{
	...
	if (g_siegeTeamSwitch.integer)
	{
		...
		{
			trap_SetConfigstring((((37+256)+1)+1), va("%i", g_siegePersistant.lastTime));
		}
		else
		{
			trap_SetConfigstring((((37+256)+1)+1), "0");
		}
	}
	else
	{ 
	...
	if (BG_SiegeGetValueGroup(siege_info, team1, gParseObjectives))
	{
		...
		{
			BG_SiegeSetTeamTheme(1, goalreq);
		}
	...
	if (BG_SiegeGetValueGroup(siege_info, team2, gParseObjectives))
	{
		if (BG_SiegeGetPairedValue(gParseObjectives, "UseTeam", goalreq))
		{
			BG_SiegeSetTeamTheme(2, goalreq);
		}
	...
	trap_SetConfigstring(((37+256)+1), gObjectiveCfgStr);

	
	BG_PrecacheSabersForSiegeTeam(1);
	BG_PrecacheSabersForSiegeTeam(2);

	G_SiegeRegisterWeaponsAndHoldables(1);
	G_SiegeRegisterWeaponsAndHoldables(2);
...
void G_SiegeSetObjectiveComplete(int team, int objective, qboolean failIt)
{
	...
	if (!p)
	{
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_saga.c", 391), 0) );
		return;
	}
...
qboolean G_SiegeGetCompletionStatus(int team, int objective)
{
	...
	if (!p)
	{
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_saga.c", 446), 0) );
		return qfalse;
	}
...
{ 
	...
	t = ((void *)0);
	while ( (t = G_Find (t, ((int)&(((gentity_t *)0)->targetname)), target)) != ((void *)0) )
	{
		if ( t == ent )
		{
...
void SiegeBroadcast_OBJECTIVECOMPLETE(int team, int client, int objective)
{
	...
	((nomatter)[0]=(nomatter)[1]=(nomatter)[2]=0);
...
{
	if (client != ((1<<10)-1) && g_entities[client].client && g_entities[client].client->sess.sessionTeam == team)
	{ 
		AddScore(&g_entities[client], g_entities[client].client->ps.origin, 20);
	}
...
void AddSiegeWinningTeamPoints(int team, int winner)
{
	...
	while (i < 32)
	{
		...
		if (ent && ent->client && ent->client->sess.sessionTeam == team)
		{
			if (i == winner)
			{
				AddScore(ent, ent->client->ps.origin, 10+30);
			}
			else
			{
				AddScore(ent, ent->client->ps.origin, 10);
			}
		}
...
void SiegeDoTeamAssign(void)
{
	...
	while (i < 32)
	{
		...
		{ 
			if (ent->client->sess.siegeDesiredTeam == 1)
			{
				ent->client->sess.siegeDesiredTeam = 2;
			}
			else if (ent->client->sess.siegeDesiredTeam == 2)
			{
				ent->client->sess.siegeDesiredTeam = 1;
			}

			if (ent->client->sess.sessionTeam == 1)
			{
				SetTeamQuick(ent, 2, qfalse);
			}
			else if (ent->client->sess.sessionTeam == 2)
			{
				SetTeamQuick(ent, 1, qfalse);
			}
		}
		i++;
	}
...
void SiegeRoundComplete(int winningteam, int winningclient)
{
	...
	if (winningclient != ((1<<10)-1) && g_entities[winningclient].client &&
		g_entities[winningclient].client->sess.sessionTeam != winningteam)
	{ 
		winningclient = ((1<<10)-1);
	}
	...
	trap_SetConfigstring((37+256), va("3|%i", level.time)); 
	...
	if (BG_SiegeGetValueGroup(siege_info, teamstr, gParseObjectives))
	{
		...
		if (originalWinningClient == ((1<<10)-1))
		{ 
            int i = 0;
			gentity_t *ent;
...
{ 
	...
	trap_SetConfigstring((37+256), va("0|%i", level.time)); 
}
...
void SiegeCheckTimers(void)
{
	...
	if (!gSiegeRoundBegun)
	{
		...
		{ 
			gSiegeBeginTime = level.time + 5000;
			trap_SetConfigstring((37+256), "1"); 
		}
		...
		else if (gSiegeBeginTime > (level.time + 5000))
		{
			gSiegeBeginTime = level.time + 5000;
		}
		else
		{
			trap_SetConfigstring((37+256), va("2|%i", gSiegeBeginTime - 5000)); 
		}
	}
}
...
void SiegeItemRemoveOwner(gentity_t *ent, gentity_t *carrier)
{
	...
	ent->genericValue8 = ((1<<10)-1); 

	if (carrier)
	{
		carrier->client->holdingObjectiveItem = 0; 
		carrier->r.svFlags &= ~0x00000020;
	}
}
...
void SiegeItemThink(gentity_t *ent)
{
	...
	if (ent->genericValue8 != ((1<<10)-1))
	{ 
		carrier = &g_entities[ent->genericValue8];

		if (carrier->inuse && carrier->client)
		{
			((ent->r.currentOrigin)[0]=(carrier->client->ps.origin)[0],(ent->r.currentOrigin)[1]=(carrier->client->ps.origin)[1],(ent->r.currentOrigin)[2]=(carrier->client->ps.origin)[2]);
			trap_LinkEntity(ent);
		}
	}
	else if (ent->genericValue1)
	{ 
		G_RunExPhys(ent, ent->radius, ent->mass, ent->random, qfalse, ((void *)0), 0);
	}

	
	if (ent->genericValue8 < 32)
	{
	...
	if (carrier)
	{
		...
		if (!carrier->inuse || !carrier->client ||
			(carrier->client->sess.sessionTeam != 1 && carrier->client->sess.sessionTeam != 2) ||
			(carrier->client->ps.pm_flags & 4096))
		{ 
			SiegeItemRespawnOnOriginalSpot(ent, ((void *)0));
		}
		...
		{ 
			...
			if ( trap_PointContents(carrier->client->ps.origin, carrier->s.number) & 0x00000800 )
			{ 
			...
			else
			{
				...
				ent->genericValue9 = level.time + 20000;
...
void SiegeItemTouch( gentity_t *self, gentity_t *other, trace_t *trace )
{
		...
		!other->client || other->s.eType == ET_NPC)
	{
		...
		{ 
			vec3_t escapePos;
			((escapePos)[0]=(self->r.currentOrigin)[0],(escapePos)[1]=(self->r.currentOrigin)[1],(escapePos)[2]=(self->r.currentOrigin)[2]);
			escapePos[2] += 1.0f;
	...
	other->r.svFlags |= 0x00000020; 
	self->genericValue8 = other->s.number; 

	self->genericValue9 = 0; 
...
{ 
	...
	{ 
		gentity_t *targ = G_Find (((void *)0), ((int)&(((gentity_t *)0)->targetname)), ent->paintarget);
...
void G_SiegeClientExData(gentity_t *msgTarg)
{
	...
	while (i < level.num_entities && count < 32)
	{
		ent = &g_entities[i];
