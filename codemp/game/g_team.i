#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_team.c"
...
#line 5 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_team.c"
...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_team.c"
...
qboolean OnSameTeam( gentity_t *ent1, gentity_t *ent2 ) {
	...
	if (g_gametype.integer == GT_SINGLE_PLAYER)
	{
		...
		if (ent1->r.svFlags & 0x00000008)
		{
			ent1IsBot = qtrue;
		}
		if (ent2->r.svFlags & 0x00000008)
		{
			ent2IsBot = qtrue;
		}
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if (targ->client->ps.powerups[enemy_flag_pw]) {
		...
		AddScore(attacker, targ->r.currentOrigin, 20);
	...
	if (tokens) {
		attacker->client->pers.teamState.lastfraggedcarrier = level.time;
		AddScore(attacker, targ->r.currentOrigin, 20 * tokens * tokens);
		attacker->client->pers.teamState.fragcarrier++;
	...
	if (targ->client->pers.teamState.lasthurtcarrier &&
		level.time - targ->client->pers.teamState.lasthurtcarrier < 8000 &&
	...
	flag = ((void *)0);
	while ((flag = G_Find (flag, ((int)&(((gentity_t *)0)->classname)), c)) != ((void *)0)) {
		if (!(flag->flags & 0x00001000))
			break;
	}
	...
	((v1)[0]=(targ->r.currentOrigin)[0]-(flag->r.currentOrigin)[0],(v1)[1]=(targ->r.currentOrigin)[1]-(flag->r.currentOrigin)[1],(v1)[2]=(targ->r.currentOrigin)[2]-(flag->r.currentOrigin)[2]);
	((v2)[0]=(attacker->r.currentOrigin)[0]-(flag->r.currentOrigin)[0],(v2)[1]=(attacker->r.currentOrigin)[1]-(flag->r.currentOrigin)[1],(v2)[2]=(attacker->r.currentOrigin)[2]-(flag->r.currentOrigin)[2]);

	if ( ( ( VectorLength(v1) < 1000 &&
		trap_InPVS(flag->r.currentOrigin, targ->r.currentOrigin ) ) ||
		( VectorLength(v2) < 1000 &&
		trap_InPVS(flag->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
		attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {

		
		AddScore(attacker, targ->r.currentOrigin, 10);
	...
	if (carrier && carrier != attacker) {
		((v1)[0]=(targ->r.currentOrigin)[0]-(carrier->r.currentOrigin)[0],(v1)[1]=(targ->r.currentOrigin)[1]-(carrier->r.currentOrigin)[1],(v1)[2]=(targ->r.currentOrigin)[2]-(carrier->r.currentOrigin)[2]);
		((v1)[0]=(attacker->r.currentOrigin)[0]-(carrier->r.currentOrigin)[0],(v1)[1]=(attacker->r.currentOrigin)[1]-(carrier->r.currentOrigin)[1],(v1)[2]=(attacker->r.currentOrigin)[2]-(carrier->r.currentOrigin)[2]);
		...
		if ( ( ( VectorLength(v1) < 1000 &&
			...
			attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
			AddScore(attacker, targ->r.currentOrigin, 2);
			attacker->client->pers.teamState.carrierdefense++;
...
gentity_t *Team_ResetFlag( int team ) {
	...
	ent = ((void *)0);
	while ((ent = G_Find (ent, ((int)&(((gentity_t *)0)->classname)), c)) != ((void *)0)) {
...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if ( ent->flags & 0x00001000 ) {
		...
		AddScore(other, ent->r.currentOrigin, 10);
	...
	other->client->rewardTime = level.time + 2000;
	other->client->ps.persistant[PERS_CAPTURES]++;

	
	AddScore(other, ent->r.currentOrigin, 100);
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			...
			cl->sess.sessionTeam) {
			if (player != other)
				AddScore(player, ent->r.currentOrigin, 25);
			
			if (player->client->pers.teamState.lastreturnedflag + 
				10000 > level.time) {
				AddScore (player, ent->r.currentOrigin, 10);
				other->client->pers.teamState.assists++;

				player->client->ps.persistant[PERS_ASSIST_COUNT]++;
				player->client->rewardTime = level.time + 2000;
...
int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if (team == TEAM_RED)
		cl->ps.powerups[PW_REDFLAG] = 2147483647; 
	else
		cl->ps.powerups[PW_BLUEFLAG] = 2147483647; 
...
gentity_t *Team_GetLocation(gentity_t *ent)
{
	...
	((origin)[0]=(ent->r.currentOrigin)[0],(origin)[1]=(ent->r.currentOrigin)[1],(origin)[2]=(ent->r.currentOrigin)[2]);
...
qboolean Team_GetLocationMsg(gentity_t *ent, char *loc, int loclen)
{
	...
	if (best->count) {
		...
		if (best->count > 7)
			best->count = 7;
		Com_sprintf(loc, loclen, "%c%c%s" "^7", '^', best->count + '0', best->message );
	} else
...
gentity_t *SelectRandomTeamSpawnPoint( int teamstate, team_t team, int siegeClass ) {
	...
	if ( !count ) {	
		return G_Find( ((void *)0), ((int)&(((gentity_t *)0)->classname)), classname);
	}

	if (g_gametype.integer == GT_SIEGE && siegeClass >= 0 &&
		bgSiegeClasses[siegeClass].name[0])
	{ 
		gentity_t *classSpots[32];
		int classCount = 0;
		int i = 0;
...
static int __cdecl SortClients( const void *a, const void *b ) {
	return *(int *)a - *(int *)b;
}
...
void TeamplayInfoMessage( gentity_t *ent ) {
	...
	for (i = 0, cnt = 0; i < g_maxclients.integer && cnt < 32; i++) {
...
void CheckTeamStatus(void) {
	...
	if (level.time - level.lastTeamLocationTime > 1000) {
