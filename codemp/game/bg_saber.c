...
saberMoveName_t PM_CheckPullAttack( void )
{
	...
	if ( (pm->ps->saberMove == LS_READY||PM_SaberInReturn(pm->ps->saberMove)||PM_SaberInReflect(pm->ps->saberMove))//ready
		...
		&& pm->ps->forcePower >= g_butterflyforce.integer//have enough power
		...
		{	
			...
			*/
			if ( doMove )
			{
				...
				pm->ps->fd.forcePower -= g_butterflyforce.integer;
...
saberMoveName_t PM_SaberAttackForMovement(saberMoveName_t curmove)
{
	...
	if ( pm->ps->weapon == WP_SABER )
	{
		...
		overrideJumpLeftAttackMove = LS_JUMPATTACK_CART_LEFT;///LS_NONE
	}
	...
	{//moving right
		if ( !noSpecials
			...
			&& BG_EnoughForcePowerForMove( g_cartwheelforce.integer ) )//have enough power
		{//cartwheel right
	...
	{//not moving left or right
		...
		{//forward= T2B slash
			if (pm->ps->weapon == WP_MELEE || !noSpecials&&
				...
				(pm->ps->groundEntityNum != ENTITYNUM_NONE || PM_GroundDistance() <= 4000/*40*/) &&
				pm->ps->velocity[2] >= 0 &&
...
qboolean PM_CanDoKata( void )
{
	...
	if ( !pm->ps->saberInFlight//not throwing saber
		...
		*/
		...
		&& BG_EnoughForcePowerForMove(g_kataforce.integer) )// have enough power
...
qboolean PM_CanDoRollStab( void )
{
	if ( pm->ps->weapon == WP_SABER )
	{
		...
		saber = BG_MySaber( pm->ps->clientNum, 1 );
		if ( saber
			&& (saber->saberFlags&SFL_NO_ROLL_STAB) || pm->ps->fd.forcePower < g_butterflyforce.integer)
		{
			return qfalse;
		}
	}
	...
	if ( PM_CanDoKata() )
	{
		...
		{//not overridden
			...
			case SS_FAST:
				...
				break;
			case SS_TAVION:
				PM_SetSaberMove( LS_SPINATTACK_ALORA);///LS_STAFF_SOULCAL );
				////PM_SetSaberMove( LS_A1_SPECIAL );
