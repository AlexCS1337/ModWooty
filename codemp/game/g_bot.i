#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_bot.c"
...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_bot.c"


static int		g_numBots;
static char		*g_botInfos[1024];


int				g_numArenas;
static char		*g_arenaInfos[1024];
...
static botSpawnQueue_t	botSpawnQueue[16];
...
#line 36 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_bot.c"
float trap_Cvar_VariableValue( const char *var_name ) {
	char buf[128];
...
#line 43 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_bot.c"
...
static void G_LoadArenasFromFile( char *filename ) {
	...
	if ( !f ) {
		trap_Printf( va( "^1" "file not found: %s\n", filename ) );
		return;
	}
	if ( len >= 8192 ) {
		trap_Printf( va( "^1" "file too large: %s is %i, max allowed is %i", filename, len, 8192 ) );
	...
	g_numArenas += G_ParseInfos( buf, 1024 - g_numArenas, &g_arenaInfos[g_numArenas] );
}
...
#line 364 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_bot.c"
...
void G_AddRandomBot( int team ) {
	...
	num = ((rand () & 0x7fff) / ((float)0x7fff)) * num;
	for ( n = 0; n < g_numBots ; n++ ) {
		value = Info_ValueForKey( g_botInfos[n], "name" );
...
static void AddBotToSpawnQueue( int clientNum, int delay ) {
	...
	G_Printf( "^3" "Unable to delay spawn\n" );
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	if ( !botinfo ) {
		G_Printf( "^1" "Error: Bot '%s' not defined\n", name );
		return;
	}
...
#line 1184 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_bot.c"
...
static void G_LoadBotsFromFile( char *filename ) {
	...
	g_numBots += G_ParseInfos( buf, 1024 - g_numBots, &g_botInfos[g_numBots] );
}
...
static void G_LoadBots( void ) {
	...
	trap_Cvar_Register( &botsFile, "g_botsFile", "", 0x00000010|0x00000040 );
	if( *botsFile.string ) {
...
char *G_GetBotInfoByNumber( int num ) {
	if( num < 0 || num >= g_numBots ) {
		trap_Printf( va( "^1" "Invalid bot number: %i\n", num ) );
		return ((void *)0);
	}
...
void G_InitBots( qboolean restart ) {
	...
	trap_Cvar_Register( &bot_minplayers, "bot_minplayers", "0", 0x00000004 );
