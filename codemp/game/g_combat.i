#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
...
#line 7 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
...
int G_GetHitLocation(gentity_t *target, vec3_t ppoint)
{
	...
	if(target->client)
	{
		...
		((tangles)[0]=(0), (tangles)[1]=(target->r.currentAngles[1]), (tangles)[2]=(0));
	}

	AngleVectors(tangles, forward, right, up);

	
	((tcenter)[0]=(target->r.absmin)[0]+(target->r.absmax)[0],(tcenter)[1]=(target->r.absmin)[1]+(target->r.absmax)[1],(tcenter)[2]=(target->r.absmin)[2]+(target->r.absmax)[2]);
	((tcenter)[0]=(tcenter)[0]*(0.5),(tcenter)[1]=(tcenter)[1]*(0.5),(tcenter)[2]=(tcenter)[2]*(0.5));
	...
	{
		((point)[0]=(ppoint)[0],(point)[1]=(ppoint)[1],(point)[2]=(ppoint)[2]);
	}
	else
	{
		return HL_NONE;
	...
	((point_dir)[0]=(point)[0]-(tcenter)[0],(point_dir)[1]=(point)[1]-(tcenter)[1],(point_dir)[2]=(point)[2]-(tcenter)[2]);
	VectorNormalize(point_dir);

	
	udot = ((up)[0]*(point_dir)[0]+(up)[1]*(point_dir)[1]+(up)[2]*(point_dir)[2]);
	if(udot>.800)
	{
		Vertical = 4;
	}
	...
	fdot = ((forward)[0]*(point_dir)[0]+(forward)[1]*(point_dir)[1]+(forward)[2]*(point_dir)[2]);
	if(fdot>.666)
	{
		Forward = 4;
	}
	...
	rdot = ((right)[0]*(point_dir)[0]+(right)[1]*(point_dir)[1]+(right)[2]*(point_dir)[2]);
	if(rdot>.666)
	{
		Lateral = 4;
	}
...
void ExplodeDeath( gentity_t *self ) 
{
	...
	AngleVectors(self->s.angles, forward, ((void *)0), ((void *)0));
...
extern qboolean g_dontPenalizeTeam; 
void AddScore( gentity_t *ent, vec3_t origin, int score )
{
	...
	if (g_leveling.integer > 0){
		...
		WriteKeys(ent->s.number,6,qtrue);
...
void LookAtKiller( gentity_t *self, gentity_t *inflictor, gentity_t *attacker ) {
	...
	if ( attacker && attacker != self ) {
		((dir)[0]=(attacker->s.pos.trBase)[0]-(self->s.pos.trBase)[0],(dir)[1]=(attacker->s.pos.trBase)[1]-(self->s.pos.trBase)[1],(dir)[2]=(attacker->s.pos.trBase)[2]-(self->s.pos.trBase)[2]);
	} else if ( inflictor && inflictor != self ) {
		((dir)[0]=(inflictor->s.pos.trBase)[0]-(self->s.pos.trBase)[0],(dir)[1]=(inflictor->s.pos.trBase)[1]-(self->s.pos.trBase)[1],(dir)[2]=(inflictor->s.pos.trBase)[2]-(self->s.pos.trBase)[2]);
	} else {
		self->client->ps.stats[STAT_DEAD_YAW] = self->s.angles[1];
		return;
	}
...
void body_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if (self->client && (self->client->ps.eFlags & (1<<26)))
	{
		return;
	}
	else if (self->s.eFlags & (1<<26))
	{
		return;
	}
	...
	if (doDisint)
	{
		if (self->client)
		{
			self->client->ps.eFlags |= (1<<26);
			((self->client->ps.lastHitLoc)[0]=(self->client->ps.origin)[0],(self->client->ps.lastHitLoc)[1]=(self->client->ps.origin)[1],(self->client->ps.lastHitLoc)[2]=(self->client->ps.origin)[2]);
		}
		else
		{
			self->s.eFlags |= (1<<26);
			((self->s.origin2)[0]=(self->r.currentOrigin)[0],(self->s.origin2)[1]=(self->r.currentOrigin)[1],(self->s.origin2)[2]=(self->r.currentOrigin)[2]);
...
#line 871 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
}
...
static int G_CheckSpecialDeathAnim( gentity_t *self, vec3_t point, int damage, int mod, int hitLoc )
{
	...
	{
		deathAnim = BOTH_DEATH_ROLL;		
	}
	else if ( BG_FlippingAnim( self->client->ps.legsAnim ) )
	{
		deathAnim = BOTH_DEATH_FLIP;		
	}
	else if ( G_InKnockDown( &self->client->ps ) )
	{
		...
		case BOTH_KNOCKDOWN4:
			...
			else
			{
				...
				AngleVectors( self->client->ps.viewangles, fwd, ((void *)0), ((void *)0) );
				thrown = ((fwd)[0]*(self->client->ps.velocity)[0]+(fwd)[1]*(self->client->ps.velocity)[1]+(fwd)[2]*(self->client->ps.velocity)[2]);

				if ( thrown < -150 )
				{
					deathAnim = BOTH_DEATHBACKWARD1;	
				}
				else
				{
					deathAnim = BOTH_DEATH_CROUCHED;	
				}
			}
			break;
		case BOTH_KNOCKDOWN5:
...
int G_PickDeathAnim( gentity_t *self, vec3_t point, int damage, int mod, int hitLoc )
{
	...
	if (self->client)
	{
		...
		if (self->client->inSpaceIndex && self->client->inSpaceIndex != ((1<<10)-1))
		{
			return BOTH_CHOKE3;
		}
	}
	...
	if (self->client)
	{
		((objVelocity)[0]=(self->client->ps.velocity)[0],(objVelocity)[1]=(self->client->ps.velocity)[1],(objVelocity)[2]=(self->client->ps.velocity)[2]);
	}
	else
	{
		((objVelocity)[0]=(self->s.pos.trDelta)[0],(objVelocity)[1]=(self->s.pos.trDelta)[1],(objVelocity)[2]=(self->s.pos.trDelta)[2]);
	}
	...
	if ( deathAnim == -1 )
	{
		...
		if (deathAnim == -1)
		{
			...
			case HL_BACK_RT:
				...
				{
					deathAnim = BOTH_DEATH9;
				}
				else if ( damage <= max_health*0.5 )
				{
					deathAnim = BOTH_DEATH3;
				}
				else if ( damage <= max_health*0.75 )
				{
					deathAnim = BOTH_DEATH6;
				}
				else 
				{
			...
			case HL_BACK_LT:
				if ( damage <= max_health*0.25 )
				{
					deathAnim = BOTH_DEATH11;
				}
				else if ( damage <= max_health*0.5 )
				{
					deathAnim = BOTH_DEATH7;
				}
				else if ( damage <= max_health*0.75 )
				{
					deathAnim = BOTH_DEATH12;
				}
				else
				{
			...
			case HL_WAIST:
				if ( damage <= max_health*0.25 || !VectorLengthSquared( objVelocity ) )
				{
					if ( !Q_irand( 0, 1 ) )
					{
						deathAnim = BOTH_DEATH18;
					}
					else
					{
						deathAnim = BOTH_DEATH19;
					}
				}
				else if ( damage <= max_health*0.5 )
				{
					deathAnim = BOTH_DEATH2;
				}
				else if ( damage <= max_health*0.75 )
				{
					if ( !Q_irand( 0, 1 ) )
					{
						deathAnim = BOTH_DEATH1;
					}
					else
					{
						deathAnim = BOTH_DEATH16;
					}
				}
				else
				{
					deathAnim = BOTH_DEATH10;
				}
				break;
			case HL_HEAD:
...
void G_AlertTeam( gentity_t *victim, gentity_t *attacker, float radius, float soundDist )
{
	...
	if ( attacker == ((void *)0) || attacker->client == ((void *)0) )
		return;
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( check->NPC->scriptFlags & 0x00002000 )
			continue;

		
		if ( !(check->NPC->scriptFlags&0x00000800) )
			continue;

		
		if ( (check->NPC->scriptFlags&0x00020000) )
			continue;
		...
		if ( check->enemy == ((void *)0) )
		{
			distSq = DistanceSquared( check->r.currentOrigin, victim->r.currentOrigin );
			if ( distSq > 16384  && !trap_InPVS( victim->r.currentOrigin, check->r.currentOrigin ) )
			{
				continue;
			}
...
{
	G_AlertTeam( victim, attacker, 512, 512 );
}
...
void DeathFX( gentity_t *ent )
{
	...
	((defaultDir)[0]=(0), (defaultDir)[1]=(0), (defaultDir)[2]=(1));

	
	switch(ent->client->NPC_class)
	{
	case CLASS_MOUSE:
		((effectPos)[0]=(ent->r.currentOrigin)[0],(effectPos)[1]=(ent->r.currentOrigin)[1],(effectPos)[2]=(ent->r.currentOrigin)[2]);
		effectPos[2] -= 20;
	...
	case CLASS_ATST: 
		AngleVectors( ent->r.currentAngles, ((void *)0), right, ((void *)0) );
		((effectPos)[0]=(ent->r.currentOrigin)[0]+(right)[0]*(20),(effectPos)[1]=(ent->r.currentOrigin)[1]+(right)[1]*(20),(effectPos)[2]=(ent->r.currentOrigin)[2]+(right)[2]*(20));
		effectPos[2] += 180;
		G_PlayEffectID( G_EffectIndex("explosions/droidexplosion1"), effectPos, defaultDir );
		((effectPos)[0]=(effectPos)[0]+(right)[0]*(-40),(effectPos)[1]=(effectPos)[1]+(right)[1]*(-40),(effectPos)[2]=(effectPos)[2]+(right)[2]*(-40));
		G_PlayEffectID( G_EffectIndex("explosions/droidexplosion1"), effectPos, defaultDir );
		break;
	...
	case CLASS_MARK1:
		AngleVectors( ent->r.currentAngles, ((void *)0), right, ((void *)0) );
		((effectPos)[0]=(ent->r.currentOrigin)[0]+(right)[0]*(10),(effectPos)[1]=(ent->r.currentOrigin)[1]+(right)[1]*(10),(effectPos)[2]=(ent->r.currentOrigin)[2]+(right)[2]*(10));
		effectPos[2] -= 15;
		G_PlayEffectID( G_EffectIndex("explosions/droidexplosion1"), effectPos, defaultDir );
		((effectPos)[0]=(effectPos)[0]+(right)[0]*(-20),(effectPos)[1]=(effectPos)[1]+(right)[1]*(-20),(effectPos)[2]=(effectPos)[2]+(right)[2]*(-20));
		G_PlayEffectID( G_EffectIndex("explosions/droidexplosion1"), effectPos, defaultDir );
		((effectPos)[0]=(effectPos)[0]+(right)[0]*(-20),(effectPos)[1]=(effectPos)[1]+(right)[1]*(-20),(effectPos)[2]=(effectPos)[2]+(right)[2]*(-20));
		G_PlayEffectID( G_EffectIndex("explosions/droidexplosion1"), effectPos, defaultDir );
...
void player_die( gentity_t *self, 
	...
	if (g_logstats.integer >= 1 
		&& attacker && self && attacker->client && self->client
		&& (attacker->client->sess.sessionTeam != TEAM_SPECTATOR || g_gametype.integer!=4)
		...
		Com_sprintf(routePath, 1040, "%s\0", "kills.log");
...
if ( attacker->client && self->client 
		&& (attacker->client->sess.sessionTeam != TEAM_SPECTATOR || g_gametype.integer!=4)
		&& attacker->client->pers.connected == CON_CONNECTED 
		&& attacker->client->sess.duelTeam != self->client->sess.duelTeam
		&& strcmp(attacker->client->pers.netname,self->client->pers.netname)
		&& !self->NPC_type
		&& !attacker->NPC_type
		){

		if (Q_irand(0,3) != 1){
			LoseKarma(attacker->s.number, "Honor.", -2);
		}
	}
	...
	for (j=0;j< level.maxclients;j++){
		if (self->s.eType != ET_NPC 
			&& attacker != self 
			&& attacker->client 
			&& level.clients[j].sess.bettor == attacker->s.number+1 
			&& level.clients[j].sess.bettor > 0){
		int xpgain;
			...
			xpgain = level.clients[j].sess.betamt;

	


			if (xpgain < 0){
			trap_SendServerCommand( -1, va("print \"^5House ^1assesses ^3%igp to ^5%s and -1 alignment.\n^5%s ^5wins a %igp commission.\n\"",  xpgain, level.clients[j].pers.netname, attacker->client->pers.netname, -1*(xpgain / 5)) );
			
			level.clients[j].sess.karma--;
			level.clients[j].sess.admin_number += xpgain;
			level.clients[0].sess.admin_number -= (xpgain + (xpgain / 5));
			attacker->client->sess.admin_number -= (xpgain / 5);
			WriteKeys(0,4,qtrue);
			WriteKeys(j,4,qtrue);
			WriteKeys(j,0,qtrue);
			WriteKeys(attacker->client->ps.clientNum,4,qtrue);

			}else{
			trap_SendServerCommand( -1, va("print \"^5House pays ^3%igp to ^5%s and +1 alignment.\n^5%s ^5wins a %igp commission.\n\"",  xpgain, level.clients[j].pers.netname, attacker->client->pers.netname, (xpgain / 5)) );
			...
			level.clients[j].sess.bettor = 0;
			level.clients[j].sess.betamt = 0;
		}

		if (attacker->client 
			&& self->client
			&& attacker->s.eType != ET_NPC 
			&& self != attacker && self->client 
			&& level.clients[j].sess.bettor == self->s.number+1 
			&& level.clients[j].sess.bettor > 0 
			...
			if (xpgain < 0){
				level.clients[j].sess.karma++;
				level.clients[j].sess.admin_number -= xpgain;
				trap_SendServerCommand( -1, va("print \"^5%s ^5won the ^1short ^5and 1 alignment.\n^5%s ^5wins a %igp commisssion.\n\"",  level.clients[j].pers.netname, attacker->client->pers.netname, -1*(xpgain / 10)) );
				level.clients[0].sess.admin_number += (xpgain / 10);
				attacker->client->sess.admin_number -= (xpgain / 10);
				WriteKeys(0,4,qtrue);
				WriteKeys(j,4,qtrue);
				WriteKeys(j,0,qtrue);
				WriteKeys(attacker->client->ps.clientNum,4,qtrue);
				...
				level.clients[j].sess.karma--;
				trap_SendServerCommand( -1, va("print \"^5%s ^5lost the bet and 1 alignment.\n^5%s ^5wins a %igp commission.\n\"",  level.clients[j].pers.netname, attacker->client->pers.netname, (xpgain / 10)) );	
				level.clients[0].sess.admin_number -= (xpgain / 10);
				attacker->client->sess.admin_number += (xpgain / 10);
				WriteKeys(0,4,qtrue);
				WriteKeys(j,4,qtrue);
		...
		if (attacker->client && self->client && attacker == self 
			&& level.clients[j].sess.bettor == self->s.number+1 
			&& level.clients[j].sess.bettor > 0 ){
			trap_SendServerCommand( -1, va("print \"^5%s ^1forfeited the bet.\n\"",  attacker->client->pers.netname) );
			level.clients[j].sess.bettor = 0;
			level.clients[j].sess.betamt = 0;
		}
	...
	{ 
		...
		if ( self->m_pVehicle->m_pVehicleInfo->hideRider )
		{
			...
			{
				G_Damage(killEnt, murderer, murderer, ((void *)0), killEnt->client->ps.origin, 99999, 0x00000008, MOD_BLASTER);
			}
	...
	self->client->bodyGrabIndex = ((1<<10)-1);
	...
	{ 
		...
		if (veh->inuse && veh->client && veh->m_pVehicle)
		{
			...
			{ 
				...
				((self->client->ps.origin)[0]=(veh->client->ps.origin)[0],(self->client->ps.origin)[1]=(veh->client->ps.origin)[1],(self->client->ps.origin)[2]=(veh->client->ps.origin)[2]);
			}
		}
		...
		case CLASS_R2D2:
			...
			{
				vec3_t	up;
				AngleVectors( self->r.currentAngles, ((void *)0), ((void *)0), up );
	...
	{
		attacker->NPC->group->enemy = ((void *)0);
	}
	...
	if ( attacker ) {
		...
		if ( attacker->client ) {
			...
			if (Q_irand(0,1) == 0){
			G_AddEvent( attacker, Q_irand( EV_VICTORY1, EV_VICTORY3 ), 0 );
				}else{
			G_AddEvent( attacker, Q_irand( EV_GLOAT1, EV_GLOAT3 ), 0 );
			}
	...
	if (self->client && self->s.eType != ET_NPC && !g_noPDuelCheck)
	{
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			if (g_gametype.integer == GT_JEDIMASTER)
			{
				...
				{ 
					...
					ThrowSaberToAttacker(self, ((void *)0));
			...
			else if (g_gametype.integer != GT_POWERDUEL && self->s.eType != ET_NPC)
			{
				AddScore( attacker, self->r.currentOrigin, 1 );
			}
			...
			if( meansOfDeath == MOD_STUN_BATON ) {
				...
				attacker->client->rewardTime = level.time + 2000;

				
				self->client->ps.persistant[PERS_PLAYEREVENTS] ^= 0x0002;
			}
		...
		{ int clientNum = -1;
			...
			if (otherClNum >= 0 && otherClNum < 32 &&
				g_entities[otherClNum].inuse && g_entities[otherClNum].client &&
				otherClNum != self->s.number && self->s.eType != ET_NPC)
			{
	...
	if ( !( contents & 0x00000800 ) && !self->client->ps.fallingToDeath) {
		if (self->s.eType != ET_NPC)
		{
	...
	if ( (self->health <= -20 && !(contents & 0x00000800) && g_blood.integer) || meansOfDeath == MOD_SUICIDE) 
	{
	...
	
	{
		...
		{ 
			...
			if (self->inuse)
			{ 
				G_SetAnim(self, ((void *)0), 1|2, anim, 1|2|4, 0);
			}
	...
	if ( self->NPC && (self->NPC->scriptFlags&0x00000100) )
	{
	...
	if (g_gametype.integer == GT_POWERDUEL && !g_noPDuelCheck && !self->NPC_type)
	{ 
...
int CheckArmor (gentity_t *ent, int damage, int dflags)
{
	...
	if (dflags & 0x00000400)
	{	
		save = ceil( damage * 0.50 );
	}
	else
	{	
		save = damage;
	...
	if (dflags & 0x00000800)		
	{
		client->ps.stats[STAT_ARMOR] -= (int)(save*0.50);
	}
	else
	{
...
void G_ApplyKnockback( gentity_t *targ, vec3_t newDir, float knockback )
{
	...
	{
		((kvel)[0]=(newDir)[0]*(g_knockback.value * (float)knockback / mass * 0.8),(kvel)[1]=(newDir)[1]*(g_knockback.value * (float)knockback / mass * 0.8),(kvel)[2]=(newDir)[2]*(g_knockback.value * (float)knockback / mass * 0.8));
		kvel[2] = newDir[2] * g_knockback.value * (float)knockback / mass * 1.5;
	}
	else
	{
		((kvel)[0]=(newDir)[0]*(g_knockback.value * (float)knockback / mass),(kvel)[1]=(newDir)[1]*(g_knockback.value * (float)knockback / mass),(kvel)[2]=(newDir)[2]*(g_knockback.value * (float)knockback / mass));
	}

	if ( targ->client )
	{
		((targ->client->ps.velocity)[0]=(targ->client->ps.velocity)[0]+(kvel)[0],(targ->client->ps.velocity)[1]=(targ->client->ps.velocity)[1]+(kvel)[1],(targ->client->ps.velocity)[2]=(targ->client->ps.velocity)[2]+(kvel)[2]);
	}
	else if ( targ->s.pos.trType != TR_STATIONARY && targ->s.pos.trType != TR_LINEAR_STOP && targ->s.pos.trType != TR_NONLINEAR_STOP )
	{
		((targ->s.pos.trDelta)[0]=(targ->s.pos.trDelta)[0]+(kvel)[0],(targ->s.pos.trDelta)[1]=(targ->s.pos.trDelta)[1]+(kvel)[1],(targ->s.pos.trDelta)[2]=(targ->s.pos.trDelta)[2]+(kvel)[2]);
		((targ->s.pos.trBase)[0]=(targ->r.currentOrigin)[0],(targ->s.pos.trBase)[1]=(targ->r.currentOrigin)[1],(targ->s.pos.trBase)[2]=(targ->r.currentOrigin)[2]);
...
int RaySphereIntersections( vec3_t origin, float radius, vec3_t point, vec3_t dir, vec3_t intersections[2] ) {
	...
	if (d > 0) {
		t = (- b + sqrt(d)) / 2;
		((intersections[0])[0]=(point)[0]+(dir)[0]*(t),(intersections[0])[1]=(point)[1]+(dir)[1]*(t),(intersections[0])[2]=(point)[2]+(dir)[2]*(t));
		t = (- b - sqrt(d)) / 2;
		((intersections[1])[0]=(point)[0]+(dir)[0]*(t),(intersections[1])[1]=(point)[1]+(dir)[1]*(t),(intersections[1])[2]=(point)[2]+(dir)[2]*(t));
		return 2;
	}
	else if (d == 0) {
		t = (- b ) / 2;
...
{ 
	...
	((boltPoint)[0]=(self->r.currentOrigin)[0],(boltPoint)[1]=(self->r.currentOrigin)[1],(boltPoint)[2]=(self->r.currentOrigin)[2]);
...
void G_GetDismemberBolt(gentity_t *self, vec3_t boltPoint, int limbType)
{
	...
	((properAngles)[0]=(self->client->ps.viewangles)[0],(properAngles)[1]=(self->client->ps.viewangles)[1],(properAngles)[2]=(self->client->ps.viewangles)[2]);
	...
	trap_G2API_GetBoltMatrix(self->ghoul2, 0, useBolt, &boltMatrix, properAngles, properOrigin, level.time, ((void *)0), self->modelScale);
	...
	{ 
		...
		te->s.otherEntityNum2 = ((1<<10)-1);
		te->s.weapon = 0;
		te->s.legsAnim = 0;

		((te->s.origin)[0]=(boltPoint)[0],(te->s.origin)[1]=(boltPoint)[1],(te->s.origin)[2]=(boltPoint)[2]);
		((te->s.angles)[0]=(boltAngles)[0],(te->s.angles)[1]=(boltAngles)[1],(te->s.angles)[2]=(boltAngles)[2]);
...
void LimbThink( gentity_t *ent )
{
	...
	{ 
		G_RunExPhys(ent, gravity, mass, bounce, qtrue, ((void *)0), 0);
...
#line 3576 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
...
#line 3578 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
...
void G_Dismember( gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath )
{
	...
	((limb->s.pos.trBase)[0]=(newPoint)[0],(limb->s.pos.trBase)[1]=(newPoint)[1],(limb->s.pos.trBase)[2]=(newPoint)[2]);
	limb->think = LimbThink;
	limb->touch = LimbTouch;
	limb->speed = level.time + Q_irand(8000, 16000);
	limb->nextthink = level.time + 50;

	limb->r.svFlags = 0x00000080;
	limb->clipmask = (0x00000001|0x00001000);
	limb->r.contents = 0x00000400;
	limb->physicsObject = qtrue;
	((limb->r.mins)[0]=(-6.0f), (limb->r.mins)[1]=(-6.0f), (limb->r.mins)[2]=(-3.0f));
	((limb->r.maxs)[0]=(6.0f), (limb->r.maxs)[1]=(6.0f), (limb->r.maxs)[2]=(6.0f));
	...
	((limb->s.apos.trDelta)[0]=(limb->s.apos.trDelta)[1]=(limb->s.apos.trDelta)[2]=0);

	if (ent->client)
	{
		((limb->r.currentAngles)[0]=(ent->client->ps.viewangles)[0],(limb->r.currentAngles)[1]=(ent->client->ps.viewangles)[1],(limb->r.currentAngles)[2]=(ent->client->ps.viewangles)[2]);
		((limb->s.apos.trBase)[0]=(ent->client->ps.viewangles)[0],(limb->s.apos.trBase)[1]=(ent->client->ps.viewangles)[1],(limb->s.apos.trBase)[2]=(ent->client->ps.viewangles)[2]);
	}
	else
	{
		((limb->r.currentAngles)[0]=(ent->r.currentAngles)[0],(limb->r.currentAngles)[1]=(ent->r.currentAngles)[1],(limb->r.currentAngles)[2]=(ent->r.currentAngles)[2]);
		((limb->s.apos.trBase)[0]=(ent->r.currentAngles)[0],(limb->s.apos.trBase)[1]=(ent->r.currentAngles)[1],(limb->s.apos.trBase)[2]=(ent->r.currentAngles)[2]);
	}

	
	limb->epGravFactor = 0;
	((limb->epVelocity)[0]=(limb->epVelocity)[1]=(limb->epVelocity)[2]=0);
	((dir)[0]=(point)[0]-(ent->r.currentOrigin)[0],(dir)[1]=(point)[1]-(ent->r.currentOrigin)[1],(dir)[2]=(point)[2]-(ent->r.currentOrigin)[2]);
	VectorNormalize( dir );
	if (ent->client)
	{
		((vel)[0]=(ent->client->ps.velocity)[0],(vel)[1]=(ent->client->ps.velocity)[1],(vel)[2]=(ent->client->ps.velocity)[2]);
	}
	else
	{
		((vel)[0]=(ent->s.pos.trDelta)[0],(vel)[1]=(ent->s.pos.trDelta)[1],(vel)[2]=(ent->s.pos.trDelta)[2]);
	}
	((limb->epVelocity)[0]=(vel)[0]+(dir)[0]*(80),(limb->epVelocity)[1]=(vel)[1]+(dir)[1]*(80),(limb->epVelocity)[2]=(vel)[2]+(dir)[2]*(80));
 ...
 && enemy->client->olderIsValid 
	...
	{ 
		...
		((limb->epVelocity)[0]=(limb->epVelocity)[0]*(0.65f),(limb->epVelocity)[1]=(limb->epVelocity)[1]*(0.65f),(limb->epVelocity)[2]=(limb->epVelocity)[2]*(0.65f));

		((dif)[0]=(enemy->client->lastSaberBase_Always)[0]-(enemy->client->olderSaberBase)[0],(dif)[1]=(enemy->client->lastSaberBase_Always)[1]-(enemy->client->olderSaberBase)[1],(dif)[2]=(enemy->client->lastSaberBase_Always)[2]-(enemy->client->olderSaberBase)[2]);
		...
		((limb->epVelocity)[0]=(limb->epVelocity)[0]+(dif)[0],(limb->epVelocity)[1]=(limb->epVelocity)[1]+(dif)[1],(limb->epVelocity)[2]=(limb->epVelocity)[2]+(dif)[2]);

		if (ent->client && (ent->client->ps.torsoTimer > 0 || !BG_InDeathAnim(ent->client->ps.torsoAnim)))
		{ 
			vec3_t preVel;

			((preVel)[0]=(limb->epVelocity)[0],(preVel)[1]=(limb->epVelocity)[1],(preVel)[2]=(limb->epVelocity)[2]);
			preVel[2] = 0;
		...
		{
			((limb->epVelocity)[0]=(limb->epVelocity)[0]*(0.3f),(limb->epVelocity)[1]=(limb->epVelocity)[1]*(0.3f),(limb->epVelocity)[2]=(limb->epVelocity)[2]*(0.3f));
		}
	}

	if ( ent->ghoul2 && limbName )
	{ 
...
int G_GetHitQuad( gentity_t *self, vec3_t hitloc )
{
	...
	{
		((clEye)[0]=(self->s.pos.trBase)[0],(clEye)[1]=(self->s.pos.trBase)[1],(clEye)[2]=(self->s.pos.trBase)[2]);
		clEye[2] += 16;
	}
...
qboolean G_GetHitLocFromSurfName( gentity_t *ent, const char *surfName, int *hitLoc, vec3_t point, vec3_t dir, vec3_t bladeDir, int mod )
{
	...
	*hitLoc = HL_NONE;
	...
	{
		*hitLoc = HL_WAIST;
		if ( ent->client != ((void *)0) && ent->ghoul2 )
		{
			mdxaBone_t	boltMatrix;
			vec3_t	tagOrg, angles;

			((angles)[0]=(0), (angles)[1]=(ent->r.currentAngles[1]), (angles)[2]=(0));
			if (kneeLBolt>=0)
			{
				trap_G2API_GetBoltMatrix( ent->ghoul2, 0, kneeLBolt, 
								&boltMatrix, angles, ent->r.currentOrigin,
								actualTime, ((void *)0), ent->modelScale );
	...
	else if ( !Q_strncmp( "torso", surfName, 5 ) )
	{
		...
		else
		{
			...
			if (ent->client->renderInfo.boltValidityTime != level.time)
			{
				...
				renderAng[0] = 0;
				renderAng[1] = ent->client->ps.viewangles[1];
				renderAng[2] = 0;

				UpdateClientRenderBolts(ent, ent->client->ps.origin, renderAng);
			}

			((dirToImpact)[0]=(point)[0]-(ent->client->renderInfo.torsoPoint)[0],(dirToImpact)[1]=(point)[1]-(ent->client->renderInfo.torsoPoint)[1],(dirToImpact)[2]=(point)[2]-(ent->client->renderInfo.torsoPoint)[2]);
			frontSide = ((t_fwd)[0]*(dirToImpact)[0]+(t_fwd)[1]*(dirToImpact)[1]+(t_fwd)[2]*(dirToImpact)[2]);
			rightSide = ((t_rt)[0]*(dirToImpact)[0]+(t_rt)[1]*(dirToImpact)[1]+(t_rt)[2]*(dirToImpact)[2]);
			upSide = ((t_up)[0]*(dirToImpact)[0]+(t_up)[1]*(dirToImpact)[1]+(t_up)[2]*(dirToImpact)[2]);
			if ( upSide < -10 )
			{
				*hitLoc = HL_WAIST;
			}
			...
			if (1) 
			{
				...
				if ( tagName )
				{
					...
					if ( tagBolt != -1 )
					{
						...
						if ( DistanceSquared( point, tagOrg ) < 256 )
						{
							float dot = ((dir)[0]*(tagDir)[0]+(dir)[1]*(tagDir)[1]+(dir)[2]*(tagDir)[2]);
							if ( dot < aoa && dot > -aoa )
							{
								dot = ((bladeDir)[0]*(tagDir)[0]+(bladeDir)[1]*(tagDir)[1]+(bladeDir)[2]*(tagDir)[2]);
								if ( dot < aoa && dot > -aoa )
								{
									dismember = qtrue;
								}
...
void G_CheckForDismemberment(gentity_t *ent, gentity_t *enemy, vec3_t point, int damage, int deathAnim, qboolean postDeath)
{
	...
	if (ent->client && ent->client->ps.customRGBA[3] != 99)
	{
		...
		G_Sound( ent, CHAN_VOICE, G_SoundIndex( "sound/fatality.wav" ) );

		if (enemy->client && enemy->client->sess.karma > Q_irand(4,64)){
			enemy->client->ps.powerups[PW_FORCE_BOON] = level.time + 2500;
		}

		
		ent->client->ps.customRGBA[3] = 99;

		if (ent->client && Q_irand(0,1) == 0 ){
		...
		if (enemy->client && enemy->client->sess.karma > Q_irand(4,64) && Q_irand(0,3) == 1){
		ent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= (1 << HI_HEALTHDISP);
		ItemUse_UseDisp(ent, HI_HEALTHDISP);
		G_AddEvent(ent, EV_USE_ITEM0+HI_HEALTHDISP, 0);
		}

		GibEntity( ent, enemy->s.number );
...
void G_LocationBasedDamageModifier(gentity_t *ent, vec3_t point, int mod, int dflags, int *damage)
{
	...
	if ((d_saberGhoul2Collision.integer && ent->client && ent->client->g2LastSurfaceTime == level.time && mod == MOD_SABER) || 
	...
	case HL_HEAD:
		...
		if (*damage >= 30){
		trap_SendServerCommand( -1, va("cp \"^1Head injury!\n\"" ) );
		}
		*damage *= 1.6;
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	char		attplayer[64], targplayer[64],modgerund[64], plural[3];


if (attacker && targ && attacker->client && targ->client && !attacker->NPC_type && !targ->NPC_type && strcmp(attacker->client->pers.netname,targ->client->pers.netname)
		&& attacker->client->sess.sessionTeam != TEAM_SPECTATOR 
		&& Q_irand(0,80) == 44 
		&& ((mod == MOD_MELEE && (dflags & 0x00008000)) || mod == MOD_SABER)
		){
		attacker->client->sess.str++;
		WriteKeys(attacker->client->ps.clientNum,1,qtrue);

		trap_SendServerCommand( -1, va("print \"%s^3 is training STR.\n\"",
			attacker->client->pers.netname) );

		if (!Q_irand(0,2) && attacker->client->sess.str+attacker->client->sess.dex+attacker->client->sess.con >= 45){
				attacker->client->sess.dex--;
				WriteKeys(attacker->client->ps.clientNum,2,qtrue);
		}else if ( attacker->client->sess.str+attacker->client->sess.dex+attacker->client->sess.con >= 45){
				attacker->client->sess.con--;
				WriteKeys(attacker->client->ps.clientNum,3,qtrue);
		}
		

	}else if (attacker && targ && attacker->client && targ->client && !attacker->NPC_type && !targ->NPC_type && strcmp(attacker->client->pers.netname,targ->client->pers.netname)
		
		&& Q_irand(0,90) == 49 
		&& (mod == MOD_FALLING || mod == MOD_MELEE)
		&& !(dflags & 0x00008000)
		){
		attacker->client->sess.dex++;
		WriteKeys(attacker->client->ps.clientNum,2,qtrue);

		trap_SendServerCommand( -1, va("print \"%s^3 is training DEX.\n\"",
			attacker->client->pers.netname) );
		if (!Q_irand(0,2) && attacker->client->sess.str+attacker->client->sess.dex+attacker->client->sess.con >= 45){
				attacker->client->sess.str--;
				WriteKeys(attacker->client->ps.clientNum,1,qtrue);
		...
		&& attacker->client->sess.sessionTeam != TEAM_SPECTATOR 
		&& Q_irand(0,100) == 59 
		&& mod != MOD_MELEE && mod != MOD_FALLING && mod != MOD_SABER
		){
		attacker->client->sess.con++;
		WriteKeys(attacker->client->ps.clientNum,3,qtrue);

		trap_SendServerCommand( -1, va("print \"%s^3 is training CON.\n\"",
		attacker->client->pers.netname) );
	...
	strcpy (modgerund, "hurting");
	strcpy (plural, "a");

	switch(mod){
		case  MOD_REPEATER_ALT:
			strcpy(modgerund,"smashing");
				strcpy (plural, "a");
			break;
		case  MOD_REPEATER_ALT_SPLASH:
			...
			break;
	case  MOD_WATER:
			strcpy(modgerund,"splashing");
				strcpy (plural, "a");
			break;
			

		case MOD_ROCKET : 
			strcpy(modgerund,"imploding");
				strcpy (plural, "an");
			break;
		...
		case MOD_ROCKET_HOMING : 
			strcpy(modgerund,"homing");
				strcpy (plural, "a");
			break;
		case MOD_THERMAL : 
			strcpy(modgerund,"exploding");
				strcpy (plural, "an");
			break;
	...
	case MOD_SENTRY : 
		...
		case MOD_VEHICLE : 
			strcpy(modgerund,"trampling");
				strcpy (plural, "a");
			break;
		case MOD_CONC : 
			strcpy(modgerund,"impacting");
				strcpy (plural, "an");
			break;
		case MOD_CONC_ALT : 
			strcpy(modgerund,"impacting");
			...
			break;
		case MOD_SABER : 
			strcpy(	modgerund,"slicing");
				strcpy (plural, "a");
			break;
		case MOD_SUICIDE : 
			strcpy(	modgerund,"hara-kiring");
				strcpy (plural, "a");
			break;
		case MOD_FALLING : 
			strcpy(	modgerund,"falling");
				strcpy (plural, "a");
			break;
		case MOD_CRUSH : 
			strcpy(	modgerund,"crushing");
				strcpy (plural, "a");
			break;
		case MOD_TELEFRAG: 
			strcpy(	modgerund,"telefunking");
				strcpy (plural, "a");
			break;
		case MOD_TRIGGER_HURT:
			...
			break;
		case MOD_MELEE:
			strcpy(	modgerund,"bludgeoning");
				strcpy (plural, "a");
			break;
		case MOD_BRYAR_PISTOL : 
			strcpy(	modgerund,"burning");
				strcpy (plural, "a");
			break;
		case MOD_BRYAR_PISTOL_ALT : 
			strcpy(	modgerund,"vaporizing");
				strcpy (plural, "a");
			break;
		case MOD_BLASTER : 
			strcpy(	modgerund,"burning");
		...
		case MOD_DISRUPTOR_SNIPER:
			strcpy(	modgerund,"sniping");
				strcpy (plural, "a");
			break;
		case MOD_DEMP2:
			strcpy(	modgerund,"shocking");
				strcpy (plural, "a");
			break;
		case MOD_DEMP2_ALT:
			strcpy(	modgerund,"electrocuting");
				strcpy (plural, "an");
			break;

		case MOD_UNKNOWN:
			strcpy(	modgerund,"unknowing");
				strcpy (plural, "an");
			break;
		case MOD_LAVA:
			strcpy(	modgerund,"immolating");
				strcpy (plural, "an");
			break;

		case MOD_BOWCASTER:
			strcpy(	modgerund,"shepish");
				strcpy (plural, "a");
			break;
		case MOD_REPEATER:
			strcpy(	modgerund,"riddling");
				strcpy (plural, "a");
			break;
	...
	if ( (targ->flags&0x00080000) && mod != MOD_SABER  && !targ->client)
	{
		return;
	}

	if ((targ->flags & 0x01000000) && mod != MOD_SABER)
	{ 
		return;
	}
	...
	if ( targ->client )
	{
		...
		if ( targ->client->ps.clientNum < 32 && targ->client->ps.m_iVehicleNum )
		{
	...
	if ( !inflictor ) {
		inflictor = &g_entities[((1<<10)-2)];
	}
	if ( !attacker ) {
		attacker = &g_entities[((1<<10)-2)];
	}
	...
	if ( targ->flags & 0x00000800 ) {
		knockback = 0;
	}
	if ( dflags & 0x00000004 ) {
		knockback = 0;
	}
	...
	if ( knockback && targ->client ) {
		...
		if (mod == MOD_SABER)
		{
			...
			((kvel)[0]=(dir)[0]*((g_knockback.value * (float)knockback / mass)*saberKnockbackScale),(kvel)[1]=(dir)[1]*((g_knockback.value * (float)knockback / mass)*saberKnockbackScale),(kvel)[2]=(dir)[2]*((g_knockback.value * (float)knockback / mass)*saberKnockbackScale));
		}
		else
		{
			((kvel)[0]=(dir)[0]*(g_knockback.value * (float)knockback / mass),(kvel)[1]=(dir)[1]*(g_knockback.value * (float)knockback / mass),(kvel)[2]=(dir)[2]*(g_knockback.value * (float)knockback / mass));
		}
		((targ->client->ps.velocity)[0]=(targ->client->ps.velocity)[0]+(kvel)[0],(targ->client->ps.velocity)[1]=(targ->client->ps.velocity)[1]+(kvel)[1],(targ->client->ps.velocity)[2]=(targ->client->ps.velocity)[2]+(kvel)[2]);
		...
		if ( !targ->client->ps.pm_time && (g_saberDmgVelocityScale.integer || mod != MOD_SABER || (dflags&0x00010000) || (dflags&0x00020000) || (dflags&0x00040000) || (dflags&0x00080000) ) ) {
			int		t;
	...
	if ( !(dflags & 0x00000008) ) {
		...
		if (targ->s.number >= 32 && targ->client 
			...
			attacker && attacker->inuse && attacker->client && targ->s.owner >= 0 && targ->s.owner < 32)
		{
		...
		if ( (targ->flags & 0x00000010) && targ->s.eType != ET_NPC ) {
			return;
		}

		if (targ && targ->client && (targ->client->ps.eFlags & (1<<27)) &&
			attacker && attacker->client && targ != attacker)
		{
			if (targ->client->invulnerableTimer <= level.time)
			{
				targ->client->ps.eFlags &= ~(1<<27);
			}
			else
			{
				return;
				...
				else if ( attacker->teamnodmg )
				{
					...
					if ( targ->teamnodmg == attacker->teamnodmg )
					{
						if (attacker->activator &&
							...
							attacker->activator->s.number < 32 &&
	...
	if ( client && client->ps.powerups[PW_BATTLESUIT] ) {
		...
		if ( ( dflags & 0x00000001 ) || ( mod == MOD_FALLING ) ) {
			return;
		}
		damage *= 0.5;
	}
	...
	if ( targ == attacker && !(dflags & 0x00004000)) {
		if ( g_gametype.integer == GT_SIEGE )
		{
			damage *= 1.5;
		}
			...
			targ->m_pVehicle )
		{
			...
			if ( targ->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL )
			{
						...
						&& targ->m_pVehicle->m_LandTrace.fraction >= 1.0f)
				{
					...
					AngleVectors( targ->m_pVehicle->m_vOrientation, ((void *)0), ((void *)0), vUp );
					((impactDir)[0]=(point)[0]-(targ->r.currentOrigin)[0],(impactDir)[1]=(point)[1]-(targ->r.currentOrigin)[1],(impactDir)[2]=(point)[2]-(targ->r.currentOrigin)[2]);
					VectorNormalize( impactDir );
					if ( surface <= 0 )
					{
						vec3_t	vFwd, vRight;
						AngleVectors( targ->m_pVehicle->m_vOrientation, vFwd, vRight, vUp );
						dot = ((vRight)[0]*(impactDir)[0]+(vRight)[1]*(impactDir)[1]+(vRight)[2]*(impactDir)[2]);
						if ( dot > 0.4f )
						{
							surface = 2;
						}
						...
						{
							dot = ((vFwd)[0]*(impactDir)[0]+(vFwd)[1]*(impactDir)[1]+(vFwd)[2]*(impactDir)[2]);
							if ( dot > 0.0f )
							{
								surface = 0;
							}
					...
					{
					case 0:
						dot = ((vUp)[0]*(impactDir)[0]+(vUp)[1]*(impactDir)[1]+(vUp)[2]*(impactDir)[2]);
						if ( dot > 0 )
						{
							targ->m_pVehicle->m_vOrientation[0] += impactStrength;
						}
						else
						{
							targ->m_pVehicle->m_vOrientation[0] -= impactStrength;
					...
					case 2:
						...
						if ( dot > 0 )
						{
							targ->m_pVehicle->m_vOrientation[2] -= impactStrength;
						}
						else
						{
							targ->m_pVehicle->m_vOrientation[2] += impactStrength;
...
#line 5692 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_combat.c"
	...
	if ( client ) {
		if ( attacker ) {
			...
			client->ps.persistant[PERS_ATTACKER] = ((1<<10)-2);
		}
		client->damage_armor += asave;
		client->damage_blood += take;
		client->damage_knockback += knockback;
		if ( dir ) {
			((client->damage_from)[0]=(dir)[0],(client->damage_from)[1]=(dir)[1],(client->damage_from)[2]=(dir)[2]);
			client->damage_fromWorld = qfalse;
		} else {
			((client->damage_from)[0]=(targ->r.currentOrigin)[0],(client->damage_from)[1]=(targ->r.currentOrigin)[1],(client->damage_from)[2]=(targ->r.currentOrigin)[2]);
	...
	if (attacker->client && attacker->client->pers.netname[1] > 0){
		strcpy(attplayer, attacker->client->pers.netname);
	}
	if (targ->client && targ->client->pers.netname[1] > 0){
		strcpy(targplayer, targ->client->pers.netname);
	}

	if (attacker->client && attacker->NPC_type && attacker->NPC_type[1] > 0){
		strcpy(attplayer, attacker->NPC_type);
	}
	if (targ->client && targ->NPC_type && targ->NPC_type[1] > 0){
		strcpy(targplayer, targ->NPC_type);
	}
	...
	if (take) 
	{
		...
		if (targ->client && targ->s.number < 32 &&
		...
		{ 
			...
			{ 
				...
				targ->client->ps.weaponTime = 900;
				targ->client->ps.electrifyTime = level.time + 900;
		...
		{ 
			...
			if (gPainHitLoc < HL_MAX && gPainHitLoc >= 0 && targ->locationDamage[gPainHitLoc] < 16777216 &&
		...
		if ( targ->health <= 0 ) {
			if ( client )
			{
				targ->flags |= 0x00000800;

				if (point)
				{
					((targ->pos1)[0]=(point)[0],(targ->pos1)[1]=(point)[1],(targ->pos1)[2]=(point)[2]);
				}
				else
				{
					((targ->pos1)[0]=(targ->client->ps.origin)[0],(targ->pos1)[1]=(targ->client->ps.origin)[1],(targ->pos1)[2]=(targ->client->ps.origin)[2]);
				}
			}
			...
			if ( targ->r.svFlags & 0x08000000 )
			{
				((targ->pos1)[0]=(point)[0],(targ->pos1)[1]=(point)[1],(targ->pos1)[2]=(point)[2]);
				if (dir)
				{
					((targ->pos2)[0]=(dir)[0],(targ->pos2)[1]=(dir)[1],(targ->pos2)[2]=(dir)[2]);
				}
				else
				{
					((targ->pos2)[0]=(targ->pos2)[1]=(targ->pos2)[2]=0);
				}
			}
			...
			{ 
				if ( (mod == MOD_SABER || (mod == MOD_MELEE ) )
					...
					&& !(dflags&0x00008000) )
				{
					G_CheckForDismemberment(targ, attacker, targ->pos1, take, targ->client->ps.torsoAnim, qfalse);
					G_CheckForDismemberment(targ, attacker, targ->pos1, take, targ->client->ps.torsoAnim, qfalse);
		...
		else 
		{
			...
			{
				if ( targ->client && (targ->client->ps.pm_flags&16384) )
				{
					G_LetGoOfWall( targ );
				}
			}
			if ( targ->pain ) 
			{
				...
				{ 
					...
					if (point)
					{
						((gPainPoint)[0]=(point)[0],(gPainPoint)[1]=(point)[1],(gPainPoint)[2]=(point)[2]);
					}
					else
					{
						((gPainPoint)[0]=(targ->r.currentOrigin)[0],(gPainPoint)[1]=(targ->r.currentOrigin)[1],(gPainPoint)[2]=(targ->r.currentOrigin)[2]);
					}
...
if (attacker->client 
		&& targ->client 
		&& strcmp(attacker->client->pers.netname, targ->client->pers.netname) 
		&& damage > 0
	...
	{
		LoseKarma(attacker->s.number, "Honor: your team.", 1);
	}	
	
	else if (attacker->client 
		&& targ->client && damage > 3 
		&& !strcmp(attacker->client->pers.netname, targ->client->pers.netname) 
...

{
		...
		LoseKarma(attacker->s.number, "Honor: your life.", 1);
	}

	if (targ->client 
		&& (targ->client->ps.eFlags & (1<<13)) 
		&& attacker->client 
		&& damage > 0 && targ->health > 0 && !targ->client->iAmALoser ){
		LoseKarma(attacker->s.number, "Honor: the truce.", 1);
	}
	
	
	
	else if ((targ->health <= 0 && !(targ->s.eFlags & (1<<1))
|| (targ->client && targ->client->iAmALoser && !(targ->client->ps.eFlags & (1<<1))))
		
&& attacker->client && Q_irand(0,4) == 2){
			gentity_t *tent;
				
		
				G_SetAnim(attacker, ((void *)0), 1, BOTH_DODGE_HOLD_FL + Q_irand(0,1), 1|2, 0);
				G_SetAnim(attacker, ((void *)0), 2, BOTH_DODGE_HOLD_FL + Q_irand(0,1), 1|2, 0);
				attacker->client->ps.legsTimer += 800;


				LoseKarma(attacker->s.number, "Honor: the social contract.", 3);	
				G_Damage(attacker, attacker, attacker, ((void *)0), attacker->client->ps.origin, -10, 0x00000200, MOD_TELEFRAG);

				
				
				
				G_Sound( attacker, CHAN_VOICE, G_SoundIndex( "sound/chr_e/rosh/misc/gloat3.mp3" ) );
				

				
				
				tent = G_TempEntity( attacker->client->ps.origin, EV_DISRUPTOR_SNIPER_MISS );
				tent->s.clientNum = attacker->s.clientNum;

			
				if (targ->client && !(targ->client->ps.eFlags & (1<<1))){
					targ->client->ps.eFlags |= (1<<1);
				}else if (targ && !(targ->s.eFlags & (1<<1))){
					targ->s.eFlags |= (1<<1);
				}
		...
		&& targ->client->ps.saberHolstered == 1
		&& strcmp(attacker->client->pers.netname, targ->client->pers.netname) 
		&& damage > 0){
	...
	else if (targ->client && attacker->client && damage > 7  
		&& PM_InKnockDown( &targ->client->ps ) 
		&& mod != MOD_FALLING
		&& mod != MOD_SUICIDE
		&& mod != MOD_UNKNOWN
		&& mod != MOD_TEAM_CHANGE
		...
		LoseKarma(attacker->s.number, "Honor: the weak.", 1);
	}

	if (attacker->client && targ->NPC_type && 
		
		(
		!Q_stricmp(targ->NPC_type,"jawa")
		|| !Q_stricmp(targ->NPC_type,"gonk")
		|| !Q_stricmp(targ->NPC_type,"r2d2")
		|| !Q_stricmp(targ->NPC_type,"protocol")
		|| !Q_stricmp(targ->NPC_type,"wampa")
		)

		){
		LoseKarma(attacker->s.number, "Honor: the innocent.", 1);
	}
	if (attacker->client && targ->NPC_type && damage >= 15 &&
		
		(
		!Q_stricmp(targ->NPC_type,"tavion")
		|| !Q_stricmp(targ->NPC_type,"alora")
		|| !Q_stricmp(targ->NPC_type,"tavion_scepter")
		|| !Q_stricmp(targ->NPC_type,"reborn")
		|| !Q_stricmp(targ->NPC_type,"luke")
		)

		){
		LoseKarma(attacker->s.number, "Justice is served.", -1);
	}

	if (targ->client 
		&& targ->client->brokenLimbs == -10 
		&& attacker->client 
		&& damage > 0){
		LoseKarma(attacker->s.number, "Honor: a fair fight.", 1);
	}
	if (targ->client 
		&& (targ->client->ps.eFlags & (1<<13)) 
		&& attacker->client 
	...
	if (attacker->client && targ->client){
			...
			+targ->locationDamage[HL_CHEST_RT] >= 60)
			{
				G_BreakArm(targ, BROKENLIMB_RARM);
			}
			else if (targ->locationDamage[HL_ARM_LT]
			+targ->locationDamage[HL_HAND_LT]
			+targ->locationDamage[HL_CHEST_LT] >= 60)
			{
				G_BreakArm(targ, BROKENLIMB_LARM);
			}


		if (damage > 34 && damage < 50){
			xpgain = Q_irand(0,2);
			
			
			attacker->client->sess.xp += xpgain;
			
			

		}else if(damage >= 50 && damage < 70 && !targ->NPC_type){
			...
			LoseKarma(attacker->s.number, "Timing.", -2);
			...
			G_Sound( targ, CHAN_VOICE, G_SoundIndex( "sound/FLAWLESS.WAV" ) );
			...
			trap_SendServerCommand( -1, va("print \"%s^1 was ^6%sly ^3@RELIEVED OF COMMAND@ ^1by %s ^1(%i dmg^1)!!\n\"",targplayer, modgerund, attplayer, damage) );
			G_Sound( targ, CHAN_VOICE, G_SoundIndex( "sound/FLAWLESS.WAV" ) );
...
qboolean CanDamage (gentity_t *targ, vec3_t origin) {
	...
	((midpoint)[0]=(targ->r.absmin)[0]+(targ->r.absmax)[0],(midpoint)[1]=(targ->r.absmin)[1]+(targ->r.absmax)[1],(midpoint)[2]=(targ->r.absmin)[2]+(targ->r.absmax)[2]);
	((midpoint)[0]=(midpoint)[0]*(0.5),(midpoint)[1]=(midpoint)[1]*(0.5),(midpoint)[2]=(midpoint)[2]*(0.5));

	((dest)[0]=(midpoint)[0],(dest)[1]=(midpoint)[1],(dest)[2]=(midpoint)[2]);
	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ((1<<10)-1), (0x00000001|0x00001000));
...
qboolean G_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius,
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) {
		...
		if( CanDamage (ent, origin) ) {
			...
			((dir)[0]=(ent->r.currentOrigin)[0]-(origin)[0],(dir)[1]=(ent->r.currentOrigin)[1]-(origin)[1],(dir)[2]=(ent->r.currentOrigin)[2]-(origin)[2]);
			...
			{ 
				G_Damage (ent, ((void *)0), (gentity_t *)attacker->m_pVehicle->m_pPilot, dir, origin, (int)points, 0x00000001, mod);
			}
			else
			{
				G_Damage (ent, ((void *)0), attacker, dir, origin, (int)points, 0x00000001, mod);
			}
			...
			{ 
				...
				evEnt->s.weapon = WP_ROCKET_LAUNCHER; 

				
				((evEnt->s.origin)[0]=(missile->r.currentOrigin)[0],(evEnt->s.origin)[1]=(missile->r.currentOrigin)[1],(evEnt->s.origin)[2]=(missile->r.currentOrigin)[2]);
				((evEnt->s.origin2)[0]=(ent->r.currentOrigin)[0],(evEnt->s.origin2)[1]=(ent->r.currentOrigin)[1],(evEnt->s.origin2)[2]=(ent->r.currentOrigin)[2]);
