...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	bs->botWeaponWeights[WP_CONCUSSION] = 15;///REPEATER
...
int WPOrgVisible(gentity_t *bot, vec3_t org1, vec3_t org2, int ignore)
{
	...
	trap_Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_SOLID/*MASK_SOLID*/);

	if (tr.fraction == 1)
	{
		trap_Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_PLAYERSOLID/*MASK_PLAYERSOLID*/);
...
int GetNearestVisibleWP(vec3_t org, int ignore)
{
	...
	{
		bestdist = 2500;//2400;///99999;/// 2400;///800;//99999;
...
void CheckForShorterRoutes(bot_state_t *bs, int newwpindex)
{
	...
	{ //we found a path we want to switch to, let's do it
		...
		bs->wpSwitchTime = level.time + 250;///1000;///3000;
...
int BotTrace_Jump(bot_state_t *bs, vec3_t traceto)
{
	...
	if (tr.fraction == 1 || Q_irand(0,100) >= 87)///95)
	{
			...
			)
		{
			...
			if (distcheck < closest && ((InFieldOfVision(bs->viewangles, 90, a) && !BotMindTricked(bs->client, i)) || BotCanHear(bs, &g_entities[i], distcheck)) && OrgVisible(bs->eye, g_entities[i].client->ps.origin, -1))
			{
				if (BotMindTricked(bs->client, i))
				{
					...
					{
						if (!hasEnemyDist || distcheck < (hasEnemyDist - 64))///128
...
int BotGetWeaponRange(bot_state_t *bs)
{
	...
	case WP_BLASTER:
		return BWEAPONRANGE_MID;
	case WP_DISRUPTOR:
		///return BWEAPONRANGE_SABER; ///MID
		return BWEAPONRANGE_MID; ///MID
	case WP_BOWCASTER:
		return BWEAPONRANGE_LONG;
	case WP_CONCUSSION: ///WP_REPEATER
		return BWEAPONRANGE_MID;
	case WP_DEMP2:
		return BWEAPONRANGE_LONG;
	case WP_FLECHETTE:
...
void MeleeCombatHandling(bot_state_t *bs)
{
	...
	if (bs->meleeStrafeTime < level.time)
	{
		...
		bs->meleeStrafeTime = level.time + Q_irand(250, 1000);///Q_irand(500, 1800);
	}
...
void SaberCombatHandling(bot_state_t *bs)
{
		...
		en_down == mid_down)
	{
		...
		if (bs->currentEnemy && bs->currentEnemy->client)
		{
			...
			{
				bs->beStill = level.time +Q_irand(250, 500); ///Q_irand(500, 1000);
				bs->saberSTime = level.time + Q_irand(600, 900);///Q_irand(1200, 1800);
			}
			else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 80 && (Q_irand(1, 10) < 6 /*8*/ && bs->saberBFTime < level.time) || bs->saberBTime > level.time || BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_DUAL)
			{
				vec3_t vs;
				vec3_t groundcheck;
				int idealDist;
...
float BotWeaponCanLead(bot_state_t *bs)
{
	...
	if (weap == WP_CONCUSSION)///WP_REPEATER
	{
		return 0.45;
	}
	if (weap == WP_THERMAL)
...
void BotAimOffsetGoalAngles(bot_state_t *bs)
{
	...
	{ //assume our goal is aiming at the enemy, seeing as he's visible and all
		...
		{
			accVal += accVal*0.025;///0.25; //if he's moving he's this much harder to hit
		}
		...
		{
			accVal += accVal*0.015;///0.15; //make it somewhat harder to aim if we're moving also
		}
	}
	...
	bs->aimOffsetTime = level.time + rand()%200 + 100;///500 + 200;
}
...
int ShouldSecondaryFire(bot_state_t *bs)
{
	...
	if ((weap == WP_BRYAR_PISTOL || weap == WP_BRYAR_OLD) && bs->frame_Enemy_Len > 400)
	{
		return 0;
	}
	else if (weap == WP_BLASTER && bs->frame_Enemy_Len > 325)
	{
		return 0;
	}

	else if (weap == WP_BOWCASTER && bs->frame_Enemy_Len > 325)
	{
		return 0;
	}
...
/*	else if (weap == WP_BOWCASTER && bs->frame_Enemy_Len > 300)
	...
	else if (weap == WP_CONCUSSION && bs->frame_Enemy_Len < 600 && bs->frame_Enemy_Len > 250)///WP_REPEATER
	{
		return 1;
	}*/
	else if (weap == WP_CONCUSSION && bs->frame_Enemy_Len < 250)///WP_REPEATER
	{
		return 0;
	}
/*	else if (weap == WP_BLASTER && bs->frame_Enemy_Len < 300)
	...
	else if (weap == WP_ROCKET_LAUNCHER && bs->frame_Enemy_Len > 500)
	{
		return 1;
	}*/
	else if (weap == WP_ROCKET_LAUNCHER && bs->frame_Enemy_Len <= 500)
...
int CombatBotAI(bot_state_t *bs, float thinktime)
{
	...
	else if (BotGetWeaponRange(bs) == BWEAPONRANGE_MELEE)
	{
		...
		{
			bs->doAttack = 1;
			if (&g_entities[bs->client].client->ps.weapon == WP_MELEE && Q_irand(0,5) == 3){//unwoot?
				G_SetAnim(&g_entities[bs->client], &g_entities[bs->client].client->pers.cmd, SETANIM_BOTH, BOTH_KYLE_GRAB, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
...
int BotSelectIdealWeapon(bot_state_t *bs)
{
	...
	if ( bs->currentEnemy && bs->frame_Enemy_Len > 150 &&////300 &&
	...
	{ //if the enemy is far away, and we have our saber selected, see if we have any good distance weapons instead
		...
		}else if (BotWeaponSelectable(bs, WP_DISRUPTOR))
		{
			...
			bestweight = 1;
		}else if (BotWeaponSelectable(bs, WP_CONCUSSION))///WP_REPEATER
		{
			bestweapon = WP_CONCUSSION;///WP_REPEATER
			bestweight = 1;
		}
...
stillmadeit:
	...
	{ //we could just call the "blow all my detpacks" function here, but I guess that's cheating.
		bs->plantKillEmAll = level.time + 250;///500;
	}
}
...
if (rand()%5000 < 25 && ent->client->ps.fd.forcePower >= g_craneforce.integer){
...
if (rand()%5000 > 4996 && ent->client->ps.fd.forcePower >= g_craneforce.integer){
		G_SetAnim(ent, NULL, SETANIM_TORSO, 870+Q_irand(0,20), 1|2, 0);
			////G_SetAnim(ent, NULL, SETANIM_LEGS, BOTH_WALK2, 1|2, 0);
			ent->client->ps.torsoTimer = level.time + 1000;
			ent->client->ps.weaponTime = ent->client->ps.torsoTimer;
			ent->client->ps.fd.forcePower -= g_craneforce.integer ;
}
	...
	if (rand()%5000 < 45 && ent->client->ps.fd.forcePower >= 100){				
		gentity_t *missile;
		vec3_t			fwd;
		int dexfactor, anim;
...
///						WP_FireHadokenBlasterMissile(ent, ent->client->renderInfo.muzzlePoint, ent->client->renderInfo.muzzleDir, 0, 9, 270, MOD_FORCE_DARK, qfalse);
			ent->chain->r.svFlags |= SVF_OWNERNOTSHARED; //only guided hadokens
	...
	if (!bs->wpCurrent)
	{
		...
		if (wp != -1)
		{
			...
			bs->wpSeenTime = level.time + 800;///1500;
			bs->wpTravelTime = level.time + 12000;///10000; //never take more than 10 seconds to travel to a waypoint
		}
	}
	...
	else //We can't find a waypoint, going to need a fallback routine.
	{
		...
		{ //helps them get out of messy situations
			...
			*/
			///g_entities[bs->client].client->ps.saberMove = Q_irand(0,162);
			bs->jumpTime = level.time+2000;/// + 1500;
			bs->jumpHoldTime = level.time + 3500;///1500;
	...
	if (bs->iHaveNoIdeaWhereIAmGoing > level.time && !bs->currentEnemy)
	{
		...
		bs->jumpHoldTime = level.time + 150;///150;
		bs->jDelay = 0;
		bs->lastSignificantChangeTime = level.time + 25000;
		//g_entities[bs->client].client->ps.pm_flags |= PMF_JUMP_HELD;
		//	bs->cur_ps.torsoAnim = 876;
		//bs->cur_ps.legsAnim = 876;
	...
	if (g_entities[bs->client].client->ps.velocity[2] < -150 ){
		//&& Q_irand(0,g_entities[bs->client].client->ps.fd.forcePower) >= (g_emoteforce.integer / 3)
		//Q_irand(0,100) > 40 && 
	
		vec3_t oppDir;
		
		///if (g_entities[bs->client].client->ps.origin[2] <= -25 ){
		if (g_entities[bs->client].client->ps.origin[2] <= -10 && Q_irand(0,10) <= 7 && g_entities[bs->client].client->ps.torsoTimer <= level.time ){

		int fooy;
	
		//bs->jumpTime = level.time + 2150;
		//bs->jDelay = 0;
		//bs->jumpHoldTime = level.time + 5550;///150;
		
		//bs->cur_ps.useTime = level.time + 2500;

			///trap_SendServerCommand( -1, va("cp \"^1Debug origin: %f veloc: %f.\n\"",g_entities[bs->client].client->ps.origin[2], g_entities[bs->client].client->ps.velocity[2]) );
		
		//bs->cur_ps.torsoAnim = 876;
		//bs->cur_ps.legsAnim = 876;
		//g_entities[bs->client].client->ps.fd.forcePower = 99;
		g_entities[bs->client].client->ps.torsoAnim = 872;
		g_entities[bs->client].client->ps.legsAnim = 872;
		g_entities[bs->client].client->ps.pm_flags |= PMF_JUMP_HELD;
		fooy = 100 * Q_irand(10,36);

		g_entities[bs->client].client->ps.torsoTimer = level.time + (int)(fooy*2.5f);
		g_entities[bs->client].client->ps.legsTimer = level.time + (int)(fooy*2.5f);
		//g_entities[bs->client].client->ps.weapontime = 0;
		
		
		//g_entities[bs->client].client->ps.pm_flags |= PMF_BACKWARDS_JUMP;
		g_entities[bs->client].client->ps.pm_time = level.time + fooy;

		g_entities[bs->client].client->ps.velocity[2] += 500;
		////g_entities[bs->client].client->ps.velocity[2] *= (float)-1.0f*(Q_irand(100,250)/100.0f);
		g_entities[bs->client].client->ps.velocity[1] += (float)Q_irand(-500,500);
		g_entities[bs->client].client->ps.velocity[0] += (float)Q_irand(-500,500);
	
		if (rand()%5000 < 50){
			NPC_Spawn_f(&g_entities[bs->client]);
		}
		g_entities[bs->client].client->ps.viewangles[2] *= (float)(Q_irand(-200,200)/100.0f);
		g_entities[bs->client].client->ps.viewangles[1] *= (float)(Q_irand(-200,200)/100.0f);
		g_entities[bs->client].client->ps.viewangles[0] *= (float)(Q_irand(-200,200)/100.0f);

		g_entities[bs->client].client->ps.torsoAnim = 872;
		g_entities[bs->client].client->ps.legsAnim = 872;
		g_entities[bs->client].client->ps.pm_flags |= PMF_JUMP_HELD;
		fooy = 100 * Q_irand(20,48);

		g_entities[bs->client].client->ps.torsoTimer = level.time + (int)(fooy*2.5f);
		g_entities[bs->client].client->ps.legsTimer = level.time + (int)(fooy*3.2f);

		
		//g_entities[bs->client].client->ps.origin[2] += Q_irand(15,45);
//Q_irand(25,75);

		bs->jumpTime = level.time + 750;
		bs->jumpHoldTime = level.time + (int)(fooy*3.5f);

		if (g_entities[bs->client].client->ps.fd.forcePower >= (g_emoteforce.integer/2)){
			g_entities[bs->client].client->ps.origin[2] += Q_irand(25,45);
			g_entities[bs->client].client->ps.fd.forcePower -= (g_emoteforce.integer/2);
			g_entities[bs->client].client->ps.pm_time = level.time + (int)(fooy*2.9f);

			if (Q_irand(0,10) <= 5){
			g_entities[bs->client].client->ps.pm_type = PM_FLOAT;
			///g_entities[bs->client].client->ps.pm_flags |= PMF_JUMP_HELD;
			g_entities[bs->client].client->ps.pm_time = level.time + (int)(fooy*3.2f);
			}
...
//g_entities[bs->client].client->ps.jetpackFuel = 90;
//g_entities[bs->client].client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_JETPACK );
//g_entities[bs->client].client->ps.pm_type = PM_JETPACK;
//g_entities[bs->client].client->jetPackOn = 1;
///g_entities[bs->client].client->ps.gravity /= 2;
...
///150;
//bs->cur_ps.useTime = level.time + 2500;
	///g_entities[bs->client].client->ps.pm_flags |= PMF_USE_ITEM_HELD;
...
*/
	...
	//g_entities[bs->client].client->ps.velocity[2] *= -1.75f;///*= -1.0f;
	
	if (g_entities[bs->client].client->ps.velocity[2] > 750){
		g_entities[bs->client].client->ps.velocity[2]= 750;
	}


	///g_entities[bs->client].client->ps.velocity[2]  *= -0.2f;///-1.0f;/// -1.25f;
	///
	...
	///g_entities[bs->client].client->pers.cmd.upmove = 127;


	///g_entities[bs->client].client->ps.velocity[1]  *= 0.75f;
		}
		

	//bs->cur_ps.velocity[1]  *= 3;
	//bs->cur_ps.velocity[0]  *= 3;

	}


	/*if (g_entities[bs->client].client->ps.fallingToDeath){
		vec3_t oppDir;
		bs->cur_ps.torsoAnim = 876;
		bs->cur_ps.legsAnim = 876;
		bs->cur_ps.torsoTimer= level.time + 1500;
		bs->cur_ps.legsTimer = level.time + 1500;
		bs->jumpTime = level.time + 250;
		bs->jumpHoldTime = level.time + 4550;///150;
		bs->cur_ps.velocity[2] *= -1;
		bs->cur_ps.useTime = level.time + 2500;
		g_entities[bs->client].client->ps.fallingToDeath=0;

VectorSubtract(g_entities[bs->client].client->ps.userVec1, g_entities[bs->client].client->ps.origin, oppDir);
		VectorNormalize(oppDir);

		g_entities[bs->client].client->ps.velocity[0] = (oppDir[0]+99998);//*(strength*600);
		g_entities[bs->client].client->ps.velocity[2] = (oppDir[2]+99998);//*(strength*600);	
		g_entities[bs->client].client->ps.velocity[1] = oppDir[1]+99998;//*(strength*2100)+300;
	...
	if (bs->currentEnemy)
	{
		...
		else if (BotGetWeaponRange(bs) == BWEAPONRANGE_MELEE)
		{
			if (bs->frame_Enemy_Len <= MELEE_ATTACK_RANGE)
			{
				...
				meleestrafe = Q_irand(0,1);//1 unwoot?
			}
		}
	}
	...
	if (bs->forceJumping > level.time)
	{
		...
		if (VectorLength(noz_x) < 320) //32 unwoot?
		{
			fjHalt = 1;
		}
	}
...
///	}
		...
		AltFiring(bs))
	{
		...
		if (friendInLOF)
		{
			if (PrimFiring(bs))
			{
				///KeepPrimFromFiring(bs); //unwoot?
			}
			if (AltFiring(bs))
			{
				///KeepAltFromFiring(bs); //unwoot?
			}
			if (useTheForce && forceHostile)
			{
				useTheForce = 0;
