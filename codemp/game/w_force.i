#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 3 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 4 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 5 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 19 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 21 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
gentity_t *G_PreDefSound(vec3_t org, int pdSound)
{
	...
	((te->s.origin)[0]=(org)[0],(te->s.origin)[1]=(org)[1],(te->s.origin)[2]=(org)[2]);
...
void WP_InitForcePowers( gentity_t *ent )
{
	...
	if (ent->s.eType == ET_NPC && ent->s.number >= 32)
	{ 
	...
	if ( (ent->r.svFlags & 0x00000008) && botstates[ent->s.number] )
	{ 
	...
	if (g_forceBasedTeams.integer)
	{
		...
		{
			warnClient = !(BG_LegalizedForcePowers(forcePowers, maxRank, HasSetSaberOnly(), 2, g_gametype.integer, g_forcePowerDisable.integer));
		}
		else if (ent->client->sess.sessionTeam == TEAM_BLUE)
		{
			warnClient = !(BG_LegalizedForcePowers(forcePowers, maxRank, HasSetSaberOnly(), 1, g_gametype.integer, g_forcePowerDisable.integer));
		}
		else
		{
	...
	if ( g_gametype.integer != GT_SIEGE && (ent->r.svFlags & 0x00000008) && botstates[ent->s.number] )
	{ 
		int oldI = i;
		i_r = 0;
		while (forcePowers[i] && forcePowers[i] != '\n' &&
		...
		{
			if (ent->client->ps.fd.forceSide == 1)
			{
				if (i_r == FP_ABSORB)
				{
					forcePowers[i] = '3';
			...
			else if (ent->client->ps.fd.forceSide == 2)
			{
				if (botstates[ent->s.number]->settings.skill >= 4)
				{
					if (i_r == FP_GRIP)
	...
	else
	{
		...
		{ 
			if (g_gametype.integer != GT_HOLOCRON && g_gametype.integer != GT_JEDIMASTER )
			{
				...
				if (!(ent->r.svFlags & 0x00000008) && ent->s.eType != ET_NPC)
				{
					if (!g_teamAutoJoin.integer)
					{
						...
						trap_SendServerCommand(ent-g_entities, "spc");	
					}
				}
...
#line 505 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
			}
			ent->client->sess.setForce = qtrue;
		}
...
#line 522 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
		}
...
void WP_SpawnInitForcePowers( gentity_t *ent )
{
	...
	ent->client->ps.fd.forcePower = ent->client->ps.fd.forcePowerMax = 101;
	ent->client->ps.fd.forcePowerRegenDebounceTime = 0;
	ent->client->ps.fd.forceGripEntityNum = ((1<<10)-1);
	...
	ent->client->ps.fd.forceDrainEntNum = ((1<<10)-1);
...
#line 694 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
#line 696 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\w_force.c"
...
qboolean WP_ForcePowerUsable( gentity_t *self, forcePowers_t forcePower )
{
	...
	if (self->health <= 0 || self->client->ps.stats[STAT_HEALTH] <= 0 ||
		(self->client->ps.eFlags & (1<<1)))
	{
		return qfalse;
	}

	if (self->client->ps.pm_flags & 4096)
	{ 
		return qfalse;
	}
	...
	{
		if ( (self->client->ps.pm_flags&16384) )
		{
			switch ( forcePower )
			{
			case FP_GRIP:
	...
	{
		if ( (self->client->saber[0].saberFlags&(1<<4)) )
		{
	...
	return WP_ForcePowerAvailable( self, forcePower, 0 );	
}
...
{ 
	...
	else if ((int)forcePower != FP_GRIP && (int)forcePower != FP_DRAIN && (int)forcePower != FP_SEE)
	{ 
...
void ForceTeamHeal( gentity_t *self )
{
	...
	while (i < 32)
	{
		...
		{
			((a)[0]=(self->client->ps.origin)[0]-(ent->client->ps.origin)[0],(a)[1]=(self->client->ps.origin)[1]-(ent->client->ps.origin)[1],(a)[2]=(self->client->ps.origin)[2]-(ent->client->ps.origin)[2]);
...
void ForceGrip( gentity_t *self )
{
	...
	((tfrom)[0]=(self->client->ps.origin)[0],(tfrom)[1]=(self->client->ps.origin)[1],(tfrom)[2]=(self->client->ps.origin)[2]);
	tfrom[2] += self->client->ps.viewheight;
	AngleVectors(self->client->ps.viewangles, fwd, ((void *)0), ((void *)0));
	tto[0] = tfrom[0] + fwd[0]*256;
	tto[1] = tfrom[1] + fwd[1]*256;
	tto[2] = tfrom[2] + fwd[2]*256;

	trap_Trace(&tr, tfrom, ((void *)0), ((void *)0), tto, self->s.number, (0x00000001|0x00000010|0x00000100|0x00001000));
	...
	{
		if (g_entities[tr.entityNum].s.number < 32 && g_entities[tr.entityNum].client->ps.m_iVehicleNum)
		{ 
	...
	{
		self->client->ps.fd.forceGripEntityNum = ((1<<10)-1);
		return;
	}
}
...
void ForceSpeed( gentity_t *self, int forceDuration )
{
	...
	if ( self->client->holdingObjectiveItem >= 32  
		&& self->client->holdingObjectiveItem < ((1<<10)-2) )
	{
...
void ForceLightningDamage( gentity_t *self, gentity_t *traceEnt, vec3_t dir, vec3_t impactPoint )
{
	...
	if ( traceEnt && traceEnt->takedamage )
	{
		...
		if ( traceEnt->client )
		{
			...
			{ 
				traceEnt->client->ps.fd.forcePower+= 10;
			...
			if (ForcePowerUsableOn(self, traceEnt, FP_LIGHTNING))
			{
				...
				if ( traceEnt->client )
				{
					...
					if (traceEnt->client->ps.electrifyTime < (level.time + 400) && traceEnt->client->ps.torsoAnim != FACE_SMILE)
					{ 
						traceEnt->client->ps.electrifyTime = level.time + 400;
...
void ForceShootLightning( gentity_t *self )
{
	...
	if ( self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 )
	{
		...
		float	radius = 300, dot, dist;
		gentity_t	*entityList[(1<<10)];
		int			iEntityList[(1<<10)];
		int		e, numListedEntities, i;

		((center)[0]=(self->client->ps.origin)[0],(center)[1]=(self->client->ps.origin)[1],(center)[2]=(self->client->ps.origin)[2]);
		for ( i = 0 ; i < 3 ; i++ ) 
		{
		...
		for ( e = 0 ; e < numListedEntities ; e++ ) 
		{
			...
			((size)[0]=(traceEnt->r.absmax)[0]-(traceEnt->r.absmin)[0],(size)[1]=(traceEnt->r.absmax)[1]-(traceEnt->r.absmin)[1],(size)[2]=(traceEnt->r.absmax)[2]-(traceEnt->r.absmin)[2]);
			((ent_org)[0]=(traceEnt->r.absmin)[0]+(size)[0]*(0.5),(ent_org)[1]=(traceEnt->r.absmin)[1]+(size)[1]*(0.5),(ent_org)[2]=(traceEnt->r.absmin)[2]+(size)[2]*(0.5));

			
			
			((dir)[0]=(ent_org)[0]-(center)[0],(dir)[1]=(ent_org)[1]-(center)[1],(dir)[2]=(ent_org)[2]-(center)[2]);
			VectorNormalize( dir );
			if ( (dot = ((dir)[0]*(forward)[0]+(dir)[1]*(forward)[1]+(dir)[2]*(forward)[2])) < 0.5 )
				continue;
			...
			trap_Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, ent_org, self->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	...
	{
		((end)[0]=(self->client->ps.origin)[0]+(forward)[0]*(2048),(end)[1]=(self->client->ps.origin)[1]+(forward)[1]*(2048),(end)[2]=(self->client->ps.origin)[2]+(forward)[2]*(2048));
		
		trap_Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, end, self->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
		if ( tr.entityNum == ((1<<10)-1) || tr.fraction == 1.0 || tr.allsolid || tr.startsolid )
		{
			return;
		}
...
int ForceShootDrain( gentity_t *self )
{
	...
	else
	{
		...
		trap_Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, end, self->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
		if ( tr.entityNum == ((1<<10)-1) || tr.fraction == 1.0 || tr.allsolid || tr.startsolid || !g_entities[tr.entityNum].client || !g_entities[tr.entityNum].inuse )
		{
			return 0;
		}
	...
	BG_ForcePowerDrain( &self->client->ps, FP_DRAIN, 5 ); 
...
{ 
	float forceJumpChargeInterval = forceJumpStrength[0] / (6400/50);
	...
	if (!self->client->ps.fd.forceJumpCharge && self->client->ps.groundEntityNum == ((1<<10)-1))
	{
		return;
	}
	...
	if ( self->client->ps.fd.forceJumpCharge/forceJumpChargeInterval/(6400/50)*forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]][FP_LEVITATION] > self->client->ps.fd.forcePower )
	{
		G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1-50], CHAN_VOICE);
		self->client->ps.fd.forceJumpCharge = self->client->ps.fd.forcePower*forceJumpChargeInterval/(6400/50);
	}
...
int WP_GetVelocityForForceJump( gentity_t *self, vec3_t jumpVel, usercmd_t *ucmd )
{
	...
	((view)[0]=(self->client->ps.viewangles)[0],(view)[1]=(self->client->ps.viewangles)[1],(view)[2]=(self->client->ps.viewangles)[2]);
	view[0] = 0;
	AngleVectors( view, forward, right, ((void *)0) );
	...
	if (self->client->ps.fd.forceJumpCharge < 225+40)
	{ 
		self->client->ps.fd.forceJumpCharge = 225+400;
	}
	...
	((jumpVel)[0]=(self->client->ps.velocity)[0]+(forward)[0]*(pushFwd),(jumpVel)[1]=(self->client->ps.velocity)[1]+(forward)[1]*(pushFwd),(jumpVel)[2]=(self->client->ps.velocity)[2]+(forward)[2]*(pushFwd));
	((jumpVel)[0]=(self->client->ps.velocity)[0]+(right)[0]*(pushRt),(jumpVel)[1]=(self->client->ps.velocity)[1]+(right)[1]*(pushRt),(jumpVel)[2]=(self->client->ps.velocity)[2]+(right)[2]*(pushRt));
...
void ForceJump( gentity_t *self, usercmd_t *ucmd )
{
	...
	if ( self->s.groundEntityNum == ((1<<10)-1) )
	{
		return;
	}
	if ( self->health <= 0 )
	...
	forceJumpChargeInterval = forceJumpStrength[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]]/(6400/50);

	WP_GetVelocityForForceJump( self, jumpVel, ucmd );

	
	self->client->ps.fd.forceJumpZStart = self->client->ps.origin[2];
	((self->client->ps.velocity)[0]=(jumpVel)[0],(self->client->ps.velocity)[1]=(jumpVel)[1],(self->client->ps.velocity)[2]=(jumpVel)[2]);
	
	

	WP_ForcePowerStart( self, FP_LEVITATION, self->client->ps.fd.forceJumpCharge/forceJumpChargeInterval/(6400/50)*forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]][FP_LEVITATION] );
...
qboolean ForceTelepathyCheckDirectNPCTarget( gentity_t *self, trace_t *tr, qboolean *tookPower )
{
	...
	if( traceEnt->NPC 
		&& traceEnt->NPC->scriptFlags & 0x00200000 )
	{
		return qfalse;
	}
	...
	if ( targetLive && traceEnt->NPC )
	{
		...
		{
			int override = 0;
			if ( (traceEnt->NPC->scriptFlags&0x00080000) )
			{
			}
				...
				&& traceEnt->client->NPC_class != CLASS_REBORN )
			{
				...
				else
				{
					...
					traceEnt->NPC->confusionTime = level.time + mindTrickTime[self->client->ps.fd.forcePowerLevel[FP_TELEPATHY]];
		...
		else if ( traceEnt->client->playerTeam == self->client->playerTeam )
		{
			...
			if ( traceEnt->client->ps.pm_type < PM_DEAD && traceEnt->NPC!=((void *)0) && !(traceEnt->NPC->scriptFlags&0x00000080) )
			{
				NPC_UseResponse( traceEnt, self, qfalse );
				WP_ForcePowerStart( self, FP_TELEPATHY, 1 );
			}
		}
		AngleVectors( traceEnt->client->renderInfo.eyeAngles, eyeDir, ((void *)0), ((void *)0) );
...
void ForceTelepathy(gentity_t *self)
{
	...
	AngleVectors( fwdangles, forward, right, ((void *)0) );
	((center)[0]=(self->client->ps.origin)[0],(center)[1]=(self->client->ps.origin)[1],(center)[2]=(self->client->ps.origin)[2]);
	...
	else	
	{
		...
		while (e < numListedEntities)
		{
			...
			{ 
				if (ent->client)
				{
					((thispush_org)[0]=(ent->client->ps.origin)[0],(thispush_org)[1]=(ent->client->ps.origin)[1],(thispush_org)[2]=(ent->client->ps.origin)[2]);
				}
				else
				{
					((thispush_org)[0]=(ent->s.pos.trBase)[0],(thispush_org)[1]=(ent->s.pos.trBase)[1],(thispush_org)[2]=(ent->s.pos.trBase)[2]);
				}
				((tto)[0]=(self->client->ps.origin)[0],(tto)[1]=(self->client->ps.origin)[1],(tto)[2]=(self->client->ps.origin)[2]);
				tto[2] += self->client->ps.viewheight;
				((a)[0]=(thispush_org)[0]-(tto)[0],(a)[1]=(thispush_org)[1]-(tto)[1],(a)[2]=(thispush_org)[2]-(tto)[2]);
				vectoangles(a, a);
...
qboolean CanCounterThrow(gentity_t *self, gentity_t *thrower, qboolean pull)
{
	...
	{ 
        ...
        ((d)[0]=(thrower->client->ps.origin)[0]-(self->client->ps.origin)[0],(d)[1]=(thrower->client->ps.origin)[1]-(self->client->ps.origin)[1],(d)[2]=(thrower->client->ps.origin)[2]-(self->client->ps.origin)[2]);
		vectoangles(d, d);

        a = AngleSubtract(d[1], self->client->ps.viewangles[1]);
...
void ForceThrow( gentity_t *self, qboolean pull )
{
	...
	if ( pull )
	{
		...
		if (self->client->ps.forceHandExtend == HANDEXTEND_NONE)
		{
			...
			{
				self->client->ps.forceHandExtendTime = level.time + 800;
			}
		}
	...
	if (pull)
	{
		...
		pushPower = self->client->ps.fd.forcePowerLevel[FP_PULL]*(self->client->sess.str*self->client->sess.dex);
	}
	else
	{
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) 
	{
		...
		{
			if ( ent->s.pos.trType == TR_STATIONARY && (ent->s.eFlags&(1<<22)) )
			{
		...
		((size)[0]=(ent->r.absmax)[0]-(ent->r.absmin)[0],(size)[1]=(ent->r.absmax)[1]-(ent->r.absmin)[1],(size)[2]=(ent->r.absmax)[2]-(ent->r.absmin)[2]);
		((ent_org)[0]=(ent->r.absmin)[0]+(size)[0]*(0.5),(ent_org)[1]=(ent->r.absmin)[1]+(size)[1]*(0.5),(ent_org)[2]=(ent->r.absmin)[2]+(size)[2]*(0.5));

		((dir)[0]=(ent_org)[0]-(center)[0],(dir)[1]=(ent_org)[1]-(center)[1],(dir)[2]=(ent_org)[2]-(center)[2]);
		VectorNormalize( dir );
		if ( (dot1 = ((dir)[0]*(forward)[0]+(dir)[1]*(forward)[1]+(dir)[2]*(forward)[2])) < 0.6 )
			continue;
		...
		if ( tr.fraction < 1.0f && tr.entityNum != ent->s.number )
		{
			...
			vec3_t eyePoint;
			((eyePoint)[0]=(self->client->ps.origin)[0],(eyePoint)[1]=(self->client->ps.origin)[1],(eyePoint)[2]=(self->client->ps.origin)[2]);
			eyePoint[2] += self->client->ps.viewheight;
			trap_Trace( &tr, eyePoint, vec3_origin, vec3_origin, ent_org, self->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	...
	if ( ent_count )
	{
		...
		for ( x = 0; x < ent_count; x++ )
		{
			...
			if (push_list[x]->client)
			{
				((thispush_org)[0]=(push_list[x]->client->ps.origin)[0],(thispush_org)[1]=(push_list[x]->client->ps.origin)[1],(thispush_org)[2]=(push_list[x]->client->ps.origin)[2]);
			}
			else
			{
				((thispush_org)[0]=(push_list[x]->s.origin)[0],(thispush_org)[1]=(push_list[x]->s.origin)[1],(thispush_org)[2]=(push_list[x]->s.origin)[2]);
			}
			...
			if ( push_list[x]->client )
			{
				...
				if ( g_debugMelee.integer )
				{
					if ( (push_list[x]->client->ps.pm_flags&16384) )
					{
				...
				if ( pull )
				{
					((pushDir)[0]=(self->client->ps.origin)[0]-(thispush_org)[0],(pushDir)[1]=(self->client->ps.origin)[1]-(thispush_org)[1],(pushDir)[2]=(self->client->ps.origin)[2]-(thispush_org)[2]);
					...
					if (push_list[x]->client && VectorLength(pushDir) <= 256)
					{
						...
						if (Q_irand(1, 10) <= randfact && canPullWeapon)
						{
							vec3_t uorg, vecnorm;

							((uorg)[0]=(self->client->ps.origin)[0],(uorg)[1]=(self->client->ps.origin)[1],(uorg)[2]=(self->client->ps.origin)[2]);
							uorg[2] += 64;

							((vecnorm)[0]=(uorg)[0]-(thispush_org)[0],(vecnorm)[1]=(uorg)[1]-(thispush_org)[1],(vecnorm)[2]=(uorg)[2]-(thispush_org)[2]);
				...
				{
					((pushDir)[0]=(thispush_org)[0]-(self->client->ps.origin)[0],(pushDir)[1]=(thispush_org)[1]-(self->client->ps.origin)[1],(pushDir)[2]=(thispush_org)[2]-(self->client->ps.origin)[2]);
				}
				...
				if ((modPowerLevel > otherPushPower || push_list[x]->client->ps.m_iVehicleNum) && push_list[x]->client)
				{
						...
						push_list[x]->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN)
					{
						...
						{ 
							...
							push_list[x]->client->ps.forceDodgeAnim = 0; 
							push_list[x]->client->ps.quickerGetup = qtrue;
						}
						else if (push_list[x]->s.number < 32 && push_list[x]->client->ps.m_iVehicleNum &&
							dirLen <= 128.0f )
						{ 
				...
				if (push_list[x]->client)
				{
					...
					self->client->pushEffectTime = level.time + 300;
					self->client->ps.velocity[0] = pushDir[0]*pushPowerMod*-1;
					self->client->ps.velocity[1] = pushDir[1]*pushPowerMod*-1;
					...
					self->client->ps.velocity[2] = -1 * push_list[x]->client->ps.velocity[2];

				}
			}
			else if ( push_list[x]->s.eType == ET_MISSILE && push_list[x]->s.pos.trType != TR_STATIONARY && (push_list[x]->s.pos.trType != TR_INTERPOLATE||push_list[x]->s.weapon != WP_THERMAL) )
			{
				if ( pull )
				{
				}
			...
			else if ( !Q_stricmp( "func_door", push_list[x]->classname ) && (push_list[x]->spawnflags&2) )
			{
				...
				((trFrom)[0]=(self->client->ps.origin)[0],(trFrom)[1]=(self->client->ps.origin)[1],(trFrom)[2]=(self->client->ps.origin)[2]);
				trFrom[2] += self->client->ps.viewheight;

				AngleVectors( self->client->ps.viewangles, forward, ((void *)0), ((void *)0) );
				VectorNormalize( forward );
				((end)[0]=(trFrom)[0]+(forward)[0]*(radius),(end)[1]=(trFrom)[1]+(forward)[1]*(radius),(end)[2]=(trFrom)[2]+(forward)[2]*(radius));
				trap_Trace( &tr, trFrom, vec3_origin, vec3_origin, end, self->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
				...
				{
					((size)[0]=(push_list[x]->r.absmax)[0]-(push_list[x]->r.absmin)[0],(size)[1]=(push_list[x]->r.absmax)[1]-(push_list[x]->r.absmin)[1],(size)[2]=(push_list[x]->r.absmax)[2]-(push_list[x]->r.absmin)[2]);
					((center)[0]=(push_list[x]->r.absmin)[0]+(size)[0]*(0.5),(center)[1]=(push_list[x]->r.absmin)[1]+(size)[1]*(0.5),(center)[2]=(push_list[x]->r.absmin)[2]+(size)[2]*(0.5));
					if ( (push_list[x]->spawnflags&1) && push_list[x]->moverState == MOVER_POS1 )
					{
						((center)[0]=(center)[0]-(push_list[x]->pos1)[0],(center)[1]=(center)[1]-(push_list[x]->pos1)[1],(center)[2]=(center)[2]-(push_list[x]->pos1)[2]);
					}
					else if ( !(push_list[x]->spawnflags&1) && push_list[x]->moverState == MOVER_POS2 )
					{
						((center)[0]=(center)[0]-(push_list[x]->pos2)[0],(center)[1]=(center)[1]-(push_list[x]->pos2)[1],(center)[2]=(center)[2]-(push_list[x]->pos2)[2]);
					}
					((pos1)[0]=(center)[0]+(push_list[x]->pos1)[0],(pos1)[1]=(center)[1]+(push_list[x]->pos1)[1],(pos1)[2]=(center)[2]+(push_list[x]->pos1)[2]);
					((pos2)[0]=(center)[0]+(push_list[x]->pos2)[0],(pos2)[1]=(center)[1]+(push_list[x]->pos2)[1],(pos2)[2]=(center)[2]+(push_list[x]->pos2)[2]);
				}
				else
				{
					((center)[0]=(push_list[x]->r.currentOrigin)[0],(center)[1]=(push_list[x]->r.currentOrigin)[1],(center)[2]=(push_list[x]->r.currentOrigin)[2]);
					((pos1)[0]=(push_list[x]->pos1)[0],(pos1)[1]=(push_list[x]->pos1)[1],(pos1)[2]=(push_list[x]->pos1)[2]);
					((pos2)[0]=(push_list[x]->pos2)[0],(pos2)[1]=(push_list[x]->pos2)[1],(pos2)[2]=(push_list[x]->pos2)[2]);
				}
			...
			{
				Touch_Button( push_list[x], self, ((void *)0) );
				continue;
			}
		}
	}
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower)
{
	...
	((a)[0]=(gripEnt->client->ps.origin)[0]-(self->client->ps.origin)[0],(a)[1]=(gripEnt->client->ps.origin)[1]-(self->client->ps.origin)[1],(a)[2]=(gripEnt->client->ps.origin)[2]-(self->client->ps.origin)[2]);
	
	trap_Trace(&tr, self->client->ps.origin, ((void *)0), ((void *)0), gripEnt->client->ps.origin, self->s.number, (0x00000001|0x00000010|0x00000100|0x00001000));
	...
	{ 
		...
		G_Damage(gripEnt, self, self, ((void *)0), ((void *)0), 2, 0x00000002, MOD_FORCE_DARK);
	}
	...
	if (gripLevel == FORCE_LEVEL_2)
	{
		...
		{ 
			...
			G_Damage(gripEnt, self, self, ((void *)0), ((void *)0), 20, 0x00000002, MOD_FORCE_DARK);
	...
	if (gripLevel == FORCE_LEVEL_3)
	{
		...
		if (gripEnt->client->ps.forceGripMoveInterval < level.time)
		{
			...
			((start_o)[0]=(gripEnt->client->ps.origin)[0],(start_o)[1]=(gripEnt->client->ps.origin)[1],(start_o)[2]=(gripEnt->client->ps.origin)[2]);
			AngleVectors(self->client->ps.viewangles, fwd, ((void *)0), ((void *)0));
			...
			fwd_o[2] += 16;
			((nvel)[0]=(fwd_o)[0]-(start_o)[0],(nvel)[1]=(fwd_o)[1]-(start_o)[1],(nvel)[2]=(fwd_o)[2]-(start_o)[2]);
		...
		{ 
			self->client->ps.fd.forceGripDamageDebounceTime = 1;
			G_Damage(gripEnt, self, self, ((void *)0), ((void *)0), 40, 0x00000002, MOD_FORCE_DARK);
...
static void WP_ForcePowerRun( gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd )
{
	...
	case FP_GRIP:
		...
		break;
	case FP_LEVITATION:
		if ( self->client->ps.groundEntityNum != ((1<<10)-1) && !self->client->ps.fd.forceJumpZStart )
		{
			WP_ForcePowerStop( self, forcePower );
		}
		break;
	...
	case FP_DRAIN:
		...
		if ( self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1 )
		{
			if ( (cmd->buttons & 2048) || ((cmd->buttons & 512) && self->client->ps.fd.forcePowerSelected == FP_DRAIN) )
			{
	...
	case FP_LIGHTNING:
		...
		{
			if ( (cmd->buttons & 1024) || ((cmd->buttons & 512) && self->client->ps.fd.forcePowerSelected == FP_LIGHTNING) )
			{
...
int WP_DoSpecificPower( gentity_t *self, usercmd_t *ucmd, forcePowers_t forcepower)
{
	...
	case FP_SPEED:
		...
		break;
	case FP_GRIP:
		if (self->client->ps.fd.forceGripEntityNum == ((1<<10)-1))
		{
			ForceGrip( self );
		}
	...
	case FP_PUSH:
		powerSucceeded = 0; 
		if (self->client->ps.fd.forceButtonNeedRelease && !(self->r.svFlags & 0x00000008))
		{ 
			break;
		}
...
{ 
	...
	while (i < 32)
	{
		...
		{
			((a)[0]=(ent->client->ps.origin)[0]-(self->client->ps.origin)[0],(a)[1]=(ent->client->ps.origin)[1]-(self->client->ps.origin)[1],(a)[2]=(ent->client->ps.origin)[2]-(self->client->ps.origin)[2]);
			tlen = VectorLength(a);
...
void SeekerDroneUpdate(gentity_t *self)
{
	...
	if (self->health < 1)
	{
		((elevated)[0]=(self->client->ps.origin)[0],(elevated)[1]=(self->client->ps.origin)[1],(elevated)[2]=(self->client->ps.origin)[2]);
		elevated[2] += 40;

		angle = ((level.time / 12) & 255) * (3.14159265358979323846f * 2) / 255; 
		dir[0] = cos(angle) * 20;
		dir[1] = sin(angle) * 20;
		dir[2] = cos(angle) * 5;
		((org)[0]=(elevated)[0]+(dir)[0],(org)[1]=(elevated)[1]+(dir)[1],(org)[2]=(elevated)[2]+(dir)[2]);
	...
	{
		self->client->ps.genericEnemyIndex = ((1<<10)-1);
	}

	if (self->client->ps.genericEnemyIndex != ((1<<10)-1) && self->client->ps.genericEnemyIndex != -1)
	{
	...
	if (self->client->ps.genericEnemyIndex != ((1<<10)-1) && self->client->ps.genericEnemyIndex != -1)
	{
		...
		if (self->client->ps.droneFireTime < level.time)
		{
			trap_Trace(&tr, org, ((void *)0), ((void *)0), en->client->ps.origin, -1, (0x00000001|0x00001000));

			if (tr.fraction == 1 && !tr.startsolid && !tr.allsolid)
			{
				((endir)[0]=(en->client->ps.origin)[0]-(org)[0],(endir)[1]=(en->client->ps.origin)[1]-(org)[1],(endir)[2]=(en->client->ps.origin)[2]-(org)[2]);
...
{ 
	...
	{
		G_SetAnim(self, &self->client->pers.cmd, 1|2, BOTH_GETUP_BROLL_R, 1 | 2, 0);
		rolled = qtrue;
	}
	else if (
		self->client->pers.cmd.rightmove < 0 &&
		!self->client->pers.cmd.forwardmove)
	{
		G_SetAnim(self, &self->client->pers.cmd, 1|2, BOTH_GETUP_BROLL_L, 1 | 2, 0);
		rolled = qtrue;
	}
	else if (
	...
	{
		G_SetAnim(self, &self->client->pers.cmd, 1|2, BOTH_GETUP_BROLL_F, 1 | 2, 0);
		rolled = qtrue;
	}
	else if (
		!self->client->pers.cmd.rightmove &&
		self->client->pers.cmd.forwardmove < 0)
	{
		G_SetAnim(self, &self->client->pers.cmd, 1|2, BOTH_GETUP_BROLL_B, 1 | 2, 0);
		rolled = qtrue;
	}
...
void WP_ForcePowersUpdate( gentity_t *self, usercmd_t *ucmd )
{
		...
		self->client->ps.forceHandExtendTime < level.time)
	{
		...
		{
			if (self->health < 1 || (self->client->ps.eFlags & (1<<1)))
			{
		...
		else if (self->client->ps.forceHandExtend == HANDEXTEND_POSTTHROWN)
		{
			...
			else if (self->client->ps.groundEntityNum != ((1<<10)-1) && !self->client->ps.forceDodgeAnim)
			{
				self->client->ps.forceDodgeAnim = 1;
				self->client->ps.forceHandExtendTime = level.time + 1000;
				G_EntitySound( self, CHAN_VOICE, G_SoundIndex("*jump1.wav") );
	...
	if (self->client->ps.fd.forceJumpCharge && self->client->ps.groundEntityNum == ((1<<10)-1) && self->client->fjDidJump)
	{ 
		if (ucmd->upmove < 10 && (!(ucmd->buttons & 512) || self->client->ps.fd.forcePowerSelected != FP_LEVITATION))
		{
			G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1-50], CHAN_VOICE);
			self->client->ps.fd.forceJumpCharge = 0;
		}
	...
	if (!(self->client->ps.pm_flags & 2) && self->client->ps.fd.forceJumpCharge)
	{
		if (!(ucmd->buttons & 512) ||
	...
	{ 
		if (self->client->ps.fd.forcePowersActive & (1 << FP_GRIP))
		{
			if (!(ucmd->buttons & 512) || self->client->ps.fd.forcePowerSelected != FP_GRIP)
			{
				WP_ForcePowerStop(self, FP_GRIP);
			}
		}
	...
	{ 
		...
		{
			if (!(ucmd->buttons & 512) || self->client->ps.fd.forcePowerSelected != FP_LIGHTNING)
			{
	...
	{ 
		...
		{
			if (!(ucmd->buttons & 512) || self->client->ps.fd.forcePowerSelected != FP_DRAIN)
			{
	...
	{
		if ( self->client->ps.saberEntityNum < ((1<<10)-1) && self->client->ps.saberEntityNum > 0 )
		{
			if ( &g_entities[self->client->ps.saberEntityNum] != ((void *)0) && g_entities[self->client->ps.saberEntityNum].s.pos.trType == TR_LINEAR )
			{
				usingForce = qtrue;
			}
		}
