...
void CalcEntitySpot ( const gentity_t *ent, const spot_t spot, vec3_t point ) 
{
	...
	case SPOT_ORIGIN:
		...
		{
			((point)[0]=(ent->r.absmax)[0]-(ent->r.absmin)[0],(point)[1]=(ent->r.absmax)[1]-(ent->r.absmin)[1],(point)[2]=(ent->r.absmax)[2]-(ent->r.absmin)[2]);
			((point)[0]=(ent->r.absmin)[0]+(point)[0]*(0.5),(point)[1]=(ent->r.absmin)[1]+(point)[1]*(0.5),(point)[2]=(ent->r.absmin)[2]+(point)[2]*(0.5));
		}
		else
		{
			((point)[0]=(ent->r.currentOrigin)[0],(point)[1]=(ent->r.currentOrigin)[1],(point)[2]=(ent->r.currentOrigin)[2]);
		}
		break;
	...
	case SPOT_HEAD:
		if ( ent->client && VectorLengthSquared( ent->client->renderInfo.eyePoint )  )
		{
			...
			((point)[0]=(ent->client->renderInfo.eyePoint)[0],(point)[1]=(ent->client->renderInfo.eyePoint)[1],(point)[2]=(ent->client->renderInfo.eyePoint)[2]);
	...
	case SPOT_GROUND:
		...
		trap_Trace( &tr, start, ent->r.mins, ent->r.maxs, end, ent->s.number, (0x00000001|0x00000010|0x00000100|0x00001000) );
		if ( tr.fraction < 1.0 ) 
		{
			((point)[0]=(tr.endpos)[0],(point)[1]=(tr.endpos)[1],(point)[2]=(tr.endpos)[2]);
			break;
		}
...
qboolean NPC_UpdateAngles ( qboolean doPitch, qboolean doYaw ) 
{
	...
	if( doYaw )
	{
		...
		error = AngleDelta ( NPC->client->ps.viewangles[1], targetYaw );
		if( fabs(error) > 0.01f )
		{
			if ( error ) 
			{
		...
		ucmd.angles[1] = ((int)((targetYaw + error)*65536/360) & 65535) - client->ps.delta_angles[1];
	}
	...
	if( doPitch )
	{
		...
		error = AngleDelta ( NPC->client->ps.viewangles[0], targetPitch );
		if ( fabs(error) > 0.01f )
		{
			if ( error ) 
			{
		...
		ucmd.angles[0] = ((int)((targetPitch + error)*65536/360) & 65535) - client->ps.delta_angles[0];
	}

	ucmd.angles[2] = ((int)((NPC->client->ps.viewangles[2])*65536/360) & 65535) - client->ps.delta_angles[2];
...
#line 516 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_utils.c"
...
void NPC_AimWiggle( vec3_t enemy_org )
{
	...
	((enemy_org)[0]=(enemy_org)[0]+(NPCInfo->aimOfs)[0],(enemy_org)[1]=(enemy_org)[1]+(NPCInfo->aimOfs)[1],(enemy_org)[2]=(enemy_org)[2]+(NPCInfo->aimOfs)[2]);
}
...
#line 608 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_utils.c"
	...
	if(doYaw)
	{
		...
		diff = AngleDelta ( NPC->client->ps.viewangles[1], targetYaw );
		...
		ucmd.angles[1] = ((int)((targetYaw + diff + error)*65536/360) & 65535) - client->ps.delta_angles[1];
	}

	if(doPitch)
	{
		
		diff = AngleDelta ( NPC->client->ps.viewangles[0], targetPitch );
		if ( diff) 
		{
			exact = qfalse;
		...
		ucmd.angles[0] = ((int)((targetPitch + diff + error)*65536/360) & 65535) - client->ps.delta_angles[0];
	}
...
#line 730 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_utils.c"
...
void NPC_UpdateShootAngles (vec3_t angles, qboolean doPitch, qboolean doYaw ) 
{
	...
	if(doYaw)
	{
		...
		error = AngleDelta ( NPCInfo->shootAngles[1], targetYaw );
		if ( error ) 
		{
			decay = 60.0 + 80.0 * NPCInfo->stats.aim;
			decay *= 100.0f / 1000.0f;
			if ( error < 0.0 ) 
			{
				error += decay;
				if ( error > 0.0 ) 
		...
		NPCInfo->shootAngles[1] = targetYaw + error;
	}

	if(doPitch)
	{
		
		error = AngleDelta ( NPCInfo->shootAngles[0], targetPitch );
		if ( error ) 
		{
			decay = 60.0 + 80.0 * NPCInfo->stats.aim;
			decay *= 100.0f / 1000.0f;
		...
		NPCInfo->shootAngles[0] = targetPitch + error;
	}
}
...
qboolean G_ActivateBehavior (gentity_t *self, int bset )
{
	...
	else
	{
		...
		trap_ICARUS_RunScript( self, va( "%s/%s", "scripts", bs_name ) );
	}
	return qtrue;
}
...
#line 915 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_utils.c"
	int i = 0;
	...
	trap_G2API_SetBoneAngles(ent->ghoul2, 0, bone, angles, flags, up, right, forward, ((void *)0), 100, level.time);
}
...
void NPC_SetSurfaceOnOff(gentity_t *ent, const char *surfaceName, int surfaceFlags)
{
	...
	while (i < 31 && bgToggleableSurfaces[i])
	{
...
qboolean NPC_SomeoneLookingAtMe(gentity_t *ent)
{
	...
	while (i < 32)
	{
		...
		if (pEnt && pEnt->inuse && pEnt->client && pEnt->client->sess.sessionTeam != TEAM_SPECTATOR &&
			!(pEnt->client->ps.pm_flags & 4096) && pEnt->s.weapon != WP_NONE)
		{
...
qboolean NPC_ValidEnemy( gentity_t *ent )
{
	...
	if ( entTeam == NPC->client->enemyTeam 
		|| (NPC->client->enemyTeam == NPCTEAM_FREE && ent->client->NPC_class != NPC->client->NPC_class )
		|| (ent->client->NPC_class == CLASS_WAMPA && ent->enemy )
		|| (ent->client->NPC_class == CLASS_RANCOR && ent->enemy )
		|| (entTeam == NPCTEAM_FREE && ent->client->enemyTeam == NPCTEAM_FREE && ent->enemy && ent->enemy->client && (ent->enemy->client->playerTeam == NPC->client->playerTeam||(ent->enemy->client->playerTeam != NPCTEAM_ENEMY&&NPC->client->playerTeam==NPCTEAM_PLAYER))) 
		)
	{
		return qtrue;
	}
...
int NPC_FindNearestEnemy( gentity_t *ent )
{
	...
	vec3_t		mins, maxs;
	int			nearestEntID = -1;
	float		nearestDist = (float)( ( 64 * 1024 ) - ( -64 * 1024 ) )*(float)( ( 64 * 1024 ) - ( -64 * 1024 ) );
	float		distance;
	...
	numEnts = trap_EntitiesInBox( mins, maxs, iradiusEnts, 256 );
...
qboolean NPC_FacePosition( vec3_t position, qboolean doPitch )
{
	...
	if ( NPC->client && (NPC->client->NPC_class == CLASS_RANCOR || NPC->client->NPC_class == CLASS_WAMPA) )
	{
	...
	yawDelta = AngleNormalize360( NPCInfo->desiredYaw - ( ((ucmd.angles[1] + client->ps.delta_angles[1])*(360.0/65536)) ) );
	
	
	if ( fabs( yawDelta ) > 2.0f )
		facing = qfalse;

	if ( doPitch )
	{
		
		float currentAngles = ( ((ucmd.angles[0] + client->ps.delta_angles[0])*(360.0/65536)) );
		float pitchDelta = NPCInfo->desiredPitch - currentAngles;
		
		
		if ( fabs( pitchDelta ) > 2.0f )
			facing = qfalse;
	}
...
void NPC_ClearLookTarget( gentity_t *self )
{
	...
	self->client->renderInfo.lookTarget = ((1<<10)-1);
...
qboolean NPC_CheckLookTarget( gentity_t *self )
{
	if ( self->client )
	{
		if ( self->client->renderInfo.lookTarget >= 0 && self->client->renderInfo.lookTarget < ((1<<10)-2) )
		{
			if ( (&g_entities[self->client->renderInfo.lookTarget] == ((void *)0)) || !g_entities[self->client->renderInfo.lookTarget].inuse )
			{
...
void G_GetBoltPosition( gentity_t *self, int boltIndex, vec3_t pos, int modelIndex )
{
	...
	if (self->client)
	{ 
		((angles)[0]=(0), (angles)[1]=(self->client->ps.viewangles[1]), (angles)[2]=(0));
	}
	else
	{
		((angles)[0]=(0), (angles)[1]=(self->r.currentAngles[1]), (angles)[2]=(0));
	}
...
int NPC_GetEntsNearBolt( int *radiusEnts, float radius, int boltIndex, vec3_t boltOrg )
{
	...
	((boltOrg)[0]=(org)[0],(boltOrg)[1]=(org)[1],(boltOrg)[2]=(org)[2]);
