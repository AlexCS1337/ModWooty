...
void MineMonster_Patrol( void )
{
	...
	((dif)[0]=(g_entities[0].r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(dif)[1]=(g_entities[0].r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(dif)[2]=(g_entities[0].r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
...
void MineMonster_TryDamage( gentity_t *enemy, int damage )
{
	...
	AngleVectors( NPC->client->ps.viewangles, dir, ((void *)0), ((void *)0) );
	((end)[0]=(NPC->r.currentOrigin)[0]+(dir)[0]*(54),(end)[1]=(NPC->r.currentOrigin)[1]+(dir)[1]*(54),(end)[2]=(NPC->r.currentOrigin)[2]+(dir)[2]*(54));

	
	trap_Trace( &tr, NPC->r.currentOrigin, vec3_origin, vec3_origin, end, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );

	if ( tr.entityNum >= 0 && tr.entityNum < ((1<<10)-1) )
	{
		G_Damage( &g_entities[tr.entityNum], NPC, NPC, dir, tr.endpos, damage, 0x00000004, MOD_MELEE );
...
void MineMonster_Attack( void )
{
	if ( !TIMER_Exists( NPC, "attacking" ))
	{
		...
		if ( NPC->enemy && ((NPC->enemy->r.currentOrigin[2] - NPC->r.currentOrigin[2] > 10 && ((rand () & 0x7fff) / ((float)0x7fff)) > 0.1f ) 
						|| ((rand () & 0x7fff) / ((float)0x7fff)) > 0.8f ))
		{
			
			TIMER_Set( NPC, "attacking", 1750 + ((rand () & 0x7fff) / ((float)0x7fff)) * 200 );
			NPC_SetAnim( NPC, 1|2, BOTH_ATTACK4, 1 | 2 );

			TIMER_Set( NPC, "attack2_dmg", 950 ); 
		}
		else if ( ((rand () & 0x7fff) / ((float)0x7fff)) > 0.5f )
		{
			if ( ((rand () & 0x7fff) / ((float)0x7fff)) > 0.8f )
			{
				
				TIMER_Set( NPC, "attacking", 850 );
				NPC_SetAnim( NPC, 1|2, BOTH_ATTACK3, 1 | 2 );

				TIMER_Set( NPC, "attack2_dmg", 400 ); 
			}
			else
			{
...
void MineMonster_Combat( void )
{
	...
	advance = (qboolean)( distance > ( 54 * 54 ) ? qtrue : qfalse  );
