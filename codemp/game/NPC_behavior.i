...
#line 10 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_behavior.c"
...
#line 11 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_behavior.c"
#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\icarus\\q3_interface.h"
...
#line 298 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\icarus\\q3_interface.h"
#line 12 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_behavior.c"
...
#line 19 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_behavior.c"
...
#line 21 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_behavior.c"
...
void NPC_BSAdvanceFight (void)
{
	...
	if ( NPCInfo->captureGoal )
	{
		...
		NPC_SetMoveGoal( NPC, NPCInfo->captureGoal->r.currentOrigin, 16, qtrue, -1, ((void *)0) );
	...
	if( NPC->enemy )
	{
		...
		((enemy_org)[0]=(NPC->enemy->r.absmin)[0]+(NPC->enemy->r.maxs)[0]*(0.5),(enemy_org)[1]=(NPC->enemy->r.absmin)[1]+(NPC->enemy->r.maxs)[1]*(0.5),(enemy_org)[2]=(NPC->enemy->r.absmin)[2]+(NPC->enemy->r.maxs)[2]*(0.5));
		...
		if(attack_ok)
		{
			...
			enemyVisibility = NPC_CheckVisibility ( NPC->enemy, 4);
			...
			if(enemyVisibility == VIS_FOV)
			{
				...
				if(attack_ok)
				{
					...
					trap_Trace ( &tr, muzzle, ((void *)0), ((void *)0), enemy_org, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
					traceEnt = &g_entities[tr.entityNum];
					if( traceEnt != NPC->enemy &&
						(!traceEnt || !traceEnt->client || !NPC->client->enemyTeam || NPC->client->enemyTeam != traceEnt->client->playerTeam) )
					{
						attack_scale *= 0.75;
						trap_Trace ( &tr, muzzle, ((void *)0), ((void *)0), enemy_head, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
						traceEnt = &g_entities[tr.entityNum];
					}
				...
				if( attack_ok )
				{
					...
					if( !dead_on )
					{
						...
						AngleVectors (NPCInfo->shootAngles, forward, ((void *)0), ((void *)0));
						...
						if(aim_off > ((rand () & 0x7fff) / ((float)0x7fff)) * max_aim_off)
						{
							...
							((diff)[0]=(hitspot)[0]-(enemy_head)[0],(diff)[1]=(hitspot)[1]-(enemy_head)[1],(diff)[2]=(hitspot)[2]-(enemy_head)[2]);
							aim_off = VectorLength(diff);
							if(aim_off > ((rand () & 0x7fff) / ((float)0x7fff)) * max_aim_off)
							{
								attack_ok = qfalse;
...
void NPC_BSCinematic( void ) 
{
	...
	if ( NPCInfo->watchTarget )
	{
		...
		((viewvec)[0]=(viewSpot)[0]-(eyes)[0],(viewvec)[1]=(viewSpot)[1]-(eyes)[1],(viewvec)[2]=(viewSpot)[2]-(eyes)[2]);
		
		vectoangles( viewvec, viewangles );

		NPCInfo->lockedDesiredYaw = NPCInfo->desiredYaw = viewangles[1];
		NPCInfo->lockedDesiredPitch = NPCInfo->desiredPitch = viewangles[0];
	}
...
qboolean NPC_CheckInvestigate( int alertEventNum )
{
	...
	((soundPos)[0]=(level.alertEvents[alertEventNum].position)[0],(soundPos)[1]=(level.alertEvents[alertEventNum].position)[1],(soundPos)[2]=(level.alertEvents[alertEventNum].position)[2]);
	...
	if ( owner->s.eFlags & (1<<8) ) 
	{
		return qfalse;
	}

	if ( owner->flags & 0x00000020 ) 
	{
		return qfalse;
	}
	...
	if ( owner->client && owner->client->playerTeam && NPC->client->playerTeam && owner->client->playerTeam != NPC->client->playerTeam )
	{
		...
		((NPCInfo->investigateGoal)[0]=(soundPos)[0],(NPCInfo->investigateGoal)[1]=(soundPos)[1],(NPCInfo->investigateGoal)[2]=(soundPos)[2]);
...
void NPC_BSFollowLeader (void)
{
	...
	if ( !NPC->enemy  )
	{
		...
		{
			if ( !(NPCInfo->scriptFlags&0x00002000) )
			{
				int eventID = NPC_CheckAlertEvents( qtrue, qtrue, -1, qfalse, AEL_MINOR );
				if ( level.alertEvents[eventID].level >= AEL_SUSPICIOUS && (NPCInfo->scriptFlags&0x00000800) )
				{
		...
		if ( !NPC->enemy )
		{
			if ( NPC->client->leader 
				...
				&& ( (NPC->client->leader->enemy->client&&NPC->client->leader->enemy->client->playerTeam==NPC->client->enemyTeam)
					||(0&&NPC->client->leader->enemy->alliedTeam==NPC->client->enemyTeam) )
	...
	else 
	{
		...
		{
			NPC_CheckEnemy( (NPCInfo->confusionTime<level.time||NPCInfo->tempBehavior!=BS_FOLLOW_LEADER), qfalse, qtrue );
		}
	}
	...
	if ( NPC->enemy && NPC->client->ps.weapon )
	{
		...
		enemyVisibility = NPC_CheckVisibility ( NPC->enemy, 4|8 );
	...
	else
	{
		...
		((delta)[0]=(leaderHead)[0]-(head)[0],(delta)[1]=(leaderHead)[1]-(head)[1],(delta)[2]=(leaderHead)[2]-(head)[2]);
		vectoangles ( delta, angleToLeader );
		VectorNormalize(delta);
		NPC->NPC->desiredYaw = angleToLeader[1];
		NPC->NPC->desiredPitch = angleToLeader[0];
		
		NPC_UpdateAngles(qtrue, qtrue);
	}

	
	leaderVis = NPC_CheckVisibility( NPC->client->leader, 1|2|8 );
	...
	if ( curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 && curAnim != BOTH_ATTACK3 && curAnim != BOTH_MELEE1 && curAnim != BOTH_MELEE2 )
	{
		...
		((vec)[0]=(NPC->client->leader->r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(vec)[1]=(NPC->client->leader->r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(vec)[2]=(NPC->client->leader->r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
...
void NPC_BSJump (void)
{
	...
	if ( NPCInfo->jumpState != JS_JUMPING && NPCInfo->jumpState != JS_LANDING )
	{
		...
		((dir)[0]=(NPCInfo->goalEntity->r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(dir)[1]=(NPCInfo->goalEntity->r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(dir)[2]=(NPCInfo->goalEntity->r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
		vectoangles(dir, angles);
		NPCInfo->desiredPitch = NPCInfo->lockedDesiredPitch = AngleNormalize360(angles[0]);
		NPCInfo->desiredYaw = NPCInfo->lockedDesiredYaw = AngleNormalize360(angles[1]);
	}

	NPC_UpdateAngles ( qtrue, qtrue );
	yawError = AngleDelta ( NPC->client->ps.viewangles[1], NPCInfo->desiredYaw );
	...
	case JS_FACING:
		if ( yawError < 0.01f )
		{
			NPC_SetAnim(NPC, 2, BOTH_CROUCH1, 1|2);
	...
	case JS_CROUCHING:
		...
		if ( NPC->r.currentOrigin[2] > NPCInfo->goalEntity->r.currentOrigin[2] )
		{
			...
			((p2)[0]=(NPCInfo->goalEntity->r.currentOrigin)[0],(p2)[1]=(NPCInfo->goalEntity->r.currentOrigin)[1],(p2)[2]=(NPCInfo->goalEntity->r.currentOrigin)[2]);
		}
		else if ( NPC->r.currentOrigin[2] < NPCInfo->goalEntity->r.currentOrigin[2] )
		{
			((p1)[0]=(NPCInfo->goalEntity->r.currentOrigin)[0],(p1)[1]=(NPCInfo->goalEntity->r.currentOrigin)[1],(p1)[2]=(NPCInfo->goalEntity->r.currentOrigin)[2]);
	...
	case JS_JUMPING:
		...
		if ( showBBoxes )
		{
			((p1)[0]=(NPC->r.mins)[0]+(NPC->pos1)[0],(p1)[1]=(NPC->r.mins)[1]+(NPC->pos1)[1],(p1)[2]=(NPC->r.mins)[2]+(NPC->pos1)[2]);
			((p2)[0]=(NPC->r.maxs)[0]+(NPC->pos1)[0],(p2)[1]=(NPC->r.maxs)[1]+(NPC->pos1)[1],(p2)[2]=(NPC->r.maxs)[2]+(NPC->pos1)[2]);
			G_Cube( p1, p2, NPCDEBUG_BLUE, 0.5 );
		}

		if ( NPC->s.groundEntityNum != ((1<<10)-1))
		{
			
			((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[2]=0);
			NPC_SetAnim(NPC, 1|2, BOTH_LAND1, 1|2);
		...
		{
			NPC_SetAnim(NPC, 1|2, BOTH_INAIR1, 1);
		}
		break;
	case JS_LANDING:
		if ( NPC->client->ps.legsTimer > 0 )
...
{
	NPC_UpdateAngles ( qtrue, qtrue );
	if( !trap_InPVS( NPC->r.currentOrigin, g_entities[0].r.currentOrigin ) )
	{ 
...
void NPC_BSSearch (void)
{
	...
	if ( !NPCInfo->investigateDebounceTime )
	{
		...
		((vec)[0]=(NPCInfo->tempGoal->r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(vec)[1]=(NPCInfo->tempGoal->r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(vec)[2]=(NPCInfo->tempGoal->r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
		if ( vec[2] < 24 )
		{
			vec[2] = 0;
		}

		if ( NPCInfo->tempGoal->waypoint != -1 )
		{
		...
		if ( VectorLengthSquared( vec ) < minGoalReachedDistSquared )
		{
			...
			NPC->waypoint = NAV_FindClosestWaypointForEnt( NPC, -1 );

			if ( ( NPCInfo->homeWp == -1 ) || ( NPC->waypoint == -1 ) )
			{
			...
			if ( NPC->waypoint == NPCInfo->homeWp )
			{
				...
				if ( NPCInfo->aiFlags & 0x00020000 )
				{
					NPCInfo->aiFlags &= ~0x00020000;
			...
			if( !Q_irand(0, 1) )
			{
				NPC_SetAnim(NPC, 1|2, BOTH_GUARD_LOOKAROUND1, 0);
			}
			else
			{
				NPC_SetAnim(NPC, 1|2, BOTH_GUARD_IDLE1, 0);
			}
			...
			if ( NPCInfo->tempGoal->waypoint != -1 )
			{
				if ( !Q_irand( 0, 30 ) )
				{
					...
					if ( numEdges != -1 )
					{
						...
						((lookDir)[0]=(branchPos)[0]-(NPCInfo->tempGoal->r.currentOrigin)[0],(lookDir)[1]=(branchPos)[1]-(NPCInfo->tempGoal->r.currentOrigin)[1],(lookDir)[2]=(branchPos)[2]-(NPCInfo->tempGoal->r.currentOrigin)[2]);
...
void NPC_BSSearchStart( int homeWp, bState_t bState )
{
	...
	if ( homeWp == -1 )
	{
		homeWp = NAV_FindClosestWaypointForEnt( NPC, -1 );
		if( NPC->waypoint == -1 )
		{
			NPC->waypoint = homeWp;
		}
...
void NPC_BSNoClip ( void )
{
	if ( UpdateGoal() )
	{
		...
		AngleVectors( NPC->r.currentAngles, forward, right, ((void *)0) );

		VectorNormalize( dir );

		fDot = ((forward)[0]*(dir)[0]+(forward)[1]*(dir)[1]+(forward)[2]*(dir)[2]) * 127;
		rDot = ((right)[0]*(dir)[0]+(right)[1]*(dir)[1]+(right)[2]*(dir)[2]) * 127;
		uDot = ((up)[0]*(dir)[0]+(up)[1]*(dir)[1]+(up)[2]*(dir)[2]) * 127;
...
qboolean NPC_CheckSurrender( void )
{
	if ( !trap_ICARUS_TaskIDPending( NPC, TID_MOVE_NAV ) 
		&& NPC->client->ps.groundEntityNum != ((1<<10)-1) 
			...
			&& NPC->s.weapon != WP_SABER )
		{
			...
			if ( NPC->s.weapon != WP_NONE )
			{
				...
				else
				{
					...
					else if ( DistanceSquared( NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) < 65536 )
					{
						return qfalse;
					}
	...
	if ( goal )
	{
		...
		if ( NPC->waypoint != -1 )
		{
			...
			if ( numEdges != -1 )
			{
				...
				((dangerDir)[0]=(NPCInfo->investigateGoal)[0]-(NPC->r.currentOrigin)[0],(dangerDir)[1]=(NPCInfo->investigateGoal)[1]-(NPC->r.currentOrigin)[1],(dangerDir)[2]=(NPCInfo->investigateGoal)[2]-(NPC->r.currentOrigin)[2]);
				...
				for ( branchNum = 0; branchNum < numEdges; branchNum++ )
				{
					...
					((runDir)[0]=(branchPos)[0]-(NPC->r.currentOrigin)[0],(runDir)[1]=(branchPos)[1]-(NPC->r.currentOrigin)[1],(runDir)[2]=(branchPos)[2]-(NPC->r.currentOrigin)[2]);
					VectorNormalize( runDir );
					if ( ((runDir)[0]*(dangerDir)[0]+(runDir)[1]*(dangerDir)[1]+(runDir)[2]*(dangerDir)[2]) > flrand( 0, 0.5 ) )
					{
						continue;
					}
					
					NPC_SetMoveGoal( NPC, branchPos, 0, qtrue, -1, ((void *)0) );
		...
		if ( moved == qfalse )
		{
			...
			if ( reverseCourse )
			{
				((dir)[0]=(NPC->r.currentOrigin)[0]-(goal->r.currentOrigin)[0],(dir)[1]=(NPC->r.currentOrigin)[1]-(goal->r.currentOrigin)[1],(dir)[2]=(NPC->r.currentOrigin)[2]-(goal->r.currentOrigin)[2]);
			}
			else
			{
				((dir)[0]=(goal->r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(dir)[1]=(goal->r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(dir)[2]=(goal->r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
			}
...
void NPC_StartFlee( gentity_t *enemy, vec3_t dangerPoint, int dangerLevel, int fleeTimeMin, int fleeTimeMax )
{
	...
	{
		cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, dangerPoint, 0x00000001|0x00000100|0x00001000|0x00010000, 128, -1 );
	}
	
	if ( cp == -1 )
	{
		cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, dangerPoint, 0x00000001|0x00000100|0x00001000, 128, -1 );
		if ( cp == -1 )
		{
			cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, dangerPoint, 0x00000001|0x00001000, 128, -1 );
			if ( cp == -1 )
			{
				cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, dangerPoint, 0x00001000, 128, -1 );
			}
		}
	}
	...
	if ( cp != -1 )
	{
		...
		NPC_SetMoveGoal( NPC, level.combatPoints[cp].origin, 8, qtrue, cp, ((void *)0) );
		NPCInfo->behaviorState = BS_HUNT_AND_KILL;
		NPCInfo->tempBehavior = BS_DEFAULT;
	}
	else
	{
		...
		else
		{
			...
			NPC_SetMoveGoal( NPC, dangerPoint, 0, qtrue, -1, ((void *)0) );
			
			((NPCInfo->investigateGoal)[0]=(dangerPoint)[0],(NPCInfo->investigateGoal)[1]=(dangerPoint)[1],(NPCInfo->investigateGoal)[2]=(dangerPoint)[2]);
		}
	}
	...
	TIMER_Set( NPC, "panic", Q_irand( 1000, 4000 ) );
