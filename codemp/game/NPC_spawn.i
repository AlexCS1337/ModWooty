...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_spawn.c"
...
#line 8 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_spawn.c"
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle, int sizefactor );
...
void NPC_SetMiscDefaultData( gentity_t *ent )
{
	...
	if ( ent->client->NPC_class == CLASS_BOBAFETT )
	{
		...
		ent->NPC->scriptFlags |= (0x00000040|0x00020000);
	}
	...
	if (ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle)
	{
		...
		{
			ent->mass = 250;
			ent->flags |= (0x00080000|0x00000800);
			ent->pain = NPC_ATST_Pain;
		}
		
		trap_G2API_SetSurfaceOnOff( ent->ghoul2, "head_hatchcover", 0 );
	}
	...
	{
		ent->NPC->scriptFlags |= 0x00200000;
	}
	if ( !Q_stricmp( "emperor", ent->NPC_type ) )
	{
		ent->NPC->scriptFlags |= 0x00004000;
	}
	...
	{		
		...
		ent->NPC->aiFlags |= 0x00200000;
		ent->client->ps.eFlags2 |= (1<<4);
		ent->count = 12; 
	...
	case NPCTEAM_PLAYER:
		...
		if ( ent->client->NPC_class == CLASS_JEDI || ent->client->NPC_class == CLASS_LUKE )
		{
			...
			{
				ent->NPC->scriptFlags |= 0x00002000;
		...
		if ( ent->client->NPC_class == CLASS_KYLE || ent->client->NPC_class == CLASS_VEHICLE || (ent->spawnflags & 32) )
		{
...
void NPC_SetWeapons( gentity_t *ent )
{
	...
	for ( curWeap = WP_SABER; curWeap < WP_NUM_WEAPONS; curWeap++ )
	{
		if ( (weapons & ( 1 << curWeap )) )
		{
			...
			ent->NPC->currentAmmo = ent->client->ps.ammo[weaponData[curWeap].ammoIndex] = 100;
...
qboolean NPC_SpotWouldTelefrag( gentity_t *npc )
{
	...
	((mins)[0]=(npc->r.currentOrigin)[0]+(npc->r.mins)[0],(mins)[1]=(npc->r.currentOrigin)[1]+(npc->r.mins)[1],(mins)[2]=(npc->r.currentOrigin)[2]+(npc->r.mins)[2]);
	((maxs)[0]=(npc->r.currentOrigin)[0]+(npc->r.maxs)[0],(maxs)[1]=(npc->r.currentOrigin)[1]+(npc->r.maxs)[1],(maxs)[2]=(npc->r.currentOrigin)[2]+(npc->r.maxs)[2]);
	...
	for (i=0 ; i<num ; i++)
	{
		...
		if (hit->inuse 
			...
			&& (hit->r.contents&(0x00000001|0x00000020|0x00000100|0x00001000))
...
void NPC_Begin (gentity_t *ent)
{
	...
	((spawn_origin)[0]=(ent->client->ps.origin)[0],(spawn_origin)[1]=(ent->client->ps.origin)[1],(spawn_origin)[2]=(ent->client->ps.origin)[2]);
	((spawn_angles)[0]=(ent->s.angles)[0],(spawn_angles)[1]=(ent->s.angles)[1],(spawn_angles)[2]=(ent->s.angles)[2]);
	spawn_angles[1] = ent->NPC->desiredYaw;
	...
	if(!(ent->spawnflags & 64))
	{
		...
		ent->clipmask = (0x00000001|0x00000020|0x00000100|0x00001000);
	}
	else
	{
		ent->r.contents = 0;
		ent->clipmask = (0x00000001|0x00000020|0x00000100|0x00001000)&~0x00000100;
	}
	...
	((client->ps.origin)[0]=(spawn_origin)[0],(client->ps.origin)[1]=(spawn_origin)[1],(client->ps.origin)[2]=(spawn_origin)[2]);
	...
	((ent->s.origin)[0]=(spawn_origin)[0],(ent->s.origin)[1]=(spawn_origin)[1],(ent->s.origin)[2]=(spawn_origin)[2]);


	SetClientViewAngle( ent, spawn_angles );
	client->renderInfo.lookTarget = ((1<<10)-1);
	...
	else if ( client->NPC_class == CLASS_SEEKER && ent->activator != ((void *)0) )
	{
	...
	{
		NPC_SetAnim( ent, 1|2, BOTH_STAND1, 0 );
	}
	...
	ent->use   = NPC_Use;
	ent->think = NPC_Think;
	ent->nextthink = level.time + 50 + Q_irand(0, 100);
	...
	((ent->NPC->lastClearOrigin)[0]=(ent->NPC->lastClearOrigin)[1]=(ent->NPC->lastClearOrigin)[2]=0);
	...
	((ent->client->renderInfo.eyePoint)[0]=(ent->r.currentOrigin)[0],(ent->client->renderInfo.eyePoint)[1]=(ent->r.currentOrigin)[1],(ent->client->renderInfo.eyePoint)[2]=(ent->r.currentOrigin)[2]);

	
	
	memset( &ucmd, 0, sizeof( ucmd ) );
	
	((ucmd.angles)[0]=(client->pers.cmd.angles)[0],(ucmd.angles)[1]=(client->pers.cmd.angles)[1],(ucmd.angles)[2]=(client->pers.cmd.angles)[2]);
	
	ent->client->ps.groundEntityNum = ((1<<10)-1);

	if ( ent->NPC->aiFlags & 0x00040000 )
	{
	...
	{
		if ( !(ent->spawnflags&32) && ent->NPC->behaviorState != BS_CINEMATIC )
		{
	...
	if ( ent->m_pVehicle )
	{
		...
		{
			char	*droidNPCType = ((void *)0);
			gentity_t *droidEnt = ((void *)0);
			if ( ent->model2 
				&& ent->model2[0] )
			{
			...
			if ( droidNPCType != ((void *)0) )
			{
				...
				droidEnt = NPC_SpawnType( ent, droidNPCType, ((void *)0), qfalse,100 );
				if ( droidEnt != ((void *)0) )
				{
					if ( droidEnt->client )
					{
						...
						((droidEnt->s.origin)[0]=(ent->r.currentOrigin)[0],(droidEnt->s.origin)[1]=(ent->r.currentOrigin)[1],(droidEnt->s.origin)[2]=(ent->r.currentOrigin)[2]);
						((droidEnt->client->ps.origin)[0]=(ent->r.currentOrigin)[0],(droidEnt->client->ps.origin)[1]=(ent->r.currentOrigin)[1],(droidEnt->client->ps.origin)[2]=(ent->r.currentOrigin)[2]);
						G_SetOrigin( droidEnt, droidEnt->s.origin );
						trap_LinkEntity( droidEnt );
						((droidEnt->s.angles)[0]=(ent->r.currentAngles)[0],(droidEnt->s.angles)[1]=(ent->r.currentAngles)[1],(droidEnt->s.angles)[2]=(ent->r.currentAngles)[2]);
						G_SetAngles( droidEnt, droidEnt->s.angles );
						if ( droidEnt->NPC )
						{
							droidEnt->NPC->desiredYaw = droidEnt->s.angles[1];
							droidEnt->NPC->desiredPitch = droidEnt->s.angles[0];
						}
						droidEnt->flags |= 0x00100000;
					}
					else
					{
...
void NPC_DefaultScriptFlags( gentity_t *ent )
{
	...
	ent->NPC->scriptFlags = (0x00400000|0x01000000);
}
...
#line 1371 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_spawn.c"
...
#line 1376 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_spawn.c"

gentity_t *NPC_Spawn_Do( gentity_t *ent, int sizefactor )
{
	gentity_t	*newent = ((void *)0);
	int			index;
	vec3_t		saveOrg;
		vec3_t oppDir;
	float sizefloat = (float)sizefactor / 100.0f;
	...
	if ( ent->spawnflags & 16 )
	{
		...
		((saveOrg)[0]=(ent->r.currentOrigin)[0],(saveOrg)[1]=(ent->r.currentOrigin)[1],(saveOrg)[2]=(ent->r.currentOrigin)[2]);
		((bottom)[0]=(ent->r.currentOrigin)[0],(bottom)[1]=(ent->r.currentOrigin)[1],(bottom)[2]=(ent->r.currentOrigin)[2]);
		bottom[2] = ( -64 * 1024 );
		trap_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, bottom, ent->s.number, (0x00000001|0x00000020|0x00000100|0x00001000) );
	...
	if ( newent == ((void *)0) || !newent ) 
	{
		Com_Printf ( "^1""ERROR: NPC G_Spawn failed\n" );
		return ((void *)0);
	}
	...
	if ( newent->NPC == ((void *)0) ) 
	{		
		Com_Printf ( "^1""ERROR: NPC G_Alloc NPC failed\n" );		
		goto finish;
		return ((void *)0);
	}	
	...
	newent->NPC->tempGoal->r.svFlags |= 0x00000001;

	if ( newent->client == ((void *)0) ) 
	{
		Com_Printf ( "^1""ERROR: NPC BG_Alloc client failed\n" );
		goto finish;
		return ((void *)0);
	}
	...
	if ( ent->r.svFlags & 0x10000000 )
	{
		newent->r.svFlags |= 0x10000000;
	}
	if ( ent->r.svFlags & 0x20000000 )
	{
		newent->r.svFlags |= 0x20000000;
	}
	if ( ent->r.svFlags & 0x40000000 )
	{
		newent->r.svFlags |= 0x40000000;
	}
	
	if ( ent->message )
	{
		newent->message = ent->message;
		newent->flags |= 0x00000800;
	}
	...
	if ( Q_stricmp( ent->classname, "NPC_Vehicle" ) == 0 )
	{
		...
		(void)( (!!(newent->m_pVehicle && newent->m_pVehicle->m_pVehicleInfo && newent->m_pVehicle->m_pVehicleInfo->Initialize)) || (_wassert(L"newent->m_pVehicle && newent->m_pVehicle->m_pVehicleInfo && newent->m_pVehicle->m_pVehicleInfo->Initialize", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_spawn.c", 1547), 0) );
		...
		{
			newent->flags |= (0x00000800|0x00080000|0x02000000);
		}
		
		
		newent->m_pVehicle->m_vOrientation[1] = ent->s.angles[1];
		newent->m_pVehicle->m_vOrientation[0] = newent->m_pVehicle->m_vOrientation[2] = 0.0f;
	...
	((newent->s.origin)[0]=(ent->s.origin)[0],(newent->s.origin)[1]=(ent->s.origin)[1],(newent->s.origin)[2]=(ent->s.origin)[2]);
	((newent->client->ps.origin)[0]=(ent->s.origin)[0],(newent->client->ps.origin)[1]=(ent->s.origin)[1],(newent->client->ps.origin)[2]=(ent->s.origin)[2]);
	((newent->r.currentOrigin)[0]=(ent->s.origin)[0],(newent->r.currentOrigin)[1]=(ent->s.origin)[1],(newent->r.currentOrigin)[2]=(ent->s.origin)[2]);
	...
	if ( ent->NPC_type )
	{
		...
		else if ( !Q_stricmp( ent->NPC_type, "test" ) )
		{
			...
			for ( n = 0; n < 1 ; n++) 
			{
				...
				{
					((newent->s.origin)[0]=(g_entities[n].s.origin)[0],(newent->s.origin)[1]=(g_entities[n].s.origin)[1],(newent->s.origin)[2]=(g_entities[n].s.origin)[2]);
	...
	if (sizefactor < 10){
		...
		sizefloat = (float)sizefactor / 100.0f;
	}
				...
				newent->client->ps.iModelScale = sizefactor;
		((newent->modelScale)[0]=(sizefloat), (newent->modelScale)[1]=(sizefloat), (newent->modelScale)[2]=(sizefloat));
	newent->client->ps.crouchheight =  (int)(float)sizefloat * 16;
	newent->client->ps.standheight =  (int)(float)sizefloat * 40;
	newent->client->saber[0].damageScale *= sizefloat;
	newent->client->ps.speed /= sizefloat;
	newent->client->ps.basespeed /= sizefloat;
	newent->client->ps.gravity *= sizefloat;
	newent->client->ps.stats[STAT_HEALTH] *= sizefloat;
	...
	((newent->s.angles)[0]=(ent->s.angles)[0],(newent->s.angles)[1]=(ent->s.angles)[1],(newent->s.angles)[2]=(ent->s.angles)[2]);
	((newent->r.currentAngles)[0]=(ent->s.angles)[0],(newent->r.currentAngles)[1]=(ent->s.angles)[1],(newent->r.currentAngles)[2]=(ent->s.angles)[2]);
	((newent->client->ps.viewangles)[0]=(ent->s.angles)[0],(newent->client->ps.viewangles)[1]=(ent->s.angles)[1],(newent->client->ps.viewangles)[2]=(ent->s.angles)[2]);
	newent->NPC->desiredYaw =ent->s.angles[1];
	...
	if ( ent->parms )
	{
		...
		for ( parmNum = 0; parmNum < 16; parmNum++ )
		{
	...
	((newent->s.pos.trBase)[0]=(newent->r.currentOrigin)[0],(newent->s.pos.trBase)[1]=(newent->r.currentOrigin)[1],(newent->s.pos.trBase)[2]=(newent->r.currentOrigin)[2]);
	((newent->s.pos.trDelta)[0]=(newent->s.pos.trDelta)[1]=(newent->s.pos.trDelta)[2]=0);
	...
	((newent->s.apos.trBase)[0]=(newent->s.angles)[0],(newent->s.apos.trBase)[1]=(newent->s.angles)[1],(newent->s.apos.trBase)[2]=(newent->s.angles)[2]);

	((newent->s.apos.trDelta)[0]=(newent->s.apos.trDelta)[1]=(newent->s.apos.trDelta)[2]=0);
	newent->s.apos.trDuration = 0;
	
	newent->NPC->combatPoint = -1;

	newent->flags |= 0x00000020;
	newent->s.eFlags |= (1<<8);

	newent->think = NPC_Begin;
	newent->nextthink = level.time + 50;
...
void NPC_ShySpawn( gentity_t *ent )
{
	...
	if ( DistanceSquared( g_entities[0].r.currentOrigin, ent->r.currentOrigin ) <= ( 256 * 256 ) )
		return;

	if ( (InFOV( ent, &g_entities[0], 80, 64 )) ) 
...
void SP_NPC_spawner( gentity_t *self)
{
...

	{
		...
		{
			self->r.svFlags |= 0x10000000;
		}
		if ( G_SpawnInt( "noCombatSounds", "0", &garbage ) )
		{
			self->r.svFlags |= 0x20000000;
		}
		if ( G_SpawnInt( "noExtraSounds", "0", &garbage ) )
		{
			self->r.svFlags |= 0x40000000;
		}
	}
	...
	else
	{
		...
		if (1) 
		{
			...
			self->nextthink = level.time + 50*3 + 50;
		}
		else
		{
...
qboolean NPC_VehiclePrecache( gentity_t *spawner )
{
	...
	G_ModelIndex(va("$%s", spawner->NPC_type)); 
		...
		{ 
			...
			if (GLAName[0])
			{
				...
				if ( slash )
				{
					...
					BG_ParseAnimationFile(GLAName, ((void *)0), qfalse);
				}
			}
...
void SP_NPC_Kyle( gentity_t *self)
{
	...
	WP_SetSaberModel( ((void *)0), CLASS_KYLE );
...
void SP_NPC_Luke( gentity_t *self)
{
	...
	WP_SetSaberModel( ((void *)0), CLASS_LUKE );
...
void SP_NPC_Tavion( gentity_t *self)
{
	...
	WP_SetSaberModel( ((void *)0), CLASS_TAVION );
...
void SP_NPC_Desann( gentity_t *self)
{
	...
	WP_SetSaberModel( ((void *)0), CLASS_DESANN );
...
if ( Q_irand( 0, 1 ) )
	...
	WP_SetSaberModel( ((void *)0), CLASS_JEDI );
...
void SP_NPC_Reborn( gentity_t *self)
{
	...
	WP_SetSaberModel( ((void *)0), CLASS_REBORN );
	SP_NPC_spawner( self );
}
...
void SP_NPC_ShadowTrooper( gentity_t *self)
{
	...
	WP_SetSaberModel( ((void *)0), CLASS_SHADOWTROOPER );
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle, int sizefactor ) 
{
	...
	if (ent->client->ps.userInt3 > 15){
		trap_SendServerCommand( ent->client->ps.clientNum, va("print \"^5Enough Desanns already!\n\"" ) );
		return ((void *)0);
	}
	ent->client->ps.userInt3++;

	if(!NPCspawner)
	{
		Com_Printf( "^1""NPC_Spawn Error: Out of entities!\n" );
		return ((void *)0);
	}

	NPCspawner->think = G_FreeEntity;
	NPCspawner->nextthink = level.time + 50;
	...
	AngleVectors(ent->client->ps.viewangles, forward, ((void *)0), ((void *)0));
	VectorNormalize(forward);
	((end)[0]=(ent->r.currentOrigin)[0]+(forward)[0]*(64),(end)[1]=(ent->r.currentOrigin)[1]+(forward)[1]*(64),(end)[2]=(ent->r.currentOrigin)[2]+(forward)[2]*(64));
	trap_Trace(&trace, ent->r.currentOrigin, ((void *)0), ((void *)0), end, 0, (0x00000001|0x00001000));
	((end)[0]=(trace.endpos)[0],(end)[1]=(trace.endpos)[1],(end)[2]=(trace.endpos)[2]);
	end[2] -= 24;
	trap_Trace(&trace, trace.endpos, ((void *)0), ((void *)0), end, 0, (0x00000001|0x00001000));
	((end)[0]=(trace.endpos)[0],(end)[1]=(trace.endpos)[1],(end)[2]=(trace.endpos)[2]);
	end[2] += 24;
	G_SetOrigin(NPCspawner, end);
	((NPCspawner->s.origin)[0]=(NPCspawner->r.currentOrigin)[0],(NPCspawner->s.origin)[1]=(NPCspawner->r.currentOrigin)[1],(NPCspawner->s.origin)[2]=(NPCspawner->r.currentOrigin)[2]);
	...
	{
		NPC_Interrogator_Precache(((void *)0));
	}
	...
	return (NPC_Spawn_Do( NPCspawner , sizefactor));
}

int GetPrices( gentity_t *ent, int price){
	int calc;
	float adj;

	if (ent->client){
		
		adj = (1.00f-(.0059f*(ent->client->sess.karma)));
...
{
	char	npc_type[1024], unpc[1024], command[1024], article[24];
	char	targetname[1024], qArg[1024], specname[1024];
	qboolean	isVehicle = qfalse, all = qfalse, isRand = qfalse;
	int required, buying = 0, sizefactor, quantity, i, j, gbought = 0;
	gentity_t *specent;
	...
	trap_Argv(0, command, 1024);
	if ( Q_stricmp( "buy", command ) == 0 ){
		buying = 1;
	}

	trap_Argv(2- buying, npc_type, 1024);
	
	if (strlen(npc_type) <= 4 && IsABot(ent->s.number)){
		int v = Q_irand(0,13);
		...
		switch(v){
			case 1:
			strcpy(npc_type,"ammo");
			quantity = Q_irand(1,5);
			break;

				case 2:
			strcpy(npc_type,"grenade");
			quantity = Q_irand(1,3);
			break;

				case 3:
			strcpy(npc_type,"rocket");
			quantity = Q_irand(1,5);
			break;

				case 4:
			strcpy(npc_type,"jetpack");
			break;

				case 5:
			strcpy(npc_type,"tripmine");
			break;
			...
			break;
				...
				case 9:
			strcpy(npc_type,"seeker");
			break;

				case 10:
			strcpy(npc_type,"detpack");
			break;

				case 11:
			strcpy(npc_type,"sentry");
			break;

				case 12:
			strcpy(npc_type,"gravgun");
			quantity = Q_irand(1,3);
			break;
			...
			default:
			strcpy(npc_type,"jawa");
			sizefactor = Q_irand(50,200);
			quantity = Q_irand(1,3);
			break;

		}

		if (ent->client->ps.pm_type == PM_FLOAT){
			strcpy(npc_type,"jetpack");
		}
	...
	{
		trap_Argv(3- buying, npc_type, 1024);
		

		
		trap_Argv(5- buying, targetname, 1024);
		sizefactor = atoi(targetname);
		trap_Argv(4- buying, qArg, 1024);
		quantity = atoi(qArg);

		if ( Q_stricmp( "atst_vehicle", npc_type ) == 0 )
		{	
			isVehicle = qtrue;
			required = 300;
		}
	...
	}else if(Q_stricmp( "all", npc_type ) == 0 )
	{
		trap_Argv(3- buying, npc_type, 1024);
		trap_Argv(5- buying, targetname, 1024);
		sizefactor = atoi(targetname);
	...
	else
	{
		...
		trap_Argv(4- buying, targetname, 1024);
		sizefactor = atoi(targetname);
		trap_Argv(3- buying, qArg, 1024);
		quantity = atoi(qArg);
...
if ( 
		...
		Q_stricmp( "emp", npc_type ) == 0 
		|| Q_stricmp( "ammo", npc_type ) == 0
		|| Q_stricmp( "grenade", npc_type ) == 0
		|| Q_stricmp( "tripmine", npc_type ) == 0
		|| Q_stricmp( "detpack", npc_type ) == 0
		)
	{
		required = 50;
	}
	...
	if ( 
		Q_stricmp( "rocket", npc_type ) == 0 
		|| Q_stricmp( "eweb", npc_type ) == 0 
		|| Q_stricmp( "repeater", npc_type ) == 0 
		|| Q_stricmp( "gravgun", npc_type ) == 0 
			|| ( Q_stricmp( "kungfu", npc_type ) == 0 )
			||  Q_stricmp( "flechette", npc_type ) == 0 
			|| Q_stricmp( "cloak", npc_type ) == 0 
		
		)
	{
		required = 75;
	}
	if (( Q_stricmp( "jetpack", npc_type ) == 0 )
		|| ( Q_stricmp( "baton", npc_type ) == 0 )
		|| ( Q_stricmp( "emplaced", npc_type ) == 0 )
		)
	
	{
		if (g_jetpacks.integer == 0 &&  Q_stricmp( "jetpack", npc_type ) == 0){
			trap_SendServerCommand( ent->s.number, va("print \"^1Jetpacks prohibited by majoritarian rule (/callvote jetpacks to enable)\n\"") );
			return;
		}
		if (ent->client->sess.karma >= 64 &&  Q_stricmp( "jetpack", npc_type ) == 0){
			trap_SendServerCommand( ent->s.number, va("print \"^1Jetpacks prohibited by ^4high lightside ^7alignment.\n\"") );
			return;
		}
	...
	if ((Q_stricmp(ent->client->sess.admin_name, "sub")!=0 )
		&&(Q_stricmp(ent->client->sess.admin_name, "tsub")!=0 ) ){
	
		sizefactor = 100;
	}
	if (sizefactor >= 10 && sizefactor <= 1000 && sizefactor != 100){
		required += 10;
	}


	if (npc_type[0] == 'a' || npc_type[0] == 'e' || npc_type[0] == 'i' || npc_type[0] == 'o' || npc_type[0] == 'u' ){
		strcpy(article, "an");
	}else{
		strcpy(article, "a");
	}
	...
	if ( 
		Q_stricmp( "jawa", npc_type ) == 0 

		||  Q_stricmp( "r2d2", npc_type ) == 0
		||  Q_stricmp( "gonk", npc_type ) == 0
		||  Q_stricmp( "protocol", npc_type ) == 0
		)
	{
		required = 15;

		if (ent->client->sess.admin_number >= required * quantity && ent->client->ps.userInt3 < 80){
		
		
		if (Q_stricmp( "jawa", npc_type ) == 0 && ent->client->sess.admin_number >= 15){
			if (Q_irand(0,2)==1){
		
				G_Sound( ent, CHAN_ANNOUNCER, G_SoundIndex( "sound/chars/kyle/04kyk002.mp3" ) );
			}else{
		
				G_Sound( ent, CHAN_ANNOUNCER, G_SoundIndex( "sound/chars/kyle/04kyk005.mp3" ) );
			}
		}
		}
	...
	if ( 
		 ...
		 Q_stricmp( "stormtrooper", npc_type ) == 0
		||  Q_stricmp( "rebel", npc_type ) == 0
		||  Q_stricmp( "tusken", npc_type ) == 0
		...
		)
	{
		...
		if( 
			Q_stricmp( "probe", npc_type ) == 0
			|| Q_stricmp( "interrogator", npc_type ) == 0 
			
			)
		{
		required = 50;
		}
		if (Q_stricmp( "str", npc_type ) == 0 
		|| Q_stricmp( "dex", npc_type ) == 0 
		
		
		||  Q_stricmp( "con", npc_type ) == 0
		)
	{
		required = 50;
	}
		...
		if (Q_stricmp( "tavion", npc_type ) == 0 
		|| Q_stricmp( "tavion_scepter", npc_type ) == 0 
		|| Q_stricmp( "alora", npc_type ) == 0 
		
		
		||  Q_stricmp( "reborn", npc_type ) == 0
		
		||  Q_stricmp( "chewie", npc_type ) == 0
		||  Q_stricmp( "gran", npc_type ) == 0
	...
	if ( Q_stricmp( "rax", npc_type ) == 0 
		||  Q_stricmp( "rockettrooper", npc_type ) == 0 
			||  Q_stricmp( "wampa", npc_type ) == 0 
		)
	{

		if (ent->client->sess.karma >= 128 &&  Q_stricmp( "wampa", npc_type ) == 0){
			trap_SendServerCommand( ent->s.number, va("print \"^1Wampas prohibited by ^4high lightside ^7alignment.\n\"") );
			return;
		}
	...
	if ( Q_stricmp( "tsa", npc_type ) == 0 
		)
	{
		required = 3000;
	}
		if ( Q_stricmp( "subadmin", npc_type ) == 0 
		)
	{
		required = 15000;
	}
	if ( Q_stricmp( "rancor", npc_type ) == 0 
		
		||  Q_stricmp( "rancor_vehicle", npc_type ) == 0
		)
	{

		if (ent->client->sess.karma >= 64){
			trap_SendServerCommand(ent->s.number, va("print \"^1Can't summon rancor: ^4lightside ^7alignment.\n\"") );
			return;
		}
	...
	if ((Q_stricmp(ent->client->sess.admin_name, "sub")==0 ) && required <= 50)
	{
		required = 12;
	}
	else if ((Q_stricmp(ent->client->sess.admin_name, "sub")==0 && required > 50 && required < 500))
	{
		required = 50;
	}
	else if ((Q_stricmp(ent->client->sess.admin_name, "sub")==0 ) && required >= 500 && required < 10000)
	{
		required = 150;
	}
	else if ((Q_stricmp(ent->client->sess.admin_name, "sub")==0))
	{
	...
	strcpy(unpc, npc_type);
	Q_strupr(unpc);
	npc_type[0] = unpc[0];

	if (ent->client->sess.spectatorState == SPECTATOR_FOLLOW){
		specent = &g_entities[ent->client->ps.clientNum];
	}else{
		specent = ent;
	}

	required = GetPrices(ent,required);

for (j = 0; j < 16; j++){

	if (g_entities[j].client 
		&& 
		(g_entities[j].client->sess.spectatorState != SPECTATOR_FREE || g_gametype.integer != 4)
		&& 
		
		(g_entities[j].client->sess.spectatorState != SPECTATOR_FOLLOW || g_gametype.integer !=4) && (g_entities[j].client->pers.netname[1] > 0 || (g_entities[j].NPC_type && g_entities[j].NPC_type[1] > 0)) && (all == qtrue || g_entities[j].s.number == specent->s.number)){

		specent = &g_entities[j];

		if (g_entities[j].client->pers.netname[1] > 0){
			strcpy(specname, g_entities[j].client->pers.netname);
		}else if (g_entities[j].NPC_type[1] > 0){
			strcpy(specname, g_entities[j].NPC_type);
		}
...
for (i = 1; i<= quantity; i++){
	...
	if (ent->client->sess.admin_number >= (required * quantity) && npc_type[1] != 0 && ent->client->ps.userInt3 < 80){
		...
		if (i == quantity){
			ent->client->sess.admin_number -= required * quantity;
		}
			
		if ( Q_stricmp( "subadmin", npc_type)  == 0)
		{
			strcpy(specent->client->sess.admin_name,"sub");
			WriteKeys(specent->s.number,7,qtrue);
			
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought subadmin privs ^4x%i^5 for %s\n\"",  ent->client->pers.netname, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought subadmin privs ^4x%i^5 for %s\n\"",  ent->client->pers.netname, quantity, specname ) );
			}
		}
		else if ( Q_stricmp( "tsa", npc_type)  == 0)
		{
			if (Q_stricmp(specent->client->sess.admin_name,"sub")){
				strcpy(specent->client->sess.admin_name,"tsub");
				WriteKeys(specent->s.number,7,qtrue);
				
				if (i == 1){
				trap_SendServerCommand( -1, va("print \"^5%s^5 bought tsa privs ^4x%i^5 for %s\n\"",  ent->client->pers.netname, quantity, specname ) );
				trap_SendServerCommand( -1, va("cp \"^5%s^5 bought tsa privs ^4x%i^5 for %s\n\"",  ent->client->pers.netname, quantity, specname ) );
				}
			}else{
				if (i == 1){
				trap_SendServerCommand( ent->s.number, va("print \"^5You can't buy tsa for an sa.\n\"" ) );
				ent->client->sess.admin_number += required;
				}
			}
		}

		else if ( Q_stricmp( "str", npc_type)  == 0)
		{
			ent->client->sess.str++;
			WriteKeys(ent->s.number, 1, qtrue);
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 trained STR +%i!\n\"",  ent->client->pers.netname, quantity ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 trained STR +%i!\n\"",  ent->client->pers.netname, quantity ) );
			}
			if (!Q_irand(0,2) && ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.dex--;
				WriteKeys(ent->s.number, 2, qtrue);
			}else if ( ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.con--;
				WriteKeys(ent->s.number, 3, qtrue);
			}
			SetRPGStats(ent, ent->s.number);
		}

		else if ( Q_stricmp( "dex", npc_type)  == 0)
		{
			ent->client->sess.dex++;
			WriteKeys(ent->s.number, 2, qtrue);
						if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 trained DEX +%i!\n\"",  ent->client->pers.netname, quantity ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 trained DEX +%i!\n\"",  ent->client->pers.netname, quantity ) );
			}
			if (!Q_irand(0,2) && ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.str--;
				WriteKeys(ent->s.number, 1, qtrue);
		...
		else if ( Q_stricmp( "con", npc_type)  == 0)
		{
			ent->client->sess.con++;
			WriteKeys(ent->s.number, 3, qtrue);
			ent->client->ps.stats[STAT_MAX_HEALTH]++;
			
			G_Damage(ent, ent, ent, ((void *)0), ent->client->ps.origin, -2, 0x00000000, MOD_SLIME);

						if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 trained CON +%i!\n\"",  ent->client->pers.netname, quantity ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 trained CON +%i!\n\"",  ent->client->pers.netname, quantity ) );
			}
			if (!Q_irand(0,2) && ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.dex--;
				WriteKeys(ent->s.number, 2, qtrue);
		...
		else if ( Q_stricmp( "emp", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_DEMP2 );
			specent->client->ps.ammo[AMMO_POWERCELL] += 40;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}		
		}else if ( Q_stricmp( "kungfu", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_MELEE );
			
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s knows kung fu!\n\"",  specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s knows kung fu!\n\"",  specname ) );
			}	
		}else if ( Q_stricmp( "baton", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_STUN_BATON );
			specent->client->ps.ammo[AMMO_POWERCELL] += 40;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "flechette", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_FLECHETTE );
			specent->client->ps.ammo[AMMO_POWERCELL] += 40;
			specent->client->ps.ammo[AMMO_METAL_BOLTS] += 40;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "emplaced", npc_type)  == 0)
			{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_EMPLACED_GUN );
			specent->client->ps.ammo[AMMO_EMPLACED] += 40;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		...
		}else if ( Q_stricmp( "gravgun", npc_type)  == 0)
			{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_CONCUSSION );
			specent->client->ps.ammo[AMMO_METAL_BOLTS] += 135;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		...
		}else if ( Q_stricmp( "grenade", npc_type)  == 0)
			{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_THERMAL );
			specent->client->ps.ammo[AMMO_THERMAL]++;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "detpack", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_DET_PACK );
			specent->client->ps.ammo[AMMO_DETPACK]++;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}

		}else if ( Q_stricmp( "tripmine", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_TRIP_MINE );
			specent->client->ps.ammo[AMMO_TRIPMINE]++;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "rocket", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_ROCKET_LAUNCHER );
			specent->client->ps.ammo[AMMO_ROCKETS]++;
			if (i == 1){
				trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "jetpack", npc_type)  == 0 && specent->client->ps.pm_type != PM_JETPACK)
		{
			
			if (ent->client){
				LoseKarma(ent->s.number, "Honor: terra firma.", 1);
			}

			specent->client->ps.jetpackFuel = 150;
			specent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_JETPACK );
			specent->client->jetPackOn = 1;
			specent->client->ps.pm_type = PM_JETPACK;
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
			...
			Q_stricmp( "jawa", npc_type ) == 0 
			||  Q_stricmp( "ugnaught", npc_type ) == 0 
			||  Q_stricmp( "r2d2", npc_type ) == 0
			||  Q_stricmp( "gonk", npc_type ) == 0
			||  Q_stricmp( "protocol", npc_type ) == 0
			)
			{
				LoseKarma(ent->s.number, "Justice: defend the innocent.", 1);
				NPC_SpawnType( ent, npc_type, targetname, isVehicle, sizefactor ); 

				if (i == 1){
				trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5\n\"",  ent->client->pers.netname, article, npc_type, quantity ) );
				trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5\n\"",  ent->client->pers.netname, article, npc_type, quantity ) );
				}
		
			}
		else if ( Q_stricmp( "sentry", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_SENTRY_GUN );
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );	
			}
		}
		else if ( Q_stricmp( "field", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_SHIELD );
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
		...
		else if ( Q_stricmp( "cloak", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_CLOAK );
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}
		else if ( Q_stricmp( "seeker", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_SEEKER );
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "eweb", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= ( 1 << HI_EWEB );
			if (i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 bought \n%s %s ^4x%i^5 for %s\n\"",  ent->client->pers.netname, article, npc_type, quantity, specname ) );
			}
		}else if ( Q_stricmp( "repeater", npc_type)  == 0)
		{
			specent->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_REPEATER );
			specent->client->ps.ammo[AMMO_METAL_BOLTS]+= 30;
			specent->client->ps.ammo[AMMO_POWERCELL]+= 30;
		...
		}else if ( Q_stricmp( "ammo", npc_type)  == 0)
		{
			
			specent->client->ps.ammo[AMMO_BLASTER] += 33;
			specent->client->ps.ammo[AMMO_POWERCELL] += 33;
			specent->client->ps.ammo[AMMO_METAL_BOLTS] += 33;
			...
			NPC_SpawnType( specent, npc_type, targetname, isVehicle, sizefactor );
		...
		else if (NPC_SpawnType( ent, npc_type, targetname, isVehicle, sizefactor ) && required <= 1200)
		{
			...
			if (i == 1){
			trap_SendServerCommand( ent->s.number, va("print \"^5You can't buy a %s.\n\"", npc_type ) );
			ent->client->sess.admin_number += required;
			}
			
		}
		WriteKeys(ent->s.number, 4, qtrue);
	}else if (npc_type[1] == 0 && i == quantity){
	
		if (Q_stricmp(ent->client->sess.admin_name, "tsub") == 0
			||  Q_stricmp(ent->client->sess.admin_name, "sub") == 0
			){

			trap_SendServerCommand( ent-g_entities, va("print \"^1+10 gold to pass size argument (e.g., /buy alora 200)\n\""));
			}
	...
	}else if (ent->client->sess.admin_number < (required*quantity) && i == quantity){

		
		if (Q_stricmp(ent->client->sess.admin_name, "sub")==0 ){
		trap_SendServerCommand( ent-g_entities, va("print \"^5You need %i more gold pieces to buy %s %s ^4x%i^5 w/employee discount.\n\"",  (required*quantity)-ent->client->sess.admin_number, article, npc_type, quantity ) );
		}else{
		trap_SendServerCommand( ent-g_entities, va("print \"^5You need %i more gold pieces to buy %s %s ^4x%i.\n\"",  ((required*quantity)-ent->client->sess.admin_number), article, npc_type, quantity ) );
		}
		
	}else if (i == quantity){
		
		trap_SendServerCommand( ent-g_entities, va("print \"^5You've bought enough this round.\n\"" ) );
...
void NPC_Kill_f( void ) 
{
	...
	if ( !name[0] )
	{
		Com_Printf( "^1""Error, Expected:\n");
		Com_Printf( "^1""NPC kill '[NPC targetname]' - kills NPCs with certain targetname\n" );
		Com_Printf( "^1""or\n" );
		Com_Printf( "^1""NPC kill 'all' - kills all NPCs\n" );
		Com_Printf( "^1""or\n" );
		Com_Printf( "^1""NPC team '[teamname]' - kills all NPCs of a certain team ('nonally' is all but your allies)\n" );
		return;
	}
	...
	if ( Q_stricmp( "team", name ) == 0 )
	{
		...
		if ( !name[0] )
		{
			Com_Printf( "^1""NPC_Kill Error: 'npc kill team' requires a team name!\n" );
			Com_Printf( "^1""Valid team names are:\n");
			for ( n = (TEAM_FREE + 1); n < TEAM_NUM_TEAMS; n++ )
			{
				Com_Printf( "^1""%s\n", TeamNames[n] );
			}
			Com_Printf( "^1""nonally - kills all but your teammates\n" );
			return;
		}
		...
		else
		{
			...
			{
				Com_Printf( "^1""NPC_Kill Error: team '%s' not recognized\n", name );
				Com_Printf( "^1""Valid team names are:\n");
				for ( n = (TEAM_FREE + 1); n < TEAM_NUM_TEAMS; n++ )
				{
					Com_Printf( "^1""%s\n", TeamNames[n] );
		...
		if ( killNonSF )
		{
			if ( player )
			{
				if ( player->client )
				{
					...
					{
						Com_Printf( "^2""Killing NPC %s named %s\n", player->NPC_type, player->targetname );
						player->health = 0;
				...
				{
					Com_Printf( "^2""Removing NPC spawner %s with NPC named %s\n", player->NPC_type, player->NPC_targetname );
					G_FreeEntity( player );
...
void Cmd_NPC_f( gentity_t *ent ) 
{
	...
	else if ( Q_stricmp( cmd, "kill" ) == 0 && ent->client->sess.admin_number >= 50) 
	{
		NPC_Kill_f();
		ent->client->sess.admin_number -= 50;
		trap_SendServerCommand( -1, va("cp \"^5%s has cast Recall NPC for 50gp.\n\"",  ent->client->pers.netname ) );
	}else if (Q_stricmp( cmd, "kill" ) == 0) {
		trap_SendServerCommand( ent->client->ps.clientNum, va("print \"^5You need 50 gold to kill NPCs.\n\"" ) );
	}
	...
	else if ( Q_stricmp ( cmd, "score" ) == 0 )
	{
		...
		{
			if ( (ent = G_Find( ((void *)0), ((int)&(((gentity_t *)0)->targetname)), cmd2 )) != ((void *)0) && ent->client )
			{
				NPC_PrintScore( ent );
			}
			else
