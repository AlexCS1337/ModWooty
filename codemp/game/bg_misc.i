#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 7 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 8 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 11 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
#line 12 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 22 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 31 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 33 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"

const char *bgToggleableSurfaces[31] = 
{
	"l_arm_key",					
	"torso_canister1",
	"torso_canister2",
...
const int bgToggleableSurfaceDebris[31] =
{
	0,					
	0,
	0,
...
const char	*bg_customSiegeSoundNames[30] = 
{
	"*att_attack",
	"*att_primary",
	"*att_second",
...
#line 340 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"


char *G_NewString( const char *string );


#line 346 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 348 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 377 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
				break;
			case F_VECTOR:
			...
			case F_PARM16:
				...
				break;
#line 416 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
			default:
			case F_IGNORE:
				break;
			}
...
#line 426 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
float vectoyaw( const vec3_t vec ) {
	...
	if (vec[1] == 0 && vec[0] == 0) {
		...
		if (vec[0]) {
			yaw = ( atan2( vec[1], vec[0]) * 180 / 3.14159265358979323846f );
		} else if (vec[1] > 0) {
			yaw = 90;
		} else {
			yaw = 270;
...
qboolean BG_CanItemBeGrabbed( int gametype, const entityState_t *ent, const playerState_t *ps ) {
	...
	case IT_WEAPON:
		...
		if (!(ent->eFlags & (1<<25)) && (ps->stats[STAT_WEAPONS] & (1 << item->giTag)) &&
...
#line 2346 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
#line 2347 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
         break;
	}
...
void BG_EvaluateTrajectory( const trajectory_t *tr, int atTime, vec3_t result ) {
	...
	case TR_STATIONARY:
	case TR_INTERPOLATE:
		((result)[0]=(tr->trBase)[0],(result)[1]=(tr->trBase)[1],(result)[2]=(tr->trBase)[2]);
		break;
	case TR_LINEAR:
		deltaTime = ( atTime - tr->trTime ) * 0.001;	
		((result)[0]=(tr->trBase)[0]+(tr->trDelta)[0]*(deltaTime),(result)[1]=(tr->trBase)[1]+(tr->trDelta)[1]*(deltaTime),(result)[2]=(tr->trBase)[2]+(tr->trDelta)[2]*(deltaTime));
		break;
	case TR_SINE:
		deltaTime = ( atTime - tr->trTime ) / (float) tr->trDuration;
		phase = sin( deltaTime * 3.14159265358979323846f * 2 );
		((result)[0]=(tr->trBase)[0]+(tr->trDelta)[0]*(phase),(result)[1]=(tr->trBase)[1]+(tr->trDelta)[1]*(phase),(result)[2]=(tr->trBase)[2]+(tr->trDelta)[2]*(phase));
		break;
	case TR_LINEAR_STOP:
	...
	case TR_NONLINEAR_STOP:
		...
		{
			deltaTime = tr->trDuration*0.001f*((float)cos( ( ( (90.0f - (90.0f*((float)atTime-tr->trTime)/(float)tr->trDuration)) * 3.14159265358979323846f ) / 180.0F ) ));
		}
		((result)[0]=(tr->trBase)[0]+(tr->trDelta)[0]*(deltaTime),(result)[1]=(tr->trBase)[1]+(tr->trDelta)[1]*(deltaTime),(result)[2]=(tr->trBase)[2]+(tr->trDelta)[2]*(deltaTime));
		break;
	case TR_GRAVITY:
...
#line 2416 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
		break;
	}
}
...
void BG_EvaluateTrajectoryDelta( const trajectory_t *tr, int atTime, vec3_t result ) {
	...
	case TR_INTERPOLATE:
		...
		break;
	case TR_LINEAR:
		((result)[0]=(tr->trDelta)[0],(result)[1]=(tr->trDelta)[1],(result)[2]=(tr->trDelta)[2]);
		break;
	case TR_SINE:
		deltaTime = ( atTime - tr->trTime ) / (float) tr->trDuration;
		phase = cos( deltaTime * 3.14159265358979323846f * 2 );	
		phase *= 0.5;
		((result)[0]=(tr->trDelta)[0]*(phase),(result)[1]=(tr->trDelta)[1]*(phase),(result)[2]=(tr->trDelta)[2]*(phase));
		break;
	case TR_LINEAR_STOP:
		if ( atTime > tr->trTime + tr->trDuration ) {
			((result)[0]=(result)[1]=(result)[2]=0);
	...
	case TR_NONLINEAR_STOP:
		...
		deltaTime = tr->trDuration*0.001f*((float)cos( ( ( (90.0f - (90.0f*((float)atTime-tr->trTime)/(float)tr->trDuration)) * 3.14159265358979323846f ) / 180.0F ) ));
		((result)[0]=(tr->trDelta)[0]*(deltaTime),(result)[1]=(tr->trDelta)[1]*(deltaTime),(result)[2]=(tr->trDelta)[2]*(deltaTime));
		break;
	case TR_GRAVITY:
		deltaTime = ( atTime - tr->trTime ) * 0.001;	
		((result)[0]=(tr->trDelta)[0],(result)[1]=(tr->trDelta)[1],(result)[2]=(tr->trDelta)[2]);
...
#line 2662 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
		}
	}
#line 2665 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
	ps->events[ps->eventSequence & (2-1)] = newEvent;
	ps->eventParms[ps->eventSequence & (2-1)] = eventParm;
	ps->eventSequence++;
}
...
void BG_TouchJumpPad( playerState_t *ps, entityState_t *jumppad ) {
	...
	if ( ps->jumppad_ent != jumppad->number ) {
		...
		p = fabs( AngleNormalize180( angles[0] ) );
		if( p < 45 ) {
			effectNum = 0;
		} else {
			effectNum = 1;
	...
	((ps->velocity)[0]=(jumppad->origin2)[0],(ps->velocity)[1]=(jumppad->origin2)[1],(ps->velocity)[2]=(jumppad->origin2)[2]);
}
...
{
	float dif = AngleSubtract(baseAngles[1], angles[1]);
		...
		dif < -constraint)
	{
		...
		*newYaw = AngleSubtract(angles[1], -dif);
...
void BG_PlayerStateToEntityState( playerState_t *ps, entityState_t *s, qboolean snap ) {
	...
	((s->pos.trBase)[0]=(ps->origin)[0],(s->pos.trBase)[1]=(ps->origin)[1],(s->pos.trBase)[2]=(ps->origin)[2]);
	if ( snap ) {
		SnapVector( s->pos.trBase );
	}
	
	((s->pos.trDelta)[0]=(ps->velocity)[0],(s->pos.trDelta)[1]=(ps->velocity)[1],(s->pos.trDelta)[2]=(ps->velocity)[2]);

	s->apos.trType = TR_INTERPOLATE;
	((s->apos.trBase)[0]=(ps->viewangles)[0],(s->apos.trBase)[1]=(ps->viewangles)[1],(s->apos.trBase)[2]=(ps->viewangles)[2]);
	if ( snap ) {
	...
	((s->origin2)[0]=(ps->lastHitLoc)[0],(s->origin2)[1]=(ps->lastHitLoc)[1],(s->origin2)[2]=(ps->lastHitLoc)[2]);
...
#line 3205 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 3232 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
}
...
#line 3326 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
static char		bg_pool[3000000];
static int		bg_poolSize = 0;
static int		bg_poolTail = 3000000;
...
qboolean BG_OutOfMemory ( void )
{
	return bg_poolSize >= 3000000;
}

#line 3406 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
...
#line 3408 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\bg_misc.c"
