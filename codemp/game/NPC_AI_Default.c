...
void NPC_BSHuntAndKill( void )
{
	...
	////061304NPC_CheckEnemy( NPCInfo->tempBehavior != BS_HUNT_AND_KILL, qfalse, qtrue );//don't find new enemy if this is tempbehav
	...
	{//ok, stand guard until we find an enemy
		////061304if( NPCInfo->tempBehavior == BS_HUNT_AND_KILL )
		////{
			NPCInfo->tempBehavior = BS_DEFAULT;
		////}
		////else
...
//	qboolean	shoot = qfalse;
	...
	{//being forced to walk
		...
		{
			NPC_SetAnim( NPC, SETANIM_TORSO, TORSO_SURRENDER_START, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
		}
	}
