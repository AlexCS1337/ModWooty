#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 6 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 7 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\q_shared.h"
...
#line 8 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
int remapCount = 0;
shaderRemap_t remappedShaders[128];
...
const char *BuildShaderStateConfig(void) {
	static char	buff[1024*4];
	char out[(64 * 2) + 5];
	int i;
  
	memset(buff, 0, 1024);
	for (i = 0; i < remapCount; i++) {
		Com_sprintf(out, (64 * 2) + 5, "%s=%s:%5.2f@", remappedShaders[i].oldShader, remappedShaders[i].newShader, remappedShaders[i].timeOffset);
...
int G_BoneIndex( const char *name ) {
	return G_FindConfigstringIndex (name, (((((((((((37+256)+1)+1)+1)+1)+1)+512)+1)+256)+64)+32), 64, qtrue);
}
...
int G_ModelIndex( const char *name ) {
	...
	return G_FindConfigstringIndex (name, ((((((37+256)+1)+1)+1)+1)+1), 512, qtrue);
}
...
int	G_IconIndex( const char* name ) 
{
	...
	if (name && name[0]){
	return G_FindConfigstringIndex (name, (((((((((37+256)+1)+1)+1)+1)+1)+512)+1)+256), 64, qtrue);
	}
}
...
int G_SoundIndex( const char *name ) {
	...
	if (name && name[0]){
		return G_FindConfigstringIndex (name, ((((((((37+256)+1)+1)+1)+1)+1)+512)+1), 256, qtrue);
	}
}

int G_SoundSetIndex(const char *name)
{
	return G_FindConfigstringIndex (name, 37, 256, qtrue);
}

int G_EffectIndex( const char *name )
{
	return G_FindConfigstringIndex (name, ((((((((((((((37+256)+1)+1)+1)+1)+1)+512)+1)+256)+64)+32)+64)+64)+64), 64, qtrue);
}

int G_BSPIndex( const char *name )
{
	return G_FindConfigstringIndex (name, (((((((((((((((((37+256)+1)+1)+1)+1)+1)+512)+1)+256)+64)+32)+64)+64)+64) + 64) + (64*3)) + 1), 32, qtrue);
}
...

{
	...
	{
		((kvel)[0]=(newDir)[0]*(g_knockback.value * (float)push / mass * 0.8),(kvel)[1]=(newDir)[1]*(g_knockback.value * (float)push / mass * 0.8),(kvel)[2]=(newDir)[2]*(g_knockback.value * (float)push / mass * 0.8));
		kvel[2] = newDir[2] * g_knockback.value * (float)push / mass * 1.5;
	}
	else
	{
		((kvel)[0]=(newDir)[0]*(g_knockback.value * (float)push / mass),(kvel)[1]=(newDir)[1]*(g_knockback.value * (float)push / mass),(kvel)[2]=(newDir)[2]*(g_knockback.value * (float)push / mass));
	}
...
static Vehicle_t g_vehiclePool[128];
static qboolean g_vehiclePoolOccupied[128];
...
#line 483 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 485 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 512 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
	(void)( (!!(ent->client)) || (_wassert(L"ent->client", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c", 512), 0) );
    BG_SetAnim(&ent->client->ps, bgAllAnims[ent->localAnimIndex].anims, setAnimParts,
		anim, setAnimFlags, blendTime);
#line 516 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
}
...
gentity_t *G_PickTarget (char *targetname)
{
	...
	while(1)
	{
		ent = G_Find (ent, ((int)&(((gentity_t *)0)->targetname)), targetname);
		if (!ent)
			break;
...
void G_UseTargets2( gentity_t *ent, gentity_t *activator, const char *string ) {
	...
	if (ent->targetShaderName && ent->targetShaderNewName) {
		...
		trap_SetConfigstring(24, BuildShaderStateConfig());
	}
	...
	t = ((void *)0);
	while ( (t = G_Find (t, ((int)&(((gentity_t *)0)->targetname)), string)) != ((void *)0) ) {
		if ( t == ent ) {
...
void G_SetMovedir( vec3_t angles, vec3_t movedir ) {
	...
	if ( VectorCompare (angles, VEC_UP) ) {
		((movedir)[0]=(MOVEDIR_UP)[0],(movedir)[1]=(MOVEDIR_UP)[1],(movedir)[2]=(MOVEDIR_UP)[2]);
	} else if ( VectorCompare (angles, VEC_DOWN) ) {
		((movedir)[0]=(MOVEDIR_DOWN)[0],(movedir)[1]=(MOVEDIR_DOWN)[1],(movedir)[2]=(MOVEDIR_DOWN)[2]);
	} else {
...
#line 727 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"


	fileHandle_t fh;
	trap_FS_FOpenFile("entspew.txt", &fh, FS_WRITE);
#line 732 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 763 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
			}
...
#line 781 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
		}
...
#line 795 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
}
...
gentity_t *G_Spawn( void ) {
	...
	for ( force = 0 ; force < 2 ; force++ ) {
		...
		e = &g_entities[32];
		for ( i = 32 ; i<level.num_entities ; i++, e++) {
			if ( e->inuse ) {
				continue;
			}
...
void G_SendG2KillQueue(void)
{
	...
	if (gG2KillNum < 0)
	{ 
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c", 913), 0) );
		gG2KillNum = 0;
	}
}
...
#line 925 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 949 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
		return;
	}

	trap_UnlinkEntity (ed);		

	trap_ICARUS_FreeEnt( ed );	
...
gentity_t *G_TempEntity( vec3_t origin, int event ) {
	...
	((snapped)[0]=(origin)[0],(snapped)[1]=(origin)[1],(snapped)[2]=(origin)[2]);
	SnapVector( snapped );		
...
void G_KillBox (gentity_t *ent) {
	...
	for (i=0 ; i<num ; i++) {
		...
		G_Damage ( hit, ent, ent, ((void *)0), ((void *)0),
			100000, 0x00000008, MOD_TELEFRAG);
	}
...
void G_AddEvent( gentity_t *ent, int event, int eventParm ) {
	...
	if ( ent->client ) {
		bits = ent->client->ps.externalEvent & (0x00000100|0x00000200);
		bits = ( bits + 0x00000100 ) & (0x00000100|0x00000200);
		ent->client->ps.externalEvent = event | bits;
		ent->client->ps.externalEventParm = eventParm;
		ent->client->ps.externalEventTime = level.time;
	} else {
		bits = ent->s.event & (0x00000100|0x00000200);
		bits = ( bits + 0x00000100 ) & (0x00000100|0x00000200);
...
gentity_t *G_PlayEffect(int fxID, vec3_t org, vec3_t ang)
{
	...
	((te->s.angles)[0]=(ang)[0],(te->s.angles)[1]=(ang)[1],(te->s.angles)[2]=(ang)[2]);
...
void G_Sound( gentity_t *ent, int channel, int soundIndex ) {
	...
	{ 
		if (g_entities[ent->client->ps.fd.killSoundEntIndex[channel-50]].inuse &&
			ent->client->ps.fd.killSoundEntIndex[channel-50] > 32)
		{
			G_MuteSound(ent->client->ps.fd.killSoundEntIndex[channel-50], CHAN_VOICE);
			if (ent->client->ps.fd.killSoundEntIndex[channel-50] > 32 && g_entities[ent->client->ps.fd.killSoundEntIndex[channel-50]].inuse)
			{
...
int G_CanUseDispOn(gentity_t *ent, int dispType)
{
	...
	{
		if (ent->client->ps.weapon <= WP_NONE || ent->client->ps.weapon > WP_BRYAR_OLD)
		{ 
			return 0;
		}
...
qboolean TryHeal(gentity_t *ent, gentity_t *target)
{
	...
	{ 
		...
		{ 
			...
			{
				G_SetAnim( ent, ((void *)0), 1, BOTH_BUTTON_HOLD, 1|2, 0 );
			}
...
void TryUse( gentity_t *ent )
{
	...
	if (!ent || !ent->client || (ent->client->ps.weaponTime > 0 && ent->client->ps.torsoAnim != BOTH_BUTTON_HOLD && ent->client->ps.torsoAnim != BOTH_CONSOLE1) || ent->health < 1 ||
		(ent->client->ps.pm_flags & 4096) || ent->client->sess.sessionTeam == TEAM_SPECTATOR ||
	...
	if (ent->s.number < 32 && ent->client && ent->client->ps.m_iVehicleNum)
	{
	...
	if (ent->client->bodyGrabIndex != ((1<<10)-1))
	{ 
		if (ent->client->bodyGrabTime < level.time)
		{
			...
			ent->client->bodyGrabIndex = ((1<<10)-1);
			ent->client->bodyGrabTime = level.time + 1000;
		}
		return;
	}

	((viewspot)[0]=(ent->client->ps.origin)[0],(viewspot)[1]=(ent->client->ps.origin)[1],(viewspot)[2]=(ent->client->ps.origin)[2]);
	viewspot[2] += ent->client->ps.viewheight;

	((src)[0]=(viewspot)[0],(src)[1]=(viewspot)[1],(src)[2]=(viewspot)[2]);
	AngleVectors( ent->client->ps.viewangles, vf, ((void *)0), ((void *)0) );

	((dest)[0]=(src)[0]+(vf)[0]*(64.0f),(dest)[1]=(src)[1]+(vf)[1]*(64.0f),(dest)[2]=(src)[2]+(vf)[2]*(64.0f));

	
	trap_Trace( &trace, src, vec3_origin, vec3_origin, dest, ent->s.number, (0x00000001|0x00020000|0x00000002|0x00001000)|0x00000001|0x00000100|0x00100000|0x00000200 );
	...
	{ 
		...
		if (!ent->s.number)
		{ 
			target->s.ragAttach = ((1<<10)-1);
		}
		else
		{
			target->s.ragAttach = ent->s.number;
		}
		ent->client->bodyGrabTime = level.time + 3000;
		ent->client->bodyGrabIndex = target->s.number;
		((ent->s.angles)[0]=(target->s.angles)[0],(ent->s.angles)[1]=(target->s.angles)[1],(ent->s.angles)[2]=(target->s.angles)[2]);
		((ent->s.angles2)[0]=(target->s.angles2)[0],(ent->s.angles2)[1]=(target->s.angles2)[1],(ent->s.angles2)[2]=(target->s.angles2)[2]);
		((ent->s.origin)[0]=(target->s.origin)[0],(ent->s.origin)[1]=(target->s.origin)[1],(ent->s.origin)[2]=(target->s.origin)[2]);
	...
	{ 
		...
		if (pVeh->m_pVehicleInfo)
		{
			...
			ent->client->pers.cmd.buttons &= ~32;
			return;
		}
	}
...
#line 1804 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
...
#line 1831 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_utils.c"
			...
			|| g_ff_objectives.integer) )
	{
		...
		{
			target->touch(target, ent, ((void *)0));
		}
		else
		{
...
tryJetPack:
	...
	{
		if (ent->client->jetPackOn || ent->client->ps.groundEntityNum == ((1<<10)-1))
		{
			ItemUse_Jetpack(ent);
			return;
		}
	...
	{ 
		...
		((fAng)[0]=(0.0f), (fAng)[1]=(ent->client->ps.viewangles[1]), (fAng)[2]=(0.0f));
		AngleVectors(fAng, fwd, 0, 0);

        ((fwd)[0]=(ent->client->ps.origin)[0]+(fwd)[0]*(64.0f),(fwd)[1]=(ent->client->ps.origin)[1]+(fwd)[1]*(64.0f),(fwd)[2]=(ent->client->ps.origin)[2]+(fwd)[2]*(64.0f));		
...
qboolean G_BoxInBounds( vec3_t point, vec3_t mins, vec3_t maxs, vec3_t boundsMins, vec3_t boundsMaxs )
{
	...
	((boxMins)[0]=(point)[0]+(mins)[0],(boxMins)[1]=(point)[1]+(mins)[1],(boxMins)[2]=(point)[2]+(mins)[2]);
	((boxMaxs)[0]=(point)[0]+(maxs)[0],(boxMaxs)[1]=(point)[1]+(maxs)[1],(boxMaxs)[2]=(point)[2]+(maxs)[2]);
...
void G_SetAngles( gentity_t *ent, vec3_t angles )
{
	((ent->r.currentAngles)[0]=(angles)[0],(ent->r.currentAngles)[1]=(angles)[1],(ent->r.currentAngles)[2]=(angles)[2]);
	((ent->s.angles)[0]=(angles)[0],(ent->s.angles)[1]=(angles)[1],(ent->s.angles)[2]=(angles)[2]);
	((ent->s.apos.trBase)[0]=(angles)[0],(ent->s.apos.trBase)[1]=(angles)[1],(ent->s.apos.trBase)[2]=(angles)[2]);
}
...
void G_SetOrigin( gentity_t *ent, vec3_t origin ) {
	((ent->s.pos.trBase)[0]=(origin)[0],(ent->s.pos.trBase)[1]=(origin)[1],(ent->s.pos.trBase)[2]=(origin)[2]);
...
qboolean G_CheckInSolid (gentity_t *self, qboolean fix)
{
	...
	((end)[0]=(self->r.currentOrigin)[0],(end)[1]=(self->r.currentOrigin)[1],(end)[2]=(self->r.currentOrigin)[2]);
	end[2] += self->r.mins[2];
	((mins)[0]=(self->r.mins)[0],(mins)[1]=(self->r.mins)[1],(mins)[2]=(self->r.mins)[2]);
	mins[2] = 0;
	...
	if(trace.fraction < 1.0)
	{
		if(fix)
		{
			...
			((neworg)[0]=(trace.endpos)[0],(neworg)[1]=(trace.endpos)[1],(neworg)[2]=(trace.endpos)[2]);
...
int DebugLine(vec3_t start, vec3_t end, int color) {
	...
	((points[0])[0]=(start)[0],(points[0])[1]=(start)[1],(points[0])[2]=(start)[2]);
	((points[1])[0]=(start)[0],(points[1])[1]=(start)[1],(points[1])[2]=(start)[2]);
	
	((points[2])[0]=(end)[0],(points[2])[1]=(end)[1],(points[2])[2]=(end)[2]);
	
	((points[3])[0]=(end)[0],(points[3])[1]=(end)[1],(points[3])[2]=(end)[2]);


	((dir)[0]=(end)[0]-(start)[0],(dir)[1]=(end)[1]-(start)[1],(dir)[2]=(end)[2]-(start)[2]);
	VectorNormalize(dir);
	dot = ((dir)[0]*(up)[0]+(dir)[1]*(up)[1]+(dir)[2]*(up)[2]);
	if (dot > 0.99 || dot < -0.99) ((cross)[0]=(1), (cross)[1]=(0), (cross)[2]=(0));
	else CrossProduct(dir, up, cross);

	VectorNormalize(cross);

	((points[0])[0]=(points[0])[0]+(cross)[0]*(2),(points[0])[1]=(points[0])[1]+(cross)[1]*(2),(points[0])[2]=(points[0])[2]+(cross)[2]*(2));
	((points[1])[0]=(points[1])[0]+(cross)[0]*(-2),(points[1])[1]=(points[1])[1]+(cross)[1]*(-2),(points[1])[2]=(points[1])[2]+(cross)[2]*(-2));
	((points[2])[0]=(points[2])[0]+(cross)[0]*(-2),(points[2])[1]=(points[2])[1]+(cross)[1]*(-2),(points[2])[2]=(points[2])[2]+(cross)[2]*(-2));
	((points[3])[0]=(points[3])[0]+(cross)[0]*(2),(points[3])[1]=(points[3])[1]+(cross)[1]*(2),(points[3])[2]=(points[3])[2]+(cross)[2]*(2));
...
void G_ROFF_NotetrackCallback( gentity_t *cent, const char *notetrack)
{
	...
	if (strcmp(type, "loop") == 0)
	{
		if (addlArg) 
		{
			((cent->s.pos.trBase)[0]=(cent->s.origin2)[0],(cent->s.pos.trBase)[1]=(cent->s.origin2)[1],(cent->s.pos.trBase)[2]=(cent->s.origin2)[2]);
			((cent->r.currentOrigin)[0]=(cent->s.origin2)[0],(cent->r.currentOrigin)[1]=(cent->s.origin2)[1],(cent->r.currentOrigin)[2]=(cent->s.origin2)[2]);
			((cent->s.apos.trBase)[0]=(cent->s.angles2)[0],(cent->s.apos.trBase)[1]=(cent->s.angles2)[1],(cent->s.apos.trBase)[2]=(cent->s.angles2)[2]);
			((cent->r.currentAngles)[0]=(cent->s.angles2)[0],(cent->r.currentAngles)[1]=(cent->s.angles2)[1],(cent->r.currentAngles)[2]=(cent->s.angles2)[2]);
		}
...
qboolean G_ExpandPointToBBox( vec3_t point, const vec3_t mins, const vec3_t maxs, int ignore, int clipmask )
{
	...
	((start)[0]=(point)[0],(start)[1]=(point)[1],(start)[2]=(point)[2]);
...
float ShortestLineSegBewteen2LineSegs( vec3_t start1, vec3_t end1, vec3_t start2, vec3_t end2, vec3_t close_pnt1, vec3_t close_pnt2 )
{
	...
	((start_dif)[0]=(start2)[0]-(start1)[0],(start_dif)[1]=(start2)[1]-(start1)[1],(start_dif)[2]=(start2)[2]-(start1)[2]);
	
	((v1)[0]=(end1)[0]-(start1)[0],(v1)[1]=(end1)[1]-(start1)[1],(v1)[2]=(end1)[2]-(start1)[2]);
	
	((v2)[0]=(end2)[0]-(start2)[0],(v2)[1]=(end2)[1]-(start2)[1],(v2)[2]=(end2)[2]-(start2)[2]);
	
	v1v1 = ((v1)[0]*(v1)[0]+(v1)[1]*(v1)[1]+(v1)[2]*(v1)[2]);
	v2v2 = ((v2)[0]*(v2)[0]+(v2)[1]*(v2)[1]+(v2)[2]*(v2)[2]);
	...
	{
		float s = -( (v2v2*((v1)[0]*(start_dif)[0]+(v1)[1]*(start_dif)[1]+(v1)[2]*(start_dif)[2])) - (v1v2*((v2)[0]*(start_dif)[0]+(v2)[1]*(start_dif)[1]+(v2)[2]*(start_dif)[2])) ) / denom;
		float t = ( (v1v1*((v2)[0]*(start_dif)[0]+(v2)[1]*(start_dif)[1]+(v2)[2]*(start_dif)[2])) - (v1v2*((v1)[0]*(start_dif)[0]+(v1)[1]*(start_dif)[1]+(v1)[2]*(start_dif)[2])) ) / denom;
		qboolean done = qtrue;
		...
		((close_pnt1)[0]=(start1)[0]+(v1)[0]*(s),(close_pnt1)[1]=(start1)[1]+(v1)[1]*(s),(close_pnt1)[2]=(start1)[2]+(v1)[2]*(s));
		
		((close_pnt2)[0]=(start2)[0]+(v2)[0]*(t),(close_pnt2)[1]=(start2)[1]+(v2)[1]*(t),(close_pnt2)[2]=(start2)[2]+(v2)[2]*(t));
	...
	{
		((close_pnt1)[0]=(start1)[0],(close_pnt1)[1]=(start1)[1],(close_pnt1)[2]=(start1)[2]);
		((close_pnt2)[0]=(start2)[0],(close_pnt2)[1]=(start2)[1],(close_pnt2)[2]=(start2)[2]);
	...
	{
		((close_pnt1)[0]=(start1)[0],(close_pnt1)[1]=(start1)[1],(close_pnt1)[2]=(start1)[2]);
		((close_pnt2)[0]=(end2)[0],(close_pnt2)[1]=(end2)[1],(close_pnt2)[2]=(end2)[2]);
		current_dist = new_dist;
	}
	...
	if ( new_dist < current_dist )
	{
		((close_pnt1)[0]=(end1)[0],(close_pnt1)[1]=(end1)[1],(close_pnt1)[2]=(end1)[2]);
		((close_pnt2)[0]=(start2)[0],(close_pnt2)[1]=(start2)[1],(close_pnt2)[2]=(start2)[2]);
		current_dist = new_dist;
	}
	...
	{
		((close_pnt1)[0]=(start1)[0],(close_pnt1)[1]=(start1)[1],(close_pnt1)[2]=(start1)[2]);
		((close_pnt2)[0]=(new_pnt)[0],(close_pnt2)[1]=(new_pnt)[1],(close_pnt2)[2]=(new_pnt)[2]);
		current_dist = new_dist;
	}
	...
	if ( new_dist < current_dist )
	{
		((close_pnt1)[0]=(new_pnt)[0],(close_pnt1)[1]=(new_pnt)[1],(close_pnt1)[2]=(new_pnt)[2]);
		((close_pnt2)[0]=(start2)[0],(close_pnt2)[1]=(start2)[1],(close_pnt2)[2]=(start2)[2]);
		current_dist = new_dist;
	}
