#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_navnew.c"
...
#line 2 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_navnew.c"
...
#line 3 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_navnew.c"
...
void NPC_ClearBlocked( gentity_t *self )
{
	...
	self->NPC->blockingEntNum = ((1<<10)-1);
}
...
void NAVNEW_PushBlocker( gentity_t *self, gentity_t *blocker, vec3_t right, qboolean setBlockedInfo )
{
	...
	((mins)[0]=(blocker->r.mins)[0],(mins)[1]=(blocker->r.mins)[1],(mins)[2]=(blocker->r.mins)[2]);
	mins[2] += 18;

	moveamt = (self->r.maxs[1] + blocker->r.maxs[1]) * 1.2;

	((end)[0]=(blocker->r.currentOrigin)[0]+(right)[0]*(-moveamt),(end)[1]=(blocker->r.currentOrigin)[1]+(right)[1]*(-moveamt),(end)[2]=(blocker->r.currentOrigin)[2]+(right)[2]*(-moveamt));
	trap_Trace( &tr, blocker->r.currentOrigin, mins, blocker->r.maxs, end, blocker->s.number, blocker->clipmask|0x00000040);
	...
	if ( leftSucc >= 1.0f )
	{
		((blocker->client->pushVec)[0]=(right)[0]*(-moveamt),(blocker->client->pushVec)[1]=(right)[1]*(-moveamt),(blocker->client->pushVec)[2]=(right)[2]*(-moveamt));
...
qboolean NAVNEW_DanceWithBlocker( gentity_t *self, gentity_t *blocker, vec3_t movedir, vec3_t right )
{
	if ( blocker->client && !VectorCompare( blocker->client->ps.velocity, vec3_origin ) )
	{
		...
		((blocker_movedir)[0]=(blocker->client->ps.velocity)[0],(blocker_movedir)[1]=(blocker->client->ps.velocity)[1],(blocker_movedir)[2]=(blocker->client->ps.velocity)[2]);
		blocker_movedir[2] = 0;
		dot = ((blocker_movedir)[0]*(right)[0]+(blocker_movedir)[1]*(right)[1]+(blocker_movedir)[2]*(right)[2]);
		if ( dot > 50.0f )
		{
			
			((movedir)[0]=(movedir)[0]+(right)[0]*(-1),(movedir)[1]=(movedir)[1]+(right)[1]*(-1),(movedir)[2]=(movedir)[2]+(right)[2]*(-1));
		...
		else if ( dot > -50.0f )
		{
			...
			((movedir)[0]=(right)[0]+(movedir)[0],(movedir)[1]=(right)[1]+(movedir)[1],(movedir)[2]=(right)[2]+(movedir)[2]);
			VectorNormalize( movedir );
			return qtrue;
		}
...
qboolean NAVNEW_SidestepBlocker( gentity_t *self, gentity_t *blocker, vec3_t blocked_dir, float blocked_dist, vec3_t movedir, vec3_t right )
{
	...
	{
		if ( self->NPC->lastSideStepSide == -1 )
		{
			arcAngle *= -1;
		}
		avoidAngles[1] = AngleNormalize360( yaw + arcAngle );
		AngleVectors( avoidAngles, movedir, ((void *)0), ((void *)0) );
		((block_pos)[0]=(self->r.currentOrigin)[0]+(movedir)[0]*(blocked_dist),(block_pos)[1]=(self->r.currentOrigin)[1]+(movedir)[1]*(blocked_dist),(block_pos)[2]=(self->r.currentOrigin)[2]+(movedir)[2]*(blocked_dist));
		trap_Trace( &tr, self->r.currentOrigin, mins, self->r.maxs, block_pos, self->s.number, self->clipmask|0x00000040 );
		return (tr.fraction==1.0&&!tr.allsolid&&!tr.startsolid);
	}

	
	avoidAngles[1] = AngleNormalize360( yaw + arcAngle );
	AngleVectors( avoidAngles, avoidRight_dir, ((void *)0), ((void *)0) );

	((block_pos)[0]=(self->r.currentOrigin)[0]+(avoidRight_dir)[0]*(blocked_dist),(block_pos)[1]=(self->r.currentOrigin)[1]+(avoidRight_dir)[1]*(blocked_dist),(block_pos)[2]=(self->r.currentOrigin)[2]+(avoidRight_dir)[2]*(blocked_dist));
	...
	if ( !tr.allsolid && !tr.startsolid )
	{
		if ( tr.fraction >= 1.0f )
		{
			((movedir)[0]=(avoidRight_dir)[0],(movedir)[1]=(avoidRight_dir)[1],(movedir)[2]=(avoidRight_dir)[2]);
	...
	avoidAngles[1] = AngleNormalize360( yaw + arcAngle );
	AngleVectors( avoidAngles, avoidLeft_dir, ((void *)0), ((void *)0) );

	((block_pos)[0]=(self->r.currentOrigin)[0]+(avoidLeft_dir)[0]*(blocked_dist),(block_pos)[1]=(self->r.currentOrigin)[1]+(avoidLeft_dir)[1]*(blocked_dist),(block_pos)[2]=(self->r.currentOrigin)[2]+(avoidLeft_dir)[2]*(blocked_dist));
	...
	if ( !tr.allsolid && !tr.startsolid )
	{
		if ( tr.fraction >= 1.0f )
		{
			((movedir)[0]=(avoidLeft_dir)[0],(movedir)[1]=(avoidLeft_dir)[1],(movedir)[2]=(avoidLeft_dir)[2]);
			self->NPC->lastSideStepSide = -1;
			self->NPC->sideStepHoldTime = level.time + 2000;
			return qtrue;
		}
...
qboolean NAVNEW_Bypass( gentity_t *self, gentity_t *blocker, vec3_t blocked_dir, float blocked_dist, vec3_t movedir, qboolean setBlockedInfo ) 
{
	...
	moveangles[2] = 0;
	AngleVectors( moveangles, ((void *)0), right, ((void *)0) );
...
qboolean NAVNEW_ResolveEntityCollision( gentity_t *self, gentity_t *blocker, vec3_t movedir, vec3_t pathDir, qboolean setBlockedInfo )
{
	...
	if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
	{
		...
		if ( DistanceSquared( self->r.currentOrigin, center ) > ( 16 * 16 ) )
			return qtrue;
	}
...
qboolean NAVNEW_TestNodeConnectionBlocked( int wp1, int wp2, gentity_t *ignoreEnt, int goalEntNum, qboolean checkWorld, qboolean checkEnts )
{
	...
	trace_t	trace;
	int		clipmask = (0x00000001|0x00000020|0x00000100|0x00001000)|0x00000040;
	int ignoreEntNum;
	...
	((playerMins)[0]=(-15), (playerMins)[1]=(-15), (playerMins)[2]=(-24));
	((playerMaxs)[0]=(15), (playerMaxs)[1]=(15), (playerMaxs)[2]=(40));
	...
	if ( !checkWorld )
	{
		clipmask &= ~(0x00000001|0x00000020|0x00000040);
	}
	if ( !checkEnts )
	{
		clipmask &= ~0x00000100;
	...
	if ( ignoreEnt )
	{
		((mins)[0]=(ignoreEnt->r.mins)[0],(mins)[1]=(ignoreEnt->r.mins)[1],(mins)[2]=(ignoreEnt->r.mins)[2]);
		((maxs)[0]=(ignoreEnt->r.maxs)[0],(maxs)[1]=(ignoreEnt->r.maxs)[1],(maxs)[2]=(ignoreEnt->r.maxs)[2]);
		ignoreEntNum = ignoreEnt->s.number;
	}
	else
	{
		((mins)[0]=(playerMins)[0],(mins)[1]=(playerMins)[1],(mins)[2]=(playerMins)[2]);
		((mins)[0]=(playerMaxs)[0],(mins)[1]=(playerMaxs)[1],(mins)[2]=(playerMaxs)[2]);
		ignoreEntNum = ((1<<10)-1);
	}
	mins[2] += 18;
...
int	NAVNEW_MoveToGoal( gentity_t *self, navInfo_t *info )
{
	...
	if ( self->waypoint == -1 && self->noWaypointTime > level.time )
	{
		return -1;
	}
	if ( self->NPC->goalEntity->waypoint == -1 && self->NPC->goalEntity->noWaypointTime > level.time )
	{
		return -1;
	}
	...
	else if ( (bestNode = trap_Nav_GetBestPathBetweenEnts( self, self->NPC->goalEntity, 0x00000002 )) == -1 )
	{
		if ( self->waypoint == -1 )
		{
	...
	while( !foundClearPath )
	{
		...
		memcpy( &tempInfo, info, sizeof( tempInfo ) );
		((tempInfo.direction)[0]=(origin)[0]-(self->r.currentOrigin)[0],(tempInfo.direction)[1]=(origin)[1]-(self->r.currentOrigin)[1],(tempInfo.direction)[2]=(origin)[2]-(self->r.currentOrigin)[2]);
		...
		if ( foundClearPath )
		{
			...
			if ( self->s.weapon == WP_SABER )
			{
				...
				{
					self->NPC->aiFlags |= 0x00000040;
					((NPCInfo->blockedDest)[0]=(origin)[0],(NPCInfo->blockedDest)[1]=(origin)[1],(NPCInfo->blockedDest)[2]=(origin)[2]);
					goto failed;
				}
			}
		}
