#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_turret.c"
...
#line 4 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_turret.c"
#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\q_shared.h"
...
#line 5 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_turret.c"
...

{
	...
	{
		self->attackDebounceTime = level.time + 800 + ((rand () & 0x7fff) / ((float)0x7fff)) * 500;
...

{
	...
	g_entities[self->r.ownerNum].think = ((void *)0);
	g_entities[self->r.ownerNum].use = ((void *)0);
	...
	((pos)[0]=(self->r.currentOrigin)[0],(pos)[1]=(self->r.currentOrigin)[1],(pos)[2]=(self->r.currentOrigin)[2]);
...

{
	...
	if ( self->enemy && self->setTime < level.time && self->attackDebounceTime < level.time )
	{
		...
		((org)[0]=(top->r.currentOrigin)[0],(org)[1]=(top->r.currentOrigin)[1],(org)[2]=(top->r.currentOrigin)[2]);
		org[2] += top->r.maxs[2]-8;
		AngleVectors( top->r.currentAngles, fwd, ((void *)0), ((void *)0) );
...

{
	...
	top->r.currentAngles[1] = AngleNormalize180( top->r.currentAngles[1] );
	top->r.currentAngles[0] = AngleNormalize180( top->r.currentAngles[0] );
	turnSpeed = top->speed;

	if ( self->painDebounceTime > level.time )
	{
		desiredAngles[1] = top->r.currentAngles[1]+flrand(-45,45);
		desiredAngles[0] = top->r.currentAngles[0]+flrand(-10,10);

		if (desiredAngles[0] < -pitchCap)
		{
			desiredAngles[0] = -pitchCap;
		}
		else if (desiredAngles[0] > pitchCap)
		{
			desiredAngles[0] = pitchCap;
		}

		diffYaw = AngleSubtract( desiredAngles[1], top->r.currentAngles[1] );
		diffPitch = AngleSubtract( desiredAngles[0], top->r.currentAngles[0] );
	...
	else if ( self->enemy )
	{
		...
		((org2)[0]=(top->r.currentOrigin)[0],(org2)[1]=(top->r.currentOrigin)[1],(org2)[2]=(top->r.currentOrigin)[2]);

		((enemyDir)[0]=(org)[0]-(org2)[0],(enemyDir)[1]=(org)[1]-(org2)[1],(enemyDir)[2]=(org)[2]-(org2)[2]);
		vectoangles( enemyDir, desiredAngles );
		desiredAngles[0] = AngleNormalize180(desiredAngles[0]);
	...
	else
	{
		...
		desiredAngles[1] = sin( level.time * 0.0001f + top->count );
		desiredAngles[1] *=  60.0f;
		desiredAngles[1] += self->s.angles[1];
		desiredAngles[1] = AngleNormalize180( desiredAngles[1] );
		diffYaw = AngleSubtract( desiredAngles[1], top->r.currentAngles[1] );
		diffPitch = AngleSubtract( 0, top->r.currentAngles[0] );
		turnSpeed = 1.0f;
	}
	...
	((setAngle)[0]=(diffPitch), (setAngle)[1]=(diffYaw), (setAngle)[2]=(0));

	((top->s.apos.trBase)[0]=(top->r.currentAngles)[0],(top->s.apos.trBase)[1]=(top->r.currentAngles)[1],(top->s.apos.trBase)[2]=(top->r.currentAngles)[2]);
	((top->s.apos.trDelta)[0]=(setAngle)[0]*((1000/50)),(top->s.apos.trDelta)[1]=(setAngle)[1]*((1000/50)),(top->s.apos.trDelta)[2]=(setAngle)[2]*((1000/50)));
	top->s.apos.trTime = level.time;
	top->s.apos.trType = TR_LINEAR_STOP;
	top->s.apos.trDuration = 50;
...

{
	...
	if ( top != ((void *)0) )
	{
		...
		((top->s.apos.trBase)[0]=(top->r.currentAngles)[0],(top->s.apos.trBase)[1]=(top->r.currentAngles)[1],(top->s.apos.trBase)[2]=(top->r.currentAngles)[2]);
		((top->s.apos.trDelta)[0]=(top->s.apos.trDelta)[1]=(top->s.apos.trDelta)[2]=0);
		top->s.apos.trTime = level.time;
...

{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( !tr.allsolid && !tr.startsolid && ( tr.fraction == 1.0 || tr.entityNum == target->s.number ))
		{
			...
			((enemyDir)[0]=(target->r.currentOrigin)[0]-(top->r.currentOrigin)[0],(enemyDir)[1]=(target->r.currentOrigin)[1]-(top->r.currentOrigin)[1],(enemyDir)[2]=(target->r.currentOrigin)[2]-(top->r.currentOrigin)[2]);
			enemyDist = VectorLengthSquared( enemyDir );

			if ( enemyDist < bestDist 
				|| (!Q_stricmp( "atst_vehicle", target->NPC_type ) && bestTarget && Q_stricmp( "atst_vehicle", bestTarget->NPC_type ) ) )
			{
...

{
	...
	base->r.contents = 0x00000100;

	((base->r.maxs)[0]=(32.0f), (base->r.maxs)[1]=(32.0f), (base->r.maxs)[2]=(128.0f));
	((base->r.mins)[0]=(-32.0f), (base->r.mins)[1]=(-32.0f), (base->r.mins)[2]=(0.0f));
...
qboolean turret_base_spawn_top( gentity_t *base )
{
	...
	((org)[0]=(base->s.origin)[0],(org)[1]=(base->s.origin)[1],(org)[2]=(base->s.origin)[2]);
	org[2] += 128;
	G_SetOrigin( top, org );
	...
	top->count = ((rand () & 0x7fff) / ((float)0x7fff)) * 9000;
	...
	if ( !base->wait )
	{
		base->wait = 300 + ((rand () & 0x7fff) / ((float)0x7fff)) * 55;
	}
	top->wait = base->wait;
	...
	((top->r.maxs)[0]=(48.0f), (top->r.maxs)[1]=(48.0f), (top->r.maxs)[2]=(16.0f));
	((top->r.mins)[0]=(-48.0f), (top->r.mins)[1]=(-48.0f), (top->r.mins)[2]=(0.0f));
