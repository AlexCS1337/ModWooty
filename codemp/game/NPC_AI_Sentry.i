...
void sentry_use( gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	NPC_SetAnim( self, 1|2, BOTH_POWERUP1, 1|2 );
...
{		
	...
	if ( mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT )
	{
		...
		self->flags |= 0x00080000;
		NPC_SetAnim( self, 1|2, BOTH_FLY_SHIELDED, 1|2 );
...
void Sentry_Fire (void)
{
	...
	else if ( NPCInfo->localState == LSTATE_ACTIVE )
	{
		...
		NPC_SetAnim( NPC, 1|2, BOTH_POWERUP1, 1|2 );
...
void Sentry_Idle( void )
{
	...
	{
		NPC_SetAnim( NPC, 1|2, BOTH_SLEEP1, 1|2 );
		NPC->flags |= 0x00080000;
...
{
	if ( TIMER_Done( NPC, "attackDelay" ) && NPC->attackDebounceTime < level.time && visible )	
	{
		if ( NPCInfo->burstCount > 6 )
		{
			...
			else if ( NPC->fly_sound_debounce_time < level.time )
			{
				...
				NPC->flags |= 0x00080000;
				NPC_SetAnim( NPC, 1|2, BOTH_FLY_SHIELDED, 1|2 );
...
void Sentry_AttackDecision( void )
{
	...
	advance		= (qboolean)(distance > ( 256 * 256 ));
