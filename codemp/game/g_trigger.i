#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_trigger.c"
...
#line 4 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_trigger.c"
...
#line 5 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_trigger.c"
...
void InitTrigger( gentity_t *self ) {
	...
	self->r.contents = 0x00000400;		
	self->r.svFlags = 0x00000001;

	if(self->spawnflags & 128)
	{
		self->flags |= 0x00010000;
	}
}
...
void multi_trigger_run( gentity_t *ent ) 
{
	...
	{
		trap_SetConfigstring( 32, ent->soundSet );
	}
	...
	else if ( ent->wait > 0 ) 
	{
		if ( ent->painDebounceTime != level.time )
		{
			...
			ent->nextthink = level.time + ( ent->wait + ent->random * (2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) ) * 1000;
...
void multi_trigger( gentity_t *ent, gentity_t *activator ) 
{
	...
	{ 
		...
		if (!activator->client ||
			(activator->client->sess.sessionTeam != 1 && activator->client->sess.sessionTeam != 2))
		{ 
			return;
		}

		
		numEnts = trap_EntitiesInBox( ent->r.absmin, ent->r.absmax, entityList, (1<<10) );
		while (i < numEnts)
		{
			if (entityList[i] < 32)
			{ 
				...
				if (cl->inuse && cl->client &&
					(cl->client->sess.sessionTeam == 1 || cl->client->sess.sessionTeam == 2) &&
					cl->health > 0 &&
					!(cl->client->ps.eFlags & (1<<1)))
				{
					
					if (cl->client->sess.sessionTeam == 1)
					{
						team1ClNum++;
					}
					else
...
void Touch_Multi( gentity_t *self, gentity_t *other, trace_t *trace ) 
{
	...
	if ( self->spawnflags & 2 )
	{
		...
		AngleVectors( other->client->ps.viewangles, forward, ((void *)0), ((void *)0) );

		if ( ((self->movedir)[0]*(forward)[0]+(self->movedir)[1]*(forward)[1]+(self->movedir)[2]*(forward)[2]) < 0.5 )
		{
			return;
		}
	}
	...
	{
		if( !( other->client->pers.cmd.buttons & 32 ) )
		{
			return;
		}
		...
		if ((other->client->ps.weaponTime > 0 && other->client->ps.torsoAnim != BOTH_BUTTON_HOLD && other->client->ps.torsoAnim != BOTH_CONSOLE1) || other->health < 1 ||
			(other->client->ps.pm_flags & 4096) || other->client->sess.sessionTeam == TEAM_SPECTATOR ||
		...
		{ 
			...
			else if (other->client->isHacking != self->s.number && other->s.number < 32 )
			{ 
				other->client->isHacking = self->s.number;
				((other->client->hackingAngles)[0]=(other->client->ps.viewangles)[0],(other->client->hackingAngles)[1]=(other->client->ps.viewangles)[1],(other->client->hackingAngles)[2]=(other->client->ps.viewangles)[2]);
	...
	{
		if( !( other->client->pers.cmd.buttons & 1 ) &&
			!( other->client->pers.cmd.buttons & 128 ) )
		{
			return;
		}
	}
	...
	if ( self->radius )
	{
		...
		((eyeSpot)[0]=(other->client->ps.origin)[0],(eyeSpot)[1]=(other->client->ps.origin)[1],(eyeSpot)[2]=(other->client->ps.origin)[2]);
	...
	if ( self->spawnflags & 4 )
	{
		...
		{
			G_SetAnim( other, ((void *)0), 1, BOTH_BUTTON_HOLD, 1|2, 0 );
		}
		else
		{
...
void trigger_cleared_fire (gentity_t *self)
{
	...
	if ( self->wait > 0 ) 
	{
		self->nextthink = level.time + ( self->wait + self->random * (2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) ) * 1000;
	}
}
...
void SP_trigger_multiple( gentity_t *ent ) 
{
	...
	if ( (ent->wait > 0) && (ent->random >= ent->wait) ) {
		ent->random = ent->wait - 50;
		Com_Printf("^3""trigger_multiple has random >= wait\n");
	}
...
void Do_Strike(gentity_t *ent)
{
	...
	trap_Trace(&localTrace, strikeFrom, ((void *)0), ((void *)0), strikePoint, ent->s.number, (0x00000001|0x00000010|0x00000100|0x00001000));
	((strikePoint)[0]=(localTrace.endpos)[0],(strikePoint)[1]=(localTrace.endpos)[1],(strikePoint)[2]=(localTrace.endpos)[2]);
	...
	if (ent->radius)
	{ 
		G_RadiusDamage(strikePoint, ent, ent->damage, ent->radius, ent, ((void *)0), MOD_SUICIDE);
	}
	else
	{ 
		...
		{ 
			G_Damage(trHit, ent, ent, ((void *)0), trHit->r.currentOrigin, ent->damage, 0, MOD_SUICIDE);
		}
	}
...
void trigger_push_touch (gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( !other->client ) {
		...
		{
			((other->s.pos.trBase)[0]=(other->r.currentOrigin)[0],(other->s.pos.trBase)[1]=(other->r.currentOrigin)[1],(other->s.pos.trBase)[2]=(other->r.currentOrigin)[2]);
			((other->s.pos.trDelta)[0]=(self->s.origin2)[0],(other->s.pos.trDelta)[1]=(self->s.origin2)[1],(other->s.pos.trDelta)[2]=(self->s.origin2)[2]);
	...
	{
		vec3_t dir;
		((dir)[0]=(self->s.origin2)[0]-(other->r.currentOrigin)[0],(dir)[1]=(self->s.origin2)[1]-(other->r.currentOrigin)[1],(dir)[2]=(self->s.origin2)[2]-(other->r.currentOrigin)[2]);
		if ( self->speed )
		{
			VectorNormalize( dir );
			((dir)[0]=(dir)[0]*(self->speed),(dir)[1]=(dir)[1]*(self->speed),(dir)[2]=(dir)[2]*(self->speed));
		}
		((other->client->ps.velocity)[0]=(dir)[0],(other->client->ps.velocity)[1]=(dir)[1],(other->client->ps.velocity)[2]=(dir)[2]);
	}
	else if ( (self->spawnflags&4) )
	{
		((other->client->ps.velocity)[0]=(self->s.origin2)[0]*(self->speed),(other->client->ps.velocity)[1]=(self->s.origin2)[1]*(self->speed),(other->client->ps.velocity)[2]=(self->s.origin2)[2]*(self->speed));
	}
	else
	{
		((other->client->ps.velocity)[0]=(self->s.origin2)[0],(other->client->ps.velocity)[1]=(self->s.origin2)[1],(other->client->ps.velocity)[2]=(self->s.origin2)[2]);
	}
...
void AimAtTarget( gentity_t *self ) {
	...
	((origin)[0]=(self->r.absmin)[0]+(self->r.absmax)[0],(origin)[1]=(self->r.absmin)[1]+(self->r.absmax)[1],(origin)[2]=(self->r.absmin)[2]+(self->r.absmax)[2]);
	((origin)[0]=(origin)[0]*(0.5f),(origin)[1]=(origin)[1]*(0.5f),(origin)[2]=(origin)[2]*(0.5f));
	...
	if ( self->classname && !Q_stricmp( "trigger_push", self->classname ) )
	{
		if ( (self->spawnflags&16) )
		{
			((self->s.origin2)[0]=(ent->r.currentOrigin)[0],(self->s.origin2)[1]=(ent->r.currentOrigin)[1],(self->s.origin2)[2]=(ent->r.currentOrigin)[2]);
			return;
		}
		else if ( (self->spawnflags&4) )
		{
			((self->s.origin2)[0]=(ent->r.currentOrigin)[0]-(origin)[0],(self->s.origin2)[1]=(ent->r.currentOrigin)[1]-(origin)[1],(self->s.origin2)[2]=(ent->r.currentOrigin)[2]-(origin)[2]);
	...
	if ( self->classname && !Q_stricmp( "target_push", self->classname ) )
	{
		if( self->spawnflags & 2 )
		{
			((self->s.origin2)[0]=(ent->s.origin)[0]-(self->s.origin)[0],(self->s.origin2)[1]=(ent->s.origin)[1]-(self->s.origin)[1],(self->s.origin2)[2]=(ent->s.origin)[2]-(self->s.origin)[2]);
			VectorNormalize( self->s.origin2);
			((self->s.origin2)[0]=(self->s.origin2)[0]*(self->speed),(self->s.origin2)[1]=(self->s.origin2)[1]*(self->speed),(self->s.origin2)[2]=(self->s.origin2)[2]*(self->speed));
			return;
		}
	}
	...
	((self->s.origin2)[0]=(ent->s.origin)[0]-(origin)[0],(self->s.origin2)[1]=(ent->s.origin)[1]-(origin)[1],(self->s.origin2)[2]=(ent->s.origin)[2]-(origin)[2]);
	self->s.origin2[2] = 0;
	dist = VectorNormalize( self->s.origin2);

	forward = dist / time;
	((self->s.origin2)[0]=(self->s.origin2)[0]*(forward),(self->s.origin2)[1]=(self->s.origin2)[1]*(forward),(self->s.origin2)[2]=(self->s.origin2)[2]*(forward));
...
void Use_target_push( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	((activator->client->ps.velocity)[0]=(self->s.origin2)[0],(activator->client->ps.velocity)[1]=(self->s.origin2)[1],(activator->client->ps.velocity)[2]=(self->s.origin2)[2]);
...
void SP_target_push( gentity_t *self ) {
	...
	if ( self->target ) {
		((self->r.absmin)[0]=(self->s.origin)[0],(self->r.absmin)[1]=(self->s.origin)[1],(self->r.absmin)[2]=(self->s.origin)[2]);
		((self->r.absmax)[0]=(self->s.origin)[0],(self->r.absmax)[1]=(self->s.origin)[1],(self->r.absmax)[2]=(self->s.origin)[2]);
		self->think = AimAtTarget;
		self->nextthink = level.time + 50;
	}
...
void hurt_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if (g_gametype.integer == GT_SIEGE && self->team && self->team[0])
	{
		...
		if (other->inuse && other->s.number < 32 && other->client &&
	...
	if (self->damage == -1 && other && other->client)
	{
		...
		{ 
			...
			((vDir)[0]=(0), (vDir)[1]=(1), (vDir)[2]=(0));
			G_Damage(other, other, other, vDir, other->client->ps.origin, 16777216, 0, MOD_FALLING);
		}
		else
		{
	...
	else	
	{
		...
		{
			G_Damage (other, self->activator, self->activator, ((void *)0), ((void *)0), dmg, dflags|0x00000008, MOD_TRIGGER_HURT);
		}
		else
		{
			G_Damage (other, self, self, ((void *)0), ((void *)0), dmg, dflags|0x00000008, MOD_TRIGGER_HURT);
		}
	}
}
...
void space_touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
	...
	if ( other->s.number < 32
		&& other->client->ps.m_iVehicleNum
		&& other->client->ps.m_iVehicleNum >= 32 )
	{
		...
		{ 
			...
			other->client->inSpaceIndex = ((1<<10)-1);
			return;
		}
	}
	...
	if (!other->client->inSpaceIndex ||
		other->client->inSpaceIndex == ((1<<10)-1))
	{ 
		other->client->inSpaceSuffocation = level.time + 500;
	}
...
void shipboundary_touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
	...
	if ( other->client->ps.hyperSpaceTime && level.time - other->client->ps.hyperSpaceTime < 4000 )
	{
		return;
	}

	ent = G_Find (((void *)0), ((int)&(((gentity_t *)0)->targetname)), self->target);
	if (!ent || !ent->inuse)
	{ 
	...
	{ 
		G_Damage(other, other, other, ((void *)0), other->client->ps.origin, 99999, 0x00000008, MOD_SUICIDE);
		return;
	}
...
void shipboundary_think(gentity_t *ent)
{
	...
	numListedEntities = trap_EntitiesInBox( ent->r.absmin, ent->r.absmax, iEntityList, (1<<10) );
	while (i < numListedEntities)
	{
		...
		if (listedEnt->inuse && listedEnt->client && listedEnt->client->ps.m_iVehicleNum)
		{
				...
				listedEnt->s.NPC_class == CLASS_VEHICLE)
			{
				...
				{
                    shipboundary_touch(ent, listedEnt, ((void *)0));
				}
			}
		}
		i++;
...
void hyperspace_touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
	...
	if ( other->client->ps.hyperSpaceTime && level.time - other->client->ps.hyperSpaceTime < 4000 )
	{
		if ( (other->client->ps.eFlags2&(1<<5)) )
		{
			float timeFrac = ((float)(level.time-other->client->ps.hyperSpaceTime))/4000;
			if ( timeFrac >= 0.75f )
			{
				vec3_t	diff, fwd, right, up, newOrg;
				float	fDiff, rDiff, uDiff;
				
				other->client->ps.eFlags2 &= ~(1<<5);
				...
				((diff)[0]=(other->client->ps.origin)[0]-(ent->s.origin)[0],(diff)[1]=(other->client->ps.origin)[1]-(ent->s.origin)[1],(diff)[2]=(other->client->ps.origin)[2]-(ent->s.origin)[2]);
				AngleVectors( ent->s.angles, fwd, right, up );
				fDiff = ((fwd)[0]*(diff)[0]+(fwd)[1]*(diff)[1]+(fwd)[2]*(diff)[2]);
				rDiff = ((right)[0]*(diff)[0]+(right)[1]*(diff)[1]+(right)[2]*(diff)[2]);
				uDiff = ((up)[0]*(diff)[0]+(up)[1]*(diff)[1]+(up)[2]*(diff)[2]);
				
				ent = G_Find (((void *)0), ((int)&(((gentity_t *)0)->targetname)), self->target2);
				if (!ent || !ent->inuse)
				{ 
					G_Error("trigger_hyperspace has invalid target2 '%s'\n", self->target2);
					return;
				}
				((newOrg)[0]=(ent->s.origin)[0],(newOrg)[1]=(ent->s.origin)[1],(newOrg)[2]=(ent->s.origin)[2]);
				
				AngleVectors( ent->s.angles, fwd, right, up );
				((newOrg)[0]=(newOrg)[0]+(fwd)[0]*(fDiff),(newOrg)[1]=(newOrg)[1]+(fwd)[1]*(fDiff),(newOrg)[2]=(newOrg)[2]+(fwd)[2]*(fDiff));
				((newOrg)[0]=(newOrg)[0]+(right)[0]*(rDiff),(newOrg)[1]=(newOrg)[1]+(right)[1]*(rDiff),(newOrg)[2]=(newOrg)[2]+(right)[2]*(rDiff));
				((newOrg)[0]=(newOrg)[0]+(up)[0]*(uDiff),(newOrg)[1]=(newOrg)[1]+(up)[1]*(uDiff),(newOrg)[2]=(newOrg)[2]+(up)[2]*(uDiff));
				...
				((other->client->ps.hyperSpaceAngles)[0]=(ent->s.angles)[0],(other->client->ps.hyperSpaceAngles)[1]=(ent->s.angles)[1],(other->client->ps.hyperSpaceAngles)[2]=(ent->s.angles)[2]);
...
void SP_trigger_hyperspace(gentity_t *self)
{
	...
	self->delay = Distance( self->r.absmax, self->r.absmin );
...
void func_timer_think( gentity_t *self ) {
	...
	self->nextthink = level.time + 1000 * ( self->wait + (2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) * self->random );
}
...
gentity_t *asteroid_pick_random_asteroid( gentity_t *self )
{
	...
	while ( (t = G_Find (t, ((int)&(((gentity_t *)0)->targetname)), self->target)) != ((void *)0) )
	{
		if (t != self)
		{
			t_count++;
