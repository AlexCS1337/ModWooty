#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
...
#line 2 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
...
#line 3 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
...
void NPC_Blocked( gentity_t *self, gentity_t *blocker )
{
	...
	self->NPC->blockedSpeechDebounceTime = level.time + 4000 + ( ((rand () & 0x7fff) / ((float)0x7fff)) * 4000 );
...
void NPC_SetMoveGoal( gentity_t *ent, vec3_t point, int radius, qboolean isNavGoal, int combatPoint, gentity_t *targetEnt )
{
	...
	((ent->NPC->tempGoal->r.currentOrigin)[0]=(point)[0],(ent->NPC->tempGoal->r.currentOrigin)[1]=(point)[1],(ent->NPC->tempGoal->r.currentOrigin)[2]=(point)[2]);
	
	
	((ent->NPC->tempGoal->r.mins)[0]=(ent->r.mins)[0],(ent->NPC->tempGoal->r.mins)[1]=(ent->r.mins)[1],(ent->NPC->tempGoal->r.mins)[2]=(ent->r.mins)[2]);
	((ent->NPC->tempGoal->r.maxs)[0]=(ent->r.mins)[0],(ent->NPC->tempGoal->r.maxs)[1]=(ent->r.mins)[1],(ent->NPC->tempGoal->r.maxs)[2]=(ent->r.mins)[2]);

	ent->NPC->tempGoal->target = ((void *)0);
	ent->NPC->tempGoal->clipmask = ent->clipmask;
	ent->NPC->tempGoal->flags &= ~0x00020000;
	...
	{
		ent->NPC->tempGoal->waypoint = -1;
	}
	ent->NPC->tempGoal->noWaypointTime = 0;

	if ( isNavGoal )
	{
		(void)( (!!(ent->NPC->tempGoal->parent)) || (_wassert(L"ent->NPC->tempGoal->parent", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 148), 0) );
		ent->NPC->tempGoal->flags |= 0x00020000;
	}
...
qboolean NAV_HitNavGoal( vec3_t point, vec3_t mins, vec3_t maxs, vec3_t dest, int radius, qboolean flying )
{
	...
	if ( radius & 16384 )
	{
		...
		{
			vec3_t	diff;
			((diff)[0]=(point)[0]-(dest)[0],(diff)[1]=(point)[1]-(dest)[1],(diff)[2]=(point)[2]-(dest)[2]);
			if ( fabs(diff[2]) <= 24 )
			{
				diff[2] = 0;
			}
	...
	else
	{
		...
		((dmins)[0]=(-radius), (dmins)[1]=(-radius), (dmins)[2]=(-radius));
		((dmaxs)[0]=(radius), (dmaxs)[1]=(radius), (dmaxs)[2]=(radius));

		
		((dmins)[0]=(dmins)[0]+(dest)[0],(dmins)[1]=(dmins)[1]+(dest)[1],(dmins)[2]=(dmins)[2]+(dest)[2]);
		((dmaxs)[0]=(dmaxs)[0]+(dest)[0],(dmaxs)[1]=(dmaxs)[1]+(dest)[1],(dmaxs)[2]=(dmaxs)[2]+(dest)[2]);

		
		((pmins)[0]=(point)[0]+(mins)[0],(pmins)[1]=(point)[1]+(mins)[1],(pmins)[2]=(point)[2]+(mins)[2]);
		((pmaxs)[0]=(point)[0]+(maxs)[0],(pmaxs)[1]=(point)[1]+(maxs)[1],(pmaxs)[2]=(point)[2]+(maxs)[2]);
...
qboolean NAV_ClearPathToPoint( gentity_t *self, vec3_t pmins, vec3_t pmaxs, vec3_t point, int clipmask, int okToHitEntNum )
{
	...
	if ( self->flags & 0x00020000 )
	{
		if ( !self->parent )
		{
			...
			(void)( (!!(self->parent)) || (_wassert(L"self->parent", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 239), 0) );
			return qfalse;
		}
		((mins)[0]=(self->parent->r.mins)[0],(mins)[1]=(self->parent->r.mins)[1],(mins)[2]=(self->parent->r.mins)[2]);
		((maxs)[0]=(self->parent->r.maxs)[0],(maxs)[1]=(self->parent->r.maxs)[1],(maxs)[2]=(self->parent->r.maxs)[2]);
	}
	else
	{
		((mins)[0]=(pmins)[0],(mins)[1]=(pmins)[1],(mins)[2]=(pmins)[2]);
		((maxs)[0]=(pmaxs)[0],(maxs)[1]=(pmaxs)[1],(maxs)[2]=(pmaxs)[2]);
	}
	
	if ( self->client || ( self->flags & 0x00020000 ) )
	{
	...
	if ( self->flags & 0x00020000 )
	{
		...
		trap_Trace( &trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask|0x00000020|0x00000040)&~0x00000100 );
		if ( trace.startsolid&&(trace.contents&0x00000040) )
		{
			clipmask &= ~0x00000040;
			trap_Trace( &trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask|0x00000020)&~0x00000100 );
		}
		...
		if ( okToHitEntNum != ((1<<10)-1) && trace.entityNum == okToHitEntNum )
		{
			return qtrue;
		}
		...
		else
		{
			...
			{
				if ( trace.entityNum < ((1<<10)-2) && (&g_entities[trace.entityNum] != ((void *)0)) && g_entities[trace.entityNum].s.eType != ET_MOVER )
				{
					vec3_t	p1, p2;
					G_DrawEdge( point, trace.endpos, EDGE_PATH );
					((p1)[0]=(g_entities[trace.entityNum].r.mins)[0]+(g_entities[trace.entityNum].r.currentOrigin)[0],(p1)[1]=(g_entities[trace.entityNum].r.mins)[1]+(g_entities[trace.entityNum].r.currentOrigin)[1],(p1)[2]=(g_entities[trace.entityNum].r.mins)[2]+(g_entities[trace.entityNum].r.currentOrigin)[2]);
					((p2)[0]=(g_entities[trace.entityNum].r.maxs)[0]+(g_entities[trace.entityNum].r.currentOrigin)[0],(p2)[1]=(g_entities[trace.entityNum].r.maxs)[1]+(g_entities[trace.entityNum].r.currentOrigin)[1],(p2)[2]=(g_entities[trace.entityNum].r.maxs)[2]+(g_entities[trace.entityNum].r.currentOrigin)[2]);
					G_CubeOutline( p1, p2, 50, 0x0000ff, 0.5 );
				}
	...
	{
		trap_Trace( &trace, self->r.currentOrigin, mins, maxs, point, self->s.number, clipmask|0x00000020|0x00000040);
		if ( trace.startsolid&&(trace.contents&0x00000040) )
		{
			clipmask &= ~0x00000040;
			trap_Trace( &trace, self->r.currentOrigin, mins, maxs, point, self->s.number, clipmask|0x00000020);
		}
...
int NAV_FindClosestWaypointForEnt( gentity_t *ent, int targWp )
{
	...
	return trap_Nav_GetNearestNode( ent, ent->waypoint, 0x00000002, targWp );
}
...
int NAV_FindClosestWaypointForPoint( gentity_t *ent, vec3_t point )
{
	...
	((marker->r.mins)[0]=(ent->r.mins)[0],(marker->r.mins)[1]=(ent->r.mins)[1],(marker->r.mins)[2]=(ent->r.mins)[2]);
	((marker->r.maxs)[0]=(ent->r.mins)[0],(marker->r.maxs)[1]=(ent->r.mins)[1],(marker->r.maxs)[2]=(ent->r.mins)[2]);

	marker->clipmask = ent->clipmask;
	marker->waypoint = -1;

	bestWP = trap_Nav_GetNearestNode( marker, marker->waypoint, 0x00000002, -1 );
...
int NAV_FindClosestWaypointForPoint2( vec3_t point )
{
	...
	((marker->r.mins)[0]=(-16), (marker->r.mins)[1]=(-16), (marker->r.mins)[2]=(-6));
	((marker->r.maxs)[0]=(16), (marker->r.maxs)[1]=(16), (marker->r.maxs)[2]=(32));

	marker->clipmask = (0x00000001|0x00000020|0x00000100|0x00001000);
	marker->waypoint = -1;
...
{
	self->NPC->aiFlags &= ~0x00000040;
	self->NPC->blockingEntNum = ((1<<10)-2);
}
...
int NAV_Steer( gentity_t *self, vec3_t dir, float distance )
{
	...
	float	right_push;
	float	left_push;
	float	right_ang	= dir[1] + 45;
	float	left_ang	= dir[1] - 45;

	
	((deviation)[0]=(dir)[0],(deviation)[1]=(dir)[1],(deviation)[2]=(dir)[2]);
	deviation[1] = right_ang;

	AngleVectors( deviation, right_test, ((void *)0), ((void *)0) );

	deviation[1] = left_ang;

	AngleVectors( deviation, left_test, ((void *)0), ((void *)0) );

	
	((right_test)[0]=(self->r.currentOrigin)[0]+(right_test)[0]*(distance),(right_test)[1]=(self->r.currentOrigin)[1]+(right_test)[1]*(distance),(right_test)[2]=(self->r.currentOrigin)[2]+(right_test)[2]*(distance));
	((left_test)[0]=(self->r.currentOrigin)[0]+(left_test)[0]*(distance),(left_test)[1]=(self->r.currentOrigin)[1]+(left_test)[1]*(distance),(left_test)[2]=(self->r.currentOrigin)[2]+(left_test)[2]*(distance));
	...
	NAV_CheckAhead( self, right_test, &tr, self->clipmask|0x00000040 );

	right_push = -45 * ( 1.0f - tr.fraction );

	
	NAV_CheckAhead( self, left_test, &tr, self->clipmask|0x00000040 );

	left_push = 45 * ( 1.0f - tr.fraction );

	
	((deviation)[0]=(dir)[0],(deviation)[1]=(dir)[1],(deviation)[2]=(dir)[2]);
	deviation[1] += ( left_push + right_push );
...
qboolean NAV_CheckAhead( gentity_t *self, vec3_t end, trace_t *trace, int clipmask )
{
	...
	((mins)[0]=(self->r.mins[0]), (mins)[1]=(self->r.mins[1]), (mins)[2]=(self->r.mins[2] + 18));
	...
	if ( trace->entityNum < ((1<<10)-2) )
	{
		...
		if ( ( blocker->classname != 0 ) && ( blocker->classname[0] != 0 ) )
		{
			if ( G_EntIsUnlockedDoor( blocker->s.number ) )
			
			{
				
				if ( DistanceSquared( self->r.currentOrigin, trace->endpos ) < ( 16 * 16 ) )
					return qfalse;
...
static qboolean NAV_TestBypass( gentity_t *self, float yaw, float blocked_dist, vec3_t movedir )
{
	...
	((avoidAngles)[0]=(avoidAngles)[1]=(avoidAngles)[2]=0);
	avoidAngles[1] = yaw;

	AngleVectors( avoidAngles, block_test, ((void *)0), ((void *)0) );
	((block_pos)[0]=(self->r.currentOrigin)[0]+(block_test)[0]*(blocked_dist),(block_pos)[1]=(self->r.currentOrigin)[1]+(block_test)[1]*(blocked_dist),(block_pos)[2]=(self->r.currentOrigin)[2]+(block_test)[2]*(blocked_dist));
	...
	if ( NAV_CheckAhead( self, block_pos, &tr, ( self->clipmask & ~0x00000100 )|0x00000040 ) )
	{
		((movedir)[0]=(block_test)[0],(movedir)[1]=(block_test)[1],(movedir)[2]=(block_test)[2]);
...
qboolean NAV_Bypass( gentity_t *self, gentity_t *blocker, vec3_t blocked_dir, float blocked_dist, vec3_t movedir ) 
{
	...
	AngleVectors( self->r.currentAngles, ((void *)0), right, ((void *)0) );
	...
	if ( blocker->client && !VectorCompare( blocker->client->ps.velocity, vec3_origin ) )
	{
		...
		dot = ((blocker_movedir)[0]*(blocked_dir)[0]+(blocker_movedir)[1]*(blocked_dir)[1]+(blocker_movedir)[2]*(blocked_dir)[2]);
		if ( dot < 0.35f && dot > -0.35f )
		{
			vec3_t	block_pos;
			trace_t	tr;
			((blocker_movedir)[0]=(blocker_movedir)[0]*(-1),(blocker_movedir)[1]=(blocker_movedir)[1]*(-1),(blocker_movedir)[2]=(blocker_movedir)[2]*(-1));
			((block_pos)[0]=(self->r.currentOrigin)[0]+(blocker_movedir)[0]*(blocked_dist),(block_pos)[1]=(self->r.currentOrigin)[1]+(blocker_movedir)[1]*(blocked_dist),(block_pos)[2]=(self->r.currentOrigin)[2]+(blocker_movedir)[2]*(blocked_dist));
			if ( NAV_CheckAhead( self, block_pos, &tr, ( self->clipmask & ~0x00000100 )|0x00000040 ) )
			{
				((movedir)[0]=(blocker_movedir)[0],(movedir)[1]=(blocker_movedir)[1],(movedir)[2]=(blocker_movedir)[2]);
				return qtrue;
			}
		}
	}
	...
	dot = ((blocked_dir)[0]*(right)[0]+(blocked_dir)[1]*(right)[1]+(blocked_dir)[2]*(right)[2]);
...
qboolean NAV_MoveBlocker( gentity_t *self, vec3_t shove_dir )
{
	...
	temp_dir[1] += 45;
	AngleVectors( temp_dir, forward, ((void *)0), ((void *)0) );

	((self->client->ps.velocity)[0]=(forward)[0]*(200),(self->client->ps.velocity)[1]=(forward)[1]*(200),(self->client->ps.velocity)[2]=(forward)[2]*(200));
...
qboolean NAV_TrueCollision( gentity_t *self, gentity_t *blocker, vec3_t movedir, vec3_t blocked_dir )
{
	...
	dot = ((movedir)[0]*(velocityDir)[0]+(movedir)[1]*(velocityDir)[1]+(movedir)[2]*(velocityDir)[2]);

	if ( dot < 0.85 )
		return qfalse;

	((testPos)[0]=(self->r.currentOrigin)[0]+(velocityDir)[0]*(speed*50),(testPos)[1]=(self->r.currentOrigin)[1]+(velocityDir)[1]*(speed*50),(testPos)[2]=(self->r.currentOrigin)[2]+(velocityDir)[2]*(speed*50));

	((tmins)[0]=(blocker->r.currentOrigin)[0]+(blocker->r.mins)[0],(tmins)[1]=(blocker->r.currentOrigin)[1]+(blocker->r.mins)[1],(tmins)[2]=(blocker->r.currentOrigin)[2]+(blocker->r.mins)[2]);
	((tmaxs)[0]=(blocker->r.currentOrigin)[0]+(blocker->r.maxs)[0],(tmaxs)[1]=(blocker->r.currentOrigin)[1]+(blocker->r.maxs)[1],(tmaxs)[2]=(blocker->r.currentOrigin)[2]+(blocker->r.maxs)[2]);

	((ptmins)[0]=(testPos)[0]+(self->r.mins)[0],(ptmins)[1]=(testPos)[1]+(self->r.mins)[1],(ptmins)[2]=(testPos)[2]+(self->r.mins)[2]);
	((ptmaxs)[0]=(testPos)[0]+(self->r.maxs)[0],(ptmaxs)[1]=(testPos)[1]+(self->r.maxs)[1],(ptmaxs)[2]=(testPos)[2]+(self->r.maxs)[2]);

	if ( G_BoundsOverlap( ptmins, ptmaxs, tmins, tmaxs ) )
	{
		((blocked_dir)[0]=(velocityDir)[0],(blocked_dir)[1]=(velocityDir)[1],(blocked_dir)[2]=(velocityDir)[2]);
		return qtrue;
	}
...
qboolean NAV_StackedCanyon( gentity_t *self, gentity_t *blocker, vec3_t pathDir )
{
	...
	((test)[0]=(blocker->r.currentOrigin)[0]+(cross)[0]*(avoidRadius),(test)[1]=(blocker->r.currentOrigin)[1]+(cross)[1]*(avoidRadius),(test)[2]=(blocker->r.currentOrigin)[2]+(cross)[2]*(avoidRadius));

	trap_Trace( &tr, test, self->r.mins, self->r.maxs, test, self->s.number, self->clipmask|extraClip );
	if ( tr.startsolid&&(tr.contents&0x00000040) )
	{
		extraClip &= ~0x00000040;
		trap_Trace( &tr, test, self->r.mins, self->r.maxs, test, self->s.number, self->clipmask|extraClip );
	}
	...
	if ( NAVDEBUG_showCollision )
	{
		...
		((mins)[0]=(test)[0]+(self->r.mins)[0],(mins)[1]=(test)[1]+(self->r.mins)[1],(mins)[2]=(test)[2]+(self->r.mins)[2]);
		((maxs)[0]=(test)[0]+(self->r.maxs)[0],(maxs)[1]=(test)[1]+(self->r.maxs)[1],(maxs)[2]=(test)[2]+(self->r.maxs)[2]);
...
qboolean NAV_ResolveEntityCollision( gentity_t *self, gentity_t *blocker, vec3_t movedir, vec3_t pathDir )
{
	...
	{
		if ( DistanceSquared( self->r.currentOrigin, blocker->r.currentOrigin ) > ( 16 * 16 ) )
			return qtrue;
	}

	((blocked_dir)[0]=(blocker->r.currentOrigin)[0]-(self->r.currentOrigin)[0],(blocked_dir)[1]=(blocker->r.currentOrigin)[1]-(self->r.currentOrigin)[1],(blocked_dir)[2]=(blocker->r.currentOrigin)[2]-(self->r.currentOrigin)[2]);
...
qboolean NAV_AvoidCollision( gentity_t *self, gentity_t *goal, navInfo_t *info )
{
	...
	if ( info->distance > 128 )
	{
		info->distance = 128;
	}

	
	((movepos)[0]=(self->r.currentOrigin)[0]+(info->direction)[0]*(info->distance),(movepos)[1]=(self->r.currentOrigin)[1]+(info->direction)[1]*(info->distance),(movepos)[2]=(self->r.currentOrigin)[2]+(info->direction)[2]*(info->distance));
	((movedir)[0]=(info->direction)[0],(movedir)[1]=(info->direction)[1],(movedir)[2]=(info->direction)[2]);

	if ( self && self->NPC && (self->NPC->aiFlags&0x00000020) )
	{
		return qtrue;
	}
	
	if ( NAV_CheckAhead( self, movepos, &info->trace, 0x00000100 ) == qfalse )
	{
		...
		((info->direction)[0]=(movedir)[0],(info->direction)[1]=(movedir)[1],(info->direction)[2]=(movedir)[2]);
...
int NAV_TestBestNode( gentity_t *self, int startID, int endID, qboolean failEdge )
{
	...
	trace_t	trace;
	vec3_t	mins;
	int		clipmask = (NPC->clipmask&~0x00000100)|0x00000040;
	...
	if ( trace.entityNum < ((1<<10)-2) )
	{
		...
		if ( ( blocker->classname != 0 ) && ( blocker->classname[0] != 0 ) )
		{
			...
			else if ( blocker->targetname && blocker->s.solid == 0xffffff && ((blocker->r.contents&0x00000020)|| (blocker->r.contents&0x00000040)) )
			{
...
int NAV_GetNearestNode( gentity_t *self, int lastNode )
{
	return trap_Nav_GetNearestNode( self, lastNode, 0x00000002, -1 );
}
...
int	NAV_MoveToGoal( gentity_t *self, navInfo_t *info )
{
	...
	if ( self->NPC->goalEntity->s.number == 0 )
	{
		...
		if ( self->NPC->goalEntity->waypoint == -1 )
			return -1;
	...
	else
	{
		...
		if ( ( self->NPC->goalEntity->waypoint = NAV_GetNearestNode( self->NPC->goalEntity, self->NPC->goalEntity->waypoint ) ) == -1 )
			return -1;
	}

	
	if ( ( self->waypoint = NAV_GetNearestNode( self, self->lastWaypoint ) ) == -1 )
		return -1;

	bestNode = trap_Nav_GetBestNode( self->waypoint, self->NPC->goalEntity->waypoint, -1 );
	...
	if ( NAV_CheckAhead( self, origin, &info->trace, (self->clipmask&~0x00000100)|0x00000040 ) == qfalse )
	{
	...
	((info->direction)[0]=(origin)[0]-(self->r.currentOrigin)[0],(info->direction)[1]=(origin)[1]-(self->r.currentOrigin)[1],(info->direction)[2]=(origin)[2]-(self->r.currentOrigin)[2]);
	info->distance = VectorNormalize( info->direction );

	((info->pathDirection)[0]=(end)[0]-(origin)[0],(info->pathDirection)[1]=(end)[1]-(origin)[1],(info->pathDirection)[2]=(end)[2]-(origin)[2]);
...
unsigned int waypoint_testDirection( vec3_t origin, float yaw, unsigned int minDist )
{
	...
	((maxs)[0]=(15), (maxs)[1]=(15), (maxs)[2]=(40));
	((mins)[0]=(-15), (mins)[1]=(-15), (mins)[2]=(-24 + 18));

	
	((angles)[0]=(0), (angles)[1]=(yaw), (angles)[2]=(0));
	AngleVectors( angles, trace_dir, ((void *)0), ((void *)0) );

	

	((test_pos)[0]=(origin)[0]+(trace_dir)[0]*(minDist),(test_pos)[1]=(origin)[1]+(trace_dir)[1]*(minDist),(test_pos)[2]=(origin)[2]+(trace_dir)[2]*(minDist));

	trap_Trace( &tr, origin, mins, maxs, test_pos, ((1<<10)-1), ( 0x00000001 | 0x00000020 | 0x00000040 ) );
...
{
	unsigned int	minDist = 1024 + 1; 
	unsigned int	dist;
	int				i;

	for ( i = 0; i < 16; i++ )
	{
		dist = waypoint_testDirection( ent->r.currentOrigin, ((360.0f/16) * i), minDist );
...
void SP_waypoint ( gentity_t *ent )
{
	if ( navCalculatePaths )
	{
		...
		((ent->r.mins)[0]=(-15), (ent->r.mins)[1]=(-15), (ent->r.mins)[2]=(-24));
		((ent->r.maxs)[0]=(15), (ent->r.maxs)[1]=(15), (ent->r.maxs)[2]=(40));
		
		ent->r.contents = 0x00000400;
		ent->clipmask = (0x00000001|0x00000010|0x00001000);
		...
		if( !(ent->spawnflags&1) && G_CheckInSolid (ent, qtrue))
		{
			...
			{
				Com_Printf("^1""ERROR: Waypoint %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
				(void)( (!!(0 && "Waypoint in solid!")) || (_wassert(L"0 && \"Waypoint in solid!\"", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1298), 0) );
				G_FreeEntity(ent);
				return;
			}
		}
...
void SP_waypoint_small (gentity_t *ent)
{
	if ( navCalculatePaths )
	{
		((ent->r.mins)[0]=(-2), (ent->r.mins)[1]=(-2), (ent->r.mins)[2]=(-24));
		((ent->r.maxs)[0]=(2), (ent->r.maxs)[1]=(2), (ent->r.maxs)[2]=(40));
		...
		if ( !(ent->spawnflags&1) && G_CheckInSolid( ent, qtrue ) )
		{
			...
			if ( G_CheckInSolid( ent, qtrue ) )
			{
				Com_Printf("^1""ERROR: Waypoint_small %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
				(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1339), 0) );
				G_FreeEntity(ent);
				return;
			}
		}
...
{
	int radius = ( ent->radius ) ? (((int)ent->radius)|16384) : 12;

	((ent->r.mins)[0]=(-16), (ent->r.mins)[1]=(-16), (ent->r.mins)[2]=(-24));
	((ent->r.maxs)[0]=(16), (ent->r.maxs)[1]=(16), (ent->r.maxs)[2]=(32));
	ent->s.origin[2] += 0.125;
	if ( !(ent->spawnflags&1) && G_CheckInSolid( ent, qfalse ) )
	{
		Com_Printf("^1""ERROR: Waypoint_navgoal %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1380), 0) );
	}
	TAG_Add( ent->targetname, ((void *)0), ent->s.origin, ent->s.angles, radius, 0x00000001 );
...
{
	((ent->r.mins)[0]=(-8), (ent->r.mins)[1]=(-8), (ent->r.mins)[2]=(-24));
	((ent->r.maxs)[0]=(8), (ent->r.maxs)[1]=(8), (ent->r.maxs)[2]=(32));
	ent->s.origin[2] += 0.125;
	if ( !(ent->spawnflags&1) && G_CheckInSolid( ent, qfalse ) )
	{
		Com_Printf("^1""ERROR: Waypoint_navgoal_8 %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1410), 0) );
	}

	TAG_Add( ent->targetname, ((void *)0), ent->s.origin, ent->s.angles, 8, 0x00000001 );
...
{
	((ent->r.mins)[0]=(-4), (ent->r.mins)[1]=(-4), (ent->r.mins)[2]=(-24));
	((ent->r.maxs)[0]=(4), (ent->r.maxs)[1]=(4), (ent->r.maxs)[2]=(32));
	ent->s.origin[2] += 0.125;
	if ( !(ent->spawnflags&1) && G_CheckInSolid( ent, qfalse ) )
	{
		Com_Printf("^1""ERROR: Waypoint_navgoal_4 %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1441), 0) );
	}

	TAG_Add( ent->targetname, ((void *)0), ent->s.origin, ent->s.angles, 4, 0x00000001 );
...
{	
	((ent->r.mins)[0]=(-2), (ent->r.mins)[1]=(-2), (ent->r.mins)[2]=(-24));
	((ent->r.maxs)[0]=(2), (ent->r.maxs)[1]=(2), (ent->r.maxs)[2]=(32));
	ent->s.origin[2] += 0.125;
	if ( !(ent->spawnflags&1) && G_CheckInSolid( ent, qfalse ) )
	{
		Com_Printf("^1""ERROR: Waypoint_navgoal_2 %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1472), 0) );
	}

	TAG_Add( ent->targetname, ((void *)0), ent->s.origin, ent->s.angles, 2, 0x00000001 );
...
{
	((ent->r.mins)[0]=(-1), (ent->r.mins)[1]=(-1), (ent->r.mins)[2]=(-24));
	((ent->r.maxs)[0]=(1), (ent->r.maxs)[1]=(1), (ent->r.maxs)[2]=(32));
	ent->s.origin[2] += 0.125;
	if ( !(ent->spawnflags&1) && G_CheckInSolid( ent, qfalse ) )
	{
		Com_Printf("^1""ERROR: Waypoint_navgoal_1 %s at %s in solid!\n", ent->targetname, vtos(ent->r.currentOrigin));
		(void)( (!!(0)) || (_wassert(L"0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c", 1503), 0) );
	}

	TAG_Add( ent->targetname, ((void *)0), ent->s.origin, ent->s.angles, 1, 0x00000001 );
...
void Svcmd_Nav_f( void )
{
	...
	else if ( Q_stricmp( cmd, "set" ) == 0 )
	{
		...
		{
			NAVDEBUG_curGoal = trap_Nav_GetNearestNode( &g_entities[0], g_entities[0].waypoint, 0x00000002, -1 );
		}
	}
...
qboolean NAV_WaypointsTooFar( gentity_t *wp1, gentity_t *wp2 )
{
	if ( Distance( wp1->r.currentOrigin, wp2->r.currentOrigin ) > 1024 )
	{
		...
		{
			Com_sprintf( temp, sizeof(temp), "^1""Waypoint conn %s->%s > 1024\n", vtos( wp1->r.currentOrigin ), vtos( wp2->r.currentOrigin ) );
		}
...
#line 1657 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"

static int numStoredWaypoints = 0;

static waypointData_t tempWaypointList[512]; 
...
void NAV_StoreWaypoint( gentity_t *ent )
{
	...
	if ( ent->targetname )
	{
		Q_strncpyz( tempWaypointList[numStoredWaypoints].targetname, ent->targetname, 64 );
	}
	if ( ent->target )
	{
		Q_strncpyz( tempWaypointList[numStoredWaypoints].target, ent->target, 64 );
	}
	if ( ent->target2 )
	{
		Q_strncpyz( tempWaypointList[numStoredWaypoints].target2, ent->target2, 64 );
	}
	if ( ent->target3 )
	{
		Q_strncpyz( tempWaypointList[numStoredWaypoints].target3, ent->target3, 64 );
	}
	if ( ent->target4 )
	{
		Q_strncpyz( tempWaypointList[numStoredWaypoints].target4, ent->target4, 64 );
	}
...
#line 1748 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
...
#line 1761 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
			{
...
#line 1774 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
			{
				trap_Nav_HardConnect( tempWaypointList[i].nodeID, tempWaypointList[target].nodeID );
			}
		}
...
#line 1787 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
			{
				trap_Nav_HardConnect( tempWaypointList[i].nodeID, tempWaypointList[target].nodeID );
			}
		}
...
#line 1800 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
			{
				trap_Nav_HardConnect( tempWaypointList[i].nodeID, tempWaypointList[target].nodeID );
			}
		}
...
#line 1807 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
...
#line 1838 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_nav.c"
}
...
void NAV_ShowDebugInfo( void )
{
	...
	if ( NAVDEBUG_showTestPath )
	{
		...
		int	nearestNode = trap_Nav_GetNearestNode( &g_entities[0], g_entities[0].waypoint, 0, -1 );
		int	testNode = trap_Nav_GetBestNode( nearestNode, NAVDEBUG_curGoal, -1 );
		vec3_t	dest, start;
...
void NAV_FindPlayerWaypoint( int clNum )
{
	g_entities[clNum].waypoint = trap_Nav_GetNearestNode( &g_entities[clNum], g_entities[clNum].lastWaypoint, 0x00000002, -1 );
}
