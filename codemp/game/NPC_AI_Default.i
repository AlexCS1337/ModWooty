...
void NPC_LostEnemyDecideChase(void)
{
	...
	case BS_HUNT_AND_KILL:
		...
		if ( NPC->enemy == NPCInfo->goalEntity && NPC->enemy->lastWaypoint != -1 )
		{
...
void NPC_BSStandGuard (void)
{
	...
	if ( NPC->enemy == ((void *)0) )
	{
		if( ((rand () & 0x7fff) / ((float)0x7fff)) < 0.5 )
		{
...
void NPC_BSHuntAndKill( void )
{
	...
	if ( NPC->enemy )
	{
		oEVis = enemyVisibility = NPC_CheckVisibility ( NPC->enemy, 4|8 );
...
void NPC_BSRunAndShoot (void)
{
	...
	if ( NPC->enemy )
	{
		...
		if ( !(ucmd.buttons & 1) && ucmd.upmove >= 0 && NPC->cantHitEnemyCounter > monitor )
		{
			vec3_t	vec;
...
void NPC_BSFace (void)
{
	...
	if( NPC_UpdateAngles ( qtrue, qtrue ) )
	{
		...
		NPCInfo->desiredYaw = client->ps.viewangles[1];
		NPCInfo->desiredPitch = client->ps.viewangles[0];
...
void NPC_BSPointShoot (qboolean shoot)
{
	...
	CalcEntitySpot(NPC->enemy, SPOT_HEAD, org);
	...
	((dir)[0]=(org)[0]-(muzzle)[0],(dir)[1]=(org)[1]-(muzzle)[1],(dir)[2]=(org)[2]-(muzzle)[2]);
	...
	if (0)
	{
...

		{
			...
			yawMiss = tan(( ( (AngleDelta ( NPC->client->ps.viewangles[1], NPCInfo->desiredYaw )) * 3.14159265358979323846f ) / 180.0F )) * dist;
			pitchMiss = tan(( ( (AngleDelta ( NPC->client->ps.viewangles[0], NPCInfo->desiredPitch)) * 3.14159265358979323846f ) / 180.0F )) * dist;
...
void NPC_BSDefault( void ) 
{
	...
	if ( NPCInfo->scriptFlags & 0x00010000 )
	{
		if( NPC->client->ps.torsoAnim != TORSO_SURRENDER_START )
		{
			NPC_SetAnim( NPC, 1, TORSO_SURRENDER_START, 1|2 );
		}
	}
	
	NPC_CheckEnemy( (NPCInfo->scriptFlags&0x00000800), qfalse, qtrue );
	if ( !NPC->enemy )
	{
	...
	if ( NPC->enemy && !(NPCInfo->scriptFlags&0x00010000) )
	{
	...
	if ( UpdateGoal() )
	{
		...
		else
		{
			...
			if ( (NPCInfo->scriptFlags & 0x00001000) || NPCInfo->goalEntity != NPC->enemy )
			{
