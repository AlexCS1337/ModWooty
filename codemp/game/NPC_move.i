...
#line 18 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_move.c"
...
#line 20 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_move.c"
...
qboolean NPC_ClearPathToGoal( vec3_t dir, gentity_t *goal )
{
	...
	if ( NAV_CheckAhead( NPC, goal->r.currentOrigin, &trace, ( NPC->clipmask & ~0x00000100 )|0x00000040 ) )
	{
...
__inline qboolean NPC_CheckCombatMove( void )
{
...
static void NPC_LadderMove( vec3_t dir )
{
	...
	if ( ( dir[2] > 0 ) || ( dir[2] < 0 && NPC->client->ps.groundEntityNum == ((1<<10)-1) ) )
	{
...
__inline qboolean NPC_GetMoveInformation( vec3_t dir, float *distance )
{
	...
	*distance = VectorNormalize( dir );
	...
	((NPCInfo->blockedDest)[0]=(NPCInfo->goalEntity->r.currentOrigin)[0],(NPCInfo->blockedDest)[1]=(NPCInfo->goalEntity->r.currentOrigin)[1],(NPCInfo->blockedDest)[2]=(NPCInfo->goalEntity->r.currentOrigin)[2]);
...
qboolean NPC_GetMoveDirection( vec3_t out, float *distance )
{
	...
	*distance = frameNavInfo.distance;
	...
	((frameNavInfo.pathDirection)[0]=(frameNavInfo.direction)[0],(frameNavInfo.pathDirection)[1]=(frameNavInfo.direction)[1],(frameNavInfo.pathDirection)[2]=(frameNavInfo.direction)[2]);

	
	if ( NPC->watertype & 0x00002000 )
	{
	...
	if ( NPC_ClearPathToGoal( frameNavInfo.direction, NPCInfo->goalEntity ) == qfalse )
	{
		...
		if ( NAV_MoveToGoal( NPC, &frameNavInfo ) == -1 )
		{
			
			vectoangles( frameNavInfo.direction, angles );
			NPCInfo->desiredYaw	= AngleNormalize360( angles[1] );		
			((out)[0]=(frameNavInfo.direction)[0],(out)[1]=(frameNavInfo.direction)[1],(out)[2]=(frameNavInfo.direction)[2]);
			*distance = frameNavInfo.distance;
			return qfalse;
		}

		frameNavInfo.flags |= 0x00000002;
	}
	...
	if ( NAV_AvoidCollision( NPC, NPCInfo->goalEntity, &frameNavInfo ) == qfalse )
	{
		...
		if ( !(frameNavInfo.flags&0x00000002) )
		{
...
qboolean NPC_GetMoveDirectionAltRoute( vec3_t out, float *distance, qboolean tryStraight )
{
	...
	*distance = frameNavInfo.distance;
	...
	if ( !tryStraight || NPC_ClearPathToGoal( frameNavInfo.direction, NPCInfo->goalEntity ) == qfalse )
	{
		...
		if ( NAVNEW_MoveToGoal( NPC, &frameNavInfo ) == -1 )
		{
...
void G_UcmdMoveForDir( gentity_t *self, usercmd_t *cmd, vec3_t dir )
{
	...
	AngleVectors( self->r.currentAngles, forward, right, ((void *)0) );

	dir[2] = 0;
	VectorNormalize( dir );
	
	((self->client->ps.moveDir)[0]=(dir)[0],(self->client->ps.moveDir)[1]=(dir)[1],(self->client->ps.moveDir)[2]=(dir)[2]);

	fDot = ((forward)[0]*(dir)[0]+(forward)[1]*(dir)[1]+(forward)[2]*(dir)[2]) * 127.0f;
	rDot = ((right)[0]*(dir)[0]+(right)[1]*(dir)[1]+(right)[2]*(dir)[2]) * 127.0f;
...
#line 382 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_move.c"
...
#line 390 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_move.c"
...
#line 411 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_move.c"
		return qfalse;
	...
	else
	{
		...
		NPCInfo->desiredYaw		= AngleNormalize360( NPCInfo->lastPathAngles[1] );
		
		
		if ( (NPC->client->ps.eFlags2&(1<<4)) )
		{
			NPCInfo->desiredPitch = AngleNormalize360( NPCInfo->lastPathAngles[0] );
...
#line 466 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_move.c"
	return qtrue;
}
...
{
	float	saveYaw = NPC->client->ps.viewangles[1];
	qboolean ret;
