...
float	pm_duckScale = 0.66f;///0.50f;
...
float	pm_spectatorfriction = 2.5f;///5.0f;
...
int PM_GetSaberStance(void)
{
	...
	if ( saber1 
		...
		&& pm->ps->saberHolstered )
	{//dual sabers, both on
		return BOTH_STAND4;///BOTH_STAND6;
	}
	...
	case SS_DESANN:	
		...
		break;
	case SS_TAVION:
		anim = BOTH_STAND4;///BOTH_STAND6;
		break;

	default:
		anim = BOTH_STAND3;/// BOTH_STAND2;
		break;
	}
	return anim;
}
...
qboolean PM_AdjustAngleForWallRun( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//wall-running and not at end of anim
		...
		if ( trace.fraction < 1.0f 
			&& (trace.plane.normal[2] >= 0.0f && trace.plane.normal[2] <= 0.7f) )//&& ent->client->ps.groundEntityNum == ENTITYNUM_NONE )
		{
			trace_t	trace2;
			vec3_t traceTo2;
		...
		if ( trace.fraction < 1.0f 
			&& (trace.plane.normal[2] >= 0.0f&&trace.plane.normal[2] <= 0.7f/*MAX_WALL_RUN_Z_NORMAL*/) )
		{//still a wall there
			if ( (ps->legsAnim) == BOTH_WALL_RUN_RIGHT )
			{
				ucmd->rightmove = 127;
			...
			if ( doMove )
			{
				...
				{//not at end of anim yet
					...
					///fwdAngles[YAW] = ps->viewangles[YAW];//unwoot?
					///AngleVectors( fwdAngles, fwd, NULL, NULL );//unwoot?
					...
					{
						speed = (float)g_speed.integer;//running speed
					}
					VectorScale( fwd, speed, ps->velocity );
				}

				ps->velocity[2] = zVel+3.14f;//unw;//preserve z velocity
		...
		{//stop it
			if ( (ps->legsAnim) == BOTH_WALL_RUN_RIGHT )
			{
				...
				PM_SetAnim(SETANIM_BOTH, BOTH_WALL_RUN_RIGHT, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_RESTART, 0);
			...
			else if ( (ps->legsAnim) == BOTH_WALL_RUN_LEFT )
			{
				...
				PM_SetAnim(SETANIM_BOTH, BOTH_WALL_RUN_LEFT, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_RESTART, 0);
				pm->ps->legsTimer = 1000;
...
#define	JUMP_OFF_WALL_SPEED	125.0f //200.0f
//nice...
...
qboolean PM_AdjustAngleForWallJump( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//hugging wall, getting ready to jump off
		...
		if ( /*!*/pm->debugMelee && pm->ps->brokenLimbs != 0)
		{//hotwalls
			if ( ucmd->upmove > 0 && pm->ps->fd.forcePower > 5)
			{//hold on until you let go manually
				if ( BG_InReboundHold( ps->legsAnim ) )
				{//keep holding
					PM_SetAnim( SETANIM_BOTH, BOTH_SPINATTACK7, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
		...
		{//jump off
			//push off of it!
			ps->pm_flags &= ~PMF_STUCK_TO_WALL;
			ps->velocity[0] = ps->velocity[1] = g_walljumpXvelocity.integer;///50;//50;
			VectorScale( checkDir, -1*g_walljumpYvelocity.integer/*-JUMP_OFF_WALL_SPEED*/, ps->velocity );
...
static qboolean PM_CheckJump( void ) 
{
	...
	{//in low grav, you push in the dir you're facing as long as there is something behind you to shove off of
		...
		!(pm->ps->pm_flags&PMF_JUMP_HELD) &&
		//unwoot?//////(pm->ps->weapon == WP_SABER || pm->ps->weapon == WP_MELEE) &&
		!PM_IsRocketTrooper() &&
		...
		BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION) )
	{
		...
		{//on the ground
			...
			{//backflip
				if ( allowFlips )
				{
					...
					///anim = BOTH_BACK_FLIP_UP;
					///BG_PickAnim( BOTH_FLIP_BACK1, BOTH_FLIP_BACK3);//?

				}
			}

			vertPush += 128; //128//give them an extra shove
			...
			if ( anim != -1 )
			{
				...
				int contents = MASK_PLAYERSOLID;//MASK_PLAYERSOLID;
				...
				if ( doTrace )
				{
					...
					///pm->trace( &trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, contents );
					///VectorSubtract( pm->ps->origin, traceto, idealNormal );
					///VectorNormalize( idealNormal );//unwoot jk2?
				}
				...
				{//there is a wall there.. or hit a client
					...
					///	|| (wallNormal[2] >= 0.0f&&wallNormal[2]<=0.7f/*MAX_WALL_RUN_Z_NORMAL*/) )
					///{
						...
						{
							///looping!!!
							////PM_SetAnim( parts, anim, SETANIM_FLAG_HOLDLESS|SETANIM_FLAG_RESTART, 0 );
							PM_SetAnim( parts, anim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_RESTART, 0 );							
		...
		{//in the air
			...
			else if ( pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START )
			{
				...
				{//not at start of anim
					VectorMA( pm->ps->origin, 16, fwd, traceto );
					anim = BOTH_FORCEWALLRUNFLIP_START;//unwoot?
				}
				if ( anim != -1 )
				{
					pm->trace( &trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, CONTENTS_SOLID|CONTENTS_BODY );
			...
			///*
				...
				//////&& PM_GroundDistance() <= 60 //unfortunately we do not have a happy ground timer like SP (this would use up more bandwidth if we wanted prediction workign right), so we'll just use the actual ground distance.
			...
			{//run up wall, flip backwards
				...
				//pm->ps->fd.forcePower = pm->ps->fd.forcePower - 20;
				...
				{//there is a wall there
					...
					pm->ps->legsTimer -= 400; //I force this anim to play to the end to prevent landing on your head and suddenly flipping over.
			...
			//*/
				...
				|| pm->ps->legsAnim == BOTH_INAIR1 ) )//not in a flip or spin or anything
			{//run up wall, flip backwards
				if ( allowWallRuns )
				{
					...
					if (1) //sure, we have it! Because I SAID SO.
					{
						...
						if ( trace.fraction < 1.0f
							...
							||DotProduct(trace.plane.normal,idealNormal)>0.4) 
							)
						{//there is a wall there
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
							if ( wallWalkAnim == BOTH_FORCEWALLRUNFLIP_START )
							{
								pm->ps->velocity[2] = forceJumpStrength[FORCE_LEVEL_3]/1.6f;////2.0f;
							}
							else
							{
								VectorMA( pm->ps->velocity, -150, fwd, pm->ps->velocity );
								pm->ps->velocity[2] += 175.0f;////150.0f;
							}
							//animate me
							///PM_SetAnim( parts, wallWalkAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_RESTART, 0 );//unwoot?
							PM_SetAnim( parts, wallWalkAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_RESTART, 0 );
				...
				(
				(!BG_InSpecialJump( legsAnim )//not in a special jump anim 
				||BG_InReboundJump( legsAnim )//we're already in a rebound
				||BG_InBackFlip( legsAnim ) 
				)//a backflip (needed so you can jump off a wall behind you)
				||
				...
				&& pm->ps->velocity[2] < -50))
					...
					&& pm->ps->velocity[2] > -2800 //not falling down very fast
					...
					//unwoot?////&& pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_2//level 3 jump or better
			...
			{//see if we're pushing at a wall and jump off it if so
				if ( allowWallGrabs )
				{
					...
					{//trace in the dir we're pushing in and see if there's a vertical wall there
						...
						if ( trace.fraction < 1.0f
							////&&fabs(trace.plane.normal[2]) <= 0.2f/* MAX_WALL_GRAB_SLOPE */
							&&fabs(trace.plane.normal[2]) <= 0.6f/* MAX_WALL_GRAB_SLOPE */
...
static void PM_WalkMove( void ) {
	...
	///if (pm->ps->weapon == WP_MELEE && (pm->ps->torsoAnim == FACE_SMILE || pm->ps->torsoAnim == BOTH_STAND5TOSTAND8 || pm->ps->torsoAnim == BOTH_STAND8TOSTAND5)){
	...
	if (pm->ps->pm_type != PM_SPECTATOR)
	{
		...
		{	
		//end divebomb
		//BG_SetAnim(pm->ps, NULL, SETANIM_BOTH, BOTH_LAND2,1|2, 0);
		//pm->ps->torsoAnim = BOTH_LAND1;
		//pm->ps->legsAnim = BOTH_LAND1;
		
		PM_SetAnim(SETANIM_BOTH, BOTH_LAND2, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
		////pm->ps->legsTimer = 1500;
...
static void PM_GroundTrace( void ) {
	...
	if ( trace.fraction == 1.0 ) {
		...
		//pm->ps->torsoAnim = BOTH_LAND1;
		//pm->ps->legsAnim = BOTH_LAND1;
		PM_SetAnim(SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
		////pm->ps->legsTimer = 1500;
		////pm->ps->weaponTime = pm->ps->legsTimer;
...
static void PM_Footsteps( void ) {
	...
	else if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		...
		if( ( g_canjk2roll.integer >= 1 && pm->cmd.buttons & BUTTON_WALKING)
		||
		
	( ( (PM_RunningAnim( pm->ps->legsAnim )
			&&VectorLengthSquared(pm->ps->velocity)>=30000/*200*200=40000*/) 
			...
			///if (g_canjk2roll.integer >= 1){
			///	rolled = PM_TryJK2Roll();
			///}else{
				rolled = PM_TryRoll();
			///}
		}
...
*/
	...
	case HI_SHIELD:
		...
		if (tr.fraction > 0.9 && !tr.startsolid && !tr.allsolid)
		{
			...
			VectorSet( dest, pos[0], pos[1], pos[2] - 512);//4096 );
...
*/
	...
	{ //reset into weapon stance
		...
		///PM_SetAnim(SETANIM_TORSO, 987, 1|2, 0);
		...
		pm->ps->weapon == WP_MELEE)
	{
			...
			pm->ps->forceHandExtend == HANDEXTEND_NONE)
		{
			...
			{ //remap the standard standing anims for melee stance
				desTAnim = BOTH_STAND6;//BOTH_STAND6
			}
	...
	{ //special anims for standard melee attacks
		...
		{ //if riding a vehicle don't do this stuff at all
			...
			{ //ok, grapple time
				...
				if (icandoit && g_canmeleekata.integer > 0)
				{
					...
					//pm->ps->torsoAnim = BOTH_KYLE_GRAB;
					//pm->ps->legsAnim = BOTH_KYLE_GRAB;
			...
			{ 
							smove = LS_A_LUNGE;
				kickMove=1273; //BOTH_ALORA_SPIN_SLASH
				G_Sound( ent, CHAN_VOICE, G_SoundIndex( "sound/chr_d/boba/24bof002.mp3" ) );
				ent->client->ps.torsoTimer = 1800;
			...
			else 	if (ent->client->pers.cmd.rightmove < 0 && ent->client->pers.cmd.forwardmove < 0)
			{
				...
				kickMove=903; //BOTH_PULL_IMPALE_SWING
				//G_Sound( ent, CHAN_VOICE, G_SoundIndex( "sound/chr_d/prisoner1/12pr1001.mp3" ) );
				ent->client->ps.torsoTimer = 1800;
			} 
			else if (ent->client->pers.cmd.rightmove > 0 && ent->client->pers.cmd.forwardmove < 0)
			...
			{ //just punch
				int desTAnim = BOTH_MELEE1; ////BOTH_MELEE1;
				if (pm->ps->torsoAnim == BOTH_MELEE1)////BOTH_MELEE1)
				{
					desTAnim = BOTH_MELEE2;////BOTH_MELEE2;
				}
				if (pm->ps->torsoAnim == BOTH_MELEE2)////BOTH_MELEE1)
				{
					desTAnim = BOTH_PULL_IMPALE_SWING;////BOTH_MELEE2;
				}
				if (pm->ps->torsoAnim == BOTH_PULL_IMPALE_SWING)
				{
					if (Q_irand(0,10) < 3){
					desTAnim = BOTH_THERMAL_THROW;////BOTH_THERMAL_READY
					}else{
					desTAnim = BOTH_THERMAL_READY;////
					}
				}
				////desTAnim = 497+Q_irand(1,120);

				if (gent->client && gent->client->saber[0].moveSpeedScale > 0.9f){

					if (gent->client->saber[0].moveSpeedScale < 3.0f){
						pm->ps->torsoTimer -= (int)(100.0f * gent->client->saber[0].moveSpeedScale ) ;
					}else{
						if (pm->ps->torsoTimer > 300){
...
//	PM_AdjustAnglesForStabDown( pm->ps, &pm->cmd );
	...
	*/
	...
	{
		pm->cmd.upmove = 0;
		pm->cmd.forwardmove = 20;///0;//50;
		pm->cmd.rightmove *= 0.08;///= 0;//*= 0.1;
	}
	...
	if (pm->ps->pm_type == PM_JETPACK)
	{
		...
		if (pm->cmd.upmove > 0 && pm->ps->velocity[2] < 384)////< 256)
