...
void R2D2_TurnAnims ( void )
{
	...
	turndelta = AngleDelta(NPC->r.currentAngles[1], NPCInfo->desiredYaw);
	...
	if ((fabs(turndelta) > 20) && ((NPC->client->NPC_class == CLASS_R2D2) || (NPC->client->NPC_class == CLASS_R5D2)))
	{
		...
		if (turndelta<0)
		{
			...
			{
				NPC_SetAnim( NPC, 1|2, BOTH_TURN_LEFT1, 1|2 );
			}
		}
		else
		{
			...
			{
				NPC_SetAnim( NPC, 1|2, BOTH_TURN_RIGHT1, 1|2 );
			}
		}
	}
	else
...
void Droid_Patrol( void )
{
	...
	if ( UpdateGoal() )
	{
		...
		{
			NPCInfo->desiredYaw += sin(level.time*.5) * 25; 
...
void Droid_Run( void )
{
	...
	else 
	{
		...
		if ( UpdateGoal() )
		{
			...
			{
				NPCInfo->desiredYaw += sin(level.time*.5) * 5; 
			}
		}
	}
		...
		|| NPC->client->NPC_class == CLASS_R2D2 )
	{
		...
		if (trap_G2API_GetSurfaceRenderStatus( NPC->ghoul2, 0, "head" )>0)
		{
			...
			{	
				...
				NPCInfo->desiredYaw = Q_irand( 0, 360 ); 
			}
		}
		else
		{
			...
			{
				NPCInfo->desiredYaw = AngleNormalize360(NPCInfo->desiredYaw + 40); 
			}
		}
	}
	else 
...
void NPC_Droid_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	if ( self->client->NPC_class == CLASS_R5D2 )
	{
		...
		if ( mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT || ((rand () & 0x7fff) / ((float)0x7fff)) < pain_chance )	
		{
				...
				&& ( self->health < 30 || mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT ) )
			{
				if (!(self->spawnflags & 2))	
				{
					...
					{
						NPC_SetSurfaceOnOff( self, "head", 0x00000100 );
	...
	else if (self->client->NPC_class == CLASS_MOUSE)
	{
		...
		self->NPC->scriptFlags &= ~0x00000800;
	}
...
void NPC_BSDroid_Default( void )
{
	...
	{
		NPC_UpdateAngles( qtrue, qtrue );
		ucmd.upmove = (2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) * 64;
	}
