#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
#line 2 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\q_shared.h"
...
#line 3 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
#line 4 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
#line 5 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
#line 12 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
wpobject_t *gWPArray[4096];
int gWPNum = 0;
#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\namespace_end.h"
...
#line 15 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"

int gLastPrintedIndex = -1;


nodeobject_t nodetable[32767];
int nodenum; 
#line 22 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
void G_TestLine(vec3_t start, vec3_t end, int color, int time)
{
	...
	((te->s.origin)[0]=(start)[0],(te->s.origin)[1]=(start)[1],(te->s.origin)[2]=(start)[2]);
	((te->s.origin2)[0]=(end)[0],(te->s.origin2)[1]=(end)[1],(te->s.origin2)[2]=(end)[2]);
	te->s.time2 = time;
	te->s.weapon = color;
...
checkprint:
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			((a)[0]=(viewent->client->ps.origin)[0]-(gWPArray[i]->origin)[0],(a)[1]=(viewent->client->ps.origin)[1]-(gWPArray[i]->origin)[1],(a)[2]=(viewent->client->ps.origin)[2]-(gWPArray[i]->origin)[2]);
...
void TransferWPData(int from, int to)
{
	...
	if (!gWPArray[to])
	{
		G_Printf("^1" "FATAL ERROR: Could not allocated memory for waypoint\n");
	}
	...
	((gWPArray[to]->origin)[0]=(gWPArray[from]->origin)[0],(gWPArray[to]->origin)[1]=(gWPArray[from]->origin)[1],(gWPArray[to]->origin)[2]=(gWPArray[from]->origin)[2]);
}
...
void CreateNewWP(vec3_t origin, int flags)
{
	if (gWPNum >= 4096)
	{
		if (!g_RMG.integer)
		{
			G_Printf("^3" "Warning: Waypoint limit hit (%i)\n", 4096);
		}
		return;
	}
	...
	if (!gWPArray[gWPNum])
	{
		G_Printf("^1" "ERROR: Could not allocated memory for waypoint\n");
	}

	gWPArray[gWPNum]->flags = flags;
	gWPArray[gWPNum]->weight = 0; 
	gWPArray[gWPNum]->associated_entity = ((1<<10)-1); 
	gWPArray[gWPNum]->forceJumpTo = 0;
	gWPArray[gWPNum]->disttonext = 0; 
	gWPArray[gWPNum]->index = gWPNum;
	gWPArray[gWPNum]->inuse = 1;
	((gWPArray[gWPNum]->origin)[0]=(origin)[0],(gWPArray[gWPNum]->origin)[1]=(origin)[1],(gWPArray[gWPNum]->origin)[2]=(origin)[2]);
	gWPNum++;
}
...
void CreateNewWP_FromObject(wpobject_t *wp)
{
	...
	gWPArray[gWPNum]->index = gWPNum;
	gWPArray[gWPNum]->inuse = 1;
	((gWPArray[gWPNum]->origin)[0]=(wp->origin)[0],(gWPArray[gWPNum]->origin)[1]=(wp->origin)[1],(gWPArray[gWPNum]->origin)[2]=(wp->origin)[2]);
	...
	if (gWPArray[gWPNum]->flags & 0x00020000)
	{
		flagRed = gWPArray[gWPNum];
		oFlagRed = flagRed;
	}
	else if (gWPArray[gWPNum]->flags & 0x00040000)
	{
...
void RemoveWP_InTrail(int afterindex)
{
	...
	{
		G_Printf("^3" "Waypoint number %i does not exist\n", afterindex);
		return;
	}
	...
	if (!foundanindex)
	{
		G_Printf("^3" "Waypoint index %i should exist, but does not (?)\n", afterindex);
		return;
	}
...
int CreateNewWP_InTrail(vec3_t origin, int flags, int afterindex)
{
	...
	while (i >= 0)
	{
		...
		else if (gWPArray[i] && gWPArray[i]->inuse && gWPArray[i]->index == foundindex)
		{
			...
			gWPArray[i]->weight = 0; 
			gWPArray[i]->associated_entity = ((1<<10)-1); 
			gWPArray[i]->disttonext = 0; 
			gWPArray[i]->forceJumpTo = 0;
			gWPArray[i]->index = i;
			gWPArray[i]->inuse = 1;
			((gWPArray[i]->origin)[0]=(origin)[0],(gWPArray[i]->origin)[1]=(origin)[1],(gWPArray[i]->origin)[2]=(origin)[2]);
			gWPNum++;
			break;
		}
...
void TeleportToWP(gentity_t *pl, int afterindex)
{
	...
	((pl->client->ps.origin)[0]=(gWPArray[foundindex]->origin)[0],(pl->client->ps.origin)[1]=(gWPArray[foundindex]->origin)[1],(pl->client->ps.origin)[2]=(gWPArray[foundindex]->origin)[2]);
...
void WPFlagsModify(int wpnum, int flags)
{
	...
	{
		G_Printf("^3" "WPFlagsModify: Waypoint %i does not exist\n", wpnum);
		return;
	}
...
#line 811 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"

int CanGetToVector(vec3_t org1, vec3_t org2, vec3_t mins, vec3_t maxs)
{
	trace_t tr;

	trap_Trace(&tr, org1, mins, maxs, org2, ((1<<10)-1), (-1));
...
#line 883 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...

{
	...
	int didMove = 0;
	int traceMask = (0x00000001|0x00000010|0x00000100|0x00001000);
	qboolean initialDone = qfalse;

	((workingOrg)[0]=(org1)[0],(workingOrg)[1]=(org1)[1],(workingOrg)[2]=(org1)[2]);
	((lastIncrement)[0]=(org1)[0],(lastIncrement)[1]=(org1)[1],(lastIncrement)[2]=(org1)[2]);

	((stepTo)[0]=(moveTo)[0],(stepTo)[1]=(moveTo)[1],(stepTo)[2]=(moveTo)[2]);
	stepTo[2] = workingOrg[2];

	((stepSub)[0]=(stepTo)[0]-(workingOrg)[0],(stepSub)[1]=(stepTo)[1]-(workingOrg)[1],(stepSub)[2]=(stepTo)[2]-(workingOrg)[2]);
	stepSize = VectorLength(stepSub); 
	...
	while (!initialDone || didMove)
	{
		...
		trap_Trace(&tr, workingOrg, mins, maxs, stepGoal, ((1<<10)-1), traceMask);

		if (!tr.startsolid && !tr.allsolid && tr.fraction)
		{
			vec3_t vecSub;
			((vecSub)[0]=(workingOrg)[0]-(tr.endpos)[0],(vecSub)[1]=(workingOrg)[1]-(tr.endpos)[1],(vecSub)[2]=(workingOrg)[2]-(tr.endpos)[2]);
		...
		{ 
			...
			((trFrom)[0]=(tr.endpos)[0],(trFrom)[1]=(tr.endpos)[1],(trFrom)[2]=(tr.endpos)[2]);
			trFrom[2] += 16;

			((trDir)[0]=(stepGoal)[0]-(workingOrg)[0],(trDir)[1]=(stepGoal)[1]-(workingOrg)[1],(trDir)[2]=(stepGoal)[2]-(workingOrg)[2]);
			...
			((vecMeasure)[0]=(trFrom)[0]-(trTo)[0],(vecMeasure)[1]=(trFrom)[1]-(trTo)[1],(vecMeasure)[2]=(trFrom)[2]-(trTo)[2]);

			if (VectorLength(vecMeasure) > 1)
			{
				trap_Trace(&tr, trFrom, mins, maxs, trTo, ((1<<10)-1), traceMask);

				if (!tr.startsolid && !tr.allsolid && tr.fraction == 1)
				{ 
					vec3_t trDown;
					vec3_t trUp;
					((trUp)[0]=(tr.endpos)[0],(trUp)[1]=(tr.endpos)[1],(trUp)[2]=(tr.endpos)[2]);
					((trDown)[0]=(tr.endpos)[0],(trDown)[1]=(tr.endpos)[1],(trDown)[2]=(tr.endpos)[2]);
					trDown[2] -= 16;
					...
					{ 
						((workingOrg)[0]=(tr.endpos)[0],(workingOrg)[1]=(tr.endpos)[1],(workingOrg)[2]=(tr.endpos)[2]);
		...
		((finalMeasure)[0]=(lastIncrement)[0]-(workingOrg)[0],(finalMeasure)[1]=(lastIncrement)[1]-(workingOrg)[1],(finalMeasure)[2]=(lastIncrement)[2]-(workingOrg)[2]);
		...
		stepSize -= measureLength; 
		if (stepSize <= 0)
		{
			break;
		}

		((lastIncrement)[0]=(workingOrg)[0],(lastIncrement)[1]=(workingOrg)[1],(lastIncrement)[2]=(workingOrg)[2]);
	}

	return didMove;
}
#line 998 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
int ConnectTrail(int startindex, int endindex, qboolean behindTheScenes)
{
	...
	if (g_RMG.integer)
	{ 
		if (!(gWPArray[startindex]->flags & 0x00800000) &&
			!(gWPArray[endindex]->flags & 0x00800000))
		{
			gWPArray[startindex]->flags |= 0x00004000;
			gWPArray[endindex]->flags |= 0x00008000;
		}
		return 0;
	}
	...
	if (!behindTheScenes)
	{
		G_Printf("^3" "Point %i is not connected to %i - Repairing...\n", startindex, endindex);
	}

	((startplace)[0]=(gWPArray[startindex]->origin)[0],(startplace)[1]=(gWPArray[startindex]->origin)[1],(startplace)[2]=(gWPArray[startindex]->origin)[2]);

	((starttrace)[0]=(startplace)[0],(starttrace)[1]=(startplace)[1],(starttrace)[2]=(startplace)[2]);

	starttrace[2] -= 4096;

	trap_Trace(&tr, startplace, ((void *)0), ((void *)0), starttrace, ((1<<10)-1), (0x00000001|0x00001000));

	baseheight = startplace[2] - tr.endpos[2];

	cancontinue = 1;

	((nodetable[nodenum].origin)[0]=(startplace)[0],(nodetable[nodenum].origin)[1]=(startplace)[1],(nodetable[nodenum].origin)[2]=(startplace)[2]);
	nodetable[nodenum].weight = 1;
	nodetable[nodenum].inuse = 1;

	nodenum++;

	while (nodenum < 32767 && !foundit && cancontinue)
	{
		if (g_RMG.integer)
		{ 
			vec3_t startDist;
			...
			((startDist)[0]=(nodetable[nodenum-1].origin)[0]-(gWPArray[startindex]->origin)[0],(startDist)[1]=(nodetable[nodenum-1].origin)[1]-(gWPArray[startindex]->origin)[1],(startDist)[2]=(nodetable[nodenum-1].origin)[2]-(gWPArray[startindex]->origin)[2]);
			((endDist)[0]=(nodetable[nodenum-1].origin)[0]-(gWPArray[endindex]->origin)[0],(endDist)[1]=(nodetable[nodenum-1].origin)[1]-(gWPArray[endindex]->origin)[1],(endDist)[2]=(nodetable[nodenum-1].origin)[2]-(gWPArray[endindex]->origin)[2]);
		...
		while (i < prenodestart)
		{
			...
			{
				((a)[0]=(gWPArray[endindex]->origin)[0]-(nodetable[i].origin)[0],(a)[1]=(gWPArray[endindex]->origin)[1]-(nodetable[i].origin)[1],(a)[2]=(gWPArray[endindex]->origin)[2]-(nodetable[i].origin)[2]);
				...
				((testspot)[0]=(nodetable[i].origin)[0],(testspot)[1]=(nodetable[i].origin)[1],(testspot)[2]=(nodetable[i].origin)[2]);
				testspot[0] += branchDistance;

				((starttrace)[0]=(testspot)[0],(starttrace)[1]=(testspot)[1],(starttrace)[2]=(testspot)[2]);

				starttrace[2] -= 4096;

				trap_Trace(&tr, testspot, ((void *)0), ((void *)0), starttrace, ((1<<10)-1), (0x00000001|0x00001000));
				...
				{
					((nodetable[nodenum].origin)[0]=(testspot)[0],(nodetable[nodenum].origin)[1]=(testspot)[1],(nodetable[nodenum].origin)[2]=(testspot)[2]);
					nodetable[nodenum].inuse = 1;
					...
					{ 
						nodetable[nodenum].flags = 0x00004000;
					}
					nodenum++;
					cancontinue = 1;
				}
	...
	if (!foundit)
	{
		...
		{
			G_Printf("^1" "Could not link %i to %i, unreachable by node branching.\n", startindex, endindex);
		}
		gWPArray[startindex]->flags |= 0x00004000;
		gWPArray[endindex]->flags |= 0x00008000;
		if (!behindTheScenes)
		{
			G_Printf("^3" "Since points cannot be connected, point %i has been flagged as only-forward and point %i has been flagged as only-backward.\n", startindex, endindex);
		}
		...
		else
		{
			...
			{
				((endDist)[0]=(nodetable[nCount].origin)[0]-(gWPArray[endindex]->origin)[0],(endDist)[1]=(nodetable[nCount].origin)[1]-(gWPArray[endindex]->origin)[1],(endDist)[2]=(nodetable[nCount].origin)[2]-(gWPArray[endindex]->origin)[2]);
	...
	failsafe = 0;
	((validspotpos)[0]=(gWPArray[startindex]->origin)[0],(validspotpos)[1]=(gWPArray[startindex]->origin)[1],(validspotpos)[2]=(gWPArray[startindex]->origin)[2]);

	while (failsafe < 32767 && i < 32767 && i >= 0)
	{
		((a)[0]=(validspotpos)[0]-(nodetable[i].origin)[0],(a)[1]=(validspotpos)[1]-(nodetable[i].origin)[1],(a)[2]=(validspotpos)[2]-(nodetable[i].origin)[2]);
		if (!nodetable[nodetable[i].neighbornum].inuse || !CanGetToVectorTravel(validspotpos, nodetable[i].origin, mins, maxs) || VectorLength(a) > maxDistFactor || (!CanGetToVectorTravel(validspotpos, gWPArray[endindex]->origin, mins, maxs) && CanGetToVectorTravel(nodetable[i].origin, gWPArray[endindex]->origin, mins, maxs)) )
		{
			nodetable[i].flags |= 0x00400000;
			if (!CreateNewWP_InTrail(nodetable[i].origin, nodetable[i].flags, insertindex))
			{
				if (!behindTheScenes)
				{
					G_Printf("^1" "Could not link %i to %i, waypoint limit hit.\n", startindex, endindex);
				}
				return 0;
			}

			((validspotpos)[0]=(nodetable[i].origin)[0],(validspotpos)[1]=(nodetable[i].origin)[1],(validspotpos)[2]=(nodetable[i].origin)[2]);
		}
	...
	if (!behindTheScenes)
	{
		G_Printf("^3" "Finished connecting %i to %i.\n", startindex, endindex);
	}

	return 1;
}
#line 1401 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
int OpposingEnds(int start, int end)
{
	...
	if ((gWPArray[start]->flags & 0x00004000) &&
		(gWPArray[end]->flags & 0x00008000))
	{
		return 1;
	}
...
{ 
	...
	trap_Trace(&tr, gWPArray[start]->origin, ((void *)0), ((void *)0), gWPArray[end]->origin, ((1<<10)-1), (0x00000001|0x00001000));
	...
	trap_Trace(&tr, gWPArray[end]->origin, ((void *)0), ((void *)0), gWPArray[start]->origin, ((1<<10)-1), (0x00000001|0x00001000));
...
int RepairPaths(qboolean behindTheScenes)
{
	...
	while (i < gWPNum)
	{
		...
		{
			((a)[0]=(gWPArray[i]->origin)[0]-(gWPArray[i+1]->origin)[0],(a)[1]=(gWPArray[i]->origin)[1]-(gWPArray[i+1]->origin)[1],(a)[2]=(gWPArray[i]->origin)[2]-(gWPArray[i+1]->origin)[2]);

			if (!(gWPArray[i+1]->flags & 0x00000400) &&
				!(gWPArray[i+1]->flags & 0x00000010) && 
				!(gWPArray[i]->flags & 0x00400000) && 
				!OpposingEnds(i, i+1) &&
				((bot_wp_distconnect.value && VectorLength(a) > maxDistFactor) || (!OrgVisible(gWPArray[i]->origin, gWPArray[i+1]->origin, ((1<<10)-1)) && bot_wp_visconnect.value) ) &&
...
#line 1525 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
int OrgVisibleCurve(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2, int ignore)
{
	...
	((evenorg1)[0]=(org1)[0],(evenorg1)[1]=(org1)[1],(evenorg1)[2]=(org1)[2]);
	evenorg1[2] = org2[2];

	trap_Trace(&tr, evenorg1, mins, maxs, org2, ignore, (0x00000001|0x00001000));

	if (tr.fraction == 1 && !tr.startsolid && !tr.allsolid)
	{
		trap_Trace(&tr, evenorg1, mins, maxs, org1, ignore, (0x00000001|0x00001000));
...
int CanForceJumpTo(int baseindex, int testingindex, float distance)
{
	...
	((xy_base)[0]=(wpBase->origin)[0],(xy_base)[1]=(wpBase->origin)[1],(xy_base)[2]=(wpBase->origin)[2]);
	((xy_test)[0]=(wpTest->origin)[0],(xy_test)[1]=(wpTest->origin)[1],(xy_test)[2]=(wpTest->origin)[2]);

	xy_base[2] = xy_test[2];

	((v)[0]=(xy_base)[0]-(xy_test)[0],(v)[1]=(xy_base)[1]-(xy_test)[1],(v)[2]=(xy_base)[2]-(xy_test)[2]);
	...
	if (!OrgVisibleCurve(wpBase->origin, mins, maxs, wpTest->origin, ((1<<10)-1)))
	{
		return 0;
	}
		...
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			while (c < gWPNum)
			{
				if (gWPArray[c] && gWPArray[c]->inuse && i != c &&
					NotWithinRange(i, c))
				{
					((a)[0]=(gWPArray[i]->origin)[0]-(gWPArray[c]->origin)[0],(a)[1]=(gWPArray[i]->origin)[1]-(gWPArray[c]->origin)[1],(a)[2]=(gWPArray[i]->origin)[2]-(gWPArray[c]->origin)[2]);
					...
					if ((nLDist < maxNeighborDist || forceJumpable) &&
						...
						(OrgVisibleBox(gWPArray[i]->origin, mins, maxs, gWPArray[c]->origin, ((1<<10)-1)) || forceJumpable))
					{
...
gentity_t *GetObjectThatTargets(gentity_t *ent)
{
	...
	next = G_Find( next, ((int)&(((gentity_t *)0)->target)), ent->targetname );
...
void CalculateSiegeGoals(void)
{
	...
	vec3_t dif;
	gentity_t *ent;
	gentity_t *tent = ((void *)0), *t2ent = ((void *)0);
	...
	while (i < level.num_entities)
	{
		...
		{ 
			...
			{ 
				if (ent->side == 1)
				{
					gWPArray[wpindex]->flags |= 0x00100000;
				}
				else
				{
					gWPArray[wpindex]->flags |= 0x00080000;
				}
	...
	{ 
		while (i < gWPNum)
		{
			if (gWPArray[i] && gWPArray[i]->inuse)
			{
				...
				if (gWPArray[i]->flags & 0x00010000)
				{
	...
	while (i < level.num_entities)
	{
		...
		if (ent && weight)
		{
			...
			{ 
				...
				gWPArray[wpindex]->flags |= 0x00010000;
...
void CalculateJumpRoutes(void)
{
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			if (gWPArray[i]->flags & 0x00000010)
			{
				nheightdif = 0;
				pheightdif = 0;
...
int LoadPathData(const char *filename)
{
	...
	if (!f)
	{
		G_Printf("^3" "Bot route data not found for %s\n", filename);
		return 2;
	}

	if (len >= 524288)
	{
		G_Printf("^1" "Route file exceeds maximum length\n");
		return 0;
	}
	...
	while (i < len)
	{
		...
		thiswp.origin[2] = 0;
		thiswp.weight = 0;
		thiswp.associated_entity = ((1<<10)-1);
...
void FlagObjects(void)
{
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			((a)[0]=(flag_red->s.pos.trBase)[0]-(gWPArray[i]->origin)[0],(a)[1]=(flag_red->s.pos.trBase)[1]-(gWPArray[i]->origin)[1],(a)[2]=(flag_red->s.pos.trBase)[2]-(gWPArray[i]->origin)[2]);
			tlen = VectorLength(a);

			if (tlen < bestdist)
			{
				trap_Trace(&tr, flag_red->s.pos.trBase, mins, maxs, gWPArray[i]->origin, flag_red->s.number, (0x00000001|0x00001000));
	...
	if (found)
	{
		gWPArray[bestindex]->flags |= 0x00020000;
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			((a)[0]=(flag_blue->s.pos.trBase)[0]-(gWPArray[i]->origin)[0],(a)[1]=(flag_blue->s.pos.trBase)[1]-(gWPArray[i]->origin)[1],(a)[2]=(flag_blue->s.pos.trBase)[2]-(gWPArray[i]->origin)[2]);
			tlen = VectorLength(a);

			if (tlen < bestdist)
			{
				trap_Trace(&tr, flag_blue->s.pos.trBase, mins, maxs, gWPArray[i]->origin, flag_blue->s.number, (0x00000001|0x00001000));
	...
	if (found)
	{
		gWPArray[bestindex]->flags |= 0x00040000;
...
int SavePathData(const char *filename)
{
	...
	if (!f)
	{
		G_Printf("^1" "ERROR: Could not open file to write path data\n");
		return 0;
	}
...
#line 2502 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"




int gSpawnPointNum = 0;
gentity_t *gSpawnPoints[64];
...
{ 
	...
	while (i < nodenum)
	{
		((vSub)[0]=(nodetable[i].origin)[0]-(point)[0],(vSub)[1]=(nodetable[i].origin)[1]-(point)[1],(vSub)[2]=(nodetable[i].origin)[2]-(point)[2]);
...
#line 2543 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
int G_RecursiveConnection(int start, int end, int weight, qboolean traceCheck, float baseHeight)
{
	...
	while (i < 4)
	{
		...
		{ 
			trap_Trace(&tr, nodetable[start].origin, ((void *)0), ((void *)0), nodetable[indexDirections[i]].origin, ((1<<10)-1), 0x00000001);
...
#line 2724 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
{ 
	...
	while (i < 4)
	{
		if (indexDirections[i] != -1)
		{
			...
			if (nodetable[indexDirections[i]].weight < lowestWeight && nodetable[indexDirections[i]].weight && !nodetable[indexDirections[i]].flags )
			{
...
#line 3054 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
{ 
	...
	int startTime = 0;
	int endTime = 0;
#line 3066 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
	vec3_t downVec, trMins, trMaxs;
	trace_t tr;
	gentity_t *terrain = G_Find( ((void *)0), ((int)&(((gentity_t *)0)->classname)), "terrain" );
...
#line 3079 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"

	nodenum = 0;
	memset(&nodetable, 0, sizeof(nodetable));

	((trMins)[0]=(-15), (trMins)[1]=(-15), (trMins)[2]=(-24));
	((trMaxs)[0]=(15), (trMaxs)[1]=(15), (trMaxs)[2]=(40));
	...
	while (placeY < terrain->r.absmax[1])
	{
		...
		while (placeX < terrain->r.absmax[0])
		{
			...
			((downVec)[0]=(nodetable[nodenum].origin)[0],(downVec)[1]=(nodetable[nodenum].origin)[1],(downVec)[2]=(nodetable[nodenum].origin)[2]);
			downVec[2] -= 3000;
			trap_Trace(&tr, nodetable[nodenum].origin, trMins, trMaxs, downVec, ((1<<10)-1), (0x00000001|0x00001000));

			if ((tr.entityNum >= ((1<<10)-2) || g_entities[tr.entityNum].s.eType == ET_TERRAIN) && tr.endpos[2] < terrain->r.absmin[2]+750)
			{ 
				((nodetable[nodenum].origin)[0]=(tr.endpos)[0],(nodetable[nodenum].origin)[1]=(tr.endpos)[1],(nodetable[nodenum].origin)[2]=(tr.endpos)[2]);
				nodenum++;
			}
			else
			{
				((nodetable[nodenum].origin)[0]=(nodetable[nodenum].origin)[1]=(nodetable[nodenum].origin)[2]=0);
			}
	...
	while (i < gSpawnPointNum-1)
	{
		...
		{ 
			...
			{ 
				...
				{ 
					CreateNewWP_InsertUnder(gSpawnPoints[i+1]->s.origin, 0x00800000, gWPNum-1);
				}
			}
...
#line 3246 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"




}
#line 3252 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
{ 
	...
	while (i < gWPNum-1)
	{ 
		((v)[0]=(gWPArray[i]->origin)[0]-(gWPArray[i+1]->origin)[0],(v)[1]=(gWPArray[i]->origin)[1]-(gWPArray[i+1]->origin)[1],(v)[2]=(gWPArray[i]->origin)[2]-(gWPArray[i+1]->origin)[2]);
...
#line 3345 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
...
void LoadPath_ThisLevel(void)
{
	...
	{ 
		...
		trap_Cvar_Register(&bot_normgpath, "bot_normgpath", "1", 0x00000200);
		...
		gLevelFlags |= 1;
#line 3375 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
	}
	else
	{
...
gentity_t *GetClosestSpawn(gentity_t *ent)
{
	...
	while (i < level.num_entities)
	{
		...
		if (spawn && spawn->inuse && (!Q_stricmp(spawn->classname, "info_player_start") || !Q_stricmp(spawn->classname, "info_player_deathmatch")) )
		{
			...
			((vSub)[0]=(ent->client->ps.origin)[0]-(spawn->r.currentOrigin)[0],(vSub)[1]=(ent->client->ps.origin)[1]-(spawn->r.currentOrigin)[1],(vSub)[2]=(ent->client->ps.origin)[2]-(spawn->r.currentOrigin)[2]);
...
int AcceptBotCommand(char *cmd, gentity_t *pl)
{
	...
	vmCvar_t mapname;
#line 3509 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
	...
	i = 0;
	FlagsFromArgument = 0;
	OptionalSArgument = ((void *)0);
	RequiredSArgument = ((void *)0);
	...
	{
		G_Printf("^3" "bot_wp_add" "^7" " - Add a waypoint (optional int parameter will insert the point after the specified waypoint index in a trail)\n\n");
		G_Printf("^3" "bot_wp_rem" "^7" " - Remove a waypoint (removes last unless waypoint index is specified as a parameter)\n\n");
		G_Printf("^3" "bot_wp_addflagged" "^7" " - Same as wp_add, but adds a flagged point (type bot_wp_addflagged for help)\n\n");
		G_Printf("^3" "bot_wp_switchflags" "^7" " - Switches flags on an existing waypoint (type bot_wp_switchflags for help)\n\n");
		G_Printf("^3" "bot_wp_tele" "^7" " - Teleport yourself to the specified waypoint's location\n");
		G_Printf("^3" "bot_wp_killoneways" "^7" " - Removes oneway (backward and forward) flags on all waypoints in the level\n\n");
		G_Printf("^3" "bot_wp_save" "^7" " - Saves all waypoint data into a file for later use\n");
	...
	if (Q_stricmp (cmd, "bot_wp_tele") == 0)
	{
		...
		{
			G_Printf("^3" "You didn't specify an index. Assuming last.\n");
	...
	if (Q_stricmp (cmd, "bot_wp_spawntele") == 0)
	{
		...
		if (closestSpawn)
		{
			((pl->client->ps.origin)[0]=(closestSpawn->r.currentOrigin)[0],(pl->client->ps.origin)[1]=(closestSpawn->r.currentOrigin)[1],(pl->client->ps.origin)[2]=(closestSpawn->r.currentOrigin)[2]);
		}
		return 1;
	}
	...
	if (Q_stricmp (cmd, "bot_wp_addflagged") == 0)
	{
		...
		while (RequiredSArgument[i])
		{
			...
			{
				FlagsFromArgument |= 0x00000010;
			}
			else if (RequiredSArgument[i] == 'd')
			{
				FlagsFromArgument |= 0x00000020;
			}
			else if (RequiredSArgument[i] == 'c')
			{
				FlagsFromArgument |= 0x00000800;
			}
			else if (RequiredSArgument[i] == 'f')
			{
				FlagsFromArgument |= 0x00001000;
			}
			else if (RequiredSArgument[i] == 's')
			{
				FlagsFromArgument |= 0x00002000;
			}
			else if (RequiredSArgument[i] == 'x')
			{
				FlagsFromArgument |= 0x00004000;
			}
			else if (RequiredSArgument[i] == 'y')
			{
				FlagsFromArgument |= 0x00008000;
			}
			else if (RequiredSArgument[i] == 'g')
			{
				FlagsFromArgument |= 0x00010000;
			}
			else if (RequiredSArgument[i] == 'n')
			{
				FlagsFromArgument |= 0x00000400;
			}
			else if (RequiredSArgument[i] == 'm')
			{
				FlagsFromArgument |= 0x00200000;
			}
	...
	if (Q_stricmp (cmd, "bot_wp_switchflags") == 0)
	{
		...
		{
			G_Printf("^3" "Waypoint number (to modify) needed for bot_wp_switchflags\nSyntax: bot_wp_switchflags <flags> <n>\n");
		}
		return 1;
	}
	...
	if (Q_stricmp (cmd, "bot_wp_killoneways") == 0)
	{
		...
		while (i < gWPNum)
		{
			if (gWPArray[i] && gWPArray[i]->inuse)
			{
				if (gWPArray[i]->flags & 0x00004000)
				{
					gWPArray[i]->flags -= 0x00004000;
				}
				if (gWPArray[i]->flags & 0x00008000)
				{
					gWPArray[i]->flags -= 0x00008000;
				}
			}
...
#line 3811 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\ai_wpnav.c"
