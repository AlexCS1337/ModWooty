...
void SaberUpdateSelf(gentity_t *ent)
{
	...
	if (
		/////g_entities[ent->r.ownerNum].client->ps.weapon != WP_SABER ||
...
void WP_SaberRadiusDamage( gentity_t *ent, vec3_t point, float radius, int damage, float knockBack )
{
	...
	else
	{
		...
		for ( i = 0; i < numEnts; i++ )
		{
			...
			if ( radiusEnt == ent )
			{//Skip myself
				/////maybe not, see if it improves staff
				/////continue;
			}
...
void saberReactivate(gentity_t *saberent, gentity_t *saberOwner)
{ 
/////	*randent = &g_entities[level.sortedClients[Q_irand(0,2)]];
	...
	saberent->s.pos.trType = TR_GRAVITY;//TR_LINEAR;
	saberent->s.eType = ET_GENERAL;
	saberent->s.eFlags = 0;

	saberent->parent = saberOwner;
	/////saberent->parent = randent;
...
#define SABER_RETRIEVE_DELAY 250 //3 seconds for now. This will leave you nice and open if you lose your saber.
...
{	
	...
	/////!
	/*
	randent->client->saberStoredIndex = saberOwner->client->saberStoredIndex;
	saberent->r.ownerNum = randent->client->ps.clientNum;
	*saberOwner = *randent;*/

	/*if (randent->client->ps.clientNum != saberOwner->client->ps.clientNum && randent->client){
		

		saberOwner->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] = 0;
		saberOwner->client->ps.stats[STAT_WEAPONS] |= (1 << WP_MELEE);
		saberOwner->client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_SABER);
		
		randent->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] = 3;
		randent->client->ps.stats[STAT_WEAPONS] |= (1 << WP_SABER);
		randent->client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_MELEE);
...
{
	gentity_t *saberOwner = &g_entities[saberent->r.ownerNum];
	/////gentity_t *saberOwner = &g_entities[level.sortedClients[Q_irand(0,2)]];
	vec3_t dir;
	float ownerLen;

	////*saberOwner = &g_entities[level.sortedClients[level.numConnectedClients]];
...
#define SABER_MAX_THROW_DISTANCE 600///700
...
void saberFirstThrown(gentity_t *saberent)
{
	...
	if ((level.time - saberOwn->client->ps.saberDidThrowTime) > 500)
	{
		...
		else if ((level.time - saberOwn->client->ps.saberDidThrowTime) > 60000)//6000
	...
	if ( trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid )
	{
		...
		{//we hit an entity
			...
			{//hurt it
				...
				if (1==2)//(d_saberKickTweak.integer)
				{
					G_Damage( hitEnt, ent, ent, kickDir, trace.endpos, kickDamage*0.33f, DAMAGE_NO_KNOCKBACK, MOD_MELEE );
				}
				else
				{
					G_Damage( hitEnt, ent, ent, kickDir, trace.endpos, (int)kickDamage*(ent->client->sess.str/18.0f), DAMAGE_NO_KNOCKBACK, MOD_MELEE );
				}
			}
			if ( hitEnt->client 
...
extern void G_GrabSomeMofos(gentity_t *self)
{
    ...
    VectorSet(flatAng, 0.0f/*0.0f*/, self->client->ps.viewangles[1], 0.0f/*0.0f*/);
	trap_G2API_GetBoltMatrix(self->ghoul2, 0, ri->handRBolt /*ri->handRBolt*/, &boltMatrix, flatAng, self->client->ps.origin,
		level.time, NULL, self->modelScale);
	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, pos);

	VectorSet(grabMins, -4.0f/*-4.0f*/,-4.0f/*-4.0f*/,-4.0f/*-4.0f*/);
	VectorSet(grabMaxs, 4.0f/*4.0f*/, 4.0f/*4.0f*/, 4.0f/*4.0f*/);
...
//	vec3_t grabMins, grabMaxs;
   ...
   //// VectorSet(flatAng, 0.0f, self->client->ps.viewangles[1], 0.0f);
	...
	////VectorSet(grabMins, -400.0f, -400.0f, -400.0f);
	////VectorSet(grabMaxs, 400.0f, 400.0f, 400.0f);

	//trace from my origin to my hand, if we hit anyone then get 'em
	////trap_G2Trace( &trace, self->client->ps.origin, grabMins, grabMaxs, pos, self->s.number, MASK_SHOT, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
    
	if (1/*trace.fraction != 1.0f &&
		trace.entityNum < ENTITYNUM_WORLD*/)
	{
		////gentity_t *grabbed = &g_entities[trace.entityNum];
		...
		{ //grabbed an active player/npc
			...
			G_Damage(grabbed, grabbed, grabbed, NULL, grabbed->client->ps.origin, (int)(Q_irand( 1, 30)*self->client->saber[0].damageScale*grabbed->client->saber[0].damageScale), DAMAGE_NORMAL, MOD_SLIME);
	...
	if (self->client->ps.torsoTimer < 300 && !self->client->grappleState)//400?
	{ //you failed to grab anyone, play the "failed to grab" anim
		////G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KYLE_MISS, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
		
		///self->client->ps.legsAnim = 1261;
		///self->client->ps.torsoAnim = 1261;
		
		///if (self->client->ps.torsoAnim == 1261)
					...
					{ //grab punch
						...
						{ //smack
							if (self->client->ps.torsoTimer < 3400)
							{
								...
								G_Damage(grappler, self, self, NULL, self->client->ps.origin, (int)8.0f*(self->client->sess.str/18.0f), 0, MOD_MELEE);
						...
						{ //smack!
							if (self->client->ps.torsoTimer < 2550)
							{
								...
								G_Damage(grappler, self, self, NULL, self->client->ps.origin, (int)10.0f*(self->client->sess.str/18.0f), 0, MOD_MELEE);
								//G_Sound( grappler, CHAN_AUTO, G_SoundIndex( va( "sound/weapons/melee/punch%d", Q_irand( 1, 4 ) ) ) );
						...
						{ //SMACK!
							if (self->client->ps.torsoTimer < 1300)
							{
								...
								G_Damage(grappler, self, self, NULL, self->client->ps.origin, (int)13.0f*(self->client->sess.str/18.0f), 0, MOD_MELEE);
								//G_Sound( grappler, CHAN_AUTO, G_SoundIndex( va( "sound/weapons/melee/punch%d", Q_irand( 1, 4 ) ) ) );
					...
					{ //knee throw
						...
						{ //knee to the face
							if (self->client->ps.torsoTimer < 3200)
							{
								...
								G_Damage(grappler, self, self, NULL, self->client->ps.origin, (int)14.0f*(self->client->sess.str/18.0f), 0, MOD_MELEE);
								//G_Sound( grappler, CHAN_AUTO, G_SoundIndex( va( "sound/weapons/melee/punch%d", Q_irand( 1, 4 ) ) ) );
...
nextStep:
	if (self->client->ps.fd.forcePowersActive & (1 << FP_RAGE))
	{
		animSpeedScale = 2;
			...
			///animSpeedScale = (self->client->sess.dex/18);
	}
	...
	////if (self->client->ps.saberEntityNum && self->client->buttons & BUTTON_ATTACK){
		///trap_SendServerCommand( -1, va("print \"pm_time %i chain %i\n\"",  self->client->ps.pm_time, self->client->ps.saberAttackChainCount) );
	////}

	if (self->client->ps.saberAttackChainCount > 8){
		///Cmd_DebugSetSaberMove_f(self);
		...
		self->client->ps.velocity[2] +=Q_irand(25,50); ///Q_irand(self->client->sess.con*1.5f,self->client->sess.con*3.5f) ;
		...
		//self->client->ps.viewangles[0] *= 2;
		//self->client->ps.viewangles[1] /= 2;
		//self->client->ps.viewangles[PITCH] *= 3;
		///self->client->ps.yawAngle += 5;
		///self->client->ps.pitchAngle += Q_irand(0,10);
	}
	...
	{ //do the thrown-saber stuff
		...
		if (saberent)
		{
			if (!self->client->ps.saberEntityState && self->client->ps.saberEntityNum)
			{
				...
				saberent->s.apos.trType = TR_LINEAR;///TR_LINEAR;
				saberent->s.apos.trDelta[0] = 0;
				saberent->s.apos.trDelta[1] = 800;
				saberent->s.apos.trDelta[2] = 0;

				saberent->s.pos.trType = TR_LINEAR;///TR_LINEAR;
				saberent->s.eType = ET_GENERAL;
				saberent->s.eFlags = 0;
...
void WP_SaberBlockNonRandom( gentity_t *self, vec3_t hitloc, qboolean missileBlock )
{
	...
	if (self->client->ps.weapon == WP_MELEE)
	{
		...
		switch ( rand() % 8){
		case 1:
		//NPC_SetAnim( self, SETANIM_BOTH, BOTH_VS_LEANR, 2);
		G_SetAnim(self, NULL, SETANIM_BOTH, BOTH_VS_LEANR, 2, 0);
		case 2:
		//NPC_SetAnim( self, SETANIM_BOTH, BOTH_CROUCH1IDLE, 2);
		G_SetAnim(self, NULL, SETANIM_BOTH, BOTH_CROUCH1IDLE, 2, 0);
		case 3:
		//NPC_SetAnim( self, SETANIM_BOTH, BOTH_COME_ON1, 2);
		G_SetAnim(self, NULL, SETANIM_BOTH, BOTH_COME_ON1, 2, 0);
		case 4:
		//NPC_SetAnim( self, SETANIM_BOTH, BOTH_STAND5, 2);
		G_SetAnim(self, NULL, SETANIM_BOTH, BOTH_STAND5, 2, 0);
		case 5:
		//NPC_SetAnim( self, SETANIM_BOTH, BOTH_VS_MOUNTTHROW, 2);
		G_SetAnim(self, NULL, SETANIM_BOTH, BOTH_VS_MOUNTTHROW, 2, 0);
		default:
		//NPC_SetAnim( self, SETANIM_BOTH, BOTH_VS_LEANL, 2);
		G_SetAnim(self, NULL, SETANIM_BOTH, BOTH_VS_LEANL, 2, 0);
		}
	}
...
int WP_SaberCanBlock(gentity_t *self, vec3_t point, int dflags, int mod, qboolean projectile, int attackStr)
{
	...
	*/
	...
	{
		blockFactor *= 2.5f;//90% deflect peel (or is it?)
		return 0;
	}
	if (projectile)
	{
