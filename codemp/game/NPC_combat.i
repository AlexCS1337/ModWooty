...
{
	if ( self && self->NPC && (self->NPC->scriptFlags&0x00020000) )
	{
		return;
	}
	...
	G_AlertTeam( self, self->enemy, 512, 256 );	
}
...
void G_AttackDelay( gentity_t *self, gentity_t *enemy )
{
	if ( enemy && self->client && self->NPC )
	{
		...
		((dir)[0]=(self->client->renderInfo.eyePoint)[0]-(enemy->r.currentOrigin)[0],(dir)[1]=(self->client->renderInfo.eyePoint)[1]-(enemy->r.currentOrigin)[1],(dir)[2]=(self->client->renderInfo.eyePoint)[2]-(enemy->r.currentOrigin)[2]);
		VectorNormalize( dir );
		AngleVectors( self->client->renderInfo.eyeAngles, fwd, ((void *)0), ((void *)0) );
		
		
		attDelay = (4-g_spskill.integer)*500;
		if ( self->client->playerTeam == NPCTEAM_PLAYER )
		{
			attDelay = 1200-attDelay;
		}
		...
		case WP_BRYAR_PISTOL:
			break;
		case WP_BLASTER:
			if ( self->NPC->scriptFlags & 0x00000040 )
			{
				attDelay += Q_irand( 0, 500 );
			}
			else
...
#line 384 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_combat.c"
...
void ChangeWeapon( gentity_t *ent, int newWeapon ) 
{
	...
	case WP_SABER:
		ent->NPC->aiFlags &= ~0x00000002;
		ent->NPC->burstSpacing = 0;
		break;

	case WP_DISRUPTOR:
		ent->NPC->aiFlags &= ~0x00000002;
		if ( ent->NPC->scriptFlags & 0x00000040 )
		{
			...
			case 0:
				...
				break;
			case 1:
				ent->NPC->burstSpacing = 2000;
				break;
			case 2:
				ent->NPC->burstSpacing = 1500;
				break;
			}
		}
		else
	...
	case WP_BOWCASTER:
		...
		if ( g_spskill.integer == 0 )
			...
			ent->NPC->burstSpacing = 750;
		else 
			ent->NPC->burstSpacing = 500;
		break;
	...
	case WP_EMPLACED_GUN:
		...
		else
		{
			...
			if ( ent->parent ) 
			{
				if ( g_spskill.integer == 0 )
				{
					ent->NPC->burstSpacing = ent->parent->wait + 400;
					ent->NPC->burstMin = ent->NPC->burstMax = 1; 
				}
				else if ( g_spskill.integer == 1 )
				{
					ent->NPC->burstSpacing = ent->parent->wait + 200;
				}
				else 
				{
					ent->NPC->burstSpacing = ent->parent->wait;
				}
			}
			else
			{
...
void WeaponThink( qboolean inCombat ) 
{
	...
	if(NPC->client->ps.ammo[ weaponData[client->ps.weapon].ammoIndex ] < 10)	
...
qboolean ShotThroughGlass (trace_t *tr, gentity_t *target, vec3_t spot, int mask)
{
	...
	if(hit != target && EntIsGlass(hit))
	{
		...
		trap_Trace (tr, muzzle, ((void *)0), ((void *)0), spot, skip, mask );
		return qtrue;
	}
...
qboolean CanShoot ( gentity_t *ent, gentity_t *shooter ) 
{
	...
	trap_Trace ( &tr, muzzle, ((void *)0), ((void *)0), spot, shooter->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	...
	if ( ShotThroughGlass( &tr, ent, spot, (0x00000001|0x00000100|0x00000200|0x00001000) ) )
	{
		traceEnt = &g_entities[ tr.entityNum ];
	}
	...
	((diff)[0]=(spot)[0]-(tr.endpos)[0],(diff)[1]=(spot)[1]-(tr.endpos)[1],(diff)[2]=(spot)[2]-(tr.endpos)[2]);
	if(VectorLength(diff) < ((rand () & 0x7fff) / ((float)0x7fff)) * 32)
	{
		return qtrue;
	}
...
void NPC_CheckPossibleEnemy( gentity_t *other, visibility_t vis ) 
{
	...
	if ( NPC->enemy && vis == VIS_FOV ) 
	{
		...
		{
			enemyVisibility = NPC_CheckVisibility ( NPC->enemy, 2|4 );
		}
	...
	if ( vis == VIS_FOV ) 
	{
		...
		((NPCInfo->enemyLastSeenLocation)[0]=(other->r.currentOrigin)[0],(NPCInfo->enemyLastSeenLocation)[1]=(other->r.currentOrigin)[1],(NPCInfo->enemyLastSeenLocation)[2]=(other->r.currentOrigin)[2]);
		NPCInfo->enemyLastHeardTime = 0;
		((NPCInfo->enemyLastHeardLocation)[0]=(NPCInfo->enemyLastHeardLocation)[1]=(NPCInfo->enemyLastHeardLocation)[2]=0);
	} 
	else 
	{
		NPCInfo->enemyLastSeenTime = 0;
		((NPCInfo->enemyLastSeenLocation)[0]=(NPCInfo->enemyLastSeenLocation)[1]=(NPCInfo->enemyLastSeenLocation)[2]=0);
		NPCInfo->enemyLastHeardTime = level.time;
		((NPCInfo->enemyLastHeardLocation)[0]=(other->r.currentOrigin)[0],(NPCInfo->enemyLastHeardLocation)[1]=(other->r.currentOrigin)[1],(NPCInfo->enemyLastHeardLocation)[2]=(other->r.currentOrigin)[2]);
	}
}
...
qboolean NPC_EnemyTooFar(gentity_t *enemy, float dist, qboolean toShoot)
{
	...
	if(!dist)
	{
		((vec)[0]=(NPC->r.currentOrigin)[0]-(enemy->r.currentOrigin)[0],(vec)[1]=(NPC->r.currentOrigin)[1]-(enemy->r.currentOrigin)[1],(vec)[2]=(NPC->r.currentOrigin)[2]-(enemy->r.currentOrigin)[2]);
...
gentity_t *NPC_PickEnemy( gentity_t *closestTo, int enemyTeam, qboolean checkVis, qboolean findPlayersFirst, qboolean findClosest )
{
	int			num_choices = 0;
	int			choice[128];
	gentity_t	*newenemy = ((void *)0);
	gentity_t	*closestEnemy = ((void *)0);
	int			entNum;
	vec3_t		diff;
	float		relDist;
	float		bestDist = 16777216;
	qboolean	failed = qfalse;
	int			visChecks = (2|4|16);
	int			minVis = VIS_FOV;
						...
						if ( trap_InPVS(newenemy->r.currentOrigin, NPC->r.currentOrigin) )
						{
							if(NPCInfo->behaviorState == BS_INVESTIGATE ||	NPCInfo->behaviorState == BS_PATROL)
							{
								if(!NPC->enemy)
								{
									...
									else if(NPC_CheckVisibility ( newenemy, 2|4|16 ) != VIS_FOV)
									{
										failed = qtrue;
									}
								}
							...
							if ( !failed )
							{
								((diff)[0]=(closestTo->r.currentOrigin)[0]-(newenemy->r.currentOrigin)[0],(diff)[1]=(closestTo->r.currentOrigin)[1]-(newenemy->r.currentOrigin)[1],(diff)[2]=(closestTo->r.currentOrigin)[2]-(newenemy->r.currentOrigin)[2]);
								...
								if ( newenemy->client->hiddenDist > 0 )
								{
									if( relDist > newenemy->client->hiddenDist*newenemy->client->hiddenDist )
									{
										...
										if ( VectorLengthSquared( newenemy->client->hiddenDir ) )
										{
											...
											dot = ((newenemy->client->hiddenDir)[0]*(diff)[0]+(newenemy->client->hiddenDir)[1]*(diff)[1]+(newenemy->client->hiddenDir)[2]*(diff)[2]); 
											if ( dot > 0.5 )
											{
												failed = qtrue;
											}
									...
									{
										Debug_Printf(&debugNPCAI, 3, "%s saw %s trying to hide - hiddenDist %f\n", NPC->targetname, newenemy->targetname, newenemy->client->hiddenDist );
									}
								}
								...
								if(!failed)
								{
									...
									else if(!NPC_EnemyTooFar(newenemy, 0, qfalse))
									{
										if(checkVis)
										{
											if( NPC_CheckVisibility ( newenemy, 2|4|16 ) == VIS_FOV )
											{
	...
	for ( entNum = 0; entNum < level.num_entities; entNum++ )
	{
		...
		if ( newenemy != NPC && (newenemy->client ) && !(newenemy->flags & 0x00000020) && !(newenemy->s.eFlags & (1<<8)))
		{
			if ( newenemy->health > 0 )
			{
					...
					if ( newenemy != NPC->lastEnemy )
					{
						...
						else if ( !NPC_EnemyTooFar( newenemy, 0, qfalse ) )
						{
							if ( checkVis )
							{
								...
								if ( NPC_CheckVisibility ( newenemy, 2|16 ) >= VIS_360 )
								{
									choice[num_choices++] = newenemy->s.number;
								}
							}
...
{
	gentity_t	*ally = ((void *)0);
	gentity_t	*closestAlly = ((void *)0);
	int			entNum;
	vec3_t		diff;
	float		relDist;
		...
		if ( ally->client )
		{
			if ( ally->health > 0 )
			{
					 ...
					 NPC->client->playerTeam == NPCTEAM_ENEMY ) )
				{
					...
					((diff)[0]=(NPC->r.currentOrigin)[0]-(ally->r.currentOrigin)[0],(diff)[1]=(NPC->r.currentOrigin)[1]-(ally->r.currentOrigin)[1],(diff)[2]=(NPC->r.currentOrigin)[2]-(ally->r.currentOrigin)[2]);
					...
					if ( relDist < bestDist )
					{
						if ( facingEachOther )
						{
							...
							AngleVectors( ally->client->ps.viewangles, vf, ((void *)0), ((void *)0) );
							VectorNormalize(vf);
							dot = ((diff)[0]*(vf)[0]+(diff)[1]*(vf)[1]+(diff)[2]*(vf)[2]);
							...
							AngleVectors( NPC->client->ps.viewangles, vf, ((void *)0), ((void *)0) );
							VectorNormalize(vf);
							dot = ((diff)[0]*(vf)[0]+(diff)[1]*(vf)[1]+(diff)[2]*(vf)[2]);
						...
						if ( NPC_CheckVisibility ( ally, 2|16 ) >= VIS_360 )
						{
							bestDist = relDist;
							closestAlly = ally;
						}
...
gentity_t *NPC_CheckEnemy( qboolean findNew, qboolean tooFarOk, qboolean setEnemy )
{
	...
	if ( NPC->enemy )
	{
		if ( NPC->enemy->health <= 0 || NPC->enemy->flags & 0x00000020 )
		{
			if ( setEnemy )
			{
				G_ClearEnemy( NPC );
...
qboolean NPC_ClearShot( gentity_t *ent )
{
	...
	if( NPC->s.weapon == WP_BLASTER  ) 
	{
		...
		trap_Trace ( &tr, muzzle, mins, maxs, ent->r.currentOrigin, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	}
	else
	{
		trap_Trace ( &tr, muzzle, ((void *)0), ((void *)0), ent->r.currentOrigin, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	}
...
int NPC_ShotEntity( gentity_t *ent, vec3_t impactPos )
{
	...
	if ( NPC->s.weapon == WP_THERMAL )
	{
		...
		((angles)[0]=(0), (angles)[1]=(NPC->client->ps.viewangles[1]), (angles)[2]=(0));
		AngleVectors( angles, forward, ((void *)0), ((void *)0) );
		((end)[0]=(muzzle)[0]+(forward)[0]*(8),(end)[1]=(muzzle)[1]+(forward)[1]*(8),(end)[2]=(muzzle)[2]+(forward)[2]*(8));
		end[2] += 24;
	...
	if( NPC->s.weapon == WP_BLASTER  ) 
	{
		...
		trap_Trace ( &tr, muzzle, mins, maxs, targ, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	}
	else
	{
		trap_Trace ( &tr, muzzle, ((void *)0), ((void *)0), targ, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	}
...
qboolean NPC_EvaluateShot( int hit, qboolean glassOK )
{
	...
	if ( hit == NPC->enemy->s.number || (&g_entities[hit] != ((void *)0) && (g_entities[hit].r.svFlags&0x08000000)) )
	{
		return qtrue;
	}
	return qfalse;
...
qboolean NPC_CheckDefend (float scale)
{
	...
	if((float)(NPCInfo->stats.evasion) > ((rand () & 0x7fff) / ((float)0x7fff)) * 4 * scale)
		return qtrue;
...
qboolean NPC_CheckCanAttack (float attack_scale, qboolean stationary)
{
	...
	((delta)[0]=(enemy_org)[0]-(muzzle)[0],(delta)[1]=(enemy_org)[1]-(muzzle)[1],(delta)[2]=(enemy_org)[2]-(muzzle)[2]);
	vectoangles ( delta, angleToEnemy );
	distanceToEnemy = VectorNormalize(delta);

	NPC->NPC->desiredYaw = angleToEnemy[1];
	...
	{
		NPC->NPC->desiredPitch = angleToEnemy[0];
	...
	if(enemyVisibility >= VIS_FOV)
	{
		...
		if ( NPC->enemy->client )
		{
			...
			{
				if ( NPC->enemy->client->buttons & 1 )
				{
					if ( NPC_CheckDefend( 1.0 ) )
					{
						attack_ok = qfalse;
						ucmd.upmove = -127;
					}
		...
		if(attack_ok)
		{
			...
			((hitspot)[0]=(muzzle)[0]+(forward)[0]*(distanceToEnemy),(hitspot)[1]=(muzzle)[1]+(forward)[1]*(distanceToEnemy),(hitspot)[2]=(muzzle)[2]+(forward)[2]*(distanceToEnemy));
			trap_Trace( &tr, muzzle, ((void *)0), ((void *)0), hitspot, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
			ShotThroughGlass( &tr, NPC->enemy, hitspot, (0x00000001|0x00000100|0x00000200|0x00001000) );
			...
			((hitspot)[0]=(tr.endpos)[0],(hitspot)[1]=(tr.endpos)[1],(hitspot)[2]=(tr.endpos)[2]);
		...
		if( attack_ok )
		{
			...
			((delta)[0]=(hitspot)[0]-(muzzle)[0],(delta)[1]=(hitspot)[1]-(muzzle)[1],(delta)[2]=(hitspot)[2]-(muzzle)[2]);
			vectoangles ( delta, angleToEnemy );
			NPC->NPC->desiredPitch = angleToEnemy[0];
			NPC_UpdateFiringAngles(qtrue, qfalse);
			...
			if( !dead_on )
			{
				...
				if(traceEnt && (traceEnt->health <= 30 || EntIsGlass(traceEnt)))
				{
					...
					if (0) 
					{
						((diff)[0]=(NPC->r.currentOrigin)[0]-(traceEnt->r.currentOrigin)[0],(diff)[1]=(NPC->r.currentOrigin)[1]-(traceEnt->r.currentOrigin)[1],(diff)[2]=(NPC->r.currentOrigin)[2]-(traceEnt->r.currentOrigin)[2]);
				...
				else
				{
					AngleVectors (client->ps.viewangles, forward, ((void *)0), ((void *)0));
					((hitspot)[0]=(muzzle)[0]+(forward)[0]*(distanceToEnemy),(hitspot)[1]=(muzzle)[1]+(forward)[1]*(distanceToEnemy),(hitspot)[2]=(muzzle)[2]+(forward)[2]*(distanceToEnemy));
					((diff)[0]=(hitspot)[0]-(enemy_org)[0],(diff)[1]=(hitspot)[1]-(enemy_org)[1],(diff)[2]=(hitspot)[2]-(enemy_org)[2]);
					aim_off = VectorLength(diff);
					if(aim_off > ((rand () & 0x7fff) / ((float)0x7fff)) * max_aim_off)
					{
						attack_scale *= 0.75;
...
{
	if(level.numCombatPoints >= 512)
	{

		Com_Printf("^1""ERROR:  Too many combat points, limit is %d\n", 512);
#line 2524 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_combat.c"
		G_FreeEntity(self);
		return;
	}
	...
	if ( G_CheckInSolid( self, qtrue ) )
	{
		...
		Com_Printf( "^1""ERROR: combat point at %s in solid!\n", vtos(self->r.currentOrigin) );
#line 2537 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_combat.c"
	}

	((level.combatPoints[level.numCombatPoints].origin)[0]=(self->r.currentOrigin)[0],(level.combatPoints[level.numCombatPoints].origin)[1]=(self->r.currentOrigin)[1],(level.combatPoints[level.numCombatPoints].origin)[2]=(self->r.currentOrigin)[2]);
...
void CP_FindCombatPointWaypoints( void )
{
	...
	for ( i = 0; i < level.numCombatPoints; i++ )
	{
		...
		if ( level.combatPoints[i].waypoint == -1 )
		{
			Com_Printf( "^1""ERROR: Combat Point at %s has no waypoint!\n", vtos(level.combatPoints[i].origin) );
		}
#line 2562 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_combat.c"
	}
}
...
static int NPC_CollectCombatPoints( const vec3_t origin, const float radius, combatPt_t *points, const int flags )
{
	...
	for ( i = 0; i < level.numCombatPoints; i++ )
	{
		...
		if ( ( flags & 0x00000008 ) && ( level.combatPoints[i].flags & 0x00000001 ) )
			continue;

		
		if ( ( flags & 0x00000004 ) && ( level.combatPoints[i].flags & 0x00000002 ) )
			continue;

		
		if ( ( flags & 0x00000040 ) && ( level.combatPoints[i].flags & 0x00000004 ) )
			continue;
		
		
		if ( ( level.combatPoints[i].flags & 0x00000008 ) && ( ( flags & 0x00000080 ) == qfalse ) )
			continue;
...
{
	combatPt_t		points[512];
	int				best = -1, cost, bestCost = 16777216, waypoint = -1;
	float			dist;
	trace_t			tr;
	float			collRad = 512.0f;
	int				numPoints;
	int				j = 0;
	...
	if ( (flags & 0x00001000) || (flags & 0x00000010) )
	{
		if ( NPC->waypoint == -1 )
		{
	...
	numPoints = NPC_CollectCombatPoints( enemyPosition, collRad, points, flags );
	...
	for ( j = 0; j < numPoints; j++ )
	{
		...
		if ( ( flags & 0x00000001 ) && ( NPC_ClearLOS( level.combatPoints[i].origin, enemyPosition ) == qtrue ) )
			continue;
		...
		if ( ( flags & 0x00000100 ) && ( DistanceSquared( level.combatPoints[i].origin, position ) < modifiedAvoidDist ) )
			continue;
		...
		if ( flags & 0x00000800  )
		{
			...
			((eDir2Me)[0]=(position)[0]-(enemyPosition)[0],(eDir2Me)[1]=(position)[1]-(enemyPosition)[1],(eDir2Me)[2]=(position)[2]-(enemyPosition)[2]);
			VectorNormalize( eDir2Me );

			((eDir2CP)[0]=(level.combatPoints[i].origin)[0]-(enemyPosition)[0],(eDir2CP)[1]=(level.combatPoints[i].origin)[1]-(enemyPosition)[1],(eDir2CP)[2]=(level.combatPoints[i].origin)[2]-(enemyPosition)[2]);
		...
		if ( flags & 0x00000020  )
		{
			...
			((eDir)[0]=(position)[0]-(enemyPosition)[0],(eDir)[1]=(position)[1]-(enemyPosition)[1],(eDir)[2]=(position)[2]-(enemyPosition)[2]);
			...
			{
				((wpOrg)[0]=(level.combatPoints[i].origin)[0],(wpOrg)[1]=(level.combatPoints[i].origin)[1],(wpOrg)[2]=(level.combatPoints[i].origin)[2]);
			}
			((gDir)[0]=(position)[0]-(wpOrg)[0],(gDir)[1]=(position)[1]-(wpOrg)[1],(gDir)[2]=(position)[2]-(wpOrg)[2]);
			VectorNormalize( gDir );

			dot = ((gDir)[0]*(eDir)[0]+(gDir)[1]*(eDir)[1]+(gDir)[2]*(eDir)[2]);
		...
		if ( flags & 0x00001000 )
		{
			...
			if ( waypoint == -1 || level.combatPoints[i].waypoint == -1 || trap_Nav_GetBestNodeAltRoute2( waypoint, level.combatPoints[i].waypoint, -1 ) == -1 )
			{
				if ( !NAV_ClearPathToPoint( NPC, NPC->r.mins, NPC->r.maxs, level.combatPoints[i].origin, NPC->clipmask, ((1<<10)-1) ) )
				{
					continue;
				}
			}
		...
		if ( flags & 0x00000010 && waypoint != -1 && level.combatPoints[i].waypoint != -1 )
		{
...
{
	float	dist, nearestDist = (float)( ( 64 * 1024 ) - ( -64 * 1024 ) )*(float)( ( 64 * 1024 ) - ( -64 * 1024 ) );
	int		nearestPoint = -1;
	int		i;
	...
	for ( i = 0; i < level.numCombatPoints; i++ )
	{
		...
		if ( ( level.combatPoints[i].flags & 0x00000008 ) == qfalse )
			continue;
...
{
	gentity_t *found = g_entities, *bestFound = ((void *)0);
	...
	for ( i = 0; i<level.num_entities; i++)
	{
		...
		if ( CheckItemCanBePickedUpByNPC( found, NPC ) )
		{
			if ( trap_InPVS( found->r.currentOrigin, NPC->r.currentOrigin ) )
			{
				...
				if ( dist < bestDist )
				{
					if ( !trap_Nav_GetBestPathBetweenEnts( NPC, found, 0x00000002 ) 
						|| trap_Nav_GetBestNodeAltRoute2( NPC->waypoint, found->waypoint, -1 ) == -1 )
					{
						if ( NAV_ClearPathToPoint( NPC, NPC->r.mins, NPC->r.maxs, found->r.currentOrigin, NPC->clipmask, ((1<<10)-1) ) )
						{
							bestDist = dist;
							bestFound = found;
						}
...
void NPC_SetPickUpGoal( gentity_t *foundWeap )
{
	...
	((org)[0]=(foundWeap->r.currentOrigin)[0],(org)[1]=(foundWeap->r.currentOrigin)[1],(org)[2]=(foundWeap->r.currentOrigin)[2]);
	org[2] += 24 - (foundWeap->r.mins[2]*-1);
...
void NPC_CheckGetNewWeapon( void )
{
	if ( NPC->s.weapon == WP_NONE && NPC->enemy )
	{
		...
		if ( TIMER_Done( NPC, "panic" ) && NPCInfo->goalEntity == ((void *)0) )
		{
