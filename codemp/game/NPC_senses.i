...
qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2, int ignore, int clipmask)
{
	...
	trap_Trace ( &tr, point1, ((void *)0), ((void *)0), point2, ignore, clipmask );
	if ( tr.fraction == 1.0 ) 
	{
		return qtrue;
	}
	...
	{
		vec3_t	newpoint1;
		((newpoint1)[0]=(tr.endpos)[0],(newpoint1)[1]=(tr.endpos)[1],(newpoint1)[2]=(tr.endpos)[2]);
		trap_Trace (&tr, newpoint1, ((void *)0), ((void *)0), point2, hit->s.number, clipmask );
...
qboolean CanSee ( gentity_t *ent ) 
{
	...
	trap_Trace ( &tr, eyes, ((void *)0), ((void *)0), spot, NPC->s.number, (0x00000001|0x00020000|0x00000002|0x00001000) );
	ShotThroughGlass (&tr, ent, spot, (0x00000001|0x00020000|0x00000002|0x00001000));
	if ( tr.fraction == 1.0 ) 
	{
		return qtrue;
	}
...
qboolean InFront( vec3_t spot, vec3_t from, vec3_t fromAngles, float threshHold )
{
	...
	((dir)[0]=(spot)[0]-(from)[0],(dir)[1]=(spot)[1]-(from)[1],(dir)[2]=(spot)[2]-(from)[2]);
	dir[2] = 0;
	VectorNormalize( dir );

	((angles)[0]=(fromAngles)[0],(angles)[1]=(fromAngles)[1],(angles)[2]=(fromAngles)[2]);
	angles[0] = 0;
	AngleVectors( angles, forward, ((void *)0), ((void *)0) );

	dot = ((dir)[0]*(forward)[0]+(dir)[1]*(forward)[1]+(dir)[2]*(forward)[2]);
...
qboolean InFOV3( vec3_t spot, vec3_t from, vec3_t fromAngles, int hFOV, int vFOV )
{
	...
	((deltaVector)[0]=(spot)[0]-(from)[0],(deltaVector)[1]=(spot)[1]-(from)[1],(deltaVector)[2]=(spot)[2]-(from)[2]);
	vectoangles ( deltaVector, angles );
	
	deltaAngles[0]	= AngleDelta ( fromAngles[0], angles[0] );
	deltaAngles[1]	= AngleDelta ( fromAngles[1], angles[1] );

	if ( fabs ( deltaAngles[0] ) <= vFOV && fabs ( deltaAngles[1] ) <= hFOV ) 
	{
		return qtrue;
	}
...
qboolean InFOV2( vec3_t origin, gentity_t *from, int hFOV, int vFOV ) 
{
	...
	if( from->client )
	{
		((fromAngles)[0]=(from->client->ps.viewangles)[0],(fromAngles)[1]=(from->client->ps.viewangles)[1],(fromAngles)[2]=(from->client->ps.viewangles)[2]);
	}
	else
	{
		((fromAngles)[0]=(from->s.angles)[0],(fromAngles)[1]=(from->s.angles)[1],(fromAngles)[2]=(from->s.angles)[2]);
	}
...
qboolean InFOV ( gentity_t *ent, gentity_t *from, int hFOV, int vFOV ) 
{
	...
	if( from->client )
	{
		if( !VectorCompare( from->client->renderInfo.eyeAngles, vec3_origin ) )
		{
			...
			((fromAngles)[0]=(from->client->renderInfo.eyeAngles)[0],(fromAngles)[1]=(from->client->renderInfo.eyeAngles)[1],(fromAngles)[2]=(from->client->renderInfo.eyeAngles)[2]);
		}
		else
		{
			((fromAngles)[0]=(from->client->ps.viewangles)[0],(fromAngles)[1]=(from->client->ps.viewangles)[1],(fromAngles)[2]=(from->client->ps.viewangles)[2]);
	...
	CalcEntitySpot( ent, SPOT_ORIGIN, spot );
	((deltaVector)[0]=(spot)[0]-(eyes)[0],(deltaVector)[1]=(spot)[1]-(eyes)[1],(deltaVector)[2]=(spot)[2]-(eyes)[2]);
...
int G_CheckAlertEvents( gentity_t *self, qboolean checkSight, qboolean checkSound, float maxSeeDist, float maxHearDist, int ignoreAlert, qboolean mustHaveOwner, int minAlertLevel )
{
	...
	if ( &g_entities[0] == ((void *)0) || g_entities[0].health <= 0 )
	{
	...
	if ( bestSightEvent >= 0 && bestSightAlert > bestSoundAlert )
	{
		...
		((sightDir)[0]=(level.alertEvents[bestSightEvent].position)[0]-(eyePoint)[0],(sightDir)[1]=(level.alertEvents[bestSightEvent].position)[1]-(eyePoint)[1],(sightDir)[2]=(level.alertEvents[bestSightEvent].position)[2]-(eyePoint)[2]);
...
qboolean G_CheckForDanger( gentity_t *self, int alertEvent )
{
	...
	if ( level.alertEvents[alertEvent].level >= AEL_DANGER )
	{
		if ( !level.alertEvents[alertEvent].owner || !level.alertEvents[alertEvent].owner->client || (level.alertEvents[alertEvent].owner!=self&&level.alertEvents[alertEvent].owner->client->playerTeam!=self->client->playerTeam) )
		{
			if ( self->NPC )
			{
				if ( self->NPC->scriptFlags & 0x00008000 )
				{
					return qfalse;
				}
				else
...
void AddSoundEvent( gentity_t *owner, vec3_t position, float radius, alertEventLevel_e alertLevel, qboolean needLOS )
{
	...
	if ( owner == ((void *)0) && alertLevel < AEL_DANGER )	
		return;
	...
	((level.alertEvents[ level.numAlertEvents ].position)[0]=(position)[0],(level.alertEvents[ level.numAlertEvents ].position)[1]=(position)[1],(level.alertEvents[ level.numAlertEvents ].position)[2]=(position)[2]);
	...
	if ( needLOS )
	{
		level.alertEvents[ level.numAlertEvents ].addLight	= 1;	
	}
	else
	{
		level.alertEvents[ level.numAlertEvents ].addLight	= 0;	
	}
...
void ClearPlayerAlertEvents( void )
{
	...
	for ( i = 0; i < curNumAlerts; i++ )
	{
		...
		if ( level.alertEvents[i].timestamp && level.alertEvents[i].timestamp + 200 < level.time )
		{
			...
			if ( level.numAlertEvents > 0 )
			{
				if ( (i+1) < 32 )
				{
					memmove( &level.alertEvents[i], &level.alertEvents[i+1], sizeof(alertEvent_t)*(32-(i+1) ) );
				}
			}
			else
			{
	...
	(void)( (!!(level.numAlertEvents >= 0)) || (_wassert(L"level.numAlertEvents >= 0", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_senses.c", 687), 0) );

	if ( eventClearTime < level.time )
	{
		eventClearTime = level.time + 200;
	}
}

qboolean RemoveOldestAlert( void )
{
	int	oldestEvent = -1, oldestTime = 16777216;
	int i;
	...
	if ( oldestEvent != -1 )
	{
		...
		if ( level.numAlertEvents > 0 )
		{
			if ( (oldestEvent+1) < 32 )
			{
				memmove( &level.alertEvents[oldestEvent], &level.alertEvents[oldestEvent+1], sizeof(alertEvent_t)*(32-(oldestEvent+1) ) );
			}
		}
		else
		{
	...
	return (level.numAlertEvents<32);
}
...
qboolean G_ClearLOS( gentity_t *self, const vec3_t start, const vec3_t end )
{
	...
	trap_Trace ( &tr, start, ((void *)0), ((void *)0), end, ((1<<10)-1), 0x00008000 );
	while ( tr.fraction < 1.0 && traceCount < 3 )
	{
		if ( tr.entityNum < ((1<<10)-2) )
		{
			if ( &g_entities[tr.entityNum] != ((void *)0) && (g_entities[tr.entityNum].r.svFlags&0x08000000) )
			{
				trap_Trace ( &tr, tr.endpos, ((void *)0), ((void *)0), end, tr.entityNum, (0x00000001|0x00020000|0x00000002|0x00001000) );
				traceCount++;
				continue;
			}
		}
...
float NPC_GetHFOVPercentage( vec3_t spot, vec3_t from, vec3_t facing, float hFOV )
{
	...
	delta = fabs( AngleDelta ( facing[1], angles[1] ) );
...
float NPC_GetVFOVPercentage( vec3_t spot, vec3_t from, vec3_t facing, float vFOV )
{
	...
	delta = fabs( AngleDelta ( facing[0], angles[0] ) );
...
int G_FindLocalInterestPoint( gentity_t *self )
{
	...
	for ( i = 0; i < level.numInterestPoints; i++ )
	{
		...
		{
			((diffVec)[0]=(level.interestPoints[i].origin)[0]-(eyes)[0],(diffVec)[1]=(level.interestPoints[i].origin)[1]-(eyes)[1],(diffVec)[2]=(level.interestPoints[i].origin)[2]-(eyes)[2]);
			...
			if ( dist < ( 256 * 256 ) && dist < bestDist )
			{
				if ( G_ClearLineOfSight( eyes, level.interestPoints[i].origin, self->s.number, (0x00000001|0x00020000|0x00000002|0x00001000) ) )
				{
					bestDist = dist;
					bestPoint = i;
				}
	...
	if ( bestPoint != ((1<<10)-1) && level.interestPoints[bestPoint].target )
	{
...
void SP_target_interest( gentity_t *self )
{
	...
	((level.interestPoints[level.numInterestPoints].origin)[0]=(self->r.currentOrigin)[0],(level.interestPoints[level.numInterestPoints].origin)[1]=(self->r.currentOrigin)[1],(level.interestPoints[level.numInterestPoints].origin)[2]=(self->r.currentOrigin)[2]);
