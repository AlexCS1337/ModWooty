...
qboolean Wampa_CheckRoar( gentity_t *self )
{
	if ( self->wait < level.time )
	{
		...
		NPC_SetAnim( self, 1|2, Q_irand(BOTH_GESTURE1,BOTH_GESTURE2), (1|2) );
...
void Wampa_Slash( int boltIndex, qboolean backhand )
{
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( DistanceSquared( radiusEnt->r.currentOrigin, boltOrg ) <= radiusSquared )
		{
			...
			G_Damage( radiusEnt, NPC, NPC, vec3_origin, radiusEnt->r.currentOrigin, damage, ((backhand)?0x00000002:(0x00000002|0x00000004)), MOD_MELEE );
			if ( backhand )
			{
...
void Wampa_Attack( float distance, qboolean doCharge )
{
	...
	if ( NPC->client->ps.legsAnim == BOTH_ATTACK1 && distance > (NPC->r.maxs[0]+48) )
	{
		ucmd.buttons |= 16;
		Wampa_Move( 1 );
	}
...
void Wampa_Combat( void )
{
	...
	else
	{
		...
		qboolean	advance = (qboolean)( distance > (NPC->r.maxs[0]+48) ? qtrue : qfalse  );
		...
		if (( advance || NPCInfo->localState == 1 ) && TIMER_Done( NPC, "attacking" )) 
		{
