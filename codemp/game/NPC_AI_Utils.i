...
int AI_GetGroupSize2( gentity_t *ent, int radius )
{
	if ( ( ent == ((void *)0) ) || ( ent->client == ((void *)0) ) )
		return -1;
...
{
	int	markerWP = -1;
	int	cost, bestCost = 16777216;
	int	closest = ((1<<10)-1);
	int i;
	
	if ( group == ((void *)0) || group->numGroup <= 0 )
	{
		return ((1<<10)-1);
	}
...
void AI_SetClosestBuddy( AIGroupInfo_t *group )
{
	...
	for ( i = 0; i < group->numGroup; i++ )
	{
		group->member[i].closestBuddy = ((1<<10)-1);
...
{
	AIGroupMember_t bestMembers[32];
	int				i, j, k;
	qboolean		sort = qfalse;

	if ( group->enemy != ((void *)0) )
	{
		group->enemyWP = NAV_FindClosestWaypointForEnt( group->enemy, -1 );
	}
	else
	{
		group->enemyWP = -1;
	...
	for ( i = 0; i < group->numGroup; i++ )
	{
		if ( group->enemyWP == -1 )
		{
			group->member[i].waypoint = -1;
			group->member[i].pathCostToEnemy = 16777216;
		}
		else
		{
			group->member[i].waypoint = NAV_FindClosestWaypointForEnt( group->enemy, -1 );
			if ( group->member[i].waypoint != -1 )
			{
	...
	if ( sort )
	{
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			for ( j = 0; j < group->numGroup; j++ )
			{
				if ( bestMembers[j].number != ((1<<10)-1) )
				{
...
qboolean AI_ValidateNoEnemyGroupMember( AIGroupInfo_t *group, gentity_t *member )
{
	...
	if ( group->commander )
	{
		((center)[0]=(group->commander->r.currentOrigin)[0],(center)[1]=(group->commander->r.currentOrigin)[1],(center)[2]=(group->commander->r.currentOrigin)[2]);
	}
	else
	{
		if ( group->member[0].number < 0 || group->member[0].number >= ((1<<10)-2) )
		{
			return qfalse;
		}
		((center)[0]=(g_entities[group->member[0].number].r.currentOrigin)[0],(center)[1]=(g_entities[group->member[0].number].r.currentOrigin)[1],(center)[2]=(g_entities[group->member[0].number].r.currentOrigin)[2]);
	}
	
	if ( DistanceSquared( center, member->r.currentOrigin ) > 147456 )
	{
		return qfalse;
	}
...
qboolean AI_ValidateGroupMember( AIGroupInfo_t *group, gentity_t *member )
{
	...
	if ( member->client == ((void *)0) )
		return qfalse;

	
	if ( member->NPC == ((void *)0) )
		return qfalse;
	...
	if ( member->NPC->scriptFlags&0x00020000 )
		return qfalse;

	
	if ( member->NPC->group != ((void *)0) && member->NPC->group != group )
	{
		return qfalse;
	}
...
void AI_GetGroup( gentity_t *self )
{
	...
	if ( self->NPC->scriptFlags&0x00020000 )
	{
		self->NPC->group = ((void *)0);
		return;
	}
	...
	if ( self->NPC->group->enemy )
	{
		...
		((self->NPC->group->enemyLastSeenPos)[0]=(self->NPC->group->enemy->r.currentOrigin)[0],(self->NPC->group->enemyLastSeenPos)[1]=(self->NPC->group->enemy->r.currentOrigin)[1],(self->NPC->group->enemyLastSeenPos)[2]=(self->NPC->group->enemy->r.currentOrigin)[2]);
	}
	...
	for ( i = 0; i < level.num_entities ; i++)
	{
		...
		if ( self->NPC->group->numGroup >= (32 - 1) )
		{
			break;
		}
	}
...
{
	gentity_t *member = ((void *)0);
	int i;

	group->commander = ((void *)0);
	for ( i = 0; i < group->numGroup; i++ )
	{
...
void AI_DeleteGroupMember( AIGroupInfo_t *group, int memberNum )
{
	...
	{
		g_entities[group->member[memberNum].number].NPC->group = ((void *)0);
	}
	...
	if ( !noflee )
	{
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			if ( member->NPC->rank < RANK_ENSIGN )
			{
				...
				else if ( DistanceSquared( member->r.currentOrigin, self->r.currentOrigin ) < 65536 )
				{
					ST_StartFlee( member, group->enemy, member->r.currentOrigin, AEL_DANGER_GREAT, 3000, 5000 );
				}
				else
...
void AI_GroupUpdateEnemyLastSeen( AIGroupInfo_t *group, vec3_t spot )
{
	...
	((group->enemyLastSeenPos)[0]=(spot)[0],(group->enemyLastSeenPos)[1]=(spot)[1],(group->enemyLastSeenPos)[2]=(spot)[2]);
}
...
qboolean AI_RefreshGroup( AIGroupInfo_t *group )
{
	...
	for ( i = 0; i < 32; i++ ) 
	{
		...
		else
		{
			...
			{
				if ( level.groups[i].numGroup+group->numGroup < (32 - 1) )
				{
					...
					for ( j = 0; j < group->numGroup; j++ )
					{
						...
						if ( level.groups[i].enemy == ((void *)0) )
						{
	...
	for ( i = 0; i < group->numGroup; i++ )
	{
		...
		if ( group->commander && debugNPCAI.integer )
		{
			...
			G_TestLine(group->commander->r.currentOrigin, member->r.currentOrigin, 0x00000ff, 50);
		}
	}
	if ( group->enemy )
	{
