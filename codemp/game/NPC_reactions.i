...
#line 27 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_reactions.c"
...
static void NPC_CheckAttacker( gentity_t *other, int mod )
{
	...
	if ( other == &g_entities[0] )
	{
		...
		if ( ((rand () & 0x7fff) / ((float)0x7fff)) > luckThreshold )
		{
			G_ClearEnemy( other );
			other->enemy = NPC;
		}
...
{
	if ( !self->NPC || !(self->NPC->aiFlags&0x00100000) )
	{
...
void NPC_ChoosePainAnimation( gentity_t *self, gentity_t *other, vec3_t point, int damage, int mod, int hitLoc, int voiceEvent )
{
	...
	if ( level.time < self->painDebounceTime &&  mod != MOD_MELEE ) 
	{
		return;
	}
 ...
 PM_SpinningAnim( self->client->ps.legsAnim )
			...
			else
			{
				...
				if (pain_anim != -1)
				{
					NPC_SetAnim( self, parts, pain_anim, 1|2 );
				}
			}
			if ( voiceEvent != -1 )
			{
...
void NPC_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	
	{
		if ( other != self->enemy && self != other->enemy )
		{
			...
			else if ( self->NPC && !other->s.number )
			{
				...
				else
				{
					...
					other->flags &= ~0x00000020;
					
					self->r.svFlags &= ~0x00008000;
					G_SetEnemy( self, other );
					
					self->NPC->scriptFlags &= ~(0x00004000|0x00000001|0x00000002|0x00000200|0x00010000);
					self->NPC->scriptFlags |= (0x00000400|0x00080000);
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) 
{
	...
	if ( self->message && self->health <= 0 )
	{
		...
		if (other && other->client && other->s.number < 32)
		{
...
void NPC_Respond( gentity_t *self, int userNum )
{
	...
	if ( event != -1 )
	{
		...
		qboolean addFlag = (self->NPC->scriptFlags&0x00000200);
		self->NPC->scriptFlags &= ~0x00000200;
...
void NPC_Use( gentity_t *self, gentity_t *other, gentity_t *activator ) 
{
	...
	if(self->client && self->NPC)
	{
		...
		if ( self->behaviorSet[BSET_USE] )
		{
			...
			&&  !(self->NPC->scriptFlags&0x00000080) )
