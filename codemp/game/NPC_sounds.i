...
void G_AddVoiceEvent( gentity_t *self, int event, int speakDebounceTime )
{
	...
	if ( (self->NPC->scriptFlags&0x00000200) && ( (event >= EV_ANGER1 && event <= EV_VICTORY3) || (event >= EV_CHASE1 && event <= EV_SUSPICIOUS5) ) )
	{
		return;
	}
	
	if ( (self->NPC->scriptFlags&0x02000000) && (event >= EV_GIVEUP1 && event <= EV_SUSPICIOUS5) )
	{
		return;
	}
