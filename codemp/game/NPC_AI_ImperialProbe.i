...
{	
	...
	else
	{
		...
		if ( goal )
		{
			...
			else
			{
				...
				{
					NPC->client->ps.velocity[2] *= 0.85f;
	...
	{
		NPC->client->ps.velocity[0] *= 0.85f;
	...
	{
		NPC->client->ps.velocity[1] *= 0.85f;
...
void ImperialProbe_Strafe( void )
{
	...
	dir = ( rand() & 1 ) ? -1 : 1;
	((end)[0]=(NPC->r.currentOrigin)[0]+(right)[0]*(200 * dir),(end)[1]=(NPC->r.currentOrigin)[1]+(right)[1]*(200 * dir),(end)[2]=(NPC->r.currentOrigin)[2]+(right)[2]*(200 * dir));
	...
	if ( tr.fraction > 0.9f )
	{
		((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[0]+(right)[0]*(256 * dir),(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[1]+(right)[1]*(256 * dir),(NPC->client->ps.velocity)[2]=(NPC->client->ps.velocity)[2]+(right)[2]*(256 * dir));

		
		NPC->client->ps.velocity[2] += 32;

		
		
		NPCInfo->standTime = level.time + 3000 + ((rand () & 0x7fff) / ((float)0x7fff)) * 500;
	}
}
...
void ImperialProbe_Hunt( qboolean visible, qboolean advance )
{
	...
	NPC_SetAnim( NPC, 1|2, BOTH_RUN1, 1|2 );
...
void ImperialProbe_FireBlaster(void)
{
	...
	if (NPC->health)
	{
		...
		((delta1)[0]=(enemy_org1)[0]-(muzzle1)[0],(delta1)[1]=(enemy_org1)[1]-(muzzle1)[1],(delta1)[2]=(enemy_org1)[2]-(muzzle1)[2]);
...
void NPC_Probe_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	if ( self->health < 30 || mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT ) 
	{
		...
		((endPos)[0]=(self->r.currentOrigin[0]), (endPos)[1]=(self->r.currentOrigin[1]), (endPos)[2]=(self->r.currentOrigin[2] - 128));
		trap_Trace( &trace, self->r.currentOrigin, ((void *)0), ((void *)0), endPos, self->s.number, (0x00000001|0x00001000) );

		if ( trace.fraction == 1.0f || mod == MOD_DEMP2 ) 
		{
			...
			if ( (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT) && other )
			{
				...
				((dir)[0]=(self->r.currentOrigin)[0]-(other->r.currentOrigin)[0],(dir)[1]=(self->r.currentOrigin)[1]-(other->r.currentOrigin)[1],(dir)[2]=(self->r.currentOrigin)[2]-(other->r.currentOrigin)[2]);
				VectorNormalize( dir );

				((self->client->ps.velocity)[0]=(self->client->ps.velocity)[0]+(dir)[0]*(550),(self->client->ps.velocity)[1]=(self->client->ps.velocity)[1]+(dir)[1]*(550),(self->client->ps.velocity)[2]=(self->client->ps.velocity)[2]+(dir)[2]*(550));
	...
	else
	{
		...
		if ( ((rand () & 0x7fff) / ((float)0x7fff)) < pain_chance )	
		{
			NPC_SetAnim( self, 1|2, BOTH_PAIN1, 1);
		}	
	}
...
void ImperialProbe_Wait(void)
{
	if ( NPCInfo->localState == LSTATE_DROP )
	{
		...
		((endPos)[0]=(NPC->r.currentOrigin[0]), (endPos)[1]=(NPC->r.currentOrigin[1]), (endPos)[2]=(NPC->r.currentOrigin[2] - 32));
		trap_Trace( &trace, NPC->r.currentOrigin, ((void *)0), ((void *)0), endPos, NPC->s.number, (0x00000001|0x00001000) );

		if ( trace.fraction != 1.0f )
		{
			G_Damage(NPC, NPC->enemy, NPC->enemy, ((void *)0), ((void *)0), 2000, 0,MOD_UNKNOWN); 
		} 
	}
