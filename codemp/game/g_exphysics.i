#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_exphysics.c"
...
#line 15 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_exphysics.c"
...
void G_RunExPhys(gentity_t *ent, float gravity, float mass, float bounce, qboolean autoKill, int *g2Bolts, int numG2Bolts)
{
	...
	(void)( (!!(mass <= 1.0f && mass >= 0.01f)) || (_wassert(L"mass <= 1.0f && mass >= 0.01f", L"c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_exphysics.c", 30), 0) );

	if (gravity)
	{ 
		((ground)[0]=(ent->r.currentOrigin)[0],(ground)[1]=(ent->r.currentOrigin)[1],(ground)[2]=(ent->r.currentOrigin)[2]);
		ground[2] -= 0.1f;
		...
		if (ent->s.groundEntityNum == ((1<<10)-1))
		{
			...
			if (ent->epGravFactor > 1200)
			{ 
				ent->epGravFactor = 1200;
			}
	...
	((projectedOrigin)[0]=(ent->r.currentOrigin)[0]+(ent->epVelocity)[0]*(velScaling),(projectedOrigin)[1]=(ent->r.currentOrigin)[1]+(ent->epVelocity)[1]*(velScaling),(projectedOrigin)[2]=(ent->r.currentOrigin)[2]+(ent->epVelocity)[2]*(velScaling));

	((ent->epVelocity)[0]=(ent->epVelocity)[0]*(1.0f-mass),(ent->epVelocity)[1]=(ent->epVelocity)[1]*(1.0f-mass),(ent->epVelocity)[2]=(ent->epVelocity)[2]*(1.0f-mass)); 

	((vNorm)[0]=(ent->epVelocity)[0],(vNorm)[1]=(ent->epVelocity)[1],(vNorm)[2]=(ent->epVelocity)[2]);
	vTotal = VectorNormalize(vNorm);

	if (vTotal < 1 && ent->s.groundEntityNum != ((1<<10)-1))
	{ 
		((ent->epVelocity)[0]=(ent->epVelocity)[1]=(ent->epVelocity)[2]=0);
		ent->epGravFactor = 0;
		trap_LinkEntity(ent);
		return;
	}
	...
	{ 
		...
		((tMins)[0]=(-3), (tMins)[1]=(-3), (tMins)[2]=(-3));
		((tMaxs)[0]=(3), (tMaxs)[1]=(3), (tMaxs)[2]=(3));

		gbmAngles[0] = gbmAngles[2] = 0;
		gbmAngles[1] = ent->s.apos.trBase[1];

		
		((trajDif)[0]=(ent->r.currentOrigin)[0]-(projectedOrigin)[0],(trajDif)[1]=(ent->r.currentOrigin)[1]-(projectedOrigin)[1],(trajDif)[2]=(ent->r.currentOrigin)[2]-(projectedOrigin)[2]);
        
		while (i < numG2Bolts)
		{
			
			trap_G2API_GetBoltMatrix(ent->ghoul2, 0, g2Bolts[i], &matrix, gbmAngles, ent->r.currentOrigin, level.time, ((void *)0), ent->modelScale);
			BG_GiveMeVectorFromMatrix(&matrix, ORIGIN, boneOrg);

			
			((projectedBoneOrg)[0]=(boneOrg)[0]+(trajDif)[0],(projectedBoneOrg)[1]=(boneOrg)[1]+(trajDif)[1],(projectedBoneOrg)[2]=(boneOrg)[2]+(trajDif)[2]);
			...
			{ 
				...
				{ 
					bestCollision = tr;
					((collisionRootPos)[0]=(boneOrg)[0],(collisionRootPos)[1]=(boneOrg)[1],(collisionRootPos)[2]=(boneOrg)[2]);
		...
		{ 
			...
			((trajDif)[0]=(collisionRootPos)[0]-(bestCollision.endpos)[0],(trajDif)[1]=(collisionRootPos)[1]-(bestCollision.endpos)[1],(trajDif)[2]=(collisionRootPos)[2]-(bestCollision.endpos)[2]);

			((projectedOrigin)[0]=(ent->r.currentOrigin)[0]+(trajDif)[0],(projectedOrigin)[1]=(ent->r.currentOrigin)[1]+(trajDif)[1],(projectedOrigin)[2]=(ent->r.currentOrigin)[2]+(trajDif)[2]);
		}
	}
...
#line 182 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_exphysics.c"
		if (autoKill)
		{
	...
	if (bounce)
	{
		...
		((vNorm)[0]=(tr.plane.normal)[0]*(vTotal),(vNorm)[1]=(tr.plane.normal)[1]*(vTotal),(vNorm)[2]=(tr.plane.normal)[2]*(vTotal)); 
		...
		if (tr.entityNum != ((1<<10)-1) && ent->touch)
		{ 
			ent->touch(ent, &g_entities[tr.entityNum], &tr);
		}

		((ent->epVelocity)[0]=(ent->epVelocity)[0]+(vNorm)[0],(ent->epVelocity)[1]=(ent->epVelocity)[1]+(vNorm)[1],(ent->epVelocity)[2]=(ent->epVelocity)[2]+(vNorm)[2]); 
	}
	else
	{ 
		ent->epVelocity[0] = 0;
