...
#line 15 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_ai_mark2.c"
...
#line 17 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_ai_mark2.c"
...
void NPC_Mark2_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	for (i=0;i<3;i++)
	{
		if ((hitLoc==HL_GENERIC1+i) && (self->locationDamage[HL_GENERIC1+i] > 1))	
		{
			if (self->locationDamage[hitLoc] >= 1)
			{			
				...
				NPC_SetSurfaceOnOff( self, va("torso_canister%d",(i+1)), 0x00000100 );
				break;
			}
		}
	}
	...
	if (self->count > 0)
	{
		G_Damage( self, ((void *)0), ((void *)0), ((void *)0), ((void *)0), self->health, 0x00000008, MOD_UNKNOWN );
	}
}
...
void Mark2_AttackDecision( void )
{
	...
	advance		= (qboolean)(distance > ( 24 * 24 ));

	
	if (NPCInfo->localState == LSTATE_RISINGUP)
	{
		NPC->flags &= ~0x00080000;
		NPC_SetAnim( NPC, 1|2, BOTH_RUN1START, 2|1 );
	...
	if ((!visible) || (!NPC_FaceEnemy(qtrue)))
	{
		...
		if ((NPCInfo->localState == LSTATE_DOWN) || (NPCInfo->localState == LSTATE_DROPPINGDOWN))
		{
			if ( TIMER_Done( NPC, "downTime" ) )	
			{
				...
				NPC_SetAnim( NPC, 1|2, BOTH_RUN1STOP, 2|1 );
	...
	else if (TIMER_Done( NPC, "runTime" ))	
	{
