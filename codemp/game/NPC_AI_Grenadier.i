...
static qboolean Grenadier_Move( void )
{
	...
	if ( moved == qfalse )
	{
		if ( (NPCInfo->scriptFlags&0x00000400) && NPC->client->ps.weapon == WP_THERMAL && NPCInfo->goalEntity && NPCInfo->goalEntity == NPC->enemy )
		{
...
void NPC_BSGrenadier_Attack( void )
{
		...
		)
	{
		if ( NPC->client->ps.weapon == WP_THERMAL )
		{
			...
			if ( !trace.allsolid && !trace.startsolid && (trace.fraction == 1.0 || trace.entityNum == NPC->enemy->s.number ) )
			{
				...
				if ( !(NPCInfo->scriptFlags&0x00000400) )
				{
					NPCInfo->scriptFlags |= 0x00000400;
				}
			}
		}
	}
	else if ( enemyDist3 > 65536 || (NPC->enemy->client && NPC->enemy->client->ps.weapon == WP_SABER && !NPC->enemy->client->ps.saberHolstered) )
	{
	...
	if ( NPC_ClearLOS4( NPC->enemy ) )
	{
		...
		{
			if ( enemyDist3 <= 4096 && InFOV3( NPC->enemy->r.currentOrigin, NPC->r.currentOrigin, NPC->client->ps.viewangles, 90, 45 ) )
			{
		...
		else if ( InFOV3( NPC->enemy->r.currentOrigin, NPC->r.currentOrigin, NPC->client->ps.viewangles, 45, 90 ) )
		{
			...
			int hit = NPC_ShotEntity( NPC->enemy, ((void *)0) );
