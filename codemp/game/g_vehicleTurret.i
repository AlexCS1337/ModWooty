#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_vehicleturret.c"
#line 1 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_headers.h"




#line 2 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_vehicleturret.c"
...
#line 3 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_vehicleturret.c"
...
#line 4 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_vehicleturret.c"
...
#line 5 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\g_vehicleturret.c"
...
{
	vec3_t	enemyDir, org;
	((org)[0]=(turretEnemy->r.currentOrigin)[0],(org)[1]=(turretEnemy->r.currentOrigin)[1],(org)[2]=(turretEnemy->r.currentOrigin)[2]);
	if ( bAILead )
	{
		vec3_t diff, velocity;
		float dist;
		((diff)[0]=(org)[0]-(pVeh->m_vMuzzlePos[curMuzzle])[0],(diff)[1]=(org)[1]-(pVeh->m_vMuzzlePos[curMuzzle])[1],(diff)[2]=(org)[2]-(pVeh->m_vMuzzlePos[curMuzzle])[2]);
		dist = VectorNormalize( diff );
		if ( turretEnemy->client )
		{
			((velocity)[0]=(turretEnemy->client->ps.velocity)[0],(velocity)[1]=(turretEnemy->client->ps.velocity)[1],(velocity)[2]=(turretEnemy->client->ps.velocity)[2]);
		}
		else
		{
			((velocity)[0]=(turretEnemy->s.pos.trDelta)[0],(velocity)[1]=(turretEnemy->s.pos.trDelta)[1],(velocity)[2]=(turretEnemy->s.pos.trDelta)[2]);
		}
		((org)[0]=(org)[0]+(velocity)[0]*((dist/fSpeed)),(org)[1]=(org)[1]+(velocity)[1]*((dist/fSpeed)),(org)[2]=(org)[2]+(velocity)[2]*((dist/fSpeed)));
	}

	
	((enemyDir)[0]=(org)[0]-(pVeh->m_vMuzzlePos[curMuzzle])[0],(enemyDir)[1]=(org)[1]-(pVeh->m_vMuzzlePos[curMuzzle])[1],(enemyDir)[2]=(org)[2]-(pVeh->m_vMuzzlePos[curMuzzle])[2]);
...

{
	...
	if ( pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft
		&& desiredAngles[1] > pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft )
	{
		aimCorrect = qfalse;
		desiredAngles[1] = pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft;
	}
	if ( pVeh->m_pVehicleInfo->turret[turretNum].yawClampRight
		&& desiredAngles[1] < pVeh->m_pVehicleInfo->turret[turretNum].yawClampRight )
	{
		aimCorrect = qfalse;
		desiredAngles[1] = pVeh->m_pVehicleInfo->turret[turretNum].yawClampRight;
	}
	...
	if ( pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown
		&& desiredAngles[0] > pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown )
	{
		aimCorrect = qfalse;
		desiredAngles[0] = pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown;
	}
	if ( pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp
		&& desiredAngles[0] < pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp )
	{
		aimCorrect = qfalse;
		desiredAngles[0] = pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp;
	}
	...
	if ( addAngles[0] > turretStats->fTurnSpeed )
	{
		
		addAngles[0] = turretStats->fTurnSpeed;
	}
	else if ( addAngles[0] < -turretStats->fTurnSpeed )
	{
		
		addAngles[0] = -turretStats->fTurnSpeed;
	}
	if ( addAngles[1] > turretStats->fTurnSpeed )
	{
		
		addAngles[1] = turretStats->fTurnSpeed;
	}
	else if ( addAngles[1] < -turretStats->fTurnSpeed )
	{
	...
	newAngles[0] = AngleNormalize180( curAngles[0]+addAngles[0] );
	newAngles[1] = AngleNormalize180( curAngles[1]+addAngles[1] );
	
	
	if ( turretStats->yawBone )
	{
		((yawAngles)[0]=(yawAngles)[1]=(yawAngles)[2]=0);
		yawAngles[turretStats->yawAxis] = newAngles[1];
		NPC_SetBoneAngles( parent, turretStats->yawBone, yawAngles );
	}
	
	if ( turretStats->pitchBone )
	{
		((pitchAngles)[0]=(pitchAngles)[1]=(pitchAngles)[2]=0);
		pitchAngles[turretStats->pitchAxis] = newAngles[0];
...

{
	...
	WP_CalcVehMuzzle( parent, curMuzzle );
	((org2)[0]=(pVeh->m_vMuzzlePos[curMuzzle])[0],(org2)[1]=(pVeh->m_vMuzzlePos[curMuzzle])[1],(org2)[2]=(pVeh->m_vMuzzlePos[curMuzzle])[2]);
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( !target->client )
		{
			if ( !(target->flags&0x04000000)
				|| !target->takedamage
				|| (target->NPC_targetname&&parent->targetname&&Q_stricmp(target->NPC_targetname,parent->targetname)!=0) )
			{
		...
		trap_Trace( &tr, org2, ((void *)0), ((void *)0), org, parent->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );

		if ( tr.entityNum == target->s.number
			|| (!tr.allsolid && !tr.startsolid && tr.fraction == 1.0 ) )
		{
			
			((enemyDir)[0]=(target->r.currentOrigin)[0]-(org2)[0],(enemyDir)[1]=(target->r.currentOrigin)[1]-(org2)[1],(enemyDir)[2]=(target->r.currentOrigin)[2]-(org2)[2]);
...
void VEH_TurretObeyPassengerControl( Vehicle_t *pVeh, gentity_t *parent, int turretNum )
{
	...
	if ( passenger && passenger->client && passenger->health > 0 )
	{
		...
		vec3_t aimAngles;
		((aimAngles)[0]=(passenger->client->ps.viewangles)[0],(aimAngles)[1]=(passenger->client->ps.viewangles)[1],(aimAngles)[2]=(passenger->client->ps.viewangles)[2]);

		VEH_TurretAim( pVeh, parent, ((void *)0), turretStats, vehWeapon, turretNum, curMuzzle, aimAngles );
		if ( (passenger->client->pers.cmd.buttons&(1|128)) )
		{
...

{
	...
	if ( pVeh->turretStatus[turretNum].enemyEntNum < ((1<<10)-2) )
	{
		turretEnemy = &g_entities[pVeh->turretStatus[turretNum].enemyEntNum];
		if ( turretEnemy->health < 0 
			|| !turretEnemy->inuse
			|| turretEnemy == ((gentity_t*)pVeh->m_pPilot)
			|| turretEnemy == parent
			|| turretEnemy->r.ownerNum == parent->s.number 
			|| ( turretEnemy->client && turretEnemy->client->sess.sessionTeam == TEAM_SPECTATOR ) )
		{
			turretEnemy = ((void *)0);
			pVeh->turretStatus[turretNum].enemyEntNum = ((1<<10)-1);
		}
	}
	...
	if ( pVeh->turretStatus[turretNum].enemyHoldTime < level.time )
	{
		...
		else if ( parent->enemy && parent->enemy->s.number < ((1<<10)-2) )
		{
	...
	if ( turretEnemy != ((void *)0) )
	{
		if ( turretEnemy->health > 0 )
		{
			
			WP_CalcVehMuzzle( parent, curMuzzle );
			((enemyDir)[0]=(turretEnemy->r.currentOrigin)[0]-(pVeh->m_vMuzzlePos[curMuzzle])[0],(enemyDir)[1]=(turretEnemy->r.currentOrigin)[1]-(pVeh->m_vMuzzlePos[curMuzzle])[1],(enemyDir)[2]=(turretEnemy->r.currentOrigin)[2]-(pVeh->m_vMuzzlePos[curMuzzle])[2]);
			enemyDist = VectorLengthSquared( enemyDir );
			...
			if ( enemyDist < rangeSq )
			{
				...
				if ( trap_InPVS( pVeh->m_vMuzzlePos[curMuzzle], turretEnemy->r.currentOrigin ) )
				{
					...
					trace_t tr;
					vec3_t start, end;
					((start)[0]=(pVeh->m_vMuzzlePos[curMuzzle])[0],(start)[1]=(pVeh->m_vMuzzlePos[curMuzzle])[1],(start)[2]=(pVeh->m_vMuzzlePos[curMuzzle])[2]);

					((end)[0]=(turretEnemy->r.currentOrigin)[0],(end)[1]=(turretEnemy->r.currentOrigin)[1],(end)[2]=(turretEnemy->r.currentOrigin)[2]);
					trap_Trace( &tr, start, ((void *)0), ((void *)0), end, parent->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
