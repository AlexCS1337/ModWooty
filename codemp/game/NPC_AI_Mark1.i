...
#line 42 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_ai_mark1.c"
...
#line 44 "c:\\projects\\jedi_academy_sdk(mp)\\codemp\\game\\npc_ai_mark1.c"
...
void Mark1_Idle( void )
{
	...
	NPC_SetAnim( NPC, 1|2, BOTH_SLEEP1, 0 );
}
...
void Mark1Dead_FireRocket (void)
{
	...
	missile = CreateMissile( muzzle1, muzzle_dir, 1300, 10000, NPC, qfalse );
...
void Mark1_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod,int dFlags,int hitLoc )
{
	...
	if (Q_irand( 1, 10) > 5)
	{
		NPC_SetAnim( self, 1|2, BOTH_DEATH2, 1|2 );
	}
	else
	{
		NPC_SetAnim( self, 1|2, BOTH_DEATH1, 1|2 );
	}
}
	...
	if (self->client->ps.torsoTimer>0)
	{
		if (TIMER_Done(self,"dyingExplosion"))
		{
			...
			else
			{
				...
				NPC_Mark1_Part_Explode(self,newBolt);
				NPC_SetSurfaceOnOff( self, va("torso_tube%d",num), 0x00000100 );
			}
		...
		if (!trap_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "l_arm" ))	
		{
			if (Q_irand( 1, 5) == 1)
			{
				SaveNPCGlobals();
		...
		if (!trap_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "r_arm" ))	
		{
			if (Q_irand( 1, 10) == 1)
			{
				SaveNPCGlobals();
...
void NPC_Mark1_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	else if ((hitLoc==HL_ARM_LT) && (self->locationDamage[HL_ARM_LT] > 40))
	{
		if (self->locationDamage[hitLoc] >= 40)	
		{
			...
			NPC_SetSurfaceOnOff( self, "l_arm", 0x00000100 );
		}
	}
	
	else if ((hitLoc==HL_ARM_RT) && (self->locationDamage[HL_ARM_RT] > 40))	
	{
		if (self->locationDamage[hitLoc] >= 40)
		{			
			...
			NPC_SetSurfaceOnOff( self, "r_arm", 0x00000100 );
		}
	}
	...
	else
	{
		for (i=0;i<6;i++)
		{
			if ((hitLoc==HL_GENERIC1+i) && (self->locationDamage[HL_GENERIC1+i] > 40))	
			{
				if (self->locationDamage[hitLoc] >= 40)
				{			
					...
					NPC_SetSurfaceOnOff( self, va("torso_tube%d",(i+1)), 0x00000100 );
					NPC_SetAnim( self, 1|2, BOTH_PAIN1, 1|2 );
					break;
				}
			}
	...
	{
		G_Damage(self,((void *)0),((void *)0),((void *)0),((void *)0),self->health,0,MOD_UNKNOWN);
	}
}
...
void Mark1_BlasterAttack(qboolean advance )
{
	...
	if ( TIMER_Done( NPC, "attackDelay" ) )	
	{
		...
		{
			if (TIMER_Done( NPC, "attackDelay2" ))	
			{
...
void Mark1_FireRocket(void)
{
	...
	missile = CreateMissile( muzzle1, forward, 1300, 10000, NPC, qfalse );
...
void Mark1_RocketAttack( qboolean advance )
{
	...
	{
		TIMER_Set( NPC, "attackDelay", Q_irand( 1000, 3000) );
 		NPC_SetAnim( NPC, 1, BOTH_ATTACK2, 1|2 );
		Mark1_FireRocket();
	}
	else if (advance)
	{
...
void Mark1_AttackDecision( void )
{
	...
	distRate	= ( distance > ( 320 * 320 ) ) ? DIST_LONG : DIST_MELEE;
