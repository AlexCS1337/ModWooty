...
//	if ( ent->client->ps.saber[0].type != SABER_NONE )
	...
	{		
		ent->NPC->defaultBehavior = BS_DEFAULT;/////BS_ADVANCE_FIGHT;////BS_DEFAULT;
		ent->client->ps.gravity = 0;
		ent->NPC->aiFlags |= NPCAI_CUSTOM_GRAVITY;
		ent->client->ps.eFlags2 |= EF2_FLYING;
		ent->count = 12; // SEEKER shot ammo count
	...
	{//a vehicle
		...
		if ( ent->m_pVehicle->m_iDroidUnitTag != -1 )
		{
			...
			if ( droidNPCType != NULL )
			{
				...
				droidEnt = NPC_SpawnType( ent, droidNPCType, NULL, qfalse,100 );
				if ( droidEnt != NULL )
				{
					if ( droidEnt->client )
					{
...
void NPC_DefaultScriptFlags( gentity_t *ent )
{
	...
	////ent->NPC->scriptFlags = (SCF_CHASE_ENEMIES|SCF_LOOK_FOR_ENEMIES);//unwoot?

	ent->NPC->scriptFlags = (SCF_NO_FALLTODEATH|SCF_USE_SUBTITLES);
}
...
//=====================================================================
	...
	if (sizefactor < 10){
		...
		VectorSet(newent->modelScale, sizefloat, sizefloat, sizefloat);
	newent->client->ps.crouchheight =  (int)(float)sizefloat * CROUCH_MAXS_2;
	newent->client->ps.standheight =  (int)(float)sizefloat * DEFAULT_MAXS_2;
...
#define SHY_SPAWN_DISTANCE		256///128
...
int GetPrices( gentity_t *ent, int price){
	...
	if (ent->client){
		///calc = (int)((1.00f-(.0667f*(ent->client->sess.karma/2.50f)))*price);
...
void NPC_Spawn_f( gentity_t *ent ) 
{
	...
	if (strlen(npc_type) <= 4 && IsABot(ent->s.number)){
		...
		switch(v){
			...
			break;
				...
				case 9:
			strcpy(npc_type,"field");
			break;

				case 10:
			strcpy(npc_type,"flechette");
			break;
			...
			sizefactor = Q_irand(25,250);
			quantity = Q_irand(1,3);
			break;

		}

		if (ent->client->ps.pm_type == PMF_JUMP_HELD){
			strcpy(npc_type,"gravgun");
		}
	...
	{
		//trap_Argv(3- buying, targetname, 1024);
...
///	||  Q_stricmp( "ugnaught", npc_type ) == 0 
	...
	/*if ( 
		Q_stricmp( "bespincop", npc_type ) == 0 
		//||  Q_stricmp( "impworker", npc_type ) == 0 
		//|| Q_stricmp( "stormpilot", npc_type ) == 0 
	...
	if ( 
		...
		//||  Q_stricmp( "cultist", npc_type ) == 0
		//||  Q_stricmp( "galak", npc_type ) == 0
		//||  Q_stricmp( "rebel", npc_type ) == 0
		//||  Q_stricmp( "Gran", npc_type ) == 0
		 Q_stricmp( "stormtrooper", npc_type ) == 0
		||  Q_stricmp( "rebel", npc_type ) == 0
		||  Q_stricmp( "tusken", npc_type ) == 0
		///||  Q_stricmp( "swamptrooper", npc_type ) == 0
		//||  Q_stricmp( "shadowtrooper", npc_type ) == 0
	    //||  Q_stricmp( "noghri", npc_type ) == 0
		...
		)
	{
		...
		//||  Q_stricmp( "boba_fett", npc_type ) == 0 
		//||  Q_stricmp( "hazardtrooperconcussion", npc_type ) == 0
		...
		)
	{
		...
		///||  Q_stricmp( "rebornfencer", npc_type ) == 0
		||  Q_stricmp( "chewie", npc_type ) == 0
		||  Q_stricmp( "gran", npc_type ) == 0
		//||  Q_stricmp( "rosh", npc_type ) == 0
	...
	if ( Q_stricmp( "rancor", npc_type ) == 0 
		//||  Q_stricmp( "mutant_rancor", npc_type ) == 0
		...
		)
	{
		...
		if (ent->client->sess.karma >= 64){
			...
			return;
		}///else if (ent->client->ps.userInt3 < 80){
		///	required = 1200;
		///	LoseKarma(ent->s.number, "Honor: natural law.", 1);
		///}
	}
...
for (j = 0; j < 16; j++){
	...
	if (g_entities[j].client 
		...
		(g_entities[j].client->sess.spectatorState != SPECTATOR_FOLLOW || g_gametype.integer !=4) 
		&& (g_entities[j].client->pers.netname[1] > 0 || (g_entities[j].NPC_type && g_entities[j].NPC_type[1] > 0)) 
		&& (all == qtrue || g_entities[j].s.number == specent->s.number)){
	...
	//userint3: number of spawners
for (i = 1; i<= quantity; i++){

	if (ent->client->sess.admin_number >= (required * quantity) && npc_type[1] != 0 && ent->client->ps.userInt3 < 80){
		
		/*if (i==1){// && required < 500){
		ent->client->ps.customRGBA[0] = Q_irand(0,255);			
		ent->client->ps.customRGBA[1] = Q_irand(0,255);
		ent->client->ps.customRGBA[2] = Q_irand(0,255);
		}*/
	
		if (i == quantity){
			ent->client->sess.admin_number -= required * quantity;
		}
			////level.clients[0].sess.admin_number += required;
		if ( Q_stricmp( "subadmin", npc_type)  == 0)
		{
			strcpy(specent->client->sess.admin_name,"sub");
			WriteKeys(specent->s.number,7,qtrue);
			//ent->client->ps.ammo[AMMO_POWERCELL] += 40;
			if (i == 1){
		...
		{
			if (ent->client->sess.dex > 0){
			ent->client->sess.str++;
			WriteKeys(ent->s.number, 1, qtrue);
			}
			if (ent->client->sess.dex > 0 && i == 1){
			...
			if (!Q_irand(0,2) && ent->client->sess.dex > 0 && ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.dex--;
				WriteKeys(ent->s.number, 2, qtrue);
			}else if ( ent->client->sess.dex > 0 && ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.con--;
		...
		else if ( Q_stricmp( "con", npc_type)  == 0)
		{
			...
			G_Damage(ent, ent, ent, NULL, ent->client->ps.origin, -2, DAMAGE_NORMAL, MOD_SLIME);

						if (ent->client->sess.dex > 0 && i == 1){
			trap_SendServerCommand( -1, va("print \"^5%s^5 trained CON +%i!\n\"",  ent->client->pers.netname, quantity ) );
			trap_SendServerCommand( -1, va("cp \"^5%s^5 trained CON +%i!\n\"",  ent->client->pers.netname, quantity ) );
			}
			if (ent->client->sess.dex > 0 && !Q_irand(0,2) && ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.dex--;
				WriteKeys(ent->s.number, 2, qtrue);
			}else if (ent->client->sess.dex > 0 &&  ent->client->sess.str+ent->client->sess.dex+ent->client->sess.con >= 45){
				ent->client->sess.str--;
		...
		else if (NPC_SpawnType( ent, npc_type, targetname, isVehicle, sizefactor ) && required <= 1200)
		{
			...
			///trap_SendServerCommand( -1, va("print \"^5%s^5 bought %s %s ^4x%i^5\"",  ent->client->pers.netname, article, npc_type ) );
			///trap_SendServerCommand( -1, va("cp \"^5%s^5 bought %s %s ^4^4x %i^5^5\"",  ent->client->pers.netname, article, npc_type ) );
			///ent->client->ps.userInt3++;
		}else{
			if (i == 1){
