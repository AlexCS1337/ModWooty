...
{
	if ( !(self->NPC->aiFlags&0x00200000 ))
	{
		G_Damage( self, ((void *)0), ((void *)0), (float*)vec3_origin, (float*)vec3_origin, 999, 0, MOD_FALLING );
	}
...
void Seeker_Strafe( void )
{
	...
	if ( ((rand () & 0x7fff) / ((float)0x7fff)) > 0.7f || !NPC->enemy || !NPC->enemy->client )
	{
		
		AngleVectors( NPC->client->renderInfo.eyeAngles, ((void *)0), right, ((void *)0) );

		
		
		side = ( rand() & 1 ) ? -1 : 1;
		((end)[0]=(NPC->r.currentOrigin)[0]+(right)[0]*(200 * side),(end)[1]=(NPC->r.currentOrigin)[1]+(right)[1]*(200 * side),(end)[2]=(NPC->r.currentOrigin)[2]+(right)[2]*(200 * side));

		trap_Trace( &tr, NPC->r.currentOrigin, ((void *)0), ((void *)0), end, NPC->s.number, (0x00000001|0x00001000) );
		...
		if ( tr.fraction > 0.9f )
		{
			...
			((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[0]+(right)[0]*(vel*side),(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[1]+(right)[1]*(vel*side),(NPC->client->ps.velocity)[2]=(NPC->client->ps.velocity)[2]+(right)[2]*(vel*side));
			
			NPC->client->ps.velocity[2] += upPush;

			NPCInfo->standTime = level.time + 1000 + ((rand () & 0x7fff) / ((float)0x7fff)) * 500;
		}
	}
	else
	{
		...
		AngleVectors( NPC->enemy->client->renderInfo.eyeAngles, dir, right, ((void *)0) );
		...
		((end)[0]=(NPC->enemy->r.currentOrigin)[0]+(right)[0]*(stDis * side),(end)[1]=(NPC->enemy->r.currentOrigin)[1]+(right)[1]*(stDis * side),(end)[2]=(NPC->enemy->r.currentOrigin)[2]+(right)[2]*(stDis * side));

		
		((end)[0]=(end)[0]+(dir)[0]*((2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) * 25),(end)[1]=(end)[1]+(dir)[1]*((2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) * 25),(end)[2]=(end)[2]+(dir)[2]*((2.0 * (((rand () & 0x7fff) / ((float)0x7fff)) - 0.5)) * 25));
		...
		if ( tr.fraction > 0.9f )
		{
			...
			((dir)[0]=(tr.endpos)[0]-(NPC->r.currentOrigin)[0],(dir)[1]=(tr.endpos)[1]-(NPC->r.currentOrigin)[1],(dir)[2]=(tr.endpos)[2]-(NPC->r.currentOrigin)[2]);
			dir[2] *= 0.25; 
			dis = VectorNormalize( dir );

			
			((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[0]+(dir)[0]*(dis),(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[1]+(dir)[1]*(dis),(NPC->client->ps.velocity)[2]=(NPC->client->ps.velocity)[2]+(dir)[2]*(dis));
			...
			NPCInfo->standTime = level.time + 2500 + ((rand () & 0x7fff) / ((float)0x7fff)) * 500;
		}
	}
}
...
void Seeker_Hunt( qboolean visible, qboolean advance )
{
	...
	{
		((forward)[0]=(NPC->enemy->r.currentOrigin)[0]-(NPC->r.currentOrigin)[0],(forward)[1]=(NPC->enemy->r.currentOrigin)[1]-(NPC->r.currentOrigin)[1],(forward)[2]=(NPC->enemy->r.currentOrigin)[2]-(NPC->r.currentOrigin)[2]);
		distance = VectorNormalize( forward );
	}

	speed = 10 + 2 * g_spskill.integer;
	((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[0]+(forward)[0]*(speed),(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[1]+(forward)[1]*(speed),(NPC->client->ps.velocity)[2]=(NPC->client->ps.velocity)[2]+(forward)[2]*(speed));
}
...
void Seeker_Fire( void )
{
	...
	((dir)[0]=(enemy_org)[0]-(NPC->r.currentOrigin)[0],(dir)[1]=(enemy_org)[1]-(NPC->r.currentOrigin)[1],(dir)[2]=(enemy_org)[2]-(NPC->r.currentOrigin)[2]);
	VectorNormalize( dir );

	
	((muzzle)[0]=(NPC->r.currentOrigin)[0]+(dir)[0]*(15),(muzzle)[1]=(NPC->r.currentOrigin)[1]+(dir)[1]*(15),(muzzle)[2]=(NPC->r.currentOrigin)[2]+(dir)[2]*(15));
...
void Seeker_Ranged( qboolean visible, qboolean advance )
{
	if ( NPC->client->NPC_class != CLASS_BOBAFETT )
	{
		...
		else
		{
			...
			G_Damage( NPC, NPC, NPC, ((void *)0), ((void *)0), 999, 0, MOD_UNKNOWN );
		}
	}
...
void Seeker_Attack( void )
{
	...
	advance		= (qboolean)(distance > ( 80 * 80 ));
...
{
	int			numFound;
	float		dis, bestDis = 1024 * 1024 + 1;
	vec3_t		mins, maxs;
	int			entityList[(1<<10)];
	gentity_t	*ent, *best = ((void *)0);
	int			i;

	((maxs)[0]=(1024), (maxs)[1]=(1024), (maxs)[2]=(1024));
	((mins)[0]=(maxs)[0]*(-1),(mins)[1]=(maxs)[1]*(-1),(mins)[2]=(maxs)[2]*(-1));

	numFound = trap_EntitiesInBox( mins, maxs, entityList, (1<<10) );
	...
	for ( i = 0 ; i < numFound ; i++ ) 
	{
		...
		if ( ent->client->playerTeam == NPC->client->playerTeam || ent->client->playerTeam == NPCTEAM_NEUTRAL ) 
		{
			continue;
		}
	...
	if ( best )
	{
		...
		NPC->random = ((rand () & 0x7fff) / ((float)0x7fff)) * 6.3f; 
...
void Seeker_FollowOwner( void )
{
	...
	if ( dis < minDistSqr )
	{
		...
		((dir)[0]=(pt)[0]-(NPC->r.currentOrigin)[0],(dir)[1]=(pt)[1]-(NPC->r.currentOrigin)[1],(dir)[2]=(pt)[2]-(NPC->r.currentOrigin)[2]);
		((NPC->client->ps.velocity)[0]=(NPC->client->ps.velocity)[0]+(dir)[0]*(0.8f),(NPC->client->ps.velocity)[1]=(NPC->client->ps.velocity)[1]+(dir)[1]*(0.8f),(NPC->client->ps.velocity)[2]=(NPC->client->ps.velocity)[2]+(dir)[2]*(0.8f));
	}
	else
	{
		if ( NPC->client->NPC_class != CLASS_BOBAFETT )
		{
			...
			{
				TIMER_Set( NPC, "seekerhiss", 1000 + ((rand () & 0x7fff) / ((float)0x7fff)) * 1000 );
				G_Sound( NPC, CHAN_AUTO, G_SoundIndex( "sound/chars/seeker/misc/hiss" ));
			}
		}
...
void NPC_BSSeeker_Default( void )
{
	...
	if ( NPC->r.ownerNum < ((1<<10)-1) )
	{
			...
			|| (owner->client && owner->client->pers.connected == CON_DISCONNECTED) )
		{
			...
			G_Damage( NPC, ((void *)0), ((void *)0), ((void *)0), ((void *)0), 10000, 0x00000008, MOD_TELEFRAG );
			return;
		}
	}
