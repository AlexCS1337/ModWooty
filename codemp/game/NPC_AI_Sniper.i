...
static qboolean Sniper_Move( void )
{
	...
	if ( moved == qfalse )
	{
		if ( (NPCInfo->scriptFlags&0x00000400) && NPCInfo->goalEntity && NPCInfo->goalEntity == NPC->enemy )
		{
			
			int cpFlags = (0x00000002|0x00001000);
			int cp;
			...
			if ( cp == -1 && !(NPCInfo->scriptFlags&0x00100000) )
			{
				cp = NPC_FindCombatPoint( NPC->r.currentOrigin, NPC->r.currentOrigin, NPC->enemy->r.currentOrigin, 0x00000002|0x00001000|0x00008000, 32, -1 );
			}
	...
	if ( NPCInfo->confusionTime < level.time )
	{
		...
		if ( !(NPCInfo->scriptFlags&0x00002000) )
		{
			...
			else
			{
				...
				if ( alertEvent >= 0 && level.alertEvents[alertEvent].ID != NPCInfo->lastAlertID )
				{
					...
					else
					{
						...
						((NPCInfo->investigateGoal)[0]=(level.alertEvents[alertEvent].position)[0],(NPCInfo->investigateGoal)[1]=(level.alertEvents[alertEvent].position)[1],(NPCInfo->investigateGoal)[2]=(level.alertEvents[alertEvent].position)[2]);
			...
			if ( NPCInfo->investigateDebounceTime > level.time )
			{
				...
				((dir)[0]=(NPCInfo->investigateGoal)[0]-(NPC->client->renderInfo.eyePoint)[0],(dir)[1]=(NPCInfo->investigateGoal)[1]-(NPC->client->renderInfo.eyePoint)[1],(dir)[2]=(NPCInfo->investigateGoal)[2]-(NPC->client->renderInfo.eyePoint)[2]);
...
static void Sniper_CheckFireState( void )
{
	...
	if ( !Q_irand( 0, 1 ) && NPCInfo->enemyLastSeenTime && level.time - NPCInfo->enemyLastSeenTime < ((5-NPCInfo->stats.aim)*1000) )
	{
...
qboolean Sniper_EvaluateShot( int hit )
{
	...
	if ( hit == NPC->enemy->s.number 
		...
		|| ( hitEnt && (hitEnt->r.svFlags&0x08000000)) )
	{
		return qtrue;
	}
	return qfalse;
...
void Sniper_FaceEnemy( void )
{
	...
	if ( NPC->enemy )
	{
		...
		if ( enemyDist2 > (16384/2) && NPCInfo->stats.aim < 5 )
		{
				...
				if ( shoot2 && TIMER_Done( NPC, "attackDelay" ) && level.time >= NPCInfo->shotTime )
				{
					...
					while ( hit && tryMissCount < 1 )
					{
						...
						if ( !Q_irand( 0, 1 ) )
						{
							...
							if ( !Q_irand( 0, 1 ) )
							{
								((target)[0]=(target)[0]+(right)[0]*(NPC->enemy->r.maxs[2]*flrand(1.5, 4)),(target)[1]=(target)[1]+(right)[1]*(NPC->enemy->r.maxs[2]*flrand(1.5, 4)),(target)[2]=(target)[2]+(right)[2]*(NPC->enemy->r.maxs[2]*flrand(1.5, 4)));
							}
							else
							{
								((target)[0]=(target)[0]+(right)[0]*(NPC->enemy->r.mins[2]*flrand(1.5, 4)),(target)[1]=(target)[1]+(right)[1]*(NPC->enemy->r.mins[2]*flrand(1.5, 4)),(target)[2]=(target)[2]+(right)[2]*(NPC->enemy->r.mins[2]*flrand(1.5, 4)));
							}
						}
						if ( !aimError || !Q_irand( 0, 1 ) )
						{
							if ( !Q_irand( 0, 1 ) )
							{
								((target)[0]=(target)[0]+(up)[0]*(NPC->enemy->r.maxs[2]*flrand(1.5, 4)),(target)[1]=(target)[1]+(up)[1]*(NPC->enemy->r.maxs[2]*flrand(1.5, 4)),(target)[2]=(target)[2]+(up)[2]*(NPC->enemy->r.maxs[2]*flrand(1.5, 4)));
							}
							else
							{
								((target)[0]=(target)[0]+(up)[0]*(NPC->enemy->r.mins[2]*flrand(1.5, 4)),(target)[1]=(target)[1]+(up)[1]*(NPC->enemy->r.mins[2]*flrand(1.5, 4)),(target)[2]=(target)[2]+(up)[2]*(NPC->enemy->r.mins[2]*flrand(1.5, 4)));
							}
						}
						trap_Trace( &trace, muzzle, vec3_origin, vec3_origin, target, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
			...
			else
			{
				...
				((target)[0]=(NPCInfo->enemyLaggedPos[missFactor])[0],(target)[1]=(NPCInfo->enemyLaggedPos[missFactor])[1],(target)[2]=(NPCInfo->enemyLaggedPos[missFactor])[2]);
			}
			GetAnglesForDirection( muzzle, target, angles );
		}
		else
...
void Sniper_UpdateEnemyPos( void )
{
	...
	for ( i = 2400-100; i >= 0; i -= 100 )
	{
		...
		{
			((NPCInfo->enemyLaggedPos[index])[0]=(NPCInfo->enemyLaggedPos[index-1])[0],(NPCInfo->enemyLaggedPos[index])[1]=(NPCInfo->enemyLaggedPos[index-1])[1],(NPCInfo->enemyLaggedPos[index])[2]=(NPCInfo->enemyLaggedPos[index-1])[2]);
		}
	}
}
...
void NPC_BSSniper_Attack( void )
{
	...
	if ( NPC_ClearLOS4( NPC->enemy ) )
	{
		...
		if ( enemyDist2 < maxShootDist )
		{
			...
			((end)[0]=(muzzle)[0]+(fwd)[0]*(8192),(end)[1]=(muzzle)[1]+(fwd)[1]*(8192),(end)[2]=(muzzle)[2]+(fwd)[2]*(8192));
			trap_Trace ( &tr, muzzle, ((void *)0), ((void *)0), end, NPC->s.number, (0x00000001|0x00000100|0x00000200|0x00001000) );
	...
	{
		if ( enemyLOS2 && (NPCInfo->scriptFlags&0x00000040) )
		{
	...
	if ( shoot2 )
	{
		...
		{
			WeaponThink( qtrue );
			if ( ucmd.buttons&(1|128) )
			{
				G_SoundOnEnt( NPC, CHAN_WEAPON, "sound/null.wav" );
			}

			
			if ( !(NPC->spawnflags&2) && !Q_irand( 0, 1 ) )
			{
