...
//		trap_S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
	...
	else if (strncmp(cgs.voteString, "moot", 4)==0)
	{
		trap_SP_GetStringTextString("MENUS_MOOT_PLAYER", sCmd, sizeof(sCmd) );
		sParm = cgs.voteString+5;
	}
	else if (strncmp(cgs.voteString, "modepluso", 9)==0)
	{
		trap_SP_GetStringTextString("MENUS_modepluso_PLAYER", sCmd, sizeof(sCmd) );
